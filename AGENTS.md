...
| 9   | **Sync**              | `sync_enforcement_to_projects.py`                                                            | Pushes governance to all projects. These files are **centrally managed** (canonical list: `scripts/fabrik_synced_manifest.py`) — never plan tickets that edit a synced copy in a project; it's overwritten on the next sync (gate-enforced by `check_synced_unmodified.py`). Changes go upstream in `/opt/fabrik`, and ONLY if correct for ALL projects.                                                                                                                                                                                      |
