...
7. **fabrik-libs** — Check `/opt/fabrik-libs/fabrik/` for existing reusable modules (llm, email, retry, translation, pause-state). USE before planning custom implementations of these capabilities. State which modules apply.
...
| Shared Code Modules | `/opt/fabrik-libs/` (`fabrik.llm`, `fabrik.email`, `fabrik.retry`, `fabrik.translation`, `fabrik.pause`) | Reusable Python modules — check before writing custom LLM client, email sender, retry logic. `pip install -e /opt/fabrik-libs` in WSL. |
