...
class MenuCreator:
    ...
    def create_menu(
        ...
        if existing_id:
            ...
            self.wp.run(f"menu item list {existing_id} --format=ids | xargs -r wp menu item delete --allow-root || true")
            menu_id = existing_id
        else:
            # Create new menu
    ...
    def _get_page_id(self, slug: str) -> Optional[int]:
    ...
    def _get_menu_id(self, name: str) -> Optional[int]:
        """Get menu ID by name."""
        try:
            output = self.wp.run(f"menu list --format=json")
