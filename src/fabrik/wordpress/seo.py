...
class SEOApplicator:
    ...
    def detect_seo_plugin(self) -> Optional[str]:
    ...
    def apply_site_seo(self, seo: dict) -> dict:
        ...
        if title := seo.get("title_template"):
            if plugin == "yoast":
                self.wp.option_update("wpseo_titles", json.dumps({
                    "title-home-wpseo": title,
                }))
            elif plugin == "rankmath":
                self.wp.option_update("rank_math_titles", json.dumps({
        ...
        if google_verification := seo.get("google_verification"):
            if plugin == "yoast":
                self.wp.option_update("wpseo", json.dumps({
                    "googleverify": google_verification,
                }))
            elif plugin == "rankmath":
                self.wp.option_update("rank_math_options", json.dumps({
