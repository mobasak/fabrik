...
class SpecValidator:
    ...
    def _validate_required(self):
        ...
        for path, expected_type in required:
            ...
            if value is None:
                ...
                self.errors.append(f"{path}: expected {expected_type.__name__}, got {type(value).__name__}")
    ...
    def _validate_types(self):
        ...
        for color_name, color_value in colors.items():
            if color_value and not self._is_valid_color(color_value):
                self.errors.append(f"brand.colors.{color_name}: invalid color format '{color_value}'")
    ...
    def _is_localized_string(self, obj: dict) -> bool:
        ...
        return all(re.match(locale_pattern, k) for k in obj.keys())
