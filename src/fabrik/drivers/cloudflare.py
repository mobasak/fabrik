...
class CloudflareClient:
    ...
    def __init__(
        self, api_token: str | None = None, account_id: str | None = None, timeout: int = 30
    ):
    ...
    def create_zone(self, domain: str) -> dict[str, Any]:
        ...
        Returns:
            Zone details including id, name, status, name_servers
        """
        if not self.account_id:
            raise ValueError("CLOUDFLARE_ACCOUNT_ID required for zone creation")

        data = {
            "name": domain,
            "account": {"id": self.account_id},
            "type": "full",  # Full zone (Cloudflare as DNS authority)
        }
    ...
    def get_zone_status(self, zone_id: str) -> dict[str, Any]:
        ...
        Returns:
            Dict with status ('pending' or 'active') and name_servers
        """
    ...
    def ensure_zone(self, domain: str) -> dict[str, Any]:
        """
        Ensure zone exists. Create if missing, return existing if found.

        Returns:
            Dict with zone_id, status, name_servers, created (bool)
        """
    ...
    def ensure_record(
        ...
        if existing:
            ...
            if (
                ...
                and record["proxied"] == proxied
                and (ttl == 1 or record["ttl"] == ttl)
            ):
        ...
        created = self.create_record(
            zone_id, record_type, full_name, content, ttl=ttl, proxied=proxied, comment=comment
        )
        return {"action": "created", "record": created}

    def delete_record_by_name(self, domain: str, record_type: str, name: str) -> dict[str, Any]:
