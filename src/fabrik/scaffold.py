...
def _logger_py_content(name: str, package_name: str) -> str:
    ...
    return (
        ...
        f"    )\n"
        f"    _bridge_stdlib_logging(level)\n"
        f"\n"
        f"\n"
        f"def _bridge_stdlib_logging(level: int) -> None:\n"
        f'    """Route stdlib loggers (uvicorn, gunicorn, SQLAlchemy) through structlog.\n'
        f"\n"
        f"    ``structlog.configure()`` configures STRUCTLOG. Third-party libraries use the\n"
        f"    stdlib ``logging`` module and keep their own handlers, so a service that has\n"
        f"    done logging correctly by our own rules STILL emits raw non-JSON lines to the\n"
        f"    same stdout. Measured live 2026-09-01 on site-provisioner, adjacent lines:\n"
        f'        {{"event": "cloudflare_connectivity_check", "level": "info", ...}}\n'
        f'        INFO:     127.0.0.1:54012 - "GET /health HTTP/1.1" 200 OK\n'
        ...
        f"        processors=[\n"
        f"            structlog.stdlib.ProcessorFormatter.remove_processors_meta,\n"
        f"            _redact_sensitive,\n"
        f"            structlog.processors.JSONRenderer(),\n"
        f"        ],\n"
        f"        foreign_pre_chain=[\n"
        ...
        f"        ],\n"
        f"    )\n"
        f"    # sys.stdout explicitly: structlog's PrintLoggerFactory writes to stdout, and a bare\n"
        f"    # StreamHandler() defaults to STDERR — which would split one service's JSON across two\n"
        f"    # streams and give Loki two different `stream` labels for the same process.\n"
        f"    handler = logging.StreamHandler(sys.stdout)\n"
        f"    handler.setFormatter(formatter)\n"
        f"    root = logging.getLogger()\n"
        f"    root.handlers = [handler]\n"
        f"    root.setLevel(level)\n"
        f"    # uvicorn/gunicorn install their OWN handlers; clear them so records reach root.\n"
        f"    #\n"
        f"    # ORDERING (why this is safe, and its one assumption): uvicorn.Config.__init__\n"
        f"    # runs configure_logging() FIRST, then load() imports the app -> imports this\n"
        f"    # module -> clears them. We win because we run LAST. Verified end-to-end against a\n"
        f"    # real uvicorn.Server (2026-09-01): 9/9 lines JSON on stdout, 0 on stderr.\n"
        f"    # If a future entrypoint imports the app BEFORE uvicorn configures logging, uvicorn\n"
        f"    # re-adds its handlers and raw lines return. The guard for that is the one-command\n"
        f"    # check in the docstring, run against the deployed container -- not this code.\n"
        f"    #\n"
        f"    # NOT using --no-access-log: the scaffolded X-Request-ID middleware binds a\n"
