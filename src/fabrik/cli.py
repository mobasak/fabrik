...
@cli.command("app-role-check")
@click.option(
    "--spec",
    "spec_path",
    required=True,
    type=click.Path(exists=True),
    help="Spec YAML to resolve db_name and repo_dir from (specs/services/<id>.yaml)",
)
def app_role_check_cmd(spec_path: str):
    """Pre-cutover check for the ``<db>_app`` DSN cutover: ownership + privilege probe + repo scan.

    Resolves ``db_name`` from the spec's ``depends.postgres`` (falling back to the
    spec's name/id with hyphens rewritten to underscores) and ``repo_dir`` from
    ``project_repo_dir()`` (``/opt/<id>``), then runs
    :func:`fabrik.app_role_check.run_check`. Prints ``✓`` and exits 0 on a clean
    pass, or one ``✗ <failure>`` line per failure and exits 1.
    """
    import yaml
