...
## Phase 1 — Finders (recall), native seats

Dispatch finders in parallel, each owning a different failure class, against the artifact + its acceptance criteria + the yardstick items. **Native seats, per `62-using-subagents.md` (the pool is OFF, D-181):** one `fabrik-reviewer` seat on Sonnet per independent failure-class GROUP plus the Haiku mechanical seats `dispatch_headroom.py --units <groups>` prints (D-191), all dispatched in one message — collapse the classes so no finding can be claimed by two groups, then the group count is the PARTITION passed as `--units` (D-191, the same collapse rule `/fabrik-review` § Breadth seats states); never a token 1–2 — **AND ≥1 native `fabrik-reviewer` on Opus** (the authoritative pass).<!-- POOL OFF (D-181): the pool breadth layer via `fanout("review", units, repo=…, project="mega-review", mode="read_only")` — the label must MATCH Phase 3's back-fill --> Cover, across finders, the classes THIS artifact can fail — locate each by NAME in the checklist (item numbers drift):
...
Never leave a ledger row whose `confirmed:` is non-zero for the operator to re-invoke. Converge here.
