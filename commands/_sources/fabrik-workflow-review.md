---
description: Force a Traycer-workflow ARTIFACT to converge to a no-op — the shared paired review for the PRODUCER `-fabrik` doers in BOTH folders — ettw `00-trigger`→`06-ticket-breakdown` and mega `00-trigger`/`02-epic-decomposition`/`03-expand-epic-files` (those are the DOER commands; the `type` you pass names their ARTIFACT — see the Phase-0 table). (Not 07/09 — their reviews are the dedicated 08/10; not 11 — a human gate.) Give it the artifact + its type; it fan-outs finders, fixes, and loops until a fresh pass changes nothing. The doer produces; THIS forces convergence (north star § Command-chain build plan, CC1).
---
