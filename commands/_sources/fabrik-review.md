...
- **Breadth seats (native, while the pool is OFF — D-181):** add **ONE native `fabrik-reviewer` seat on Sonnet per independent failure-class group** of the Coverage Checklist — the class list IS the seat count, typically 4–6 on a substantial diff and never a token 1–2 (D-186) — **all dispatched in ONE message** so they run in parallel with the Opus seat; a seat that returns nothing is a FAILED seat (re-dispatch), never a clean round. Nothing records to the flywheel; no `NO-POOL:` is owed.
<!-- POOL OFF (D-181, 2026-09-07) — kept verbatim for re-enable:
