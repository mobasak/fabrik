...
1. **Suggest (pool, multi-model)** — `fanout("review", units=[…], mode="read_only", k=3)` (3 diverse cheap families, code inlined in each unit's task) to each propose the distinct user-observable behaviors; **union** them. A single suggester is the blind spot — diverse families catch what one misses, for cents.
