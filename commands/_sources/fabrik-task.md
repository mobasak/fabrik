...
`handoff` is the fragment's third sanctioned terminal: UPGRADE uses it.
...
author) — or a LANE verdict naming where the work belongs. Read every line; gaps print one
each. **A refusal is the answer, not an obstacle**: take the named lane, no record was opened. On
...
commit. All twelve, each with its counter or its honest lack of one, are (1)–(12) in § Constraints C3
of `docs/superpowers/specs/2026-09-17-fabrik-task-lane-design.md`; read them there, not here.

## Phase 1 — MEASURE

FIX DIRECTIVE step 1: reproduce, attribute, name the ROOT CAUSE as a falsifiable claim. Then
...
`--design` stores that file's TEXT — refusing it over 2,000 characters rather than truncating — so the
...
A declared path inside another lock's `owned_paths` is a STOP; for an active lock with NO `owned_paths`,
so is one appearing in `git diff --name-status -M <lock.baseline_commit> HEAD` on EITHER side of an
`R`/`C` record (`--name-only` hides the source). Then the change and its grader, red-first per FIX
...
the declared files? did it add a mechanism the declaration said `no` to? can you name a second approach
...
5. Commit with explicit pathspecs + provenance trailers, and **in the SAME shell as the commit**:
...
block alone), then closes. ⚠️ **The recorded value is the FIRST WHITESPACE TOKEN after `UPGRADE:`** —
