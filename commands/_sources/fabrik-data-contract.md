---
description: Freeze the project's data contract — the single frozen truth mapping every GUI/form field to its exact DB column (+ type, required, validation, PII class, FK references, enums, dependencies) so parallel agents (Traycer/Kilo, frontend, backend) build against one naming truth and nobody invents or mismatches a field. Three modes: spec-driven (A), reverse-generated from the live schema for existing-project backfill (B), or a fresh stub (C). Self-converges to an edit-free FROZEN file. Sits between /fabrik-spec and /fabrik-plan-after-chat.
