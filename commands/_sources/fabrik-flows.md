...
**Parallelism — UNCONDITIONAL, every run.** Dispatch **one native `general-purpose` seat per persona (or per journey), all in a single message** to walk that persona's path through the spec, the code and any existing flows and return its entry → actions → feedback → exit with the gaps it hit; the persona count is the PARTITION and `dispatch_headroom.py --units <N>` prints the seats (D-191). YOU own the journey judgment, the [PRIMARY PATH] call and every edit — the seats ground, they do not author.
