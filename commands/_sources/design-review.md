...
⚠️ CONVERGENCE — this review is a LOOP, not a one-shot. A design is "done" ONLY when a fresh, demonstrably-thorough design-review pass finds ZERO issues (no Blockers / High-Priority / Medium findings) AND applies zero fixes — a no-op pass. **The pass in which anything was found or fixed is NEVER the converged pass**: the surviving findings must be fixed and the screen RE-REVIEWED, and you keep looping (review → fix → re-review) until one pass comes back empty. Refuting/deferring FRESH findings does not count as empty — a pass that raised N fresh findings is `found: N`, so you owe the next pass; **a re-raise of an already-adjudicated STANDING finding (an accepted UX tradeoff, a deferred nit) is CITED, never counted (D-048)** — counting it made honest termination impossible while the finding stayed true; the report has no disposition ledger, so the citation lives in a `Standing:` line directly under the Pass Ledger (one entry per standing finding + where it was adjudicated — an uncited re-raise has silently vanished, the exact outcome cite-not-count exists to prevent). The report MUST end with a `Converged: yes | no` line, a one-line Pass Ledger (`Pass k — found: N, fixed: M`), and that `Standing:` line whenever a standing finding was re-raised; `Converged: yes` is permitted only when the last row is `found: 0, fixed: 0`. **Record each pass in the run record too** — `python3 scripts/command_run.py round --seats <seats dispatched this pass> --findings <n> --classes-swept <the axes swept> --classes-new <…>` — the report is chat-only, so the round ledger is this loop's only machine-read trace. (When run inside `/fabrik-ui-design`'s Build Verification Loop, that loop drives the re-review to this no-op.) **Context is never a reason to stop:** the harness auto-compacts and the run continues — keep going.
