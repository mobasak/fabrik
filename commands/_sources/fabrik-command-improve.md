...
- a verdict that wants a NEW mechanism, a gate, a hook, or a schema → not a command edit: it is
  ...
  D-row names it — D-315) — say so and open it yourself as this run's `NEXT:`, after the close,
  never a mail; else SPEC/PLAN work (`/fabrik-spec` → `/fabrik-plan-after-chat`) — say so and
  file it.
...
then open it: it is this run's `NEXT:`, ahead of any re-run over another queue.
