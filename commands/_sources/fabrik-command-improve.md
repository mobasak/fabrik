...
- a verdict that wants a NEW mechanism, a gate, a hook, or a schema → not a command edit: it is
  ...
  D-row names it — D-315) — say so, close this run, then OPEN it yourself in the same session,
  never a mail (own-session work is dispatched, not narrated in a `NEXT:` line — `CLAUDE.md`
