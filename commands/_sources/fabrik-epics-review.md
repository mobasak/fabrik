...
- **Breadth** — one native `fabrik-reviewer` seat on Sonnet PER LENS, all dispatched in one message: the lens count is the PARTITION and `dispatch_headroom.py --units <N> --mechanical 0` prints the seats — stamped BEFORE they go out with `python3 scripts/command_run.py dispatch --seats <n>` so sibling sessions subtract them — (D-191 — a lens is a judgement, no Haiku mechanical seat), never a token 1–2. <!-- POOL OFF (D-181): **Pool breadth** — `fanout("review", units, repo=…, project="mega-review", mode="read_only")` picks
