...
- **Browser/device work = native `fabrik-gui` subagents, in PARALLEL** — the seats `python3 /opt/fabrik/scripts/sysadmin/dispatch_headroom.py --units <bundles>` prints: one Sonnet driver per flow-bundle plus that bundle's Haiku mechanical gate and the Opus authoritative re-drive (roles in § Subagents),
...
- **≥2 parallel native `fabrik-gui` subagents for any gauntlet with ≥2 flows** (scale up with the
  ...
  never handed to the mechanical seat — and the Opus authoritative seat (`model: "opus"`, ≥1 per
  gauntlet) re-drives the highest-risk bundle end to end before the verdict. Native is the ONLY option for browser/device legs (the pool has no browser
