...
- `CONVERGED` whose inputs changed (the spec, compose, or code moved under it — it is `DRAFT` in fact),
  OR that `/fabrik-deploy` routed back with a NAMED defect (its `BLOCKED` report is the evidence — e.g.
  a fused credentialed step; the plan itself is defective even though no input moved): flip back and
  converge again. Unchanged inputs AND no routed-back defect → report already-converged, do not re-run.
