...
| 9 | COLD START | the plan NAMES every first-boot init the service needs (DB schema/module init, first-run migrations, seed users) and PROVES what INVOKES it at path:line or runbook step id — an image whose entrypoint scans no init dir plus a registrar that creates an EMPTY DB is a `--wait` hang, not a deploy (live: tryton-crm — the triad modeled init's CONSEQUENCES everywhere and its INVOKER nowhere); `needs_database: true` with no init step requires an explicit "no init required because <reason>" |
| 10 | Capability-check honesty | every OPERATOR-GATE naming a credential carries the proven fleet-ABSENCE evidence (hub .env key names, site-provisioner env, `src/fabrik/drivers/`) — a gate for a credential the fleet holds is a manufactured gate |
...
seat on Sonnet PER UNIT, all in one message** (the unit count is the PARTITION — `dispatch_headroom.py --units <N>` prints the seats, never a token 1–2; D-191)
