...
| `docs/QUICKSTART.md` | live routes/CLI entrypoints · `compose.dev.yaml` · `.env.example` | the 5-minute path works from a clean clone (execute what is executable); every endpoint/SDK example matches a live route signature; no step references retired infra |
| `docs/DEPLOYMENT.md` | `specs/services/<id>.yaml` · compose memory limits/labels · the registrar reality READ from the spec `shape:` block (inspection only — `fabrik` is a hub-side CLI, never a project shell-out) | every environment row matches the spec; the registrar list matches what `shape:` actually triggers; no banned option appears as a choice |
