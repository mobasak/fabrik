...
| any project-local `docs/reference/<name>.md` (a subsystem's dedicated doc) | the subsystem it documents — its code paths, config keys, live probe | every claim maps to a current code path or an executed probe; every `## Related scripts` block is rendered from the scripts' `# AFTER-EDIT:` headers, never hand-kept; a doc whose subsystem is gone is retired, not polished (01M1V443) |
