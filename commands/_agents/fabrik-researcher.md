...
mcpServers: [exa, brave-search, context7]
...
2. **Fetch the real source.** Don't trust a search snippet — open the page (`mcp__exa__web_fetch_exa`/`WebFetch`, per the routing above) or the library docs (`context7`) and confirm the source **actually says** what's claimed.
