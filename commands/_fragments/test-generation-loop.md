...
1. **Suggest (native, 2–3 seats)** — dispatch 2–3 native `general-purpose`/`fabrik-reviewer` subagents (Sonnet; the code under test inlined or pointed at by path) to each propose the distinct user-observable behaviors; **union** them. A single suggester is the blind spot — differently-briefed seats catch what one misses.
