...
**There is NO round ceiling.** While anything is still being raised, the next round is owed — 5, 15, 30 rounds; the loop runs until the exit conditions hold. The ONLY non-quiet stop is a **BLOCKED escalation**: a specific finding that survived **3 consecutive fix attempts** (same test failing three times) → pause THAT finding, post it to the operator (Telegram / report `## BLOCKED` section naming the finding + the 3 attempts), and keep looping on everything else. An agent never self-exits with an "accepted risk" — unresolved means BLOCKED-and-escalated, visible to the operator, not declared away. Re-invoking this command on unchanged code re-adjudicates this SAME checklist — free-hunting outside it must be justified as a new class, which is a rubric gap to report.
