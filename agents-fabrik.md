...
| 12  | **AI Sysadmin**       | `scripts/sysadmin/` — `bot.py` (Telegram FastAPI), `proactive-check.sh` (every 15 min via cron), `daily-digest.sh` (09:XX hash-slotted), `morning-report.sh`, `weekly-security.sh`, `weekly-maintenance.sh`, `monthly-backup-verify.sh`, `detect_reversals.py` (`*/5 min`), `send-telegram.sh`, `system-prompt.txt` + the **quota-governance layer** (`quota_governor.py` router · `claude_broker.py` [dormant until a container consumer] · `incident_context.py` marshaller · the `claude-run.sh` gate) | **Deployed on ALL 3 hosts** (vps1 hub + vps2 + vps3 spokes), each with its OWN per-host ob@ OAuth grant (2026-08-30). **Models:** diagnosis surfaces run Opus 5 (`CLAUDE_SYSADMIN_MODEL`), the morning report Sonnet 5 (`CLAUDE_MORNING_MODEL`) with a zero-Claude RAW fallback on a governor shed — the daily heartbeat never silently dies. **Quota:** the governor sheds routine at ≥80% on every usage window (`QUOTA_RESERVE_PCT` pinned in `.env.sysadmin`); incidents are never blocked; the keepalive is a quota-free `--probe-current` health probe (the `claude -p ping` was retired 2026-08-30). `detect_reversals.py` revived 2026-08-30 (dual-format ts crash had silenced it). CLI auto-update staggered: hub Sun (canary) → spokes Mon. Telegram ↔ Claude Code on each host; on-host `aro-wake.service` (FastAPI :8201 with `/metrics`). Proactive checks include Authelia + GlitchTip + GlitchTip-worker probes (2026-06-17). System prompt instructs LESSONS_LEARNT.md consultation (140+ incidents, ~250KB) before deciding fixes. Peer protocol `consult` verb LIVE since 2026-06-06 (loop-prevention guards). Stage 2 hourly rate-limited (wake slots counted only on real wakes). Runbook: `docs/workstation/vps-claude-quota-governance.md`. |
...
| pgvector / vector search | `fabrik-lib/rag` (pgvector+tsvector+pg_trgm+RRF hybrid) — the LIBRARY exists; the fleet DB does NOT carry the extension | ⚠️ **NOT installed** — probed 2026-09-01 (infra) and re-probed 2026-09-03 (fleet): `postgres-main` runs `postgres:16-alpine`, not `pgvector/pgvector:pg16`, and `select count(*) from pg_available_extensions where name='vector'` returns **0** AND `select count(*) from pg_extension where extname='vector'` returns **0** — so it is neither installed NOR installable on this image (the available-extensions zero is the stronger fact: `CREATE EXTENSION vector` would fail, not merely be un-run). This row read "✅ fully self-hosted" from INTENT since self-hosting; no probe ever verified it. Any project needing vector search must first move postgres-main to the pgvector image (an operator decision — it restarts the shared DB): mail 01M1EHNXT4829615ZARZF9WJP7, `docs/STRATEGIC_BACKLOG.md` § fleet capability claims |
...
5. **Design System** — For any project type with a UI surface (saas-skeleton, static-site, docusaurus, chrome-extension, office-extension, mobile-app, desktop-app), read `.windsurf/rules/core/design-system-template.md` and the project's `docs/design-system.md` (resolved by the ladder in `saas/60-saas-ui.md`) before generating any spec or copy; read `.windsurf/rules/core/ocoron-design-system.md` only when that file declares the Ocoron house identity (D-051). For mobile-app, also read `.windsurf/rules/mobile-app/ocoron-mobile-design-system.md`. Apply color tokens, typography, scaffold-specific adaptations, verbal identity (forbidden language, voice, microcopy rules) to all planning output. State: "Design system read."
...
The packs under `.windsurf/rules/**` ARE the discipline base `[canonical: the packs themselves]`.
...
| Design System Template | `.windsurf/rules/core/design-system-template.md` | The structure and token slots every UI project inherits; the shape of `docs/design-system.md` |
| Ocoron Design System | `.windsurf/rules/core/ocoron-design-system.md` | The Ocoron house brand (visual + verbal) — only for a project that declares it |
