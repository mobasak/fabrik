...
| [test_liveness_audit.py](tests/test_liveness_audit.py) | 53 tests — the liveness auditor's three proofs on fake boxes, its own heartbeat (one `LOG_STAMP`-shaped stderr line, flushed after the report so the cron's merged streams keep it on its own line; only a stamped marker line is evidence) and the dead-output paths (`\| head`, `2>&1 \| head`: exit 0; a full disk: exit 1, never a raise; `>&-`: exit 1; `2>&-`: the stamp is skipped and the report stays pure JSON) |
...
| [test_check_command_corpus.py](tests/test_check_command_corpus.py) | 74 tests — the command-corpus gate's eight predicates on fixtures, plus `--selftest` run as a subprocess — its canaries are the predicates' only grader and nothing else on the box ran them |
...
| [test_check_script_headers.py](tests/enforcement/test_check_script_headers.py) | 31 tests — the `# AFTER-EDIT:` coupling header must parse BOTH separator styles (`a.py, b.md` and `a.py \| b.md`); the pipe form used to parse as files named `\|`, so every `scripts/sysadmin/` script warned on its own edit — invisible until the row was declared advisory and its stdout stopped being discarded. |
