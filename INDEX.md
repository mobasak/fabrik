...
| [test_phase_tests_gate.py](tests/enforcement/test_phase_tests_gate.py) | 15 end-to-end behavior tests for the plan-window gate — throwaway git repos per case: WARN path (red-on-revert proven), test-accompaniment silence, docs-only, no-rows, no-lock, released lock, broken JSON fail-soft, deleted-test regression, absolute-path confinement, sibling-window scoping, scalar-owned_paths, non-dict-lock isolation, Behavior-Contract scoping (both plan forms), utility-script boundary. |
| [test_final_gate_registration.py](tests/enforcement/test_final_gate_registration.py) | Source-structure pins for final_gate.py registrations: check_phase_tests.py stays tier-2-only; the mutation registration stays inside the FABRIK_MUTMUT pop/finally-restore guard (both shipped regressions, review-caught). |
