...
| [test_liveness_audit.py](tests/test_liveness_audit.py) | 49 tests — the liveness auditor's three proofs on fake boxes, its own heartbeat (one `LOG_STAMP`-shaped stderr line, flushed after the report so the cron's merged streams keep it on its own line; only a stamped marker line is evidence) and the dead-output paths (`\| head`, `2>&1 \| head`, a full disk: never raises, exit 0, the stamp still lands wherever stderr works) |
