...
| [install_user_hooks.py](scripts/sysadmin/install_user_hooks.py) | Installs / verifies (`--check`, a gate row) the three user-level hook entries in the canonical `~/.claude/settings.json` and every `~/.claude-fleet/*/settings.json`; idempotent, registration timeout 10 s (above the gate's inner 8 s). Review B10: the entries had no creator, verifier or restorer. |
...
| [test_selfwatch_check.py](tests/test_selfwatch_check.py) | 5 graders: unarmed → order with sid; held lock → silent; stale lock file → unarmed; /tmp, headless, sid-less → silent; `source=compact` NOT exempt. |
| [test_user_hook_gate.py](tests/test_user_hook_gate.py) | 5 graders: runs when the project does not wire the hook; defers when it does; no settings → runs; exit code passes through; unreadable settings fail open to running. |
...
| [test_stop_hook_quota_hold_exemption.py](tests/test_stop_hook_quota_hold_exemption.py) | 2 graders: the Stop hook blocks normally with no hold (control) and ALLOWS the stop while the fleet-exhausted stamp stands. |
