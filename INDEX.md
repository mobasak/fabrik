...
| [test_external_services_chain.py](tests/test_external_services_chain.py) | 53 tests — the external-services chain is ONE script with a gated heartbeat: `external_services_chain.sh`'s five steps in order (classify bounded, registry_sync with credits, gen_dashboard only on chain success), both entry points run it and inline no step, the `external-services-chain` liveness surface, no second entry point, and `gen_dashboard --help` writing no file (the 2026-09-02 defect) |
| [test_liveness_audit.py](tests/test_liveness_audit.py) | 69 tests — the liveness auditor's three proofs on fake boxes, its own heartbeat (one `LOG_STAMP`-shaped stderr line, flushed after the report so the cron's merged streams keep it on its own line; only a stamped marker line is evidence) and the dead-output paths (`\| head`, `2>&1 \| head`: exit 0; a full disk: exit 1, never a raise; `>&-`: exit 1; `2>&-`: the stamp is skipped and the report stays pure JSON) |
...
| [test_check_command_corpus.py](tests/test_check_command_corpus.py) | 120 tests — the command-corpus gate's eight predicates on fixtures, plus `--selftest` run as a subprocess — its canaries are the predicates' only grader and nothing else on the box ran them |
