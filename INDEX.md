...
- `commands/_sources/fabrik-rivals.md` - Source for **`/fabrik-rivals`** (stage `1-design`, before `/fabrik-spec`): competitive evidence — finds rivals, mines their reviews, builds the feature MATRIX, then the ranked MATCH (what they have that we lack) and BEAT (their review-proven weaknesses) lists, plus a pricing wedge and white-space. Two modes by repo identity (PROJECT briefs, HUB runs). NOT market-sizing.
- `scripts/rivals_run.py` - Hub-side driver for `/fabrik-rivals`: wires ONE `Deps(...)` for the vendored `competitor-intel` engine and pre-flights every wiring trap BEFORE spending (the engine never raises for money/staging, so a wiring bug otherwise yields a dossier that LOOKS complete). LLM is subscription `claude -p` from a neutral cwd — never a metered API, never an agent. `--free-legs-only` for a zero-marginal-cost scan.
