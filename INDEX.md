...
| [test_phase_tests_gate.py](tests/enforcement/test_phase_tests_gate.py) | 9 end-to-end behavior tests for the plan-window gate — throwaway git repos per case: WARN path (red-on-revert proven), test-accompaniment silence, docs-only, no-rows, no-lock, released lock, broken JSON fail-soft, deleted-test regression, absolute-path confinement. |
