...
| [test_quota_dashboard.py](tests/test_quota_dashboard.py) | 34 tests — the box-local quota dashboard (`scripts/sysadmin/quota_dashboard.py`), its three load-bearing risks: the regeneration floor bounds probe volume, a failed probe never blanks the board, the board shows REMAINING (an inversion would read as plenty when there is none) |
