...
| [test_liveness_audit.py](tests/test_liveness_audit.py) | 51 tests — the liveness auditor's three proofs on fake boxes, its own heartbeat (one `LOG_STAMP`-shaped stderr line, flushed after the report so the cron's merged streams keep it on its own line; only a stamped marker line is evidence) and the dead-output paths (`\| head`, `2>&1 \| head`: exit 0; a full disk: exit 1, never a raise; `2>&-`: the report stays pure JSON — the stamp lands wherever stderr works) |
