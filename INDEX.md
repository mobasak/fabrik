...
| [check_commit_trailers.py](scripts/check_commit_trailers.py) | **commit-msg** hook (hub-local, not synced) rejecting a commit whose `Agent-Role:` trailer git cannot parse — the last point the message is still editable, since a pushed bad block needs a force-push. Delegates to `git interpret-trailers --parse`. Tests: `scripts/kilo-benchmarks/tests/test_commit_trailer_guard.py`. |
