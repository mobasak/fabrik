...
| [_check_refresh_before_ranker.py](scripts/enforcement/_check_refresh_before_ranker.py) | Phase-C gate: asserts `daily_refresh.sh` rebuilds the cost sidecar (`claude_p_cost.py --refresh`) BEFORE it regenerates the subagent ranking — wired the other way the selection doc publishes yesterday's rate for a full cycle. Matches only real `_step` lines, never comments. |
