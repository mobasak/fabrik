...
- `scripts/kilo-benchmarks/derive_quality_v2.py` - Multi-signal quality_tier deriver. Combines Chatbot Arena ELO + Terminal Bench + weighted_coding + OpenRouter's design_arena ELOs + Artificial Analysis index + model family pattern + cost proxy + reasoning + context-length boosts. Without it, ~85% of models default to T1 and the category selector drops frontier models that lack public benchmarks. Replaces the old `migrate_selector_columns.derive_quality_tier()` arena+tbench-only logic.
