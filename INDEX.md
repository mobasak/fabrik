...
| [check_governance_tables.py](scripts/enforcement/check_governance_tables.py) | Tier-2 gate (ADVISORY): every markdown table row in the governance contracts renders at its header's width — an unescaped pipe inside a code span makes GFM discard the overflow cells, which silently deletes the RULE that row carries for every rendered reader while the raw file still reads fine (measured 2026-09-10: three shapes of the denominator HARD STOP invisible since 2026-09-03, in both fleet-synced copies) |
