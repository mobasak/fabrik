...
- `/opt/ai-model-catalog/engine/claude_p.py` - Single-shot `claude -p` dispatch shim (VENDOR+ENHANCE claude-evaluator) that lets the native Claude Max tiers be scored as first-class review/code candidates beside the OpenRouter pool. `claude_p_call(model, prompt, *, system="", timeout) -> (text, usage)` shells `npx @anthropic-ai/claude-code --print --output-format json --model <alias> [--system-prompt]` (stdin prompt, fail-closed on non-zero exit), capturing the snake_case `usage` block (`input_tokens`/`output_tokens`/`cache_read_input_tokens`/`cache_creation_input_tokens`) `_call_cli` discards under `text`. NO tools, single turn, `--system-prompt` omitted when empty — transport parity with the OR single completion (methodology rides in the prompt). `ALIASES` maps `claude-code/{opus,sonnet,haiku,fable}` → the live model ids. Test: `tests/test_claude_p.py`. Landed 2026-07-20 as plan-2.
...
- `/opt/ai-model-catalog/engine/derive_quality_v2.py` - Multi-signal quality_tier deriver. 9 signals: arena + tbench + weighted_coding + design_arena_avg + AA index (OR-embedded) + AA scraped (own leaderboard) + SWE-bench Verified % + Aider Polyglot % + design_arena coding-only ELO + family regex + cost proxy + reasoning + context. Without it, ~85% of models default to T1.
- `/opt/ai-model-catalog/engine/scrape_coding_benchmarks.py` - Mines three public coding leaderboards (SWE-bench Verified swebench.com, Aider Polyglot aider.chat, OpenRouter design_arena coding categories) into agents.swe_bench_verified_pct / aider_polyglot_pct / design_arena_coding_elo. No inference cost. Permissive canonical-name matching across word-order/date-suffix/agent-prefix variants.
