...
| [command-loop-performance.md](docs/reference/command-loop-performance.md) | The command loop's MEASURED performance — the 14-aim contract the operator holds the corpus to, the 803 h / 43 G two-week cost with the 10.2 h spec-chain median, why a month of convergence fixes moved per-run time but not the total (corpus +111%, CLAUDE.md +232%, tokens +10%), the two causes, and the five-item program. DRAFT — § 5 awaits the operator's ruling |
...
| [private_index_commit.py](scripts/private_index_commit.py) | The shared-append commit recipe as a script: HEAD's blob + your hunk after an anchor in a PRIVATE index, CAS on the base SHA with a bounded retry (working file three-way-merged up to the new base), blob/mode/ref assertions, shared-index realignment, carry by INSERT; refuses a message without trailers and a shared-append file passed as `--own`; prints the `--force` sync notice on a trigger path (D-328) |
| [test_private_index_commit.py](tests/test_private_index_commit.py) | Red-first graders for `scripts/private_index_commit.py`: a sibling commit in the CAS window, a sibling's staged blob, a lost exec bit, a missing anchor, a message without trailers, a sync-trigger path, the CLI — the 5b merge proven by mutant |
