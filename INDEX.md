...
| [tests/enforcement/test_check_governance_tables.py](tests/enforcement/test_check_governance_tables.py) | Behavior contract for the governance-table guard — the load-bearing test is that a FINDING must exit 0 (`warn_only` means no failing exit path, and a warn_only check exiting non-zero is promoted to a gate failure in every synced repo), plus the three GFM boundaries measured with markdown_it: a 3-space-indented table, a fenced example, and a pipe in the HEADER row |
