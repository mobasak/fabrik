...
| [check_commit_trailers.py](scripts/check_commit_trailers.py) | **commit-msg** hook (hub-local, not synced) rejecting a commit whose `Agent-Role:` trailer git cannot parse — the last point the message is still editable, since a pushed bad block needs a force-push. Delegates to `git interpret-trailers --parse`. Tests: `engine/tests/test_commit_trailer_guard.py (ai-model-catalog)`. |
...
- `engine/derive_quality_v2.py` - Multi-signal quality_tier deriver. 9 signals: arena + tbench + weighted_coding + design_arena_avg + AA index (OR-embedded) + AA scraped (own leaderboard) + SWE-bench Verified % + Aider Polyglot % + design_arena coding-only ELO + family regex + cost proxy + reasoning + context. Without it, ~85% of models default to T1.
- `engine/scrape_coding_benchmarks.py` - Mines three public coding leaderboards (SWE-bench Verified swebench.com, Aider Polyglot aider.chat, OpenRouter design_arena coding categories) into agents.swe_bench_verified_pct / aider_polyglot_pct / design_arena_coding_elo. No inference cost. Permissive canonical-name matching across word-order/date-suffix/agent-prefix variants.
