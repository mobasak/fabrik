...
| [test_check_command_corpus.py](tests/test_check_command_corpus.py) | 53 tests — the command-corpus gate's eight predicates on fixtures, plus `--selftest` run as a subprocess — its canaries are the predicates' only grader and nothing else on the box ran them |
...
| [test_check_script_headers.py](tests/enforcement/test_check_script_headers.py) | 13 tests — the `# AFTER-EDIT:` coupling header must parse BOTH separator styles (`a.py, b.md` and `a.py | b.md`); the pipe form used to parse as files named `|`, so every `scripts/sysadmin/` script warned on its own edit — invisible until the row was declared advisory and its stdout stopped being discarded. |
