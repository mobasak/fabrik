...
| [test_render_chat_history.py](tests/test_render_chat_history.py) | 14 red-first graders for render_chat_history — same-label and relabel collision guards, non-dict sidecars, an unreadable transcript skipped with WARN, dated compaction headings + the reload marker, tool-result/system-reminder stripping, persisted session names, incremental skip, newest-first INDEX, named error on an unknown project, repo path → project key |
