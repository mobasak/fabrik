...
| [test_check_command_corpus.py](tests/test_check_command_corpus.py) | 75 tests — the command-corpus gate's eight predicates on fixtures, plus `--selftest` run as a subprocess — its canaries are the predicates' only grader and nothing else on the box ran them |
