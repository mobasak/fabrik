...
- `scripts/claude_usage_daily.json` - OUR daily per-model token store, merged forward from the Claude Manager extension's `usage-history.json` by `claude_p_cost.merge_usage_store()`. Raw per-model tokens (never tiered totals, so history can be re-weighted); the first merge was the 111-day backfill. ⚠️ This is the copy that OUTLIVES the extension — once it is uninstalled nothing writes the upstream file and this store is the only history there is
