...
| [final_gate_stop.py](.claude/hooks/final_gate_stop.py) | Claude Code `Stop` hook — blocks end-of-turn when the worktree is dirty and `final_gate --lean` is red (in-session "definition of done"). Fail-open, loop-capped, scoped. Wired in `.claude/settings.json`. |
| [test_final_gate_stop_hook.py](tests/test_final_gate_stop_hook.py) | 5 tests for the Stop hook `decide()` loop-guard (clean→allow, green→allow, red→block to cap, over-cap→allow_warn). |
