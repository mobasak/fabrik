...
| [test_mail.py](tests/test_mail.py) | 19 behavior-contract tests for `scripts/mail.py` — watched-fail-first (red-on-revert proven) on the ★ risky invariants (ULID lexical sort, O_EXCL publish collision, star-topology refusal) + recipient/secret/size/ack/requeue/digest/malformed coverage. All real-filesystem (tmp roots via `FABRIK_MAIL_ROOT`/`FABRIK_OPT_ROOT`). |
