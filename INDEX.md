...
| [test_agent_memory.py](tests/test_agent_memory.py) | graders for the workstation RAM policy — the swapoff/swapon asymmetry (swapon -a reads only fstab, which has no swap entry here), the fail-closed ENOMEM guard, install over a truncated conf, and the skip-vs-critical exit contract the cron heartbeat depends on. Drives the real script with sudo/pgrep/sysctl stubbed. |
