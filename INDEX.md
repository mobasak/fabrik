...
- `scripts/kilo-benchmarks/audit_ui_values.py` - Systematic 5-phase audit that cross-checks every value the browser surfaces against its live upstream (OR /api/v1/models, OR /endpoints, Kilo CLI, benchmark caches). Reports drifts by field-type + verifier-tracked vs verifier-untracked bucket. Wired into `daily_refresh.sh` so any future upstream drift trips the log immediately instead of accumulating.
