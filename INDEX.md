...
- `scripts/kilo-benchmarks/daily_refresh.sh` - Cron-installed (`0 6 * * *` UTC) wrapper that runs the full daily chain: verifier → classifier → route mapper → pack markdown export → models_browser regen. Cron-safe (no PATH assumptions, no shell-activity dependency).
