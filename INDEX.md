...
| [test_sidecar_staleness.py](tests/test_sidecar_staleness.py) | Phase B's guard: the ② rate renders with the window it came from, and a build older than the 24h rebuild cadence is marked STALE — while a MISSING sidecar still renders nothing at all, which is a different state |
