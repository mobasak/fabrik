...
│   ├── kaizen.md                   # The weekly kaizen loop: the wake-proof weekly MEASUREMENT (kaizen_metrics.py via weekly_catchup.sh hourly stamp check — no agent, no quota) vs the agent's ≤90-min ANALYSIS pass; which of the 5 pinned metrics are real vs `—` and why; ISO-week idempotence; the fabrik-mail hand-off
