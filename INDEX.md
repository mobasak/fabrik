...
| [test_phase_tests_gate.py](tests/enforcement/test_phase_tests_gate.py) | 20 end-to-end behavior tests for the plan-window gate — throwaway git repos per case: WARN path (red-on-revert proven), accompaniment boundaries (code tests vs test-adjacent conftest/fixtures vs utility scripts), docs-only, no-rows, no-lock, released lock, broken/hostile-lock fault isolation, deleted-test regression, absolute-path confinement, sibling-window scoping, Behavior-Contract scoping (heading + bold-label forms, nested dedupe). |
