...
For multi-tenant projects using `postgres-main` RLS with `tenant_id` (RLS is owned by `fabrik-lib/fastapi-user-auth`: native mode — the default — scopes by `current_tenant_id()` over `app.tenant_id`; compat mode's `auth.uid()` reads `request.jwt.claims`). The required probe and the test-role assertion (neither superuser nor `BYPASSRLS`) are in `saas/95-multi-tenant-saas.md` § Hardening and the Cross-Tenant Probe. Legacy Supabase-RLS projects run the same tests unchanged after migrating to `postgres-main` — see `agents-fabrik.md § Supabase`:
...
| Destructive test (DROP SCHEMA/TABLE, TRUNCATE, migration re-apply) connecting unguarded | Call `require_throwaway(url)` (scaffold-emitted `tests/conftest.py`) FIRST — an UNSCAFFOLDED module (fabrik-lib) has no such helper and copies the rule as prose, so here is the predicate to copy VERBATIM, anchored: `dbname = urlparse(url).path.lstrip('/'); assert dbname.lower().endswith(('_test', 'throwaway', 'scratch')), dbname` — an unanchored `'test' in dbname` let `invoicing`, `pricing`, `latest_app`, `contest` be DROPped (fabrik-lib payments, 2026-09-02 — 01M1GQC1) — fail-closed: only DB names ending `_test`/`throwaway`/`scratch` (the helper folds CASE and the copied predicate must too — `scaffold.py::_DISPOSABLE_NAME` carries `re.IGNORECASE` because Postgres folds an unquoted name, so `CREATE DATABASE MY_TEST` gives you `my_test`; `test_guard_marker_is_case_insensitive` pins it. A bare `endswith` is case-SENSITIVE and would reject `MY_TEST` while the scaffolded helper accepts it — same direction, still fail-closed, but the two no longer say the same thing, which is why the predicate above carries `.lower()`. CI uses `ci_test`), or `CI=true` **against a localhost DB**, may proceed; a mispointed URL errors instead of wiping a dev DB |
