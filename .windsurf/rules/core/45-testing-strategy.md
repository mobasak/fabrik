...
| Destructive test (DROP SCHEMA/TABLE, TRUNCATE, migration re-apply) connecting unguarded | Call `require_throwaway(url)` (scaffold-emitted `tests/conftest.py`) FIRST — an UNSCAFFOLDED module (fabrik-lib) has no such helper and copies the rule as prose, so here is the predicate to copy VERBATIM, anchored: `dbname = urlparse(url).path.lstrip('/'); assert re.fullmatch(r'.*(_test\|throwaway\|scratch)', dbname), dbname` — an unanchored `'test' in dbname` let `invoicing`, `pricing`, `latest_app`, `contest` be DROPped (fabrik-lib payments, 2026-09-02 — 01M1GQC1) — fail-closed: only DB names ending `_test`/`throwaway`/`scratch` (case-insensitive; CI uses `ci_test`), or `CI=true` **against a localhost DB**, may proceed; a mispointed URL errors instead of wiping a dev DB |
