...
**No always-on price cap (removed 2026-07-19).** Per-run task cost is pennies regardless of $/Mtok (a review/spec/research fan-out especially), so the former blanket ≤$1.5/Mtok auto-cap wrongly excluded the flywheel's top-ranked models — it is gone. Price is now **opt-in**: pass `max_cost_per_mtok=` to `pick_models`/`fanout` *only* when you want a hard budget ceiling for a run; `allow_above_cap=` is a retained **no-op**.
