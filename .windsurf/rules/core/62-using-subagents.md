...
**Select with `pick_models(task_type, max_cost_per_mtok=1.2, …)` — never hand-pick.** It returns the flywheel-ranked pool best-first; take the cheapest that clears your bar. What enforces the cap today: the module's price table is **cost-vetted** (only ≤$1.5 models are priced/rankable) AND `max_cost_per_mtok` is a **fail-closed ceiling** (`select.py` drops any unpriced/over-ceiling model a refreshed doc surfaced). So **always pass the ceiling** (`=1.2`, or `=1.5` for the fleet cap) — then the cap holds regardless of module version or what the synced doc contains. **The one named default is `minimax/minimax-m3`** (best overall / best reviewer — review/plan/docs/research); for code, take the cheapest `pick_models("code")` returns. **No `anthropic/*` via the pool** — Claude is subscription-native; use the `fabrik-reviewer` Claude Code subagent, not the pool. Don't invent OR IDs — verify the ID exists.
