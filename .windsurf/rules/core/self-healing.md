...
- [`55-observability`](55-observability.md) — sees the symptom (alerts, logs, traces). Doesn't act.
...
| 8 | **Webhook delivery failure** (outgoing webhook >5 consecutive failures) | webhook `delivery_status='failed'` chain | `webhooks/` module auto-disables the subscription after the configured failure threshold ([fabrik-lib README](../../../../opt/fabrik-lib/README.md) `webhooks/` row) | Watchdog Tier C → operator (auto-disable is the destructive step; we don't go further) | n/a |
| 9 | **Stuck row locks** (application-level lock held >max_age_sec) | `locked_at` column past `now() − interval` in workers' queue table | Worker's own orphan-sweep task ([75-workers-jobs § Orphan Sweep](75-workers-jobs.md)) | Watchdog Tier A `rotate_locks` (bounded by `max_age_sec` ∈ [30, 86400]) | Watchdog Tier C → operator; investigate why workers crash mid-lock |
...
- **Tier A (autonomous, default):** rows 1, 2, 5, 7, 9 use `restart_container`, `drop_queue_items`, `pause_worker`, `rotate_locks` respectively. No operator approval, no opt-in flag.
...
4. **Self-healing without observability.** A pause flag, breaker, or rate-limit reject that doesn't increment a Prometheus counter and emit a structured log line is invisible — when it misfires, you can't tell. Every ladder step MUST emit a counter ([55-observability § Metrics](55-observability.md)) AND a `structlog.info()` row carrying the resource name + reason.
...
- Visibility (alerts, traces, dashboards): [`55-observability`](55-observability.md)
- Vendor modules cited: [`pause-state/`](../../../../opt/fabrik-lib/pause-state/), [`async-http-client/`](../../../../opt/fabrik-lib/async-http-client/), [`abuse-prevention/`](../../../../opt/fabrik-lib/abuse-prevention/), [`webhooks/`](../../../../opt/fabrik-lib/webhooks/)
