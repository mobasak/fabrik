...
- Chunk size: `66-rag-chunking.md` is the authoritative source — **512–800 tokens** for a vector-embedding pipeline (the module's `chunk_text()` default), **300–500** for pure keyword retrieval, **1,200 hard max**, **120 minimum**, **10–20% overlap**; `chunk_text(target_min=…, target_max=…)` is where a project moves off the default, with a measurement.
