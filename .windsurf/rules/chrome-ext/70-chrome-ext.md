...
- **Use `optional_permissions`** for features the user may not need. Request at runtime with `chrome.permissions.request()` — just-in-time, with priming (explain why before the prompt). For the runtime-permission UX, use the fregante bloc: **`webext-permission-toggle`** (v7.0.0 — a context-menu toggle for optional host permissions), **`webext-dynamic-content-scripts`** (auto-register content scripts on newly-granted hosts), and **`webext-permissions`** (declared-vs-granted diffing). Don't hand-roll the grant→inject wiring.
...
- **Typed settings/preferences via `@wxt-dev/storage`** (`defineItem` with types + versioned `migrations`) — the first-party option under WXT. Do **not** add `webext-options-sync` (redundant under WXT).
