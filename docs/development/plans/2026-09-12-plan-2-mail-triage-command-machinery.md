...
| 01M20K4FK6TKPDF529Q1HQ2N4P | trade-intelligence | the bounded-search rule names no canonical denominator recipe; three seats produced 1,275 / 1,832 / 72,768 for one question | OPEN — no pack or fragment names `.claude/worktrees` (`command grep -rn` over `.windsurf/rules/` + `commands/_fragments/` → 0); the two CLAUDE.md contracts carry it only as the scratch-sweep clause (3 occurrences); the one exclusion recipe on disk is `commands/_sources/fabrik-review.md:129` (`.claude/worktrees/**` + `.tmp/**` for a review's own grep) — no CLAUDE.md-level denominator recipe exists | T11.4 a denominator recipe in CLAUDE.md's HARD-STOP row (tracked set + uncommitted siblings; the implied exclusions) that EXTENDS `fabrik-review.md:129`'s exclusion list by citation — never a second copy — cited from `subagents-core.md`'s brief text |
