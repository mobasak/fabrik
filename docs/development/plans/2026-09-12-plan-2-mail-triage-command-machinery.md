...
| 01M20K4FK6TKPDF529Q1HQ2N4P | trade-intelligence | the bounded-search rule names no canonical denominator recipe; three seats produced 1,275 / 1,832 / 72,768 for one question | OPEN — no pack or fragment names `.claude/worktrees` (`command grep -rn` over `.windsurf/rules/` + `commands/_fragments/` → 0); across the six roots `commands/`, `.windsurf/`, `CLAUDE.md`, `templates/governance/CLAUDE.md`, `docs/reference/`, `README.md` the string has 9 hits — 2 scratch-sweep clauses, 2 launch texts, 4 descriptions, and ONE exclusion recipe: `commands/_sources/fabrik-review.md` § the stale-copies exclusion (`.claude/worktrees/**` + `.tmp/**` for a review's own grep; :129 today, re-derived at execution because Phase A edits that file above it) — no CLAUDE.md-level denominator recipe exists | T11.4 a denominator recipe in the HARD-STOP row of BOTH CLAUDE.md files, byte-identical (tracked set + uncommitted siblings; the implied exclusions), that EXTENDS the stale-copies exclusion by an absolute anchor — `/opt/fabrik/commands/_sources/fabrik-review.md` § the stale-copies exclusion, T11.2's absolute-path shape — never a second copy, never a line number; cited from `subagents-core.md`'s brief text |
...
**Steps (the gate).** 6. `python3 scripts/enforcement/check_rule_grounding.py --root .` and `check_citations_resolve.py --changed <the three packs and both CLAUDE.md files>` green; `command grep -n` for each new phrase in its own target (the three packs; both CLAUDE.md files for T11.4, byte-identical; `review_rubric.py` or core/45 for T11.5) → exactly one hit per target; `final_gate.py --check --json` success; `sync_enforcement_to_projects.py --dry-run` lists the three packs and `templates/governance/CLAUDE.md`; `python3 -m pytest tests/enforcement/test_check_citations_resolve.py -q` green; `/fabrik-review-scoped` over the phase.

**Behavior Contract.** **Given** the five T11 edits (three packs, both CLAUDE.md files, the rubric or core/45), **When** each target is grepped, **Then** each phrase is present once at its target and the fleet sync carries the synced ones (dry-run then force at Finish).
