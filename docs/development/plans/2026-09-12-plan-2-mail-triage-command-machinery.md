...
|---|---|---|---|
| T13.1 | 01M1S2B0S, 01M1S2DQM, 01M1S2P9D, 01M1SN267, 01M1WD26H | **DONE 2026-09-15** — FIXED — `git commit` is admitted under the hold since 20c3b557 (`_GIT_VERB_FLAGS["commit"]` = a POSITIVE flag set, `--file` included); trailers under the hold FIXED per 01M1YR362; this session committed and pushed under the 2026-09-12 hold | reply + ack; the held-subagent parent signal (01M1S2B0S) → T9 backlog row |
| T13.2 | 01M1S6CWX, 01M1VN1D7, 01M1XJ3XT | **DONE 2026-09-15** — two halves REFUTED at HEAD (W11 landed 2026-09-07; duplicate arms re-measured 0 of 7, was 11 of 15), the transcript-age ceiling ROUTED (box-local file, read-only rule); baseline `174 ok, 1 fail` (A0a) recorded and matches hooks-index.md:113. Was: OPEN — the flock dir `/tmp/claude-sound-locks-$(id -u)` (selfwatch.sh:18) is reapable so duplicate arms pass; watchers have no "my pane is gone" exit; the mesh harness was red at A0a on 2026-09-07 (one run) | lock under a non-reaped dir (`~/.claude/state/`); a transcript-age ceiling backstop; run `claude-mesh-test.sh` once and record the baseline before touching either |
| T13.3 | **DONE 2026-09-15** (8c9642d3) | 01M1VRVWS, 01M1VVNWY, 01M1VVX03, 01M1VXH6B, 01M1VWJWX, 01M1W6SS5, 01M1YB2AK, 01M21JAET, 01M28YN1F, 01M25Y93RB | OPEN — the sixth cause (T5.1–T5.3); plus `_this_sessions_edits` (:716) not holding on a 5.7-day resumed transcript and the block naming no file — reproduced on the hub 2026-09-12 16:2x: this session's sixth cause counted 20 code files last edited 2026-06-04…06-16 (the transcript's pre-compaction history) as uncovered while a `/fabrik-plan-review` record with 55 covered windows was live | T5.1–T5.3 + T5.4: the floor mandatory with a bounded fallback; the block NAMES three files |
| T13.4 | 01M20E1QN | **DONE 2026-09-15** (8c9642d3) — OPEN was — the UNPUSHED cause measures `origin/main..HEAD` and says "push YOUR work" — on a shared tree it orders a sibling's commit published | T5.5: fire only on commits this session authored (the record's session id, the Agent-Context trailer) |
| T13.5 | 01M23HB2M, 01M23K7XT | **DONE 2026-09-15** (8c9642d3) — OPEN was — `mail_notify.py` and `mcp_watch.py` carry no headless guard (grep FABRIK_HEADLESS → 0); `rivals_run.py:_make_llm` spawns a full-turn `claude -p` | the dispatcher sets `FABRIK_HEADLESS=1`; the two advisory hooks stand down on it; rivals' spawn routed through llm-dispatch with the bounded flags (intel's rivals beat informed) |
| T13.6 | 01M1S5DGF | **DONE 2026-09-15** — backlog row written; RECORDED — `isolation: "worktree"` cuts from origin/main (harness) | T9 backlog row |
| T13.7 | 01M280CV7 | **DONE 2026-09-15** — acked; FIXED since — records nest (`command_run.py:1836/:1867` build the child envelope with the parent parked in `stack`) | reply + ack |
| T13.8 | 01M1V59MK, 01M1V597C | **DONE 2026-09-15** — PARTLY REFUTED: 2 of 4 present (both git-config items live), the two file artifacts routed to fabrik-lib; replied + acked. Was: OPEN — fabrik-lib is sync-EXCLUDED, so the four multi-agent adoption artifacts never reach it (the four artifacts — `.worktreeinclude` (synced from `templates/governance/.worktreeinclude`, manifest :114), the `.gitignore` `.claude/worktrees` line (the generated Fabrik-synced block), the `.claude/settings.json` worktree block and the `rerere.enabled`/`push.autoSetupRemote` git config (the model doc's adoption recipe) — ride the governance sync and the adoption recipe, both infra's: `infra.md:15` names the sync surfaces, `infra.md:9` the box mesh); re-routed from fleet to infra | reply: a one-time hand-vendor of the four artifacts (the recipe in `docs/reference/multi-agent-operating-model.md`), or a line in fabrik-lib's CLAUDE.md stating the deliberate exclusion |
| T13.9 | 01M1Z0YKQ | **DONE 2026-09-15** — all four questions ruled, replied + acked. Was: OPEN — wef2's four migration questions (2026-09-08), unanswered | reply from the model doc + D-196/D-198 (see § Out of Scope) |
...
- **Phase B — DONE 2026-09-14, its review CLOSED ON THE SCOPE-GROWTH STOP (D-252), not converged.** T3 (`command_run.py`) and T4 (the convergence/coverage/hygiene/plan-tickets/hooks-index/doc-index graders, the rubric and the gate) landed across 24 commits, `bb995e60` → `a86950e2`: the Phase B change itself plus twenty review rounds, every confirmed finding fixed with its grader in the same round. Receipt `docs/development/reviews/2026-09-12-plan-2-mail-triage-phase-B-review.md`, `Status: IN-PROGRESS` by rule — confirmed 43 · 28 · 20 · 24 · 17 · 20 · 13 · 10 · 7 · 9 · 9 · 7 · 10 · 5 · 8 · 6 · 5 · 7 · 4 · 2.
  - **Why there is no round 21.** The Phase B CODE has been quiet since round 13. Rounds 14–20 confirmed only defects inside the review's OWN fix prose, with a falling count (7 · 4 · 2) that the stall circuit-breaker is structurally blind to — it needs three rounds of non-decreasing, nonzero `confirmed`. The counted stop built for exactly this (D-252, `scripts/command_run.py::scope_growth_warning`, receipt `docs/development/reviews/2026-09-14-scope-growth-stop-review.md`) was run against rounds 19 and 20 — `4/4 → 2/2`, `own_fix == confirmed > 0` twice — and FIRED; the control with round 20's findings placed outside the previous fix stays silent. Its exit is: stop, route the remaining own-fix work to a backlog row with a named destination, close on the original delta's state. That state is `a86950e2`, fully fixed.
  - **Evidence at the close, re-run in the closing turn:** the 145-mutant battery on a fresh `__pycache__`-free copy of the live tree — `145 mutants, killed 145, survived 0, other 0`, no anchor miss; `final_gate.py --check --json` `"status": "success"`, 64 passed, 0 failed, `skipped_checks: ["pytest"]`; `check_review_coverage.py --root .` 0 unproven claims; `check_review_hygiene.py --surface` over both changed sources 0 hits — and over the receipt 7 ADVISORY `dual-verdict` hits, which the receipt's § CLOSE measures, explains as false positives of that advisory's word-count rule, and uses to correct two earlier Pass rows that claimed 0.
  - **Residuals to the backlog at Finish:** § Residual's 115 RECORDED rows with their named destinations, plus the `dual-verdict` advisory class above. Two findings were filed to the infra beat during the phase: `01M2FYPCT64F5XAWCQ8KPGXSAZ` (a paused run record ages past the 12h stale bound and its reviewed work then reads as UNREVIEWED SPONTANEOUS WORK) and `01M2G0TAWQJF0D3E7KRYF85YCB` (a hub working `CHANGELOG.md` missing four committed entries).
- **Phase C — DONE 2026-09-14.** T5 (`.claude/hooks/final_gate_stop.py`, the sixth cause) and T6 (both CLAUDE.md contracts), every claim EXECUTED before it was written.
  - **T5.1 (01M288YHD, 01M25Y93RB, 01M1YB2AK) — a parked parent's window was invisible.** `command_run.py::start` parks the running parent on `stack` and gives the nested child an EMPTY `covered` ledger on purpose (copying it down and joining it back up doubled the ledger per nest cycle); the hook never read `stack`, so for the whole life of a nested `/fabrik-review` both the parent's live window AND every window the session had already closed were gone. Measured: an edit at T0+60 under a parent started at T0 counted 1 unreviewed once a child started at T0+120, and 0 with the same parent live. `_review_windows` now walks the flat frame list; a frame's RUNNING window rides the live record's freshness, its CLOSED pairs survive staleness.
  - **T5.2 (01M28YN1F) — the cause fired on the files the running review was reviewing.** A review's window opens at its `start`, so the code it exists to review is outside every window until it closes `done` and the reach-back applies; the reach-back cannot move earlier (F1: a `blocked`/`handoff` close must not certify the gap). A RUNNING review-family record now exempts BY NAME the paths its `--surface` spells — equality on a whole token, never a prefix, `./` and a trailing `:12`/`::node` stripped. `_REVIEW_FAMILY` is bound to `command_run.py::REVIEW_FAMILY` by a parity grader, as `_CLOSED_STATES` is, rather than becoming a fifth hand-kept copy.
  - **T5.3 (01M21JAET) — verified against 28ca7443 and still open.** That commit closed the PRE-LEDGER half and is transitional by construction. The other half is not: with an EMPTY ledger the writer's reach-back has nothing to reach back to and falls back to the review's own start, so a session interrupted before any command closed can never cover the work preceding its first review — reproduced at 1 unreviewed file, unchanged after a clean `done`. `_first_review_base_case` supplies the missing base case, covering back to the session floor (which exists only on the hook side). Narrow: a window ADDED not a pair rewritten, `done` only, review-family only, stands down the moment any ledger window predates the record's start. RECORDED with destination `docs/STRATEGIC_BACKLOG.md`: a NESTED review pops into its caller before the next stop, so this side cannot identify the joined pair — closing that means the writer tagging the pair it appends.
  - All three compose in ONE gradeable reader, `_unreviewed_spontaneous`, because the call site sits 233 lines inside `main` where nothing could grade the wiring (D-252 round 3's lesson: a fix whose wiring no test reaches can be deleted with a green suite). Graders: 5 new tests in `tests/test_stop_hook_spontaneous_review.py` + 2 in `tests/test_governance_template_split.py`; a 10-mutant battery run on a fresh `__pycache__`-free copy, `killed 10, survived 0`, each dead by the failing test's NAME.
  - **T6.1/T6.2/T6.3 — the three sentences, byte-identical in `CLAUDE.md` and `templates/governance/CLAUDE.md`**, each placed where its subject already lives (READ BEFORE YOU EDIT) rather than in the section the step's label named: T6.1 beside the realign sentence (a pathspec fails LOUDLY on an untracked path — rc 1, nothing committed — but says NOTHING when a tracked path is merely OMITTED; `git show --numstat HEAD` against the intended list is the only check that catches the silent one); T6.2 beside the stale-blob paragraph (a pathspec protects the FILE LIST, never the CONTENT — it commits the WORKING TREE, so a shared-append file needs the private-index recipe, now spelled out); T6.3 beside the `expect 0 0` sentence (a pure MODE change numstats as `0 0 <path>`, so `0 0` means "no line changed", never "nothing staged" — `--summary`/`--raw` show it). The parity grader caught a NON-BREAKING SPACE I had written inside `0 0`, in both files; removed, and neither file carries another.
  - `docs/workstation/hooks-index.md` row 22 brought current in the same change — it is the authority for this subsystem and carried the superseded coverage rule.
- **Phase C's heavy `/fabrik-review` — CLOSED ON THE SCOPE-GROWTH STOP (D-252), 2026-09-14.** Three rounds over `dffcca1a`; receipt `docs/development/reviews/2026-09-14-plan-2-mail-triage-phase-C-review.md`, `Status: IN-PROGRESS` by rule. Round 1 reviewed the Phase C change itself — confirmed 24, `own_fix: 0`. Rounds 2 and 3 each confirmed defects ONLY inside the previous round's fixes, which is the counted condition D-252 defines, so there is no round 4.
  - **The loop earned its cost and the findings say so.** Round 1: the T5.1 stack walk had REOPENED the coroner-laundering hole closing review C-2 had shut; T5.3's fix carried unreachable code and an ungraded half; and the private-index recipe I had just shipped to BOTH fleet-synced contracts carried a compare-and-swap that can never fire — which I had been running for every commit that session (audited: reflog strictly linear, nothing lost). Round 2: round 1's own fix laundered through nesting (an unreviewed edit went 1 → 0 with nesting the only difference), it had reproduced T5.1 on its own new field, and SIX of the recipe's seven steps destroyed data when a seat executed them verbatim. Round 3: a `printf` whose format-string position silently eats any hunk containing `%`, an APPEND that relocates a CHANGELOG entry below the released sections, and two counts measured wrong rather than merely stale.
  - **Fixes at** `e6046139` · `8a8d8b26` · `552428e5` · `3d8baa9d` · `9bcebd42` · `2d559515` · `e821548c` · `806b6eb9`. Battery 10 → 29 mutants, killed 29 on a fresh copy; 372 tests green over the nine named files. Five mutants that survived along the way were real grading gaps in my own fixes, all closed; two dropped as inexpressible with their proofs recorded rather than left surviving.
  - **Routed to `docs/STRATEGIC_BACKLOG.md` at Finish, not built here:** the recipe's defects are all TRANSCRIPTION failures, and the answer to that class is an EXECUTABLE SCRIPT plus one sentence and a pointer — a new fleet-synced mechanism, which is SPEC/PLAN work, not a round-3 in-run fix. Beside it: `command_run.py:2124` discards parked frames on a `start` over a non-running record; the hook is fleet-synced while its 39 graders are not; a tally-vs-rows check for `check_review_hygiene.py` (five defects across this review and Phase B would have been caught by it); and T5.2's `--surface` going stale across a long review's own rounds, which the sixth cause demonstrated on THIS session at round 3.
- **Phase D — DONE 2026-09-14.** All five T11 verdicts RE-DERIVED today before editing (the plan's greps were dated 2026-09-12 and one had moved: the `58-resilience` dedup claim is at `:504`, not `:581`).
  - **T11.1 (01M1SSV9YR, youtube) — the claim was false and the code says so.** `core/58-resilience.md` asserted `fabrik-lib/alerting/`'s title dedup "gives the exactly-one property". Read at `alerting/__init__.py`: `_last_sent: dict[str, float] = {}` is a module-level dict — PER-PROCESS, dying with the worker — and `ALERT_MIN_INTERVAL` defaults to 300 s on a SLIDING window. That is suppression, not a latch: it cannot deliver "ONE alert, cleared on recovery" across restarts or two workers, and re-fires every 5 minutes while the cause persists. The rule now says the durable latch is yours to build.
  - **T11.2 (01M25CVK8N, iterative_image_editor)** — both `ai/20-vision.md` cites of the hub-only provider map were project-relative in a FLEET-SYNCED pack, so every project's Doc Link check reds on them. Both absolute now.
  - **T11.3 (01M1RHJZ3, fleet relaying trade-intelligence)** — `core/57` gains the MEASURABLE half, not selector advice: a render that satisfies its selector and extracts zero rows is a FAILED fetch, because a 404 or challenge page carries the site's own navigation chrome. Writing a cleverer selector is the same bet one redesign later.
  - **T11.4 (01M20K4FK6TKPDF529Q1HQ2N4P, trade-intelligence)** — the denominator HARD-STOP row in BOTH contracts gains a canonical recipe naming the three defensible populations (tracked · tracked + uncommitted siblings · on disk), byte-identical at 1,763 chars, citing the stale-copy exclusion by ABSOLUTE path with no line number. ⚠️ **Executed before shipping, and the first cut was broken:** `tr -d '\0'` deletes the separators instead of translating them, so `wc -l` returned **0** — a clean-looking zero, inside the very rule about counts that fail silently. Corrected to `tr '\0' '\n'` and re-run: 770 · 770 · 23,459 · 36,787, with stale copies inflating the unscoped count by 57%. Those numbers are now IN the rule.
  - **T11.5 (01M1S4D78K, fleet relaying site-provisioner) — ADOPTED, and placed by measurement.** "Write the guard's subject five legitimate ways and count how many it still catches" went to `core/45-testing-strategy.md`'s anti-pattern table, NOT `review_rubric.py`: the rubric injects into every review (301 receipts on disk) whether or not a guard was added, while core/45 fires where a grader is authored — same benefit, far lower fire rate, which is what FIX DIRECTIVE 5 asks. Its evidence line carries the measured rate (4× in one day, 2 repos) plus two more from the day it landed, both from this session's own Phase C review.
  - **Gate:** the cited absolute paths exist (rc 0); each phrase present exactly once at its target; `check_rule_grounding.py --root .` rc 0 with no finding naming a Phase D file; `test_rule_grounding.py` + `test_check_citations_resolve.py` 16 passed; `sync_enforcement_to_projects.py --dry-run` carries all four synced surfaces. `final_gate.py --check` flagged ONE broken ref that was mine — a backlog row naming `scripts/shared_tree_commit.sh`, a script the row PROPOSES creating — reworded so it is not a path ref; the two remaining refs belong to a sibling's staged-for-deletion plan file.
- **Phase D's `/fabrik-review-scoped` — EIGHT rounds, 54 confirmed defects in five text edits, CLOSED ON THE D-252 SCOPE-GROWTH STOP** (`9/9 → 8/8`, the counted condition). Series `1 · 1 · 24 · 0 · 11 · 0 · 9 · 8` (rounds 4 and 6 are the ORCHESTRATOR's class sweeps, not seat passes — recorded in the same series, which is what tripped the oscillation advisory; the real shape is a stable ledger over a surface that kept yielding). Fixes at `4a58df44` · `7b5bdb86` · `912aabd1` · `25e8b041` · `540d103f` · `08c0d005` · `9567d383`.
  - **Two of the 37 had already SHIPPED to 46 of 49 repos, both mine.** (1) Escaping two pipes inside a Python regex to stop a table row shredding: `\|` renders as a pipe but is a LITERAL pipe in the raw file a pack actually reaches an agent as, so the destructive-test guard rejected `app_test`, `throwaway` and `scratch` and matched only the literal string — fail-closed, and its predictable repair is deletion. Rephrased to `dbname.endswith((...))`, which carries no pipe and was verified identical over seven names. (2) The denominator clause's own pipes took the HARD-STOP row from 4 rendered cells to 7 against a 2-cell header, truncating every number, the `find` recipe and the whole cobra clause away from any rendered reader. `check_governance_tables.py` was reporting both on BOTH contracts.
  - **The detector gap that let (1) through:** that check's finding message names the correct remedy verbatim — "inside a CODE SPAN rephrase the example so it carries no literal pipe" — and I escaped instead; its `_TARGETS` are the two CLAUDE.md files only, so `.windsurf/rules/`, the surface where it shipped fleet-wide, has no detector at all. Backlog row filed with the measurement.
  - **Round 1 found I had fixed the WRONG LINE.** youtube's mail named the coverage-map row; I edited a paragraph eighty lines above it, leaving the false claim live where readers look AND contradicting it. Both the row and the paragraph are now correct, and the fact is stated once.
  - **Class sweeps at the close, with denominators:** `code-span-escape` — 116 governance files, 11 spans carry a backslash-pipe, none a copy-and-run predicate (routed); `rendered-truncation` — 1,659 table rows across 58 files, ZERO off their header width; `wrong-line-fixed` — all five T11 fixes land where their source named; `undefined-escape-hatch` and `counter-without-cobra-note` both closed in-run.
  - **Rounds 7 and 8 reviewed only this review's own fix prose, which is what the stop counts.**
    Round 8's worst finding was mine and had already SYNCED: `core/45-testing-strategy.md`'s
    destructive-test row called the throwaway-DB guard `case-SENSITIVE` and called it deliberate,
    while the helper it instructs you to call — `scaffold.py::_DISPOSABLE_NAME` — is `re.IGNORECASE`
    with a passing regression test. Executed side by side, the two predicates disagreed on 5 of 7
    names; present in 46 of the 49 `/opt` repos carrying the pack. Claim and predicate both fixed,
    7 of 7 agree.
  - **Two claims were RETRACTED rather than defended,** both mine, both caught by a fresh seat: a
    fabricated backlog example (`command grep -ci three` over the file it named returns 0, in the
    tree and across its history) and an invented `4+4=8` whose source row carries three numbers that
    do not reconcile. And a round-7 fix that never landed — its script aborted on a later edit's
    assertion — shipped a CHANGELOG line claiming it had. Nothing in the loop cross-checks a round's
    claimed fixes against its own diff; that is now the backlog row's second item.
