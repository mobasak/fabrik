...
23. Render → `--check` → `check_command_corpus.py` → hygiene; the graders in `tests/test_assemble_dispatch_step.py` / `test_review_exit_contract.py` and, for T2.15, `tests/test_review_receipt.py` for each new sentence, red first; commit by private index; the scoped review routes up (>5 files) — one heavy round.
...
| 01M20K4FK6TKPDF529Q1HQ2N4P | trade-intelligence | the bounded-search rule names no canonical denominator recipe; three seats produced 1,275 / 1,832 / 72,768 for one question | OPEN — no pack or fragment names `.claude/worktrees` (`command grep -rn` over `.windsurf/rules/` + `commands/_fragments/` → 0); the two CLAUDE.md contracts carry it only as the scratch-sweep clause (3 occurrences); the one exclusion recipe in the six roots searched (`commands/`, `.windsurf/`, both CLAUDE.md files, `docs/reference/`, `README.md` — 9 `.claude/worktrees` hits, 1 recipe) is `commands/_sources/fabrik-review.md` § the stale-copies exclusion (`.claude/worktrees/**` + `.tmp/**` for a review's own grep; at :129 today, re-derived at execution because Phase A edits that file above it) — no CLAUDE.md-level denominator recipe exists | T11.4 a denominator recipe in the HUB `CLAUDE.md`'s HARD-STOP row (tracked set + uncommitted siblings; the implied exclusions) that EXTENDS `fabrik-review.md`'s stale-copies exclusion by anchor — never a second copy, never a line number; the project-facing `templates/governance/CLAUDE.md` gets the recipe WITHOUT the hub-only `commands/_sources/` cite (T11.2's shape) — cited from `subagents-core.md`'s brief text |
...
**Steps (the gate).** 6. `python3 scripts/enforcement/check_rule_grounding.py --root .` and `check_citations_resolve.py --changed <the three packs>` green; `command grep -n` for each new phrase in `.windsurf/rules/` → exactly one hit; `final_gate.py --check --json` success; `sync_enforcement_to_projects.py --dry-run` lists the three packs; `python3 -m pytest tests/enforcement/test_check_citations_resolve.py -q` green; `/fabrik-review-scoped` over the phase.
