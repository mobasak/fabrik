...
| 01M28K3F44 | fabrik-lib | VERDICT rejects RECORDED; graders blind to an unchanged artifact | OPEN | `check_review_coverage.py:63` |
...
21. T2.21 the corpus audit for bare-root `grep` verification steps (01M25H4SR (2)) — bounded to the 36 sources, the fragments and the agent definitions (`commands/_sources/*.md`, `commands/_fragments/*.md`, `commands/_agents/*.md` — 0 hits in the last two today): `command grep -n 'grep -r' <those globs>` listed with the root each step names, a rewrite per hit (the shim bites at ANY root, so a step rooted at `/opt/*` or `--exclude-dir` is in the class too).
