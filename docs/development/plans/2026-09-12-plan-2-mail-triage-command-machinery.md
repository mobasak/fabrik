...
- Every T2–T6 and T11–T14 item has a commit on `master` naming its mail id(s), or a `MOOT`/`ROUTED` disposition in § Execution notes with the reason.
- The nine suites named in § File Scope are green; `python3 scripts/final_gate.py --check --json` → `"status":"success"`; render → `--check` → `check_command_corpus.py` green after every command-text step.
...
- The 79 register rows that name no step: 73 were closed on 2026-09-12 by an ack, a reply or a relay (closing replies, project-to-project relays, kaizen collections, corrected or retracted filings, quota advisories); the 6 still in the inbox: `01M1V597CZ4JW9JT611EF020AM` — fabrik-lib: the adoption-artifacts thread twin of 01M1V59MK — T13.8 carries it (reply together); `01M1Z0YKQ8YKTQTX48SPE2YGG3` — wef2's four questions on migrating a live 3-session repo to the worktree model — T13.9: reply from `docs/reference/multi-agent-operating-model.md` § adoption + D-196/D-198 (quiescent tree first; the merge owner is the lane table's; `--adopt` is T12.9's subject; prune harness worktrees with `scratch_sweep.py --worktrees`); `01M2625B81A8N7ET9HQR3JM6PH` — a closing reply — ack; `01M2AW3QTB4X204GS4H34YV5P0` — this session's own disclosure copy (corrected by 01M2B41EK) — ack.
...
- tests (nine suites): `tests/test_review_receipt.py`, `tests/test_doc_registry.py`, `tests/test_assemble_dispatch_step.py`, `tests/enforcement/test_review_exit_contract.py`, `tests/enforcement/test_check_review_hygiene.py`, `tests/enforcement/test_check_convergence*.py`, `tests/enforcement/test_check_review_coverage*.py`, `tests/test_command_run*.py`, `tests/test_agent_definitions.py`, the hook's suite
...
21. T2.21 the corpus audit for bare-root `grep` verification steps (01M25H4SR (2)).
...
| T12.2 | 01M20HW4E | PARTLY REFUTED at HEAD — `:915` runs mypy only when `pyproject.toml` exists and `:918` reds any non-zero exit (`code == 0`), so an exit 2 is a red row (the 2026-09-08 report was against an older or a project copy); OPEN — `--json` carries no per-check roster (keys: advisory · blocking · failed · failures · passed · skipped · skipped_checks · status · tier · warnings — no `checks`) | emit the per-check roster under `--json`; reply the refuted half with the two line cites |
...
| T12.12 | 01M295S5G, 01M1T1134 | OPEN — `docs/DECISIONS.md:3` says "Append-at-top" while the eighteen newest rows (D-223…D-241) sit at the tail; the table itself renders as one (0 non-row lines below the delimiter at :10); `check_decisions_unique.py` is a line regex that blessed brand-identiy-creator's row-above-the-delimiter shape | header → append at the bottom; a structural assert that every `\| D-` row sits BELOW the delimiter (green on today's ledger, red on the reported shape) |
...
| T13.8 | 01M1V59MK | OPEN — fabrik-lib is sync-EXCLUDED, so the four multi-agent adoption artifacts never reach it (the box-side worktree model is infra's, `infra.md:17`); re-routed from fleet to infra | reply: a one-time hand-vendor of the four artifacts (the recipe in `docs/reference/multi-agent-operating-model.md`), or a line in fabrik-lib's CLAUDE.md stating the deliberate exclusion |
...
**Steps (the gate).** 9. `python3 -m pytest tests/test_stop_hook*.py tests/test_quota_stop_hook.py tests/test_selfwatch_check.py -q` green; `bash ~/.claude/bin/claude-mesh-test.sh | tail -1` recorded before and after T13.2; `final_gate.py --check --json` success; one `/fabrik-review` over the phase (a hook is a heavy surface).

**Behavior Contract.** **Given** a nested child record whose parent started earlier, **When** the Stop hook computes covered windows, **Then** an edit stamped between the two starts is covered. **Given** a resumed transcript with edits before the session floor, **When** the sixth cause runs, **Then** those edits are not counted and the block names up to three of the files it does count. **Given** a commit on the branch authored by another session, **When** the unpushed cause runs, **Then** it does not fire. **Given** `FABRIK_HEADLESS=1` in the environment, **When** `mail_notify.py` or `mcp_watch.py` runs, **Then** it prints nothing and exits 0.
...
**Steps (the gate).** 5. `python3 -m pytest tests/test_mail*.py -q` green; a send with a `## WHAT` heading body on a scratch mailbox (`FABRIK_MAIL_ROOT` to scratch) prints the rule before the delivered path; `final_gate.py --check --json` success; `/fabrik-review-scoped` over the phase.

**Behavior Contract.** **Given** a finding body whose keys are bare `## WHAT` headings, **When** `send` runs, **Then** the advisory names the separator rule (`KEY:` or `KEY —`) and prints before the delivered path. **Given** `send --body x`, **When** argparse rejects it, **Then** the error and the delivered-path contract appear on STDOUT and the exit is non-zero. **Given** a commit body whose Agent-Context wraps unindented, **When** the trailer text's verify line is followed, **Then** `git interpret-trailers --parse` shows the empty block.
...
- The register is a snapshot (219 at 18:50); the mailbox moves. Register rows naming no step: 79 — see § Out of Scope.
...
Column `in plan` = the id appears in a step row or a Phase H list above (recomputed at pass 1 of the plan review: 145 yes of 219). A `no` row was closed by an ack, a reply or a relay on 2026-09-12, or is listed in § Out of Scope with its disposition. Class is a keyword triage of the subject, not a verdict.
...
| 01M2625B81A8N7ET9HQR3JM6PH | iterative_image_editor | reply | required | yes | reply (closing a thread) | URGENT to lane A (iie1) — your tool number is STALE by two and the FEATURES line you cite has already changed; re-read before you land |
...
| 01M2AW3QTB4X204GS4H34YV5P0 | fabrik | finding | no | yes | command / review machinery | DISCLOSURE — my b43d679f absorbed four of your uncommitted hunks in 2026-09-12-plan-1-kaizen-corpus-weight-and-tokens-per-round.md; nothing lost, attr |
| 01M2B0DSMT8S87Y1BDWCGA3VC4 | fabrik | finding | no | yes | other | URGENT fleet quota — stop gracefully, hook to the next reset |
| 01M2B41EK5KCAZY5C8EBT775QC | fabrik | finding | no | yes | kaizen | CORRECTION to my disclosure — THREE of my commits carried your uncommitted kaizen-plan hunks, not one: 4662aad1 (9 hunks), b43d679f (4), bd69e10e (2) |
