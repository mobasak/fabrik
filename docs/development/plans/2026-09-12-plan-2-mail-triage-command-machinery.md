...
**Steps (the gate).** 6. `python3 scripts/enforcement/check_rule_grounding.py --root .` green; every absolute path a T11 edit cites exists (`test -f /opt/fabrik/docs/reference/ai-media-generation-provider-map.md /opt/fabrik/commands/_sources/fabrik-review.md` — `check_citations_resolve.py` scans `docs/**` only, `SOURCE_GLOBS` at :37-42, so it cannot grade a pack or a CLAUDE.md); `command grep -n` for each new phrase in its own target (the three packs; both CLAUDE.md files for T11.4, byte-identical) → exactly one hit per target; for T11.5 either exactly one hit at its chosen target or a recorded rejection row naming the measured fire rate; `final_gate.py --check --json` success; `sync_enforcement_to_projects.py --dry-run` lists the three packs and `templates/governance/CLAUDE.md`; `python3 -m pytest tests/enforcement/test_check_citations_resolve.py -q` green; `/fabrik-review-scoped` over the phase.

**Behavior Contract.** **Given** the five T11 edits (three packs, both CLAUDE.md files, the rubric or core/45), **When** each target is grepped, **Then** each phrase is present once at its target — or, for T11.5, a rejection row names the measured fire rate — and the fleet sync carries the synced ones (dry-run then force at Finish).
