...
| 01M28K3F44 | fabrik-lib | VERDICT rejects RECORDED; graders blind to an unchanged artifact | PARTLY REFUTED — `:63` accepts `RECORDED —` with one of four reason tokens (T4.4 aligns the command text, T2.10); the second half (graders blind to an unchanged artifact) stays OPEN (T4.5) | `check_review_coverage.py:63` |
...
| 01M20K4FK6TKPDF529Q1HQ2N4P | trade-intelligence | the bounded-search rule names no canonical denominator recipe; three seats produced 1,275 / 1,832 / 72,768 for one question | OPEN — `.claude/worktrees` appears in no pack or fragment (`command grep -rn` over `.windsurf/rules/` + `commands/_fragments/` → 0) and only as the scratch-sweep clause in the two CLAUDE.md contracts (3 hits), never as an exclusion recipe | T11.4 a denominator recipe in CLAUDE.md's HARD-STOP row (tracked set + uncommitted siblings; the implied exclusions) cited from `subagents-core.md`'s brief text |
