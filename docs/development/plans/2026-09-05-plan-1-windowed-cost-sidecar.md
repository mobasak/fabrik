...
- **A3.** Make the Fable 5.1 cache rate **actually apply**: `api_equiv` (`claude_p_cost.py:84,89`) multiplies cache reads by ONE global `_cache["read"]` (0.1). Fable 5.1's true rate is 0.025 — a 4× overprice on what `claude_price_ratios.json`'s own `_comment` calls the dominant term. Add per-model cache multipliers to that file **and make `api_equiv` prefer a per-model value, falling back to `_cache`**. ⚠️ `tests/test_price_ratios_current.py:43-57` currently pins the blind spot as *documented* (asserting `_cache.read == 0.1` and the `_comment` text). **Updating those three assertions and the now-false `_comment` is a MANDATED STEP here, because no gate will force it** — ⚠️ SUPERSEDED IN EXECUTION (2026-09-05): under the per-model-id fix that shipped, `_cache.read == 0.1` is the RIGHT assertion about the right thing (the default), so it was kept and only its message reworded. The `_comment` was rewritten. The two prose assertions were DELETED rather than updated — the Phase-A review ruled them wallpaper once the rate became executable (`"0.25" in comment` passes on "the 0.25 figure was withdrawn"). Do not "restore" them — executed proof: applying A3 as specified leaves the suite green 3/3, since A3 touches neither `_cache.read` nor `_comment`.
...
- **Gate (round-trip, not a file read):** `python -m pytest tests/test_claude_p_cost_refresh.py -q` — the test monkeypatches the module constants `claude_p_cost._USAGE_HISTORY` and `._MANAGER_ACCOUNTS` onto a fixture tree and `CLAUDE_P_COST` onto a `tmp_path` output, calls `refresh()`, then asserts (i) all five new keys present and non-`null`, (ii) `window_start < window_end` and `tokens > 0`, and (iii) **`amortized_per_mtok_by_family` SURVIVES the round-trip** — the live data-loss bug of I11, which revision 2 flagged as its headline finding and then left with no gate at all. **RED today** (exit 4, file absent). ⚠️ Revision 2 specified this as *"a fixture usage-history via the `CLAUDE_P_COST` env override"* — unbuildable: `refresh()` and the then-existing `_live_amortized_per_mtok()` took **zero arguments**, `_USAGE_HISTORY` (`:43`) and `_MANAGER_ACCOUNTS` (`:45`) are module constants with no env knob, and `CLAUDE_P_COST` redirects the OUTPUT sidecar, not the input. Monkeypatching the constants is the buildable form; A1 may instead parameterise `refresh()`, but then it must SAY so. ⚠️ **Use RELATIVE fixture dates, never hardcoded ones:** the window counts only days inside `cutoff = today − (_MONTHLY_DAYS − 1)` (`_live_usage_window`; Phase A corrected this from `today − _MONTHLY_DAYS`, which spanned 31 dates against one month of spend, and deleted the `_live_amortized_per_mtok` shim this line named), so a static fixture silently falls out of the window and the test reds about a month after it is written — measured: dates 96 days old → total 0 → anchor fallback, and `tokens > 0` fails.
...
## Phase B — the readers make staleness LOUD — ✅ EXECUTED 2026-09-05 (B4 landed early in Phase A; B1-B3 in this phase's commit)
...
- **B4.** Fix `claude_p_cost.py:17` — it enumerates three keys while four are written today, and B **depends on A**, so the truthful post-A enumeration is **NINE**: the 3 existing + `amortized_per_mtok_by_family` that A1 stops destroying + A1's 5 new window/denominator keys. Writing "four" here would leave `:17` false about the very reshape this plan delivers. Also `:10-12` claims the file "is synced to every project"; it is in **no manifest** (grep exit 1) and **1** of 57 `/opt/*` dirs carries it by `ls -1d /opt/*/` (59 counting hidden — the method is part of the number) — the hub's own. (Revision 3 wrote "0 of 57", false under the same counting convention `§ Global Constraints` uses, which includes `/opt/fabrik`.)
- **Gate:** `python -m pytest tests/test_claude_p_cost.py -q` — green and must stay green (9 tests when this was written at `e1710420`; 14 after Phase A added the id-form and env-knob guards — the count is a snapshot, the GREEN is the gate). ⚠️ **`test_cached_rate_failsoft_to_anchor_when_sidecar_missing` must pass UNCHANGED** — it pins the `$0.093` fail-soft anchor for a MISSING sidecar via `CLAUDE_P_COST`, and B3 is precisely the step that touches fail-soft behaviour. (Cited by NAME, not by line: it was written as `:65-68`, and Phase A's new tests pushed it to `:112-114` — a line anchor in a gate rots the moment the file above it grows, and an executor obeying it literally would md5 the wrong four lines. Verified unchanged by md5 of the named function, not of a line range.) Revision 1 carried that word; revision 2 dropped it, leaving the guard against "make stale loud" quietly softening "missing fails soft" to the executor's memory.
...
- tests/test_claude_p_cost_refresh.py
...
## Execution notes — corrections to this plan's own record

⚠️ **The Phase-A commit message (`7aa4d7a4`) contains a FALSE claim, retracted here.** It says A3 "put the
0.025 multiplier on the `claude-code/fable` FAMILY key" and credits the executor with overriding it on live
evidence. **A3 says no such thing** — it says *"Add per-model cache multipliers … and make `api_equiv` prefer
a per-model value, falling back to `_cache`"*, and the word "family" appears nowhere in it. The executor
misread the plan, shipped the correct implementation anyway, and then wrote a self-congratulatory correction
narrative into the permanent record. The four grounding facts in that paragraph are each independently true
(re-derived by an author-blind finder); the claim that they overturned the plan is not. The commit cannot be
amended on a shared branch, so this note is the correction of record — found by an author-blind finder, not
by the author.
...
**RESOLVED at Phase A start (2026-09-05), recorded here because the plan made this a gating pre-step:**
- **Whether `refresh()` can derive `accounts` and `spend_usd` at all.** PROBED before any code was written: both are derivable — `accounts` is the count of non-dotted dirs under `~/.claude/manager-accounts` (4 live), `spend_usd` is `_SUBSCRIPTION_USD_PER_ACCOUNT × accounts`. A1's gate was therefore NOT re-scoped. ⚠️ The Phase-A review then found the harder half the probe missed: the count has a `max(1, n)` FLOOR, so an unreadable or empty accounts dir silently yields `1`, and publishing that as `accounts` would have been exactly the fabricated denominator this clause forbids. Fixed — an unmeasurable count publishes `null` for both keys while the rate keeps the historical floor.

**Still open, each with a named resolution step:**
- **The carried `amortized_per_mtok_by_family` is stale AND was computed with the wrong cache rate.** Measured 2026-09-05: the carried split is 79.6% off a live recompute (88.0% against the PRE-fix `amortized_by_family`, which read the global `_cache` — this phase fixed that site, which is why the two figures differ; quote either with its as-of). `refresh()` cannot regenerate the split (it deliberately does not import `derive_cost`), and the only function that can — `write_cost_sidecar` — destroys the five window keys when it runs. Mitigated, not closed: the carried map ships with `amortized_per_mtok_by_family_carried: true` — a FLAG, not a date. Two earlier revisions tried to stamp the split's age from `prev["built_at"]` (as `..._built_at`, then `..._carried_from`) and both overstated it by weeks, because that value is the previous REFRESH's clock and not the split's build time. The lineage is unrecoverable from the file, so the producer states the one thing it knows — it carried the map, it did not compute it — and pops both superseded key names on write. Resolution: a producer that refreshes the split WITHOUT overwriting the window — either `refresh()` grows an optional import, or `write_cost_sidecar` learns to merge. Sized as a follow-up, not smuggled into this plan.
