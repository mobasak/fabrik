...
- **The carried `amortized_per_mtok_by_family` is stale AND was computed with the wrong cache rate.** Measured 2026-09-05: the carried split is up to 88% off a live recompute, and the site that produces it (`derive_cost.amortized_by_family`) was reading the global `_cache` until this phase fixed it. `refresh()` cannot regenerate the split (it deliberately does not import `derive_cost`), and the only function that can — `write_cost_sidecar` — destroys the five window keys when it runs. Mitigated, not closed: the carried map now ships with `amortized_per_mtok_by_family_built_at` so it cannot borrow the fresh window's authority. Resolution: a producer that refreshes the split WITHOUT overwriting the window — either `refresh()` grows an optional import, or `write_cost_sidecar` learns to merge. Sized as a follow-up, not smuggled into this plan.
