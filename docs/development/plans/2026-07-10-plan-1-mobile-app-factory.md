...
A2. **Pin Expo SDK 57 + mmkv v4** in `package.json` (`expo` → SDK 57 line, RN 0.86; `react-native-mmkv` → `^4`, add `react-native-nitro-modules`). Replace any `new MMKV()` with `createMMKV()` and `.delete(`→`.remove(` in `src/lib/storage.tsx`. *Gate:* `grep -qE '"expo": ?"[~^]?57' templates/mobile-app/package.json && grep -qE '"react-native-mmkv": ?"[~^]?4' templates/mobile-app/package.json && grep -q 'react-native-nitro-modules' templates/mobile-app/package.json && ! grep -rn 'new MMKV(' templates/mobile-app/src && ! grep -rn '\.delete(' templates/mobile-app/src/lib/storage.tsx` → true (**pins the SDK-57 + mmkv-^4 versions by pattern, not just key-presence** — a stale SDK-54/mmkv-v3 install would otherwise pass; `react-native-nitro-modules` is mmkv v4's required peer; the `.delete(` ban is scoped to the mmkv wrapper `storage.tsx` — verify the Obytes file is `.tsx`, else check `.ts`).
...
**Behaviors 5–8 (consent, offline, force-update, nav-error) are RN-runtime** → tested by **named jest/RTL tests** committed in the template (`src/lib/consent/__tests__/consent.test.tsx`, `src/lib/api/__tests__/offline.test.tsx`, `src/lib/update/__tests__/force-update.test.tsx`) + a **Maestro** `launch → consent-gate blocks → home` flow, all run in **CI** (the ported Obytes workflows) — **WSL cannot run RN tests**, so the phase gates above are only the local static proxies. The plan's "one test per behavior" rule is met by these CI tests, not the grep gates.
