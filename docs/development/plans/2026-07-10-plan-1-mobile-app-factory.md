...
**Status:** CONVERGED (via `/fabrik-plan-review` 2026-07-10 — round 2 mirrored the `chrome-extension` precedent; round 3 verified the Phase A0 `src/lib` gitignore blocker + corrected its fix to a surgical `!` negation, Pass 2 edit-free no-op. Next: user-triggered `/fabrik-execute-plan`)
...
A4. **[TDD, highest risk] SWAP axios → `@hey-api/openapi-ts`.** Add hey-api dev config (`openapi-ts.config.ts`) with `@tanstack/react-query` + `zod` + explicit `{ name:'@hey-api/sdk', validator:true }` plugins, sourcing `EXPO_PUBLIC_API_URL`'s `/openapi.json` (Phase B backend). Replace `src/lib/api/client.tsx` (axios) + the per-feature query hooks with generated client/hooks; keep `src/lib/api/provider.tsx`. **RUN the generator** (`npx @hey-api/openapi-ts -f openapi-ts.config.ts`, sourcing the bundled backend's committed `openapi.json` from Phase B — so this step is sequenced *after* B, or against a committed `openapi.json` snapshot) so the **generated client+hooks are committed into the template** — the template must SHIP the generated module or a scaffolded app fails at import. Add a **CI OpenAPI-drift gate** workflow (regenerate → `git diff --exit-code`). *Behavior test (CI only — pnpm/node build is CI, not WSL, per Global Constraints):* `type-check` + a jest test asserting the generated client imports a query hook + a zod validator. *Gate (WSL, static):* `grep -q 'validator: true' templates/mobile-app/openapi-ts.config.ts && ! grep -rn "axios.create" templates/mobile-app/src/lib/api && test -e templates/mobile-app/src/lib/api/generated` (the generated module is committed).
