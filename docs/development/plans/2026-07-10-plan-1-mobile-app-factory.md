...
A2. **Pin Expo SDK 57 + mmkv v4** in `package.json` (`expo` → SDK 57 line, RN 0.86; `react-native-mmkv` → `^4`, add `react-native-nitro-modules`). Replace any `new MMKV()` with `createMMKV()` and `.delete(`→`.remove(` in `src/lib/storage.tsx`. *Gate:* `grep -q '"expo"' templates/mobile-app/package.json && grep -q 'react-native-nitro-modules' templates/mobile-app/package.json && ! grep -rn 'new MMKV(' templates/mobile-app/src && ! grep -rn '\.delete(' templates/mobile-app/src/lib/storage.tsx` → true (`react-native-nitro-modules` is mmkv v4's required peer — the native link fails without it).
...
B4. **Export `openapi.json` + generate the hey-api client (resolves the A4→B dependency).** The backend now exists, so: (i) commit a snapshot `templates/mobile-app/openapi.json` exported from the FastAPI app (`python -c "import json,app.main; json.dump(app.main.app.openapi(), open('openapi.json','w'))"` or an equivalent export step); (ii) run `npx @hey-api/openapi-ts -f openapi-ts.config.ts` (config from A4) against it so **`src/lib/api/generated/` is committed into the template** — the template MUST ship the generated client or a scaffolded app fails at import. (iii) Enumerate the `/app-config` env contract into `.env.example`: `APP_MIN_VERSION`, `APP_LATEST_VERSION`, `STORE_URL_ANDROID`, `STORE_URL_IOS`, plus feature-flag toggles. *Gate:* `test -e templates/mobile-app/src/lib/api/generated && test -f templates/mobile-app/openapi.json && grep -q 'APP_MIN_VERSION' templates/mobile-app/.env.example`.
