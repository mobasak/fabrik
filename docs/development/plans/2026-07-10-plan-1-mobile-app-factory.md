...
B4. **Export `openapi.json` + generate the hey-api client (resolves the A4→B dependency).** The backend now exists, so: (i) commit a snapshot `templates/mobile-app/openapi.json` exported from the FastAPI app (`cd templates/mobile-app/server && PYTHONPATH=src python -c "import json, app.main; json.dump(app.main.app.openapi(), open('../openapi.json','w'))"` — `app` lives at `server/src/app`, so `PYTHONPATH=src` is required or the import fails; or an equivalent export step); (ii) run `npx @hey-api/openapi-ts -f openapi-ts.config.ts` (config from A4) against it so **`src/lib/api/generated/` is committed into the template** — the template MUST ship the generated client or a scaffolded app fails at import. (iii) Enumerate the `/app-config` env contract into `.env.example`: `APP_MIN_VERSION`, `APP_LATEST_VERSION`, `STORE_URL_ANDROID`, `STORE_URL_IOS`, plus feature-flag toggles. *Gate:* `test -e templates/mobile-app/src/lib/api/generated && test -f templates/mobile-app/openapi.json && grep -q 'APP_MIN_VERSION' templates/mobile-app/.env.example`.
