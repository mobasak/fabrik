...
- **Read:** `src/fabrik/orchestrator/deployer_ssh.py:197` — registrar-injected vars merge into the project's **shared `.env`**; all three compose services read it via `env_file`, the backend + worker consume them. **Correction found in review:** the postgres registrar did NOT inject `DATABASE_URL` (only redis/glitchtip injected theirs), and `POSTGRES_PASSWORD` is provisioned nowhere — so a `needs_database` app got no working DB credential (saas is the first such type). Fixed: `_provision_postgres` now mints a dedicated **non-superuser role that owns the DB** and injects its `DATABASE_URL` (so RLS applies — the superuser bypasses it). See LESSONS_LEARNT Lesson 79.
