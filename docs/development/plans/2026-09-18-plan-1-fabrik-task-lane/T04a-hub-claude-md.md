...
4. `CLAUDE.md:198`: replace `either SPEC/PLAN work (a new mechanism, a vendored/synced surface, schema, auth, >5 files) — say so in the reply and open the pipeline at its stage, never half-build it inline — or a RIGHT-NOW fix` with `SIZE it against § Orient step 0's lane table — spec-chain work is said so in the reply and opens the pipeline at its stage, never half-built inline; a RIGHT-NOW fix` keeping the rest of the clause (`every right-now fix ships with /fabrik-review-scoped …`) verbatim.
