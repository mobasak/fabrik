...
- **Re-verification pass (2026-06-30):** treated CONVERGED as unproven and re-ran. No drift in counts/citations/absence. **Grounded the Paddle external dependency by live fetch** (was previously asserted) — event names confirmed real; found + fixed the one error (`transaction.refunded` → `adjustment.created`). Added the iyzico residual (#10). One independent grounder re-confirmed (below). Net: one external-dep correction, zero internal drift.
...
10. **iyzico webhook specifics (named, not deferred silently).** The plan invokes iyzico only for Turkish-domestic projects; its exact webhook event names + signature scheme are NOT yet grounded (no TR-domestic launch is imminent). *Resolution:* fetch iyzico's webhook reference and apply the same `webhook_events` idempotency pattern when the first TR-domestic project is launched. Non-blocking until then.
