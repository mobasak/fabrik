...
- **Fixed-point pass (solo, after fixes):** re-read every changed section — residuals now 1–11 in order, all gates carry a runnable command + expected result, and the fixes introduce no new ungrounded claim (they reference already-`(to-build)`-tagged tooling). No new items. This is the demonstrably-thorough empty pass.
...
11. **Qualified-session threshold + bot/self-exclusion are config, not hardcoded.** `MIN_QUALIFIED_SESSIONS` (default 100) and the exclusion list (operator IP/UA + bot UAs + watchdog UA + require JS `page_view`) drive the selection gate; defaults are provisional until real traffic exists. *Resolution:* set in the brake config at Day 4–5; revisit after the first real sessions. Blocking for a *meaningful* kill/graduate verdict (not for the schema).
