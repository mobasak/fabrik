...
**Status:** CONVERGED on facts (2026-06-30) — every existing-asset claim grounded against code/path:line, every future deliverable verified absent, every external dependency grounded (incl. live Paddle fetch) or flagged with a resolution step. **Caveat (honest):** three independent adversarial passes each found items (shrinking each time); the *factual* claims are stable and re-confirmed, but no independent pass has returned literally empty — the remaining tail is definitional/gate-precision inherent to an all-`(to-build)` roadmap, tracked as Residuals #11–13 and closed at implementation. See `## Self-audit` → "HONEST CONVERGENCE STATUS". Evidence in `## Evidence`; residual unknowns in `## Residual unknowns`.
...
- **Kill:** ~$0 revenue and below threshold *after a fair shot* → scale down, **issue a Paddle refund via the Adjustments API + cancel the subscription for any paying users (MoR handles tax reversal — Residual #9/#12)**, archive DB/repo, park DNS.
...
9. **Paddle refund tax-reversal.** The kill→refund path assumes Paddle-MoR reverses tax automatically; the pack documents outbound invoicing, not refund tax-reversal. *Resolution:* verify in Paddle Sandbox before the auto-kill refund path goes live.
