...
CANNOT drive the doc-audit → Phase B rebuilt as a BUILD (corrects the spec's inherited "VENDOR+ENHANCE doc_reconcile"
verdict); plus the CLI probe (`sys.executable` + whole-surface-broken guard), the flat `.commands` walk (→ recurse the
7 nested groups, 54 vs 23), and the untestable `manual` status (→ marker/no-safe-probe rule); + dry-run-by-default for
the dead-doc delete; + a wrong `path:line` citation. **Round 2 (re-review)** caught **3 more internal-consistency
defects a self-review would have missed** — a stale "not yet a fixed point" line contradicting this header, the
Context-Ledger "7 surfaces" list that omitted the `script` kind, and a Behavior-1/Behavior-7 TDD-fixture conflict —
each caught by an independent Opus pass, all fixed, then re-verified to a 0-defect no-op. Design is sound + internally
consistent; execution-ready.
...
| Enumeration points (introspection targets) | the **7 surfaces = 7 `kind`s** the generator reads | **cli** `@cli.command(`/`@cli.group(` `src/fabrik/cli.py`; **driver** `src/fabrik/drivers/*.py`; **registrar** `_REGISTRAR_ORDER` `src/fabrik/orchestrator/infrastructure.py:90`; **script** top-level `scripts/*.{py,sh}` + named subdirs − `.archive/`; **lib-module** `/opt/fabrik-lib/README.md`+dirs; **scaffold** `templates/*/` (tagged via `SCAFFOLD_TYPES` `src/fabrik/scaffold.py:138`); **rules-pack** `.windsurf/rules/**/*.md` |
...
1. **[RISK, TDD] A probe that errors → `status:"broken"` + excluded from the usable set, and the generator does NOT crash** (fail-soft). *Test first: seed a fixture with a fake broken CLI verb (`--help` exits non-zero) **alongside ≥1 healthy verb** (so it doesn't trip the Behavior-7 whole-surface guard) → assert the broken one is `broken`, the healthy one `ok`, and the run completes.*
...
**Steps:**
1. **[TDD-red]** Write `tests/test_generate_capability_index.py::test_broken_probe_is_flagged_not_crashing` — build the catalog over a tmp repo fixture containing a deliberately-broken CLI stub **plus ≥1 healthy verb** (so the CLI surface isn't 100% broken → the Behavior-7 whole-surface guard does NOT fire); assert the broken record is `status:"broken"` + excluded from `[c for c in caps if c["status"]=="ok"]`, the healthy one is `ok`, and `build_catalog` returned (no exception). Run → **confirm RED** (no generator yet). Gate: `.venv/bin/python -m pytest tests/test_generate_capability_index.py -k broken -x` → fails for the right reason (ImportError/AttributeError on the missing module).
...
- **Pillars present:** `/fabrik-review` is a literal step (7/c, B-7/c) in **both** phases; pool-default subagents named (Phase A-7/c review finders via `fanout`); the final `/fabrik-docs-review` is Phase B step 6. **Fixed point reached** — `/fabrik-plan-review` converged it over two independently-verified review rounds to a 0-defect no-op (see Status).
