...
| cost / quota / limit edges (standing) | FIXED (3) | The per-ticket READ budget: T04 (293,629 B), T05 (267,161 B), T08 (258,556 B, 3.6 KB of headroom while a sibling grew both its files) and — after spec r11 grew the design doc — T02 (264,036 B) each forced a split; re-measured across all 33 tickets after every edit. Also recorded: the OpenRouter pool returned HTTP 402 on all 24 grounder units — $225.00 of $225.00 spent — so the breadth layer of this review was unavailable and the authoritative native pass carried it (§ Residual unknowns). |
...
| ticket breadth (advisory, adjudicated) | FIXED (2) | 20 of the 33 tickets score ≥ 5 today (re-derived this pass; the 16-of-24 figure below the fold was the pre-split measurement). Two split (T01 → T01a/T01b, T07 → T07a/T07b); 14 kept with a recorded reason — see § Breadth adjudication. |
...
| T02a | 6 | KEEP | One hook, one regex bound, one enum relaxation. The score comes from counting its charter/containment/silent-no-op behaviours, which are all the SAME invariant read three ways. |
...
| T02a · T02b · T03b (post-split) | 6 · 5 · 5 | SPLIT already applied | T02 scored 9 (three areas, code+governance mix) and its read set broke the budget at 264,036 B when the spec grew at r11 — split into the hook change (T02a) and the contract row (T02b). T03a is listed separately above because it did NOT fall below the threshold: it still measures **7**, which is the honest reading of a ticket that kept six behaviours after shedding the disjointness half. |
| T01a · T03b · T04a · T04b · T06a · T06b · T07a · T07b · T13 · T14d | 5 | KEEP (each) | All ten sit exactly at the threshold where the check's measured precision is ~0.50. Each is already one file (or one contract sentence) with one gate, and splitting at 5 trades a real review round for a nominal score drop. |
| T05a · T06c | 6 | KEEP | Re-measured this pass at **6**, not the 5 an earlier round recorded. Both are still single-mechanism tickets — T05a is one optional-tier registration, T06c one assembled command — and the extra class each picked up is its doc-sync surface, which travels with the code rather than forming an independent risk. |
| T05b | 7 | KEEP, rewritten | Re-measured at **7**, tied for the worst ticket in the set, and an earlier round wrongly adjudicated it at 5. The score is not why it was rewritten: this pass found its two named blockers unfixable inside its own Touches and its Behavior Contract self-contradictory. Both are fixed in the ticket rather than by splitting it — the breadth is one mechanism (an optional Tier-2 registration) read through several behaviours. |
...
- `scripts/enforcement/check_traycer_chain.py:31 (`DIRS`, cited by symbol — it was asserted as `:28-33`)` `DIRS` (two of its four roots do not exist), `:89` the glob.
