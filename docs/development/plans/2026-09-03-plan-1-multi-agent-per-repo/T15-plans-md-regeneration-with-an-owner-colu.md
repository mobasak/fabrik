...
Gate: test -f docs/reference/multi-agent-operating-model.md   # RED today: the reference doc does not exist. The pytest gate above passes 80 tests today with the work undone (14 in test_docs_updater.py + 66 in test_plan_shape_gates.py — round 6 widened the command and left the pre-widening 14 beside it, the same shape as the regression round 6 itself caught), so it cannot tell 'done' from 'not started'; these two markers can. NOTE on `docs_updater.py --check`: it is NOT a gate here. It is REPO-GLOBAL with no --path flag and exits 1 today on 123 lines of pre-existing debt in OTHER plans (0 of which name this ticket's three docs), so as a gate it would be unsatisfiable inside these Touches. The orchestrator still runs it at the Docs step; it is simply not this ticket's pass/fail.
