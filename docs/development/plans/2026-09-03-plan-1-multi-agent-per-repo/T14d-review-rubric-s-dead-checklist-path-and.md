...
Gate: test -z "$(git grep -nE 'epic-to-ticket-workflow|EVALUATION_CHECKLIST_FOR_EPIC_COMMANDS|\bettw\b' -- scripts/review_rubric.py tests/test_review_rubric.py tests/test_review_rubric_edges.py)" && test -z "$(git grep -n '\.traycer' -- README.md)"   # TWO conjuncts, one per deliverable. The ettw half covers the three code Touches — `tests/test_review_rubric.py:61` and `tests/test_review_rubric_edges.py:60` each BUILD the token inside `_mk_tree()`, a helper five tests share, so a coder who rewrites the assertions and leaves the fixture line passes the pytest gate and reds T16 at Merge Order 33. The `.traycer` half covers the OTHER deliverable (spec § Chain consolidation (b), 7 references at README.md:112,115,152,163,174,175,445) and is what BC row 2 asserts. ⚠️ An earlier round collapsed these into one conjunct over four files: README carries 0 hits of the ettw pattern — not today, not ever — so listing it there was decorative and the whole Traycer-install retirement went unenforced.
