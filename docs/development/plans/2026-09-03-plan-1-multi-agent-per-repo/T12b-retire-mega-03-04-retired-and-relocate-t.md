...
Gate: bash -c 'R=$(git log -1 --format=%H --diff-filter=R -- docs/orchestrator/); test -n "$R" && test -z "$(git show --numstat --format= -M $R | awk '"'"'$2 > 0'"'"')"'   # The pathspec MUST cover BOTH sides of the rename (`docs/orchestrator/`, not just the `_retired/` destination): git scores a change as A, not R, when the pathspec excludes the source, so `--diff-filter=R` returns EMPTY and the gate exits 1 on the correct move. Proven on a real rename and on a fixture. Pinned to the rename COMMIT rather than HEAD because HEAD is a shared moving ref on a three-session tree and this ticket's Docs: repair lands in a later commit. Negative control: a move that also deletes content exits 1, as it must.
Docs: INDEX.md (moved files) · docs/README.md · CHANGELOG.md · **doc-link repair caused by this move**: the relocated `03-expand-epic-files-fabrik.md:318` cites `scripts/traycer_mirror.py`, deleted at T09 — `_retired/` is still SCANNED by check_doc_links (it is not in the skip-prefix list at check_doc_links.py:129-133), so the tombstone must be scrubbed in a SEPARATE commit after the rename — orchestrator-applied
