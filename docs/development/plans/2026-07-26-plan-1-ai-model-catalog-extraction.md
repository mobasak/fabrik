...
**Status:** HARDENED — plan-review reached an md5-no-op, then a 5-round `/fabrik-review` (native Opus) found + fixed **24 excise-boundary defects** and replaced grep-discovery with a mandated **import-graph audit** (E.1). Residual excise-inventory completeness is CLOSED-BY-CONSTRUCTION at execution time by that audit (plan-time hand-enumeration provably kept missing members — a missed runtime subsystem `kilo_auto_route.py` was the last, Pass 7). **Not a clean plan-time no-op; the import-graph audit is the completeness mechanism.** Recommend one more `/fabrik-review` pass post-fix (or proceed, trusting E.1) before `/fabrik-execute-plan`.
**Date:** 2026-07-26
...
- **12-Factor (binding):** logs → unbuffered **stdout** — because Phase B.4/E.4 stand up the engine as a **deployed worker container**, its `cache/update.log` file-write must become stdout-only **in Phase B (B.2f), before the container is wired**, not deferred (a deployed container writing a logfile is an active XI violation — Promtail→Loki owns routing); the WSL-dev batch invocation still tees to a local log if the operator wants, but the container path is stdout-only (XII); **no migrations from web startup** (XII, n/a — SQLite); **same backing services dev/prod** (X — SQLite is the catalog store in both; the flywheel is Postgres in both); config = env vars, no secrets in code (III); backing services by DSN not code (IV — the flywheel DSN).
...
- CREATE `engine/` ← copy of `/opt/fabrik/scripts/kilo-benchmarks/**` (98 scripts + 50 in-tree tests + config YAMLs + `kilo_agents.db` + `models_browser_template.html` + vendored `libs/`, `alerting/`, `web_scrape/`, `direct_vendor_parsers/`, `specialty_clients/`, `translation_bench/`) **PLUS the 14 repo-root engine test files `/opt/fabrik/tests/kilo_benchmarks/**` → `engine/tests/`** (they `importlib`-load engine scripts, e.g. `test_llm_selector.py:29`, so they move with the code they test — 64 engine test files total; Phase E `git rm`s them from fabrik). **Exclude:** `process.py`, `process_v2.py` (throwaway — spec §3a), the `.lcb-venv/`/`.lcb-src/` vendored trees (rebuilt by `setup_lcb_grader.sh`).
