...
### Steps
**B.1 — Copy + prune.** `rsync -a --exclude process.py --exclude process_v2.py --exclude '.lcb-*' --exclude classify_ticket.py --exclude db_models.py --exclude kilo_telemetry.py /opt/fabrik/scripts/kilo-benchmarks/ /opt/ai-model-catalog/engine/` — the last three are the **rule-6 fabrik-consumer-only** modules (N5): no engine script imports them, and copying them would create the dual-editable "shared file" Global Constraints forbid.
1. Gate: `test -f /opt/ai-model-catalog/engine/kilo_agents.db && ls /opt/ai-model-catalog/engine/*.py | wc -l` → **Expected:** DB present, **~93 scripts (98 − 2 throwaway − 3 rule-6)**; and `ls /opt/ai-model-catalog/engine/{classify_ticket,db_models,kilo_telemetry}.py 2>&1 | grep -c "No such file"` → **Expected: 3** (none copied).
...
def local_imports(p):                      # names imported by this file
