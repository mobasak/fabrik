...
- `main(argv: list[str] | None = None) -> int`. Flags: `--check` — the report prints in full and NOTHING is written, whatever else is passed (`--check --seed` and `--check --reseed` print the would-write line and write nothing) · `--seed` — writes the baseline only when none exists; with one present prints the hint line and writes nothing · `--reseed` — writes today's sizes whether or not a baseline exists (the tool of the D-row-citing commit) · `--strict` — exit 1 iff `grew`; never passed by the gate. A plain run with a baseline tightens every surface whose size fell below its baseline value (a write, printed) and leaves the rest. Writes happen only in the main checkout (`is_main_checkout`). **Exit code is 0 on every path without `--strict`.** No `--json`.
...
3. then each of these when it applies (none replaces another): `corpus-weight: no baseline — run --seed at a corpus state you accept` · `corpus-weight: baseline exists — use --reseed` (`--seed` over a present baseline) · `corpus-weight: seeded <n> surface(s) at <ref|—> → .fabrik/corpus-weight-baseline.json` / `corpus-weight: reseeded <n> surface(s) …` on a seed/reseed write · `corpus-weight: ratcheted DOWN <surface> <b> → <bytes>` per tightening write · `corpus-weight: would ratchet DOWN <surface> <b> → <bytes> (--check: not written)` / `corpus-weight: would seed … (--check: not written)` under `--check` · `corpus-weight: worktree — reporting only, nothing written` outside the main checkout · `⚠ corpus-weight: .fabrik/corpus-weight-baseline.json is gitignored here — the ratchet is local-only` (the lint ratchet's CI-parity degradation, restated).
...
- **Given** the same growth, **When** run with `--strict`, **Then** exit 1 (`test_strict_reds_on_growth_vs_base`) — the future-promotion and regression hook, never passed by the gate. — row A4
- **Given** any state (no baseline · growth · shrink · equal · non-owner · gitignored baseline · no base ref), **When** run without `--strict`, **Then** exit is 0 — one grader loops every state (`test_exit_is_zero_on_every_path_without_strict`; red-on-revert proven; row A5).
- **Given** `--check`, alone or with `--seed` or `--reseed`, **When** run over any state, **Then** no file under `.fabrik/` is created or modified (`test_check_never_writes_even_with_seed_or_reseed`; red-on-revert proven; row A6).
...
- **Given** any figure THIS change adds (`tok_per_round`, `mass_ratio`, their reason), **When** emitted, **Then** `--json` carries `rows_with_numerator`, `rows_with_denominator`, `rows_both` and `rows_total` beside it, the rendered cell carries `num/den/both`, and the population line carries `rows_total` (`test_every_added_figure_carries_its_row_counts`) — the two pre-existing columns without a row count (`max wall`, `cache hit`) are the backlog row Phase C step 5 writes, not this plan's grader. — row B5
...
```
$ replay — one commit per day (newest first-parent on master), last 30 days, six hub surfaces, EVERY blob (row A13's walk); + = rose vs the previous day
day         CLAUDE.md  tmpl/CLAUDE.md  cmd/_sources  cmd/_fragments  cmd/_agents  .windsurf/rules
2026-08-13        36,535         33,536        542,066         28,688              0      1,077,977 
...
days=30 rises (would-WARN days under the shipped walk): CLAUDE.md=19, CLAUDE.md=21, commands/_sources=21, commands/_fragments=16, commands/_agents=6, .windsurf/rules=19
```
(eight of the 30 rows shown; the full table reproduces from `git log --since='30 days ago' --first-parent master` + `git ls-tree -r -l <sha> -- <surface>` summing every blob, and the two `.yaml` files under `.windsurf/rules` land on 2026-09-01/02 as the +77,442 B step the earlier `.md`-only series could not show). A ratchet that WARNs whenever a surface sits above its baseline would have spoken on two days in three; the WARN is therefore on the change's own delta (tree vs base) and the baseline is the trend line — Q6's narrowing, recorded as D-240 and amended for the sixth surface by D-241.

```
$ base-branch sizes at origin/master 2234c830 (pinned — origin/master has since moved; the numbers are the snapshot's) vs the working tree (the second gate's inputs)
...
|---|---|---|
| FLOOR core/35-security-auth.md | UNCHECKED | no auth surface, no JWT, no CORS, no secret: the two scripts read a JSONL and walk bytes — hunt: any `os.getenv`, any token, any network call in the Interfaces |
| FLOOR core/25-data-postgres.md | UNCHECKED | no DB — hunt: any SQL, any migration, any `postgres` in the phases |
| FLOOR core/30-ops.md | UNCHECKED | no container, no compose, no port — hunt: any Docker or deploy step |
| FLOOR 12-FACTOR (all twelve) | UNCHECKED | § Global Constraints carries XI/III/VIII/X verbatim; hunt: a logfile write, a grouped env set, a daemon, a substituted backing service in tests |
| MATCHED core/10-python.md | UNCHECKED | type hints on every signature; `list[str]` / `str \| None`; `uv run pytest` gates — hunt: the Interfaces' signatures, the gate commands |
| MATCHED core/40-documentation.md | UNCHECKED | CHANGELOG under `[Unreleased]` (Phase C 5); TROUBLESHOOTING row (Phase A 6); INDEX rows (Phase C 5); the lock-owned docs mailed, not edited — hunt: every Doc Sync Matrix trigger the diff fires |
| MATCHED core/45-testing-strategy.md | UNCHECKED | one grader per Behavior row (24), each seen RED first — B9 asserts 13 cells so it is RED on the shipped table; two red-on-revert proofs per phase named — hunt: a row with no grader, a grader that passes with the feature reverted |
| Fail-open vs fail-closed on every gate/guard | UNCHECKED | the check exits 0 on every path (A5 loops the states); `--strict` is the only red and the gate never passes it; the base verdict prints even with no baseline (A1) — an absent baseline can silence nothing; `tok_per_round`'s absence renders `—` WITH its reason (the pre-existing `—` cells carry none: backlog, not this plan) |
| Cost / quota / limit accounting edges | UNCHECKED | two stdlib reads; the 30-day replay states its bound (one commit per day, first-parent); every figure this change adds carries its row counts (`max wall` and `cache hit`, pre-existing, carry none — the backlog row Phase C step 5 writes); `unknown ≠ 0`: a row without tokens is counted, never zeroed |
| Boundary / sentinel / prefix collisions | UNCHECKED | `T = 0` (a constructed fixture, never `test_cmd`); exactly ⅔ publishes; `rounds == 0` rows excluded from both sides; every file on BOTH tree and base side (A13); a surface absent at base is not an addition (A12) — the on-disk vs tracked-at-ref population gap; no resolvable base → `—` (A11); no `commands/_sources/` → nothing owned (A9) |
| Behaviour without a test | UNCHECKED | 24 named graders incl. `--reseed` (A8), `--seed` over an existing baseline (A8), `--check` with a writing flag (A6), the table's column count (B9); `--json` was removed rather than left ungraded; the gate registration (infra's) carries its own grader in the mail |
| Fleet blast radius (this surface's own class) | UNCHECKED | `scripts/enforcement/` ships to ~46 repos: ownership is the positive hub marker, so a synced project AND a repo that is neither hub nor synced (fabrik-lib) own nothing and the check says so (A9); a hub WORKTREE owns but never writes (A15); `Path.rglob`, never git; always-0 exit; the registration infra lands is `warn_only=True` |
| Recurrence: stale figures | UNCHECKED | the ledger grew 102 → 126 during the design; stamps + a grader |
| Recurrence: fix residue | UNCHECKED | if the review's confirmed count stops falling with findings inside the previous round's fix diff, change WHO writes the fixes (`docs/LESSONS_LEARNT.md` § "The finder and the fixer must not be the same agent") |
...
- **Still open, with its step:** when the hub baseline is seeded — Phase C step 6 (conditioned on infra's lock, both branches self-service). The gate registration + its grader + the workflow-doc bullet — infra's, requested by mail `01M2AJKKVGH8Q2PK51CM2GJ5FC` (Phase A Interfaces); the check runs standalone until then. The `command-run-protocol.md` paragraph — Phase B step 5 (mailed to the lock-holder). The cron line — Phase C step 7 (the operator installs it).
