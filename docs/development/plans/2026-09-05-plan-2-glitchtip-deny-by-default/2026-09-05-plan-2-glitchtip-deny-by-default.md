...
- `sentry-sdk[fastapi]>=2.18.0` stays the pin the scaffold emits (`src/fabrik/scaffold.py:2125`); the guard runs against the hub's installed sentry-sdk and RECORDS its version in the test output.
...
- **Given** the Node paragraph (`:247-255`), **When** diffed, **Then** it is byte-identical (the asymmetry correction of 2026-08-28 is not touched).
