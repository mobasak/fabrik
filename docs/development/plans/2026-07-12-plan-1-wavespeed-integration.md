...
**Status:** SUPERSEDED (2026-09-26, D-428) — never executed; its target, the hub's kilo-benchmarks catalog pipeline (`scripts/kilo-benchmarks/` scrapers, the hub `daily_refresh.sh`, `kilo_agents.db`), left fabrik with the catalog engine on 2026-08-15 (`73bde59a`), and every sibling scraper this plan copies (SiliconFlow, ModelScope, fal, Replicate) now lives in `/opt/ai-model-catalog/engine/`. Ingesting WaveSpeed's catalog is that repo's call — sent as prior art. WaveSpeed as a direct provider is unaffected (iterative_image_editor uses it).
**Date:** 2026-07-12
