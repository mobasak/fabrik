...
Status: EXECUTED 2026-08-29 — Zitadel v4 umbrella IdP LIVE at https://auth.ocoron.com (vps1): OIDC issuer + backchannel logout, valid LE cert, DNS resolving, DB+DSN, GlitchTip DSN, Gatus green. Battery green except the Prometheus /debug/metrics scrape (spec defect: metrics env not set — filed follow-up, not a rollback trigger). Whole-plan review: docs/development/reviews/2026-08-28-plan-deploy-zitadel-review.md
Service: zitadel
...
— ✅ S3 2026-08-29T00:27:13Z RUN 4 — `fabrik apply` COMPLETE (attempt 2; attempt 1 hit a transient SSH burst-throttle `kex_exchange_identification: Connection reset` at the readiness-poll → orchestrator rolled back but its rollback SSH also partially failed, leaving container+DB; torn down to clean slate + retried). The verifier health.disabled fix WORKED: verify() skipped the in-band probe, did NOT roll back → "✅ Deployment complete: https://auth.ocoron.com". All 6 registrars ran.
— ✅ S5 2026-08-29T00:27:13Z docker inspect: ZITADEL_DATABASE_POSTGRES_DSN=postgresql://zitadel:***@postgres-main:5432/zitadel (resolved) + GLITCHTIP_DSN/SENTRY_DSN=https://***@errors.vps1.ocoron.com/87 injected; container Up; in-network /debug/ready=200.
— ⚠️ S1 2026-08-29T00:27:13Z DNS — _provision_dns SILENTLY no-op'd (FINDING [fleet]: the hub DNSClient/site-provisioner SSH-proxy path returns EMPTY not error → get_records('ocoron.com')=0 while direct CloudflareClient sees 18; add_subdomain returned a dict without success=False so _provision_dns recorded success + never hit the CloudflareClient fallback → auth.ocoron.com was NOT in the zone). RECOVERED via the code's own fallback path (CloudflareClient.add_subdomain, proxied=False, matching fleet convention) → auth.ocoron.com → 172.93.160.197, confirmed in-zone, resolving via kiki/lex.ns.cloudflare.com + 8.8.8.8.
— ✅ BATTERY (in-network + DNS) 2026-08-29T00:27:13Z — /debug/ready=200 (in-network) · OIDC discovery issuer=https://auth.ocoron.com, backchannel_logout_supported=true, backchannel_logout_session_supported=true · DNS resolving (authoritative NS + Google) · 6 registrars provisioned (GLITCHTIP_DSN present, DSN resolved). PENDING: public TLS cert — Traefik's ACME hit LE 429 "5 failed authorizations/hostname/hour" from the DNS-absent attempts (RUN 3 + RUN 4-a); router correctly configured (Host(auth.ocoron.com), certresolver=letsencrypt, enabled); SELF-HEALING once the LE window clears (~01:15 UTC) now that DNS is live. Polling for issuance.
— ✅ CERT 2026-08-29T00:34:59Z — Let's Encrypt cert ISSUED (issuer O=Let's Encrypt CN=YR1, valid Aug 28–Nov 26); LE 429 self-healed once DNS was live. https://auth.ocoron.com/debug/ready=200 (ssl_verify ok), OIDC discovery over HTTPS: issuer=https://auth.ocoron.com backchannel_logout_supported=true.
— ✅ MONITORING 2026-08-29T00:34:59Z — Gatus: apps_zitadel endpoint GREEN (probes /debug/ready). GlitchTip: DSN injected.
— ⚠️ MONITORING-GAP 2026-08-29T00:34:59Z — Prometheus target registered (https://auth.ocoron.com/debug/metrics) but health=DOWN (404). FINDING [fleet]: Zitadel v4.17.1 does NOT serve /debug/metrics by default — the spec sets monitoring.metrics_path but never sets the enabling env, and docs/reference/zitadel.md:52 wrongly claims metrics is "enabled by default via ZITADEL_METRICS_TYPE: otel". FOLLOW-UP (spec fix, not a rollback trigger — the IdP is fully functional): add the metrics-enable env to specs/services/zitadel.yaml + re-apply, and correct the reference doc. Filed to STRATEGIC_BACKLOG.
