...
claim was premature until 2026-08-16**: `excise_manifest.py --check` was in fact exiting 1 on the
correct tree, because its ORDERING gate still allowed 6 steps while D.1 had deliberately added a
7th, `deliver_to_fabrik`. The remnant was always right; the certifier was wrong. Fixed by checking
the exact step NAME SET instead of a count — a count scored seven correct steps and seven wrong
ones identically. It now exits 0 with KEEP=30, and is mutation-proven in both directions). ⚠️ The review history below is kept deliberately: **11 passes found 150+ wrong claims**, and the lesson that mattered is that a ~100-file cross-referenced excise cannot be specified in prose — the set had to become a program. ⚠️ **NOT CONVERGED — corrected 2026-08-15.** This line
