...
- **A1** — **Given** the vendored `libs/web_scrape/` copy, **When** a caller runs `from libs.web_scrape import WebScraper, extract_nextjs_data`, **Then** the import succeeds without error. **Mocked:** none (real vendored module).
- **A2** — **Given** vendored `WebScraper`, **When** a caller constructs `WebScraper(cache_dir=Path("/tmp/x"), browserless_url="…", browserless_token="…")`, **Then** the instance is created without exception. **Mocked:** none (real constructor call).
- **A3** — **Given** the empty `ms_enrich.py` scaffold in place, **When** a caller runs `import ms_enrich`, **Then** the import succeeds (scaffold is Phase B/C-extensible). **Mocked:** none.
...
## Phase D — Ingest orchestration + `--ingest-new` CLI + daily_refresh wire + docs — ✅ EXECUTED 2026-07-10
