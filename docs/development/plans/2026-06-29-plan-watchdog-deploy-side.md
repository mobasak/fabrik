...
1. In the spec `watchdog:` block set `trigger_sources: [emitter, health, error_webhook]`. Empty/absent leaves `WATCHDOG_TRIGGER_SOURCES` unset → legacy poll path (no bus), unchanged. Setting `error_webhook` starts the sidecar's `:8889` ingest server.
   - **Also set `critical_paths`** (e.g. `[PaymentError, /checkout]`) — Option A pages an error_webhook signal only when its `error_type`/url-path substring-matches one. The driver renders `WATCHDOG_CRITICAL_PATHS` whenever set, **independent of Tier-D** (an alerting-only target has no bootstrap to carry `deps.critical_paths`). Without it the sidecar **captures but never pages** — the driver logs a warning. For Tier-D targets the env is additive with the bootstrap's `configure(critical_paths=…)`; the library unions both.
