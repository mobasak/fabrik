...
**Decision (Pass-2)**: Auto-*registration* (a) CANNOT ship — DEFERRED. Operator registers the GlitchTip webhook manually in the UI after each new watchdog-enabled deploy. **Closing sweep SHIPPED 2026-06-30** (commit on master): (b) operator recipe documented in `docs/operations/deployment.md` → "GlitchTip Webhook Registration"; (c) apply-time `ACTION REQUIRED` reminder for every `error_webhook` spec via `glitchtip.webhook_registration_reminder` (`src/fabrik/drivers/glitchtip.py`) wired into **`fabrik apply`** (the single deploy entry point, on `DeploymentState.COMPLETE`, via `cli._emit_glitchtip_webhook_reminder`) **and** `reconcile-all` — the apply wiring was added in adversarial review (it was reconcile-all-only, which would have silently skipped the reminder on the primary deploy path); (d) payload-shape contract test `tests/test_watchdog_ingest_payload.py` pinning the captured fixture (the real `:8889` parser lives in the fabrik-lib watchdog sidecar, not `drivers/watchdog.py` — the original item-3 grounding was slightly off). Tests: 5 green across `test_apply_emits_glitchtip_webhook_reminder.py` + `test_watchdog_ingest_payload.py`. Only **7a** remains a true DEFERRED leaf.
