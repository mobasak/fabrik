...
1. `cmd_harvest` (`scripts/thread_anchor.py:535-600`) computes `anchored = bool(matches) and _is_anchor(matches[-1])`
...
3. **The seam test** — `tests/test_work_hook_seam.py` gains the V1 walk through the REAL Stop hook
   ...
   naming an id claims nothing.
4. The register is untouched: every existing `tests/test_thread_anchor.py` test stays green unchanged,
