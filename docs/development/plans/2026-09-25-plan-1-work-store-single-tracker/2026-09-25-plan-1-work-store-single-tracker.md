...
- Decided HERE, at plan time, from the grounding (one row, D-419, minted with the plan's commit):
  ...
  - `kind: next` items belong to their session: they never appear in `ready`, `next` or the prompt's
    ready count (T02) — otherwise another session would pick up this session's current thread.
  ...
  - The prompt keeps this session's `your claim` lines and adds one `on it:` line per OTHER session, so
    every live claim appears exactly once (T02).
...
| T02 | The view: obligations, every claim, crisp ready, unnamed line | T01, T04 | ⛓️ | ⬜ | |
| T03 | mail.py claim/ack/requeue create and close the mail item | T01 | ⚡ | ⬜ | |
| T05a | The harvest's NEXT rules in on_harvest | T02 | ⛓️ | ⬜ | |
| T05b | The harvest passes next_anchored; the V1 seam test | T05a | ⛓️ | ⬜ | |
| T06 | next_census.py: the NEXT measurement | — | ⚡ | ⬜ | |
| T07a | The contract sentence in all three copies | T05b | ⚡ | ⬜ | |
| T07b | The landing docs | T02, T03, T04, T05b, T06 | ⚡ | ⬜ | |
...
(T04 feedback, T03 mail), then the view that reads them (T02), then the harvest (T05a, T05b), then the
contracts and docs that describe the merged code (T07a, T07b). `scripts/work.py` is one file, so T01 →
T02 → T05a run in sequence; `tests/test_work_claims.py` is shared by T02 and T05a, which the same
Depends chain orders. No two Depends-unconnected tickets share a path, so no `Serialized:` row is needed.
...
- **T04 → T02 — `queue_depths(ledger: Path | None = None) -> dict[str, int]`** in
  `scripts/command_feedback_report.py`, equal per command to `--queue`'s `N unanswered`, `{}` on any read
  failure. Seam test: `tests/test_work_view.py` (T02) loads the real script by path against a temp ledger.
...
- **mail.py's `_parse` → T02** (existing, `scripts/mail.py:1208-1226`), loaded by path by the mail
  obligation reader. Seam test: `tests/test_work_view.py`.
...
- **Review floor** — "every ticket, on the coder's return, runs `/fabrik-review` on its changed
  ...
  are governance-sync paths (`.pre-commit-config.yaml:164`), so T01, T02, T03, T05a, T05b and T07a take
...
- **Dispatch policy** — native Claude seats for every fan-out (the pool is OFF, D-181/D-182):
  ...
  before each fan-out. Coders: Opus for T01, T02, T05a, T05b (the lock-held harvest, the view's budget, the
...
- **Parallelism + merge** — T01 and T06 start together. Once T01 is merged, T04 and T03 run in their
  own worktrees alongside each other (disjoint Touches). T02 starts once T01 and T04 are merged; T05a
  after T02; T05b after T05a. T07a starts after T05b; T07b after T02, T03, T04, T05b and T06 — the two run
  together. Every merge happens in the main checkout in `## Merge Order`; a finished ticket whose
  predecessor in that order is not yet merged waits. The dedupe point is the orchestrator's merge step:
  each coder's return is reviewed, then merged, one at a time.
...
- **Given** the real Stop hook and an initialised temp repo, **When** one session stops three times on accepted free text, then on `NEXT: <open item id>`, then on `NEXT: operator decision: … <id>`, **Then** the store holds one superseded `next` item, the named item claimed by that session, and no change from the third Stop (spec § Validation V1)
...
- T02 `scripts/work.py:293` (`_worktrees`, main checkout first), `:672-688` (`_ready_from`, never reads
  `kind`), `:2503` (`cmd_ready`), `:2749-2776` (`cmd_status`), `:3067-3103` (`prompt_block`);
  `scripts/mail.py:1208-1226` (`_parse`), `:1509-1511` (`list_msgs` quarantines); `scripts/thread_anchor.py:196`
...
- T04 `scripts/command_feedback_report.py:45` (`_rows`), `:1248-1300` (`queue`), `:327-404`
  (`mark_answered`), `:1509-1519` (no window with `--queue` alone); `commands/_sources/fabrik-command-improve.md:85`, `:229-248`.
...
- T07a `CLAUDE.md:661`, `templates/governance/CLAUDE.md:650`, `:739`, `tests/test_work_contract_rule.py:59-68`.
...
- **The harvest seam** (Opus): `on_harvest` receives the raw last NEXT and cannot tell whether the
  register accepted it; an allowed Stop harvests twice with the same text; an open `kind: next` item
  would appear in every session's `ready`; `_set_next` stops at the first id; the Stop hook's session may
  be `nosession`; six tests pin today's `_set_next` and prompt behaviour (named in T05a, T05b, T02).
...
**(a) Coverage** — every "What we already agreed" line maps to a ticket: D1 → T02 (+ T04's
`queue_depths`); D2 → T01 (store), T03 (mail), T04 (feedback); D3 → T05a, T05b; D4 → T02; D5 → T01;
D6 → T02; D7 → T07a; the census and V5 → T06, T08; V1 → T05a, T05b; V2 → T03; V3, V4 → T02; V6 → T01;
the docs → T07b; the adoption and I16's announcement → T08; the plan-time decisions → T02, T03, T04,
T05a, T05b. No gap.

**(b) Cross-ticket signatures** — `open_linked`/`close_linked` read identically in T01 (producer), T03
and T04 (consumers) and § Interfaces; `queue_depths` in T04 and T02; `next_anchored` in T05a and T05b.
...
|---|---|---|---|
| 1 | FLOOR core/10-python — typed stdlib code, no file logging | PENDING | every new function in T01–T06; the census prints to stdout only |
| 2 | FLOOR 12-FACTOR (all twelve axes) — § Global Constraints | PENDING | § Global Constraints; no daemon, no logfile, no backing service |
| 3 | MATCHED core/40-documentation — the landing docs, fenced code only | PENDING | T07a, T07b |
| 4 | MATCHED core/45-testing-strategy — a test per behaviour, seen red | PENDING | every ticket's Behavior Contract names a test file in its Touches |
| 5 | fail-open vs fail-closed on every gate/guard — hooks and mail open, CLI loud | PENDING | T01 `open_linked`/`close_linked`, T02 `obligations`, T03, T04 `--take`, T05a's rules, T05b's signature guard |
| 6 | cost / quota / limit edges — the 2 s lock inside the 5 s harvest, the 3 s prompt budget, the READ budgets, the 7-day and 6-day windows, the top-10 cut | PENDING | T02 (V4 timing), T05a (one lock, one pass), § Evidence |
| 7 | boundary / sentinel / prefix — `W-` ids, `nosession`, the rule-1 prefixes, the mailbox name, `ack: required` | PENDING | T03, T05a, T02 |
| 8 | behaviour without a test — every G/W/T row names its grader | PENDING | the 46-row roll-up below |
...
- Whether the contract sentence breaks a pinned test: `tests/test_work_contract_rule.py:59-68` is updated in the same ticket (T07a).
...
- **The hub windows are unnamed**, so owners and `ready`'s "owned by this agent" section resolve to nothing until each is named (`CLAUDE_AGENT` at launch, or `python3 scripts/whoami_agent.py --as <name>`). The unnamed-window line (T02) makes it visible; naming is the operator's action, named again in T08's announcement.
