# T03 acceptance review — the pre-cutover check and fabrik app-role-check
...
```text
$ python scripts/review_rubric.py --changed src/fabrik/app_role_check.py src/fabrik/cli.py tests/test_app_role_check.py docs/QUICKSTART.md
...
### core/10-python.md  (hit: src/fabrik/app_role_check.py, src/fabrik/cli.py, tests/test_app_role_check.py)
...
### core/40-documentation.md  (hit: docs/QUICKSTART.md)
...
### core/45-testing-strategy.md  (hit: tests/test_app_role_check.py)
...
| Class | Status |
|---|---|
| Hunt: `src/fabrik/app_role_check.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (S1–S4, H2, H3, O1–O14, O16 — raw-line scan, structural compose services, connection-source suppression, every exception a failure), FIXED r2 (O17–O26, NEW1), FIXED r3 (O27–O29) — every hunk of `src/fabrik/app_role_check.py` hunted by sonnet×1 + haiku×1 + opus×1 |
| Hunt: `src/fabrik/cli.py` — every changed hunk, its enclosing function, its callers | FIXED r2 (O21 — a non-mapping spec prints SpecResolutionError and exits 1 in `src/fabrik/cli.py`) |
| Hunt: `tests/test_app_role_check.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (O16 — '--'/'//' markers, fetch failure and timeout, non-git repo, probe exceptions covered), FIXED r2 (O23 — the probe-exception test plants a real finding) in `tests/test_app_role_check.py` |
| Hunt: `docs/QUICKSTART.md` — every changed hunk, its enclosing function, its callers | CLEAN (the `fabrik app-role-check --spec` entry in `docs/QUICKSTART.md` matches the CLI's flag and exit codes, re-read in pass 4) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1–r3 (O6/O7/O25 exceptions and git timeouts now named failures; O5/O20 unparseable compose fails closed; O27/O29 null and case-variant keys no longer suppress) in `src/fabrik/app_role_check.py`; O30 recorded below |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (no counter or quota in the `src/fabrik/app_role_check.py` hunks) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r2 (O24 — `owner` means the value fullmatches the owner variable, never a substring; O19 pass-through spellings) in `src/fabrik/app_role_check.py` |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (O16) and r2 (O23 — the vacuous test) in `tests/test_app_role_check.py`; every fix item carries a test watched red on the prior head |
| Recurrence: denominator on every count — bounded searches state their bound | FIXED r1 (zero scanned files is a failure, not a clean pass, in `src/fabrik/app_role_check.py`); test totals quoted from pytest's own summary |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every claim was executed against a pinned copy of `src/fabrik/app_role_check.py` with fixture repos; the orchestrator re-ran the suites each round, 219 passed on the integrated tree) |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1 + opus×1 (T03 slice), one sonnet refuter | found: 23, new: 23, confirmed: 21, fixed: 21, unexecuted: 0 | method: citation — full slice pass over the T03 hunks + one hop; 2 refuted |
| Pass 2 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T03 owners) | found: 11, new: 11, confirmed: 10, fixed: 10, unexecuted: 0 | method: re-derivation — 21 ledger claims re-executed (all fixed); 10 new inside the r1 fix hunks (own-fix: round 1; NEW1 merged into O22) |
| Pass 3 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T03 owners) | found: 3, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | method: re-derivation — 10 ledger claims re-executed (all fixed); 3 new inside the r2 hunks (own-fix: round 2); scope-growth stop fired |
| Pass 4 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T03 owners), remainder round | found: 1, new: 1, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — O27–O29 re-executed (fixed); O30 recorded below per the scope-growth rule |
...
|---|---|
| T03-check-H1 | REFUTED (the registrar's call site passes spec.get('name') or spec.get('id') at infrastructure.py:449, which the check mirrors) |
| T03-check-O15 | REFUTED (the fetch argv never carries the remote URL and git redacts user:token in its own error text; reproduced) |
| T03-check-O9-multiline | RECORDED — by design (D-386) — the scan is line-by-line; multi-line DDL run as the app role fails at runtime, since it holds USAGE and never CREATE, and the privilege probe proves that |
| T03-check-O12-nested | RECORDED — by design (D-386) — nested test/build/libs directories hold vendored or test code; any DDL there run as the app role fails, since it holds no CREATE |
| T03-check-O30 | RECORDED — measured (own-fix residue raised after the scope-growth stop: a YAML null or an empty value with a trailing comment on a migrate service's DATABASE_URL key still suppresses the migrate finding; routed to D7 whole-plan validation for T03's coder to fix) |
...
### Phase 1 — T03 the pre-cutover check and `fabrik app-role-check`: CLEAN after r1–r3 fixes — 219 tests pass on the integrated tree (T01–T03 plus the CLI suites); O30 carried to D7
