# Review — `command_run.py done` artifact gate: a sibling's mid-loop report vetoed a committed close — 2026-09-02

Status: CONVERGED
Surface: 8fec476e0d026c13cb33aa7db343f62c8f5fa3e9 ef4827073a270438d94f26bdf192e1c9
Scope: `scripts/command_run.py` (the `done` artifact gate — the commit walk also runs when every working-tree candidate is mid-loop; `_midloop_report` hoisted to module level) · `tests/test_command_run.py` (one appended test) · CHANGELOG entry.
Anchor: no prior review for this scope → full WIDE pass 1.
NO-POOL: standing session directive — no Agent tool; every finder partition run in-line by the reviewing session with executed probes.

## Rubric (verbatim — `python scripts/review_rubric.py --changed scripts/command_run.py tests/test_command_run.py`)
...
### core/10-python.md  (hit: scripts/command_run.py, tests/test_command_run.py)
...
|---|---|---|
| core/35-security-auth (FLOOR) | CLEAN | no auth/secret surface; the change reads review headers and git metadata only |
| core/25-data-postgres (FLOOR) | CLEAN | no DB — grep `psycopg|DATABASE_URL` in the diff: 0 |
| core/30-ops (FLOOR) | CLEAN | no deploy/compose surface; `command_run.py` is governance-synced — the change is correct for every project (same gate everywhere) |
| 12-FACTOR (FLOOR) | CLEAN | no config, no state added; one pure predicate hoisted |
| core/10-python (MATCHED) | REFUTED(1) | ruff format reflowed one unrelated over-long `print` at `:458` (whitespace only, same string) — kept; `re` imported at `:56`; the hoisted predicate is byte-identical in regex and 10-line header zone to the removed nested def (diffed) |
| core/45-testing-strategy (MATCHED) | CLEAN | the new test seen red first (`1 failed` before the fix), `sleep(1.1)` justified by the gate's whole-second `commit_epoch >= int(started_epoch)` rule, `_artifact_repo` pins no author date (the only `GIT_AUTHOR_DATE` is another test's `env_extra` at `:1878`); both suites 122/122 ×3 |
| Fail-open vs fail-closed on every gate/guard | CLEAN | the round-135 refusal is still reached when the tree AND the window are mid-loop-only (`test_done_refused_when_every_artifact_is_midloop` green; branch read: the commit walk adds nothing → `all(_midloop_report)` over the same list → refusal); the widening only ADDS candidates, never removes a refusal |
| Cost/quota/limit accounting edges | CLEAN | the commit walk keeps its bounds (`--since=<run start>`, `-n 500`, `-m`, `%at`) — untouched by the diff |
| Boundary/sentinel/prefix collisions | CLEAN | a sibling's COMMITTED IN-PROGRESS report inside my window is a candidate too; the union is judged with `all()`, so one CONVERGED report from either walk suffices — the text's promise, now the control flow |
| Behavior-without-a-test | FIXED(1) | the veto class had no test — `test_a_siblings_dirty_midloop_report_cannot_veto_a_committed_converged_close` |

## Disposition ledger

| # | candidate | class | disposition |
|---|---|---|---|
| 1 | an unrelated `print` at `:458` reflowed by ruff format | core/10-python | REFUTED — whitespace-only, string identical (`git diff` hunk read); the file was one line short of formatted at HEAD |
| 2 | the widened condition might re-open the round-135 mid-loop-only hole | fail-open | REFUTED — traced: with no committed report in the window the commit walk adds nothing and the same `all(_midloop_report)` refusal fires; the round-135 test stays green |

## Pass Ledger

| Pass | method | found | new | fixed | finders |
|---|---|---|---|---|---|
| Pass 1 | method: citation | found: 2 | new: 2 | fixed: 0 | finders: in-line (NO-POOL) — hole re-opening trace · union semantics · hoist diff · round-139 seams · test timing/harness · CHANGELOG claims; dispatched: 6 partitions, returned: 6 |
| Pass 2 | method: re-derivation | found: 0 | new: 0 | fixed: 0 | finders: in-line, FULL fresh read of the final diff; suites re-run 3× (122/122); the refusal branch re-read; regex diffed byte-for-byte; both candidates re-adjudicated with no new evidence |
...
- (a) pre-existing: the working-tree walk still accepts ANY fresh `*.md` under reviews/ as an artifact (the round-139 symlink/deletion guards remain); a sibling's mid-loop report continues to appear in the roster — by design, it is named so the operator can see who poisoned the walk.
