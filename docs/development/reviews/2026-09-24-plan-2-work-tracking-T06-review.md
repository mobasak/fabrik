# T06 review — the completion gate's advisory work-items sync row
...
```text
$ python scripts/review_rubric.py --changed scripts/final_gate.py docs/workflows/FINAL_GATE_WORKFLOW.md tests/test_final_gate_work_row.py
...
### core/45-testing-strategy.md  (hit: tests/test_final_gate_work_row.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/final_gate.py` — every changed hunk, its enclosing function, its callers | FIXED r1-r2 (round 1: the bare run_optional_check replaced by `_work_sync_row` — red only on exit 1 with a `DRIFT <n> (blocking)` line, advisory drift ⚠-prefixed into --json warnings, a 30 s `TIMEOUTS["work_sync"]`, everything else a ⚠ pass; round 2: a traceback never reds, a could-not-run row is the named ` (NOT RUN — <why>)` skip, the ⚠ names the last non-fallback line) |
| Hunt: `docs/workflows/FINAL_GATE_WORKFLOW.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (the Gate-wired list re-derived by instrumented run: 64 rows, 60 through run_optional_check and 4 appended directly, gaining Corpus Weight and INDEX.md ↔ new docs; GATE-COUNTS tier2=56 and the 35-of-36 inheritance re-derived from code by two seats; the Work items bullet matches the helper) |
| Hunt: `tests/test_final_gate_work_row.py` — every changed hunk, its enclosing function, its callers | FIXED r1-r3 (6 → 12 tests: old work.py, timeout, WorkError, traceback, fallback-line ordering and --json visibility added; the blocking test now reads status and failures from the real `main --check --json` envelope; every fix's test seen red on revert by the seats) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1-r2 — a broken work.py (timeout, exit 2, WorkError, traceback) is a VISIBLE skip, never a red and never a silent pass: named ` (NOT RUN — <why>)`, listed in skipped_checks, roster `skipped`, ⚠ in warnings; paths hunted: _work_sync_row, run_cmd, _check_roster, _summarize_skipped, the --json warnings builder |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | FIXED r1 — one subprocess per Tier-2 gate run, bounded by `TIMEOUTS["work_sync"]` = 30 s (was the 120 s default); the row ran in 0.06 s with no store on the pin; no paid calls |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r2 — the blocking match is the anchored `^DRIFT \d+ \(blocking\)` and is refused when the output holds a traceback, since work.py labels classes 2-6 blocking even in the advisory window; the ⚠ line skips `local regex fallback` notices; paths hunted: _BLOCKING_DRIFT, the crashed guard, the last-line selection |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r3 — the 4 Behavior-Contract rows each proven by a named test; the fallback-line filter had no killing test until r3 (A-O11), now killed by test_a_fallback_notice_on_stderr_after_the_stdout_error_is_skipped (1 of 12 fails with the filter removed) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (every seat stated its denominators: tests passed of total — 10, 13, 15, 12; ledger rows verified 12 of 12, 3 of 3, 2 of 2; fleet `scripts/work.py` present in 1 of 50 /opt/*/scripts/final_gate.py repos; Gate-wired rows 64 of 64 listed) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (the orchestrator re-executed each confirmed class on a pin copy by importing final_gate.py and calling the row with stub work.py scripts: old work.py, hang, non-git, no store in r1; traceback after a blocking line and the timeout roster in r2; the refuted-by-measurement A-S1 was executed by its seat) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2 (A: scripts/final_gate.py + FINAL_GATE_WORKFLOW.md sonnet+haiku+opus; B: tests/test_final_gate_work_row.py sonnet+haiku) + 2 refuters | found: 14, new: 14, confirmed: 12, fixed: 12, unexecuted: 0 | citation — the full partitioned pass (workflow wf_a1aaa84b-cca) over c38f6fe1d..d02298bb1; the orchestrator re-executed the fleet-safety cluster on a pin copy (old work.py without `sync` → False, a hang past the timeout → False, a non-git directory → False, a git repo with no store → True); confirmed — the row reds on a timeout, an old work.py, a WorkError or a non-git directory (A-S1, A-S2, A-O2, A-O3, A-O6, B-S1, B-S2), it takes the 120 s default timeout (A-S4), advisory DRIFT lines never reach --json (A-O4), the Gate-wired list misses 2 rows and its totals (A-O1), the missing tests (A-S3) and a tautological status assertion (B-S3); RECORDED A-O5 (the ticket names the fleet ⚠ not-present row until T07), A-O7 (one reading per sync --check run, spec :100); seats: A-sonnet 4/4 · A-haiku 0/0 · A-opus 7/7 · B-sonnet 3/3 · B-haiku 0/0; stop: confirmed 12 |
| Pass 2 | native opus×1 + sonnet×2 + haiku×2 (A: sonnet+haiku+opus; B: sonnet+haiku) + refuters | found: 4, new: 4, confirmed: 4, fixed: 4, unexecuted: 0 | re-verification — the slice owners over their own ledgers (workflow wf_0999eaf7-ac1) on d02298bb1..29ba490d5 plus one hop; all 12 pass-1 rows verified FIXED by execution (A 9/9, B 3/3; reverting the helper turns 4 of 9 tests red); re-executed by the orchestrator on the pin, the fix's own lines: a work.py printing a `(blocking)` DRIFT line then raising reds the gate, because work.py labels classes 2-6 blocking even in the advisory window (A-S5, A-O9), a could-not-run row reads roster `pass` and stays out of skipped_checks (A-O8), and the ⚠ line shows the first output line, a benign import warning, not the error (A-O10); seats: A-sonnet 1/1 · A-haiku 0/0 · A-opus 3/3 · B-sonnet 0/0 · B-haiku 0/0; stop: confirmed 4 |
| Pass 3 | native opus×1 + sonnet×2 + haiku×2 (A: sonnet+haiku+opus; B: sonnet+haiku) + refuters | found: 3, new: 3, confirmed: 2, fixed: 2, unexecuted: 0 | re-verification under the scope-growth stop — the slice owners over the named pass-2 set (workflow wf_b517a28e-3e5) on 29ba490d5..b73f5a07e; A-S5, A-O8, A-O10 verified FIXED by execution (a traceback after a blocking line → the `(NOT RUN — exit 1)` skip; a timeout → roster `skipped` and named in skipped_checks; the ⚠ names the real error); CONFIRMED (A-O11, B-S1, the same gap): with the fallback-line filter removed, all 11 tests still pass, because the fixture puts the real error last — a test-only fix; RECORDED A-S1 (measured); seats: A-sonnet 1/1 · A-haiku 0/0 · A-opus 1/1 · B-sonnet 1/1 · B-haiku 0/0; stop: confirmed 2 |
| Pass 4 | native opus×1 + sonnet×2 + haiku×2 (A: sonnet+haiku+opus; B: sonnet+haiku) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass under the scope-growth stop: the same slice owners re-ran the named check on the pin of 6961a658a (workflow wf_e5bb1a6d-41d); A-O11 and B-S1 FIXED by execution (unmutated 12 passed; the fallback filter removed → exactly test_a_fallback_notice_on_stderr_after_the_stdout_error_is_skipped fails, 1 of 12), counts re-derived on the pin; seats: A-sonnet 0/0 · A-haiku 0/0 · A-opus 0/0 · B-sonnet 0/0 · B-haiku 0/0; stop: confirmed 0 |
...
| A-O5 | RECORDED — measured (the T06 ticket's Scope names the `⚠ check not present` row for a repo without scripts/work.py; it lasts in the fleet only until T07 distributes work.py) |
| A-O7 | RECORDED — measured (every Tier-2 gate run appends one reading; the spec defines one line per sync --check run, spec :100, and the blocking rule counts calendar days, not runs) |
| A-O3 (CLI half) | RECORDED — measured (work.py sync --check in a non-git directory exits 1 through WorkError; CLI verbs fail loud by the spec, and the gate side is the named skip) |
| A-S1 | RECORDED — measured (the crashed guard matches the interpreter's traceback header anywhere in the output; work.py emits that line only from an uncaught exception, and the probe needed a synthetic stub that prints it without crashing) |

## Per-phase verdicts

### Phase 1 — T06, the completion gate's advisory work-items sync row: CONVERGED — 4 passes, confirmed 12 → 4 → 2 → 0; 12 tests in tests/test_final_gate_work_row.py; the 4 Behavior-Contract rows each proven by a named test; tier2 stays 56
