# T06 of plan 2026-09-25-plan-1-work-store-single-tracker
...
```text
$ python scripts/review_rubric.py --changed scripts/sysadmin/next_census.py tests/test_next_census.py
...
### core/45-testing-strategy.md  (hit: tests/test_next_census.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/sysadmin/next_census.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (C-S1/C-O1 the last NEXT-bearing row kept; C-S2/C-O4/C-H1 the V5 window bounded both ways; C-S3/C-O7 rows deduplicated; C-S4/C-O3 Claude Code's project-dir mapping; C-S5/C-O12 streamed reading; C-O5 any parse error a skipped line; C-O6 every bad line counted, files and lines reported apart; C-O8 fenced NEXT lines skipped; C-O9 an import failure reported plainly; C-O11 --since must be positive), r2 (C2-S1/C2-O1/T2-sonnet1 the dedup keyed on the row uuid, so a message's later content block still counts) — scripts/sysadmin/next_census.py |
| Hunt: `tests/test_next_census.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (C-O2 unequal none/blocked counts, the V5 item bound, window, sidechain and closed-id tests; T-S2/T-S3/T-S4), r2 (T-L1 an unreadable file pins skipped files; the same-message second-block test) — every test red on its named mutant; tests/test_next_census.py 28 tests |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (C-O9 an import failure no longer prints a real-looking zero census; C-O6 no bad line dropped uncounted; C-O5 no traceback escapes) — scripts/sysadmin/next_census.py main and _scan |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | FIXED r1 (C-S5/C-O12 transcripts streamed line by line instead of loaded whole) — scripts/sysadmin/next_census.py _scan |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (C-S2 the V5 window's lower bound; C-O11 --since zero and negative refused; C-O3 the dir-name mapping) — scripts/sysadmin/next_census.py |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (C-O2 five surviving mutants now killed), r2 (T-L1 the file-skip path, the uuid dedup key) — tests/test_next_census.py |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the receipt's counts carry their totals: 27 + 3 + 0 candidates over three passes; 28 passed in tests/test_next_census.py for scripts/sysadmin/next_census.py) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every confirmed candidate executed by its refuter on a copy of scripts/sysadmin/next_census.py against a temp --root; the pass-2 seat re-ran the none/blocked mutant the coder had only inspected) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 | found: 27, new: 27, confirmed: 22, fixed: 22, unexecuted: 0 | method: citation — file partition (C: scripts/sysadmin/next_census.py with the Opus floor; T: the test file), workflow wf_04d05e31-ed5, every seat on a temp --root; every candidate executed by its refuter; 1 refuted, 4 RECORDED |
| Pass 2 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 (the round-1 slice owners) | found: 4, new: 4, confirmed: 4, fixed: 4, unexecuted: 0 | method: re-derivation — 21 of 22 ledger claims NOW_FALSE, T-L1 STILL_TRUE; 3 new inside pass 1's dedup hunk (own-fix 3 of 4): the orchestrator's round-1 recipe keyed the dedup on message.id |
| Pass 3 | native opus×1 + haiku×1 (the round-1 owners of slices C and T) | found: 1, new: 1, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: C2-S1, C2-O1, T2-sonnet1 and T-L1 NOW_FALSE with their mutants red; the one raise, a resumed copy under a new uuid, measured absent on the box and RECORDED; standing clean since pass 1: denominator, proxy-evidence |
...
| P1-T-H1 | REFUTED (an underscore-joined word after none is not a leading whole-word hold; the refuter's probe classified it free text) |
| P1-C-O13 | RECORDED — measured (a session started in a subdirectory is named by that directory; `-opt-fabrik-lib` and a subdirectory of fabrik cannot be told apart by name, so V5 counts only sessions started at the repo root) |
| P1-C-O10 | RECORDED — measured (`over <n> sessions` is the ticket's own line format: every session scanned, NEXT or not) |
| P1-T-H3 | RECORDED — measured (a transcript whose mtime equals the cutoff to the millisecond may fall either side) |
| P1-C-H2 | RECORDED — by design (D-420) — a hold on `none-…` follows the classify_next the store shares |
| P3-C-O1 | RECORDED — measured (a resumed copy under a new uuid would count twice; over the hub's 35 transcripts and 326,976 assistant rows, 0 of 128,166 message ids appear in more than one file, while 56,360 rows repeat a uuid within a file and 2-6 rows share one message id — the uuid key is the one that matches the data) |

## Per-phase verdicts

### Phase 1 — T06 next_census.py, the NEXT-line measurement: CLEAN
