# Review — secrets-binary-decode

**Status:** CONVERGED on the D-278 scope-growth stop — the reported crash and its whole class are closed and mutation-proven; confirmed/own-fix ran 14/3, 9/8, 6/6 and `command_run.py` printed the stop at 2 of the last 3. Every class the closing round opened is FIXED, not routed; the one routed item is the fleet-wide survey of the other 35 scripts
**Surface:** `git rev-parse HEAD` = ea4e6bb733b8838649c7a82105a9a7617a3d8610; `git diff HEAD -- scripts/enforcement/check_secrets.py tests/test_check_secrets_placeholders.py` md5 d535fac1bf163c121ae12e086086d54e (5195 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/enforcement/check_secrets.py`, `tests/test_check_secrets_placeholders.py`
...
```text
$ python scripts/review_rubric.py --changed scripts/enforcement/check_secrets.py tests/test_check_secrets_placeholders.py
...
### core/45-testing-strategy.md  (hit: tests/test_check_secrets_placeholders.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/enforcement/check_secrets.py` — every changed hunk, its enclosing function, its callers | FIXED r3 (the reported strict-UTF-8 decode, plus five more in the same two functions: quoted paths silently skipped, undecodable paths crashing, a binary diff silencing its own file, a manufactured hunk header reaching MemoryError, and `Path.is_file()` propagating ENAMETOOLONG; 78 import sites surveyed fleet-wide, zero external callers of the changed private helpers) |
| Hunt: `tests/test_check_secrets_placeholders.py` — every changed hunk, its enclosing function, its callers | FIXED r3 (14 graders added; four of my own were false or blind — one passed only under pytest's capture, one derived its bound from the constant under test, one asserted the truncation that was itself the defect, one monkeypatched the predicate whose real behaviour was broken) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r3 (the review's centre: widening the fail-open arms made an unusable git read SILENTLY CLEAN on a secrets gate, and `run_optional_check` discards stdout on rc 0 so no warning could surface — now a loud rc 1, with the work-tree probe moved off the very git that is broken) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | RECORDED — measured (the scope bound is the limit at its edge: per-header clamping left the accumulated set unbounded at ~1.85 MB of child RSS per byte of crafted content, so it now fails OPEN; pool OFF per D-181/D-182, five native seats across three rounds, each stamped before dispatch) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r3 (the sentinel WAS the bug: `set()` meant "allow nothing" downstream while `None` means "scan everything", so a binary diff silenced its own file; `_HUNK_RE`'s digit runs are bounded to `\d{1,9}` because `int()` raises above 4300 digits; git's text/binary boundary re-derived at exactly 8000 bytes) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r3 (15 mutants in round 3 left four survivors — two `-z` probes ungraded, the work-tree predicate, and one equivalent; the first three are now killed, re-proven in a throwaway worktree: 2 failed / 1 failed / 1 failed) |
| Recurrence: denominator on every count — bounded searches state their bound | FIXED r3 (three wrong denominators were caught and corrected: "36 files" was the count of files CONTAINING a `text=True` call against a directory of 78; "4 exposed" was a file-count subtraction, not a set difference, against 85 of 88 call sites; and a fleet scan of 184 bad files was a truncation artifact that cleared to 63 of 102,145 once full blobs were decoded) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | FIXED r3 (a grader that called `main()` in-process passed while the shipped code crashed, because pytest replaces stdout with an `errors="replace"` writer — it now runs a real subprocess the way `final_gate` does, and the whole suite is run `-q` AND `-q -s`) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×1 | found: 20, new: 0, confirmed: 14, fixed: 11, unexecuted: 0 | full partitioned pass, disjoint slices by file |
| Pass 2 | native opus×1 + sonnet×1 | found: 12, new: 0, confirmed: 9, fixed: 9, unexecuted: 0 | delta over the round-1 fix plus one hop of callers |
| Pass 3 | native opus×1 | found: 8, new: 0, confirmed: 6, fixed: 6, unexecuted: 0 | method: re-derivation — a fresh non-authoring seat re-derived every shipped claim from its primary source (the 8000-byte text boundary, `pathlib`'s ignored errnos, `update-index` accepting an overlong path at rc 0, `splitlines()`'s exact split set) and ran a 16-scenario totality battery, an `index.lock` concurrency case and a 16-file detection-parity corpus against HEAD; closed on the D-278 scope-growth stop (14/3 → 9/8 → 6/6) |
...
| F-survey | RECORDED — by design (D-308) — 85 of 88 `subprocess.run(text=True)` call sites across 35 other files in `scripts/enforcement/` carry the same unguarded decode; routed with its measured classification, not fixed here |
| F6-r3 | RECORDED — measured (`errors="ignore"` is indistinguishable from `"replace"` to this suite; both are safe because `\n` is never part of a multi-byte sequence, so neither can merge lines) |
| F9-r3 | RECORDED — measured (`SubprocessError` in the widened arms is an equivalent mutant: no `timeout=` was in play when it was raised, so `TimeoutExpired` was unreachable — a `timeout=` has since been added) |
| F8-r3 | RECORDED — by design (D-308) — the printed spelling of an undecodable path matches no shell tool, but `backslashreplace` is the only one of three that survives `final_gate`'s own strict-text parent: `surrogateescape` relocates the crash into the runner, `replace` loses the byte |
| F7-r3 | RECORDED — by design (D-308) — `sys.stdout.reconfigure` mutates the caller's stdout for the process; harmless (strictly more permissive) and the only in-process callers are two hub test modules |
...
### Phase 1 — the Secrets leg survives what git hands it: CONVERGED on the D-278 scope-growth stop
...
(the `ruff` rows read SKIPPED because `--check` is the read-only tier; ruff was run directly on both changed files — `All checks passed!`. The hub's pytest leg is off by contract, so the suites were run by hand: 23 in the changed file both with and without pytest capture, plus 37 in `test_enforcement*.py`.)
