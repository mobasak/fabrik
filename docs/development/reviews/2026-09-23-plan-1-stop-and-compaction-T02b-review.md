# T02b — templates/governance/CLAUDE.md: the DECISION block in every § FINAL OUTPUT copy
...
```text
$ python scripts/review_rubric.py --changed templates/governance/CLAUDE.md tests/test_governance_template_split.py
...
| Class | Status |
|---|---|
| Hunt: `templates/governance/CLAUDE.md` — every changed hunk, its enclosing section, its callers | FIXED r1 (R1-T02b-O1/O2: `hub D-054` in both copies, `hub D-374` in # Compact instructions; byte-identity with the hub checked in python by three seats across both § FINAL OUTPUT copies; every rule T02a restored present in each copy; the index line keeps its anchor case-exact) |
| Hunt: `tests/test_governance_template_split.py` — every changed hunk, its enclosing section, its callers | FIXED r1-r2 (R1-T02b-O3 closed hub-prefix mapping; R1-T02b-O4 per-copy span through the REFUSED sentence; R1-T02b-O5 exactly two copies; R2-T02b-O1 the span ends at the next paragraph — every change red on a named mutant, pass 3 re-ran four) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1-r2 (graders that stayed green on an inverted rationale, a renamed copy, or appended text — R1-T02b-O4/O5, R2-T02b-O1) |
| Recurrence: cost/quota accounting — a counter or limit at its edges | CLEAN (no counter or limit in the template hunks or the graders) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1-r2 (a bare `D-054` resolving to a different project row — R1-T02b-O1; a span ending at a literal word — R2-T02b-O1) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (R1-T02b-O4) |

## Pass Ledger

| Pass | Finders | Counters | Method |
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×1 (finders sonnet, haiku, opus; refuter sonnet) | found: 7, new: 7, confirmed: 4, fixed: 4, unexecuted: 0 | method: citation — wide pass over 43a53e99e; byte-identity of every copy extracted and compared in python by three seats; confirmed claims re-executed by the orchestrator |
| Pass 2 | native opus×1 + sonnet×2 + haiku×1 (the round-1 seats over their own slice ledger; refuter sonnet) | found: 1, new: 1, confirmed: 1, fixed: 1, unexecuted: 0 | method: re-derivation — 4 ledger claims re-executed on 537f359ab from a full scratch tree, all NOW_FALSE; 1 new in a round-1 hunk (own-fix: round 1) |
| Pass 3 | native opus×1 (the round-1 owner of the slice) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the one round-2 claim re-executed on f688d6e0f with four mutants (append and insert, in each copy), all red; unmutated green; orchestrator ran two of them independently |

## Dispositions

| ID | Disposition |
|---|---|
| R1-T02b-O1 | CONFIRMED → FIXED r1 (`hub D-054` in both copies; D-054 is a different row in 2 of 49 project repos) |
| R1-T02b-S1 | CONFIRMED (dup of R1-T02b-O1 and R1-T02b-O2) |
| R1-T02b-O2 | CONFIRMED → FIXED r1 (`hub D-374` in # Compact instructions; no project repo carries D-374) |
| R1-T02b-O3 | CONFIRMED → FIXED r1 (the byte-identity graders take a closed hub-prefix mapping as their only sanctioned divergence) |
| R1-T02b-S2 | CONFIRMED (dup of R1-T02b-O3) |
| R1-T02b-O4 | CONFIRMED → FIXED r1 (per-copy byte-identity now spans the format block through the REFUSED verdict sentence) |
| R1-T02b-O5 | CONFIRMED → FIXED r1 (exactly two § FINAL OUTPUT copies asserted) |
| R1-T02b-note | RECORDED — measured (pre-existing bare hub D-ids at template :37, :70, :444, :471 with no written rule for the `hub` prefix; one hop out of this diff; docs/STRATEGIC_BACKLOG.md row filed with the T02b merge) |
| R2-T02b-O1 | CONFIRMED → FIXED r2 (own-fix: round 1) (the span ends at the next paragraph marker) |

## Gate

On the branch head f688d6e0f, run by the orchestrator: `tests/test_governance_template_split.py` — 39 passed; ruff clean. The template is a governance-sync path: the merge is a plumbing commit, so the post-commit sync is run by hand right after it. The whole-plan `final_gate.py` run is in the plan's integration receipt (T06).

## Per-phase verdicts

### T02b — the fleet template: PASS
