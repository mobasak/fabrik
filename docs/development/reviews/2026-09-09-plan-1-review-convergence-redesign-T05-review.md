# T05 — the board banner sentence, its doc and its own grader — per-ticket review
...
```text
$ python scripts/review_rubric.py --changed scripts/sysadmin/quota_dashboard.py docs/workstation/quota-dashboard.md tests/test_quota_dashboard_banner.py
...
### core/10-python.md  (hit: scripts/sysadmin/quota_dashboard.py, tests/test_quota_dashboard_banner.py)
...
### core/40-documentation.md  (hit: docs/workstation/quota-dashboard.md)
...
### core/45-testing-strategy.md  (hit: tests/test_quota_dashboard_banner.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/sysadmin/quota_dashboard.py` — every changed hunk, its enclosing function, its callers | CLEAN — the banner rendered from the pinned module with a stubbed probe payload and from the pre-fix module (`git show 0c1f2d50:…`): the new sentence names the partition (Opus on the risky slices, Sonnet on the rest, at most one Haiku class seat, `--slices`; `--units` + the three-seat floor for grounding/adjudication) and the retired literal is absent; no other banner text changed (the DO-NOT clause), `_budget_caveats` untouched; RECORDED: the shipped sentence joins with a colon where the ticket's Scope quote has an em dash (the sentence already carries one em dash before it — wording, no contract row asserts the punctuation) |
| Hunt: `docs/workstation/quota-dashboard.md` — every changed hunk, its enclosing function, its callers | CLEAN — the 4 added lines match the rendered sentence's parenthetical verbatim and the spec's partition semantics (§ D2/D3/D5); the retained `(2026-09-08, D-191)` heading citation and the banner's `D-189/D-191` label cite decision ids, not the retired wording; the script's `# AFTER-EDIT:` header names this doc and the doc moved in the same diff |
| Hunt: `tests/test_quota_dashboard_banner.py` — every changed hunk, its enclosing function, its callers | CLEAN — the one test executed against the pre-fix module (positive control passes, the absence assertion fails: red) and the pin (green); the absence substring `one Sonnet + one Haiku` cannot collide with the new sentence (`at most one Haiku class seat`); the ticket's Gate line run against the pin plus the real `tests/test_quota_dashboard.py`: 3 passed, 122 deselected |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN — the banner's `except Exception` fallback ("Box budget unavailable") pre-exists and is out of scope; the seat verified the rendered html carries the real numbers, not the fallback string, so the test's assertions are not vacuous |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — the T04→T05 seam executed: T04's pinned `dispatch_headroom.py --units 1 --json` run live and every key `_budget_probe` reads (`caps` incl. `concurrency_cap`/`quota_cap`, `quota`, `box_caps`, `siblings`, `box_caps_floored`) present with the expected shape; no numeric change on this surface |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — a prose change; the 9 retired-literal phrases grepped across the whole pinned script and doc (0 live hits of 9 phrases in 2 files) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN — the ticket's two Behavior Contract rows (the partition named; the literal gone) are the two halves of the one test, each proven red on the pre-fix module by the coder (copy-based revert) and by the seat |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — 3 of 3 changed files read whole, 8 of 8 probe keys matched, 9 phrases × 2 files, 7 of 90 added lines over 100 chars (E501 is ignored by `pyproject.toml`), 2 seats stamped |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — the banner was RENDERED (not read) on both modules, the seam probe was RUN against T04's script, the Gate line was RUN against the pin; the CHANGELOG obligation the seat raised is the orchestrator's merge Delta by the plan's own words (the ticket's Docs row: "CHANGELOG entry (Deltas)") — REFUTED as a coder defect |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1 (2 seats stamped — one code/doc/test unit and the mechanical classes, D-203 partition; the surface has no risky slice) | found: 2, new: 2, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — citation — the 90-line diff at b407555b pinned by md5; the banner rendered from the pinned and the pre-fix module, the seam probe against T04's pinned script, the new test red on the base and green on the pin, the ticket's Gate line run (3 passed, 122 deselected), the mechanical sweep (retired literals 0 of 9 phrases live, ruff clean incl. format, AFTER-EDIT header names the doc, test name/body agree). 2 candidates: the missing CHANGELOG entry — REFUTED (an orchestrator merge Delta by the ticket's Docs row); the colon-for-em-dash join — RECORDED (wording). 0 CONFIRMED — quiet under D-206: the review closes on its first round |
| Pass 2 | native sonnet×1 (1 seat stamped — the corpus gate's confirming round) | found: 6, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the banner RENDERED again from the pinned and the pre-fix module with a stubbed probe (the retired literal present in the old, absent in the new; the partition sentence verbatim), the new test run against both modules (red on the base, green on the pin), the T04→T05 seam re-run against T04's script (5 of 5 keys read are emitted), the 9 retired phrases grepped across both files (18 checks; the `D-191`/`D-186` hits are decision citations, not the retired sentence; `three-seat floor` is the new wording), ruff clean. 6 candidates, 5 REFUTED by re-derivation, 1 PLAUSIBLE (the CHANGELOG entry — the orchestrator's merge Delta by the ticket's Docs row). 0 CONFIRMED — quiet under D-206 on the confirming round: the review closes |
...
### Phase 1 — the board banner sentence (T05): CLEAN — one executed review round (2 candidates, 0 confirmed): the banner rendered from the pinned and the pre-fix module, the T04→T05 `--json` seam executed against T04's pinned script, the new test red on the base; the CHANGELOG obligation is the orchestrator's merge Delta by the ticket's own Docs row.
...
`final_gate.py --lean --check --json` on a shared clone of master (bd58e5c0) carrying the T05 merge content, 2026-09-09.
