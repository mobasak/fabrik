# Review — T01 of plan 2026-09-25-plan-1-work-store-single-tracker
...
```text
$ python scripts/review_rubric.py --changed scripts/work.py tests/test_work_linked.py
...
### core/45-testing-strategy.md  (hit: tests/test_work_linked.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/work.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (W-S1/W-O1 the duplicate drop's partial write — keep's pre-image restored; W-O2 msg digests absorbed and a live holder wins; W-O6 close_linked closes every item of the link), r2 (W-O7 restore only when the duplicate is uncommitted; W-O8 close_linked per item), r3 (W-O9/W-O10 class rewrite: _close raises only when nothing was committed, scripts/work.py:2581-2604) |
| Hunt: `tests/test_work_linked.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (W-O3, W-O4, W-O5 — seven mutations of scripts/work.py that stayed green now red), r2 (T-S1 test made real — _write_item fails for the duplicate's id only); tests/test_work_linked.py 17 tests |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r3 (W-O9/W-O10: a committed close was reported as failed by close_linked and drop --duplicate-of; _close now raises only when nothing was committed, scripts/work.py:2581-2604) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (scripts/work.py open_linked/close_linked/_drop_duplicate hunted for lock and git budgets — both hook-facing entry points run inside _hook_git_budget under one store lock; no quota or pool accounting in this surface) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (scripts/work.py _check_linked empty link values refused before the lock; _drop_duplicate empty identity never matches an empty creator; id == keep refused — tests/test_work_linked.py drop tests) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (W-O3, W-O4, W-O5 — tests added and each proven red under the named mutation of scripts/work.py) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (receipt counts carry their totals — 13 + 3 + 3 + 0 candidates over four passes; gate 275 passed over six named test files in the worktree of scripts/work.py) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every confirmed candidate re-run by its refuter and by the orchestrator on a copy of scripts/work.py; every fix proven by reverting it on a copy and naming the red test) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 | found: 13, new: 13, confirmed: 8, fixed: 8, unexecuted: 0 | method: citation — full file partition (W: scripts/work.py with the Opus floor as third finder; T: tests/test_work_linked.py), workflow wf_c6b0e22a-33c; every candidate executed by its slice refuter and re-read by the orchestrator at 493e8587f; 5 RECORDED by design |
| Pass 2 | native opus×1 + sonnet×2, refuters sonnet×2 (the round-1 slice owners) | found: 3, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | method: re-derivation — 8 of 8 ledger claims NOW_FALSE, each test proven red under its claim's mutation; 3 new, all inside pass 1's fix hunks (own-fix 3 of 3) |
| Pass 3 | native opus×1 + sonnet×2, refuters sonnet×2 (the round-1 slice owners) | found: 3, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | method: re-derivation — W-O7, W-O8 NOW_FALSE; T-S1 REFUTED (the swapped-order mutation turns test_a_duplicate_already_closed_keeps_the_kept_items_record red); 2 new inside pass 2's hunks; class rewrite — _close (the scope-growth stop fired: new hunting suspended, the fixed set re-verified) |
| Pass 4 | native opus×1 + sonnet×1, refuter sonnet×1 (the round-1 owner of slice W) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: W-O9, W-O10 NOW_FALSE, all three new tests red when the _close rewrite is reverted on a copy; 17 of 17 tests/test_work_linked.py green; standing clean since pass 1: cost-quota, boundary-sentinel, denominator, proxy-evidence |
...
| P1-W-S2 | RECORDED — by design (D-419) — close_linked does not fence: the mail's rename lock is the fence and the close follows the mail's disposition (spec § The delta D2) |
| P1-W-S3 | RECORDED — by design (D-394) — identity is environment-supplied, the store's existing trust model (work.py's claims COBRA note) |
| P1-W-O7 | RECORDED — by design (D-394) — drift class 6's stale-marker arm flags an unmerged close of every kind; the spec's exemption is the evidence arm |
| P1-W-O8 | RECORDED — by design (D-419) — open_linked returns the id when another session holds the claim; mail claims cannot collide (the rename lock) and T04's --take reports the holder (routed to T04's brief) |
| P1-W-O9 | RECORDED — by design (D-394) — the same environment-identity trust model as P1-W-S3 |
| P3-T-S1 | REFUTED (the swapped-order mutation of scripts/work.py turns test_a_duplicate_already_closed_keeps_the_kept_items_record red — executed by the pass-3 seats and the coder) |
| P3-R1 | RECORDED — measured (the status re-read in _drop_duplicate's except arm runs inside suppress(Exception); under the pass-3 contract that arm only runs when nothing was committed, so the re-read is a guard with no effect — no behaviour claim) |

## Per-phase verdicts

### Phase 1 — T01 the store — linked items and the duplicate drop: CLEAN
...
GATE-SCOPE: out-of-surface — Doc Link Integrity (live tree); findings naming this surface: 0 of 8; measured by: `final_gate.py --check --json` in the T01 worktree — the 8 broken refs point at gitignored cache files (scripts/kilo_47_agents_final.json, scripts/kilo-benchmarks/cache/*) absent from a fresh worktree
