# Chunk 6 — the section reviews and execute-plan's nested reviews on the review-loop workflow (D-361)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 3d1387845e68e9116fd95a1b612054b0d5fd5b5f; `git diff HEAD -- .claude/workflows/fabrik-review-loop.js tests/test_review_loop_workflow.py commands/_sources/fabrik-spec-review.md commands/_sources/fabrik-plan-review.md commands/_sources/fabrik-execute-plan.md docs/reference/review-loop-workflow.md docs/reference/command-loop-performance.md scripts/review_loop_ledger.py tests/test_review_loop_ledger.py` (+288 −101)
**Command:** /fabrik-review · **Changed:** `.claude/workflows/fabrik-review-loop.js`, `tests/test_review_loop_workflow.py`, `commands/_sources/fabrik-spec-review.md`, `commands/_sources/fabrik-plan-review.md`, `commands/_sources/fabrik-execute-plan.md`, `docs/reference/review-loop-workflow.md`, `docs/reference/command-loop-performance.md`, `scripts/review_loop_ledger.py`, `tests/test_review_loop_ledger.py`
...
### core/10-python.md  (hit: scripts/review_loop_ledger.py, tests/test_review_loop_ledger.py, tests/test_review_loop_workflow.py)
...
### core/40-documentation.md  (hit: commands/_sources/fabrik-execute-plan.md, commands/_sources/fabrik-plan-review.md, commands/_sources/fabrik-spec-review.md)
...
### core/45-testing-strategy.md  (hit: tests/test_review_loop_ledger.py, tests/test_review_loop_workflow.py)
...
| Class | Status |
|---|---|
| Hunt: `.claude/workflows/fabrik-review-loop.js` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S4 a researcher refuter was given shell instructions; the per-round Opus floor needed a third finder) · FIXED r2 (A-NEW1 a candidate absorbed one twin only, so a defect three finders raised split in two) |
| Hunt: `tests/test_review_loop_workflow.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (the literal-agent pin became executed graders: default pair, section seats, researcher refuter, refusals, the three-finder slice) · FIXED r2 (a grader for the three-way twin); 25 graders, each new one watched red on the script before its fix |
| Hunt: `commands/_sources/fabrik-spec-review.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (B-S1 the `SEATS: 0` stop dropped) · FIXED r2 (B-NEW1 the restored stop now names the invocation as a possible binder, agreeing with /fabrik-review) |
| Hunt: `commands/_sources/fabrik-plan-review.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (B-S2 the same dropped stop) · FIXED r2 (B-NEW1 the same wording) |
| Hunt: `commands/_sources/fabrik-execute-plan.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (B-H2/C-S2/C-S3 the per-round Opus finder of D4 and the phase review's risky-diff Opus seat were dropped under D-344, which never covered this command — restored as the riskiest slice's third finder) |
| Hunt: `docs/reference/review-loop-workflow.md` — every changed hunk, its enclosing function, its callers | CLEAN (docs/reference/review-loop-workflow.md's slices row states models one to three, the agent, the scope and the refusals the script enforces; slice B seats, passes 1-3) |
| Hunt: `docs/reference/command-loop-performance.md` — every changed hunk, its enclosing function, its callers | CLEAN (docs/reference/command-loop-performance.md row 6 is one progress line naming D-361 and what landed; slice C seats, passes 1-3) |
| Hunt: `scripts/review_loop_ledger.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S1 `next` split a hyphenated slice name on its first `-`; the slice now comes from the finder label) |
| Hunt: `tests/test_review_loop_ledger.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (a grader for the hyphenated slice name; 9 graders) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (normalizeSeats in .claude/workflows/fabrik-review-loop.js refuses an unknown model or agent and a repeated or fourth finder before any seat runs) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (pool OFF; 9, 9, 6 seats stamped across .claude/workflows/fabrik-review-loop.js passes 1-3; the execute-plan Count discipline sizes `--slices opus=1,sonnet=N,haiku=N`) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (the first-hyphen split in scripts/review_loop_ledger.py) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (every new behaviour of the script has an executed grader in tests/test_review_loop_workflow.py, each red on the version before its fix) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (25 of 25 graders in tests/test_review_loop_workflow.py and 9 of 9 in tests/test_review_loop_ledger.py; 38 of 38 commands rendered) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every candidate executed by its slice refuter; the three-finder twin and the hyphenated slice reproduced by the orchestrator through the harness before the fix) |

## Pass Ledger

| Pass | Finders | Counters | Method |
|---|---|---|---|
| Pass 1 | native sonnet×3 + haiku×3 (two finders per slice over three slices, ONE `Workflow` call — 9 agents incl. 3 refuters) | found: 19, new: 19, confirmed: 8, fixed: 8, unexecuted: 0 | method: citation — 19 candidates collapse to 8 defects; the rest recorded or refuted; seats: A-sonnet 5/5 · A-haiku 1/1 · B and C per the pass file; stop: confirmed 8; fix: the Opus floor kept as a third finder, the stop restored, the ledger split, the researcher prompt |
| Pass 2 | native sonnet×3 + haiku×3 (the round-1 seats over their own ledgers, ONE `Workflow` call — 9 agents) | found: 8, new: 8, confirmed: 2, fixed: 2, unexecuted: 0 | method: re-derivation — 8 of 8 ledger defects NOW_FALSE; A-NEW1 NEW inside the three-finder fix (one twin per candidate), raised by four seats; B-NEW1 the restored stop's wording; own-fix 2; stop: confirmed 2 |
| Pass 3 | native sonnet×3 + haiku×3 (the round-1 seats over their own ledgers, ONE `Workflow` call — 6 agents, no candidate to refute) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — 4 of 4 ledger defects re-executed NOW_FALSE on 25 + 9 graders; a two-finder slice's union and a same-seat neighbour re-run unchanged; stop: closable |

## Residual

| A-S2 | RECORDED — by design (D-361) |
| A-H1 | RECORDED — by design (D-361) |
| B-S6 | RECORDED — by design (D-361) |
| C-S1 | RECORDED — by design (D-361) |
| A-S3 | RECORDED — by design (D-361) |
| C-S4 | RECORDED — by design (D-361) |
| B-S4 | RECORDED — by design (D-361) |
| B-S5 | RECORDED — by design (D-361) |
| B-S3 | RECORDED — measured (CLAUDE.md's Subagent fan-out bullet reads its Sonnet + Haiku pair as covering the section loops too; older than this change and on a governance-sync path, routed to docs/STRATEGIC_BACKLOG.md) |

A-S2, A-H1, B-S6 and C-S1 are one observation: the D-361 row the change cites lands in the same commit. A-S3 and C-S4: a section slice's coverage is not diffed, written as a cobra note beside the code. B-S4: the script does not enforce the section shape's no-Haiku rule — the shape belongs to the command. B-S5: two section slices on one file can each raise a boundary defect; the lead merges them in adjudication.

## Per-phase verdicts

### Phase 0 — Establish scope: CLEAN — record opened, nine files pinned with their HEAD copies and the diff
### Phase 1 — Finders: CLEAN — one `Workflow` call per pass on the changed script itself, every seat returned, no gap
### Phase 2 — Execute: FIXED(10) — 10 confirmed across two passes, the three-finder twin and the hyphen split reproduced before their fixes
### Phase 3 — Prove and fix: FIXED — 6 new graders, each watched red on the version before its fix
### Phase 4 — Converge: CLEAN — pass 3 confirmed 0 · fixed 0, every slice verified

## Gate

`final_gate.py --check --json` over this receipt (the code landed in `8b733f034`, the ledgers in `fdc95e77d`), pasted verbatim:

```json
{"status": "failure", "tier": 2, "passed": 65, "failed": 1, "skipped_checks": ["bandit", "semgrep", "pytest"]}
```

GATE-SCOPE: out-of-surface — INDEX.md ↔ docs tree drift; findings naming this surface: 0 of 1; measured by: `.venv/bin/python scripts/final_gate.py --check --json`

The one failure names `docs/reference/research/2026-09-23-self-healing-currency-ledger.md`, a sibling session's untracked file (its INDEX row is that run's to add). The hub's pytest leg is off by design; the two touched test files were run by hand, 25 of 25 and 9 of 9 green, and the corpus renders with `--check` OK.
