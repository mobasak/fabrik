# T05b of plan 2026-09-25-plan-1-work-store-single-tracker
...
```text
$ python scripts/review_rubric.py --changed scripts/thread_anchor.py tests/test_thread_anchor.py tests/test_work_hook_seam.py
...
### core/45-testing-strategy.md  (hit: tests/test_thread_anchor.py, tests/test_work_hook_seam.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/thread_anchor.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S1/A-O1/T-S1 next_anchored only when the register wrote the NEXT; A-S2/A-O5/T-S3 a **kwargs on_harvest accepts the keyword; A-O4/T-H1 the verdict cached per on_harvest function; A-O6 inspect imported only on the harvest path; A-O8 the store handed the same 300 characters the register judges) — scripts/thread_anchor.py cmd_harvest and _takes_next_anchored |
| Hunt: `tests/test_thread_anchor.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-O2 the D-392 byte-identity test compares old and new after every step over two-NEXT messages, the anchor cap, a >300-character anchor and a cleared-message echo, and the four register mutants now turn it red; a test per round-1 fix) — tests/test_thread_anchor.py 125 tests |
| Hunt: `tests/test_work_hook_seam.py` — every changed hunk, its enclosing function, its callers | CLEAN (tests/test_work_hook_seam.py: the V1 walk through the real Stop hook covers every ticket clause — three free-text Stops, a named claim with supersede, an operator decision, an awaiting-then-open line, another session's item, the 8-day close by a second session; red under the coder's work.py mutants W1-W4) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (A-S1 a skipped register write no longer reads as accepted; A-O4 a stale signature verdict can no longer cost the decision write) — scripts/thread_anchor.py |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | FIXED r1 (A-O6 the prompt-time line path no longer imports inspect); the whole Stop hook measured 0.23-0.26 s on the seam fixture against the 5 s kill — scripts/thread_anchor.py |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (A-O8 the 300-character boundary shared by the register and the store) — scripts/thread_anchor.py cmd_harvest |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (A-O2 the identity test now kills the four register mutants; F1-F6 each red on a named test) — tests/test_thread_anchor.py |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the receipt's counts carry their totals: 18 + 0 candidates over two passes; 259 passed over five named test files for scripts/thread_anchor.py) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every confirmed candidate executed by its refuter on a copy of scripts/thread_anchor.py; the orchestrator re-ran the 259-test gate on the worktree at 6f9d3c36a and read _update's return contract) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 | found: 18, new: 18, confirmed: 11, fixed: 11, unexecuted: 0 | method: citation — file partition (A: scripts/thread_anchor.py with the Opus floor; T: the two test files), workflow wf_c598ba92-325, seat scratch at absolute paths; every candidate executed by its refuter; 1 refuted, 6 RECORDED |
| Pass 2 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×1 (the round-1 slice owners) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: 11 of 11 ledger claims NOW_FALSE, the four register mutants red on the identity test, every UNCHANGED input re-verified; workflow wf_70dd4b0a-e77; standing clean since pass 1: denominator, proxy-evidence |
...
| P1-A-H1 | REFUTED (the decision harvest passes its own NEXT to the store; the refuter's captured on_harvest call shows next_text set, not None) |
| P1-A-S3 | RECORDED — measured (the Stop hook's two harvests can pass different NEXT texts; the decision harvest runs last and decides, as T05a's ticket step 3 anticipates) |
| P1-A-O7 | RECORDED — measured (the flush-race previous-turn NEXT reaches rule 3 on the plain harvest; an identical text writes nothing and this turn's decision harvest decides) |
| P1-A-H3 | RECORDED — measured (the seam walk asserts outcomes, not each harvest's verdict; the 5-case unit test pins the verdicts) |
| P1-A-H2 | RECORDED — measured (the identity test needs bde8b388d in the clone; the hub always carries it, tests/ is not synced, and it fails loud rather than skipping) |
| P1-A-O3 | RECORDED — measured (the same as P1-A-H2: a shallow clone or pinned copy without the commit fails the test with a named message) |
| P1-T-S2 | RECORDED — measured (the same as P1-A-H2) |

## Per-phase verdicts

### Phase 1 — T05b the harvest tells the store whether the register accepted the NEXT: CLEAN
