# D-181/D-182 — the hub's scripted pool dispatchers gated on the committed policy; the :5051 commands tab

**Status:** CONVERGED
**Surface:** final HEAD 242b6a53; range b00d8bbd..242b6a53 (5 commits: acffe9c1 the gates + dashboard, ccf7cb02 round-1 fixes, 242b6a53 round-2 fix); `git diff b00d8bbd..242b6a53 | md5sum` = a3019990e5ff8307135ba452e212d63d (142708 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/classify_services.py`, `scripts/sysadmin/canary_grounding.py`, `scripts/sysadmin/claude_broker.py`, `scripts/sysadmin/incident_context.py`, `scripts/sysadmin/quota_dashboard.py`, `tests/test_quota_dashboard.py`, `tests/test_incident_context.py`, `tests/test_claude_broker.py`, `tests/test_canary_grounding.py`, `tests/test_classify_services_policy.py`, `docs/workstation/quota-dashboard.md`, `CHANGELOG.md`
...
### core/10-python.md  (hit: scripts/classify_services.py, scripts/sysadmin/canary_grounding.py, scripts/sysadmin/claude_broker.py)
...
### core/40-documentation.md  (hit: CHANGELOG.md, docs/workstation/quota-dashboard.md)
...
### core/45-testing-strategy.md  (hit: tests/test_canary_grounding.py, tests/test_classify_services_policy.py, tests/test_claude_broker.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/classify_services.py` — every changed hunk, its enclosing function, its callers | FIXED r1 — gate moved after `parse_args()` (`--help` → SystemExit 0, bad flag → 2, pinned); policy loaded by path; exit 0 + stderr line when OFF (the 06:00 chain's `_step` reads 0 as success — a stderr line is its heartbeat's contract) |
| Hunt: `scripts/sysadmin/canary_grounding.py` — every changed hunk, its enclosing function, its callers | FIXED r1 — gate after argparse; policy by path; Sunday cron returns 0 without `run_agents` (test: run_agents raises if reached) |
| Hunt: `scripts/sysadmin/claude_broker.py` — every changed hunk, its enclosing function, its callers | FIXED r1 — `PoolOffByRulingError` → 503 `via: refused`, rollover persisted like the 429 path, audited, NO window counted (test reads the budgets file back) |
| Hunt: `scripts/sysadmin/incident_context.py` — every changed hunk, its enclosing function, its callers | CLEAN (r1) — refusal string is the diagnosis text and says 'bundle written for manual review'; the operator notify carries it; seat A: materially less severe than the broker, read-only |
| Hunt: `scripts/sysadmin/quota_dashboard.py` — every changed hunk, its enclosing function, its callers | FIXED r1 — HTML comments blanked before every detector (pool column 0 of 35 by construction), Native subagents column with assembly boilerplate excluded, governor banner never advertises a shed while OFF, the flywheel column removed (operator, mid-turn); the live board restarted twice and re-read by curl |
| Hunt: `tests/test_quota_dashboard.py` — every changed hunk, its enclosing function, its callers | CLEAN — 3 new tests (commented pool contract, native column incl. boilerplate exclusion, governor panel); the `fly` probe removed with the column |
| Hunt: `tests/test_incident_context.py` — every changed hunk, its enclosing function, its callers | CLEAN — `_default_pool` refuses OFF / dispatches ON; ruff reflow verified token-identical outside the appended test |
| Hunt: `tests/test_claude_broker.py` — every changed hunk, its enclosing function, its callers | CLEAN — refusal now asserted as `PoolOffByRulingError`; the 503 test uses the SAME module object the Broker class comes from (a second load carried a different exception class and read 502 — caught in-run); ruff reflow verified wrapping-only |
| Hunt: `tests/test_canary_grounding.py` — every changed hunk, its enclosing function, its callers | CLEAN — `main()` OFF → 0 and run_agents untouched; ON → reaches dispatch (nonzero) |
| Hunt: `tests/test_classify_services_policy.py` — every changed hunk, its enclosing function, its callers | CLEAN — new file: OFF → 0, no dispatch, stderr names the ruling; `--help`/bad flag reach argparse |
| Hunt: `scripts/doc_reconcile.py` — every changed hunk, its enclosing function, its callers | FIXED r1 — the by-path loader's first path was `_ENF.parent/scripts/enforcement` (19 tests red in-run); now `_ENF / check_subagent_flywheel.py`, verified via a real import both ways |
| Hunt: `docs/workstation/quota-dashboard.md` — every changed hunk, its enclosing function, its callers | CLEAN — § The Commands tab: comment-aware detectors, the Native subagents column and its boilerplate exclusion, the governor banner under OFF, the flywheel column's removal |
| Hunt: `CHANGELOG.md` — every changed hunk, its enclosing function, its callers | CLEAN — one entry, prepended on a clean file |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN — every wrapper reads OFF on any failure (unknown ⇒ no spend); the broker's refusal is a 503, never a 200; the classify/canary gates exit 0 by design (a scheduled skip is not a failure) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | FIXED r1 — the broker charged a window for a refusal (seat #1); now nothing counted, audited as refused |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — `_NATIVE_COMPILED` lookarounds exclude `/fabrik-reviewer`, `fabrik-gui-driver`; the boilerplate regexes match the rendered banner/fragment byte-for-byte (seat (e)) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN — 9 new tests across 6 files, each red on HEAD (7 of 7 in the worktree run, then 2 of 2 for the round-1 additions) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — 4 of 4 hub dispatchers gated (grep over scripts/, .claude/hooks, src: 7 files with dispatch calls, 3 of them message/selftest strings), 35 of 35 commands in the natives column, 244 tests green across 7 suites |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every gate smoked on the live box (6 wrappers False with the seam unset, True with it on); the board restarted and curl-read for both column changes; gate `--check --json` success ×3 |
...
|---|---|---|---|
| Pass 1 (acffe9c1) | native sonnet×1 (a–g: module shadow, argparse placement, broker/incident result handling, dashboard regexes, tests' mocking strategy, the ruff reflow) + orchestrator re-derivation (6 wrappers smoked both ways on the box; 244 tests; gate success; board curl-read) + operator (mid-turn: the flywheel column) | found: 4, fixed: 4 | citation |
| Pass 2 (ccf7cb02, the round-1 fixes) | native sonnet×1 exit seat (broker 503 path incl. serve(), gate placement executed both ways, six by-path loaders from a foreign cwd, the matrix column count on the live board) + orchestrator re-derivation (244 tests; 6 wrappers smoked; gate green on own files, one FOREIGN red — a sibling's D-184 row → scripts/scratch_sweep.py, messaged) | found: 1, fixed: 1 | citation |
| Pass 3 (242b6a53, the exit) | native sonnet×1 exit seat (200-call exec-load loop: sys.path, sys.modules, fds, RSS; the guard's spelling; the test's discriminating power) + orchestrator re-derivation (185 tests on the three touched suites, 6 wrappers smoked, the live board curl-read) | found: 0, fixed: 0 | method: re-derivation |
...
### Phase 1 — Gate the four scripted dispatchers + the dashboard columns: FIXED r1 (four gates, the by-path policy loader in six scripts, the comment-aware matrix, the Native subagents column)
### Phase 2 — Land: CLEAN (acffe9c1 + ccf7cb02, private-index commits by explicit pathspec, pushed, doc_reconcile synced 45 of 45)
### Phase 3 — Review: CONVERGED (pass 3 over 242b6a53: found: 0; 5 candidates across three rounds, all FIXED red-first)
...
```json
{
  ...
  "failures": [
    "Doc Link Integrity (live tree): ERROR: docs/DECISIONS.md: broken ref -> scripts/scratch_sweep.py"
  ],
