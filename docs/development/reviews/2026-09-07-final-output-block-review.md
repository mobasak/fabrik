# Review — one seven-line FINAL OUTPUT block in every repo (D-173)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 2923e48f7a2dc8261b2de16a8578219bf369f309 (the working-tree diff, committed as the next commit); `git diff HEAD -- .claude/hooks/final_gate_stop.py tests/test_final_gate_stop_hook.py CLAUDE.md docs/DECISIONS.md CHANGELOG.md` md5 1dea649d98f38bd0307e6b8999dc070c (31788 bytes, 5 paths; CHANGELOG counted with the sibling's hunk present in the tree, committed from a HEAD-carved blob)
**Command:** /fabrik-review · **Changed:** `.claude/hooks/final_gate_stop.py`, `tests/test_final_gate_stop_hook.py`, `CLAUDE.md`, `docs/DECISIONS.md`, `CHANGELOG.md`
...
```text
$ python scripts/review_rubric.py --changed .claude/hooks/final_gate_stop.py tests/test_final_gate_stop_hook.py CLAUDE.md docs/DECISIONS.md CHANGELOG.md
...
### core/10-python.md  (hit: .claude/hooks/final_gate_stop.py, tests/test_final_gate_stop_hook.py)
...
### core/45-testing-strategy.md  (hit: tests/test_final_gate_stop_hook.py)
...
| Class | Status |
|---|---|
| Hunt: `.claude/hooks/final_gate_stop.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (`_detect_stall`: the widened trigger was fence-blind and whole-text, so an explainer quoting a partial example mid-message would stall — three readers; scoped to the last 10 lines, the contract's own "LAST 7 lines"; the rendering names every missing key; executed: the real hook blocks a 4-key tail and passes a quoted example followed by prose) |
| Hunt: `tests/test_final_gate_stop_hook.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (four new tests — partial block names the missing keys; FEEDBACK-only shape still named; full block and the STATE/NEXT footer pass; a quoted mid-message example is prose while the same block at the end stalls; 2 watched red before the mechanism, 153 pass across the four hook suites) |
| Hunt: `CLAUDE.md` — every changed hunk, its enclosing function, its callers | CLEAN (one UNIVERSAL-marker bullet; the anchor `last 7 lines of every task-completing response` lives in the § FINAL OUTPUT heading OUTSIDE the index — executed with fabrik-lib's own `_strip_anchor_index`: present in the hub's stripped text, absent from fabrik-lib's, so their drift check flags the 6-line heading; deepseek's "anchor only in the index" REFUTED by that run) |
| Hunt: `docs/DECISIONS.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (D-173 said "some of the seven keys"; the mechanism is two-or-more within the last 10 lines — the row now says so; deepseek's boundary finding) |
| Hunt: `CHANGELOG.md` — every changed hunk, its enclosing function, its callers | CLEAN (one entry under [Unreleased]; the file also carries a sibling's uncommitted dashboard entry, committed from a HEAD-carved blob so their hunk stays theirs) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1+r2 (over-blocking direction: the whole-text scan → the 10-line tail; the BLOCKED: escalation is now computed BEFORE the check and waives it, recorded in the waiver ledger — executed: a 3-strikes BLOCKED close passes) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (pool r1 3 units ~$0.003 scored 3/2/3 via `set_quality`; native Sonnet ×1; the 3-attempt warn-through of the stall lane is untouched — `main()` counters) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (the trigger is ≥2 non-NEXT keys: footer alone = 0, footer + one prose key = 1, both executed; `LESSONS LEARNED:` is a different key by contract — the directive is these seven lines verbatim) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (`tests/test_final_gate_stop_hook.py`: the partial-block test was red before the branch existed; the tail-window test was red at 12 lines and green at 10 — the constant is pinned by behaviour) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (49 of 49 `/opt/*/CLAUDE.md` scanned, 48 carry the seven lines; 45 of 45 project hooks match the hub, 3 fabrik-lib trees differ; fire rate 0 of 840 assistant turns in this session's transcript — one session, hub only, is the bound) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (the hook itself run on scratch transcripts under a probe session id for both shapes; fabrik-lib's drift-check function imported and run on both contracts) |
...
|---|---|---|---|
| Pass 1 | pool deepseek-v4-flash×1 + nemotron-3-super:free×1 + minimax-m3:free×1 + orchestrator probes | found: 4, fixed: 2, refuted: 2 (the anchor IS outside the index — executed with fabrik-lib's own strip function; the misspelled-key shape is by contract) | citation |
| Pass 2 | native sonnet×1 (round-1 seat, returned after the tail window landed) + pool deepseek-v4-flash×1 + nemotron-3-super:free×1 + minimax-m3:free×1 over the pre-fix diff (19 items, all re-raises of adjudicated rows — cited, not counted) + orchestrator | found: 5, fixed: 3, refuted: 2 (a BLOCKED: escalation was refused before the global exemption existed → computed first, waived and recorded; two incidental prose labels beside a footer tripped the 2-key trigger → three keys; the fix-it text always said FEEDBACK → names the missing keys; refuted: fence-awareness inside the 10-line window — the block is emitted fenced by contract, and the fire rate on 840 turns is 0; `LESSONS LEARNED:` is a different key by contract) | citation |
| Pass 3 | pool deepseek-v4-flash×1 + nemotron-3-super:free×1 + minimax-m3:free×1 (fresh sweep of the final diff) + the orchestrator's re-derivation (the real hook executed on the three decisive shapes, 155 hook tests, the anchor re-checked with fabrik-lib's strip function, fire rate re-measured) | found: 0, fixed: 0 (10 re-raises of adjudicated rows cited, not counted: the fenced example at the tail is the contract's own emission form; the anchor IS in the stripped hub text; key spelling is by contract) | method: re-derivation |
...
### Phase 1 — measure + implement (three contracts diffed, 49 CLAUDE.md files and 48 hook copies compared, the hook's stall lane generalised, the anchor, D-173, CHANGELOG, the request to fabrik-lib 01M1W9BJT801DKGD58Z0Y07MAQ): FIXED (round 1)

### Phase 2 — review rounds: see the Pass Ledger

### Phase 3 — commit, push, sync: CLEAN (private-index commit of the five paths with the CHANGELOG blob carved from HEAD; fetch + fast-forward push; forced sync of the hook to the 45 project repos)
...
`python3 scripts/final_gate.py --check --json` at the flip (00:35, after the last edit), verbatim:
...
```json
{
 ...
 "advisory": [
  ...
  },
  {
   ...
   "output": "⚠ check_citations_resolve ADVISORY — 2 citation(s) do not land, of 4 examined across 3 docs (a wrong `path:line` reads as verified and is not):\n   - docs/development/plans/2026-09-06-plan-2-multi-agent-adoption/T03-check-advisory.md: BLANK-TARGET scripts/docs_updater.py:1357 → ''\n   - docs/development/plans/2026-09-06-plan-2-multi-agent-adoption/T03-check-advisory.md: BLANK-TARGET scripts/docs_updater.py:1357 → ''",
   "truncated": false,
   "omitted_lines": 0,
   "rerun": null
  },
 ...
 "warnings": [
  ...
  },
  {
   ...
   "output": "⚠ 1 untracked source file(s) NOT in gate scope (unstaged → unscanned): docs/development/reviews/2026-09-07-final-output-block-review.md — if yours: `git add` them and RE-RUN the gate (they ship unlinted otherwise); if a sibling's: leave them.",
   "truncated": false,
   "omitted_lines": 0,
   "rerun": null
  }
