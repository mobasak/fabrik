# Review — plan 2026-09-01-plan-1-deployment-verification, Phase C (scaffolder seeding: parity stub · vendored health-probe · fleet-AI doc sections) — 2026-09-02

Status: CONVERGED
Surface: fae20651f82bb4a39819a50368a2a281e0f58107 47b9d09267129718ccf8aeae7ff5d79c
Scope: `templates/scaffold/scripts/verify_prod_parity.py` (NEW) · `scripts/verify_prod_parity.py` (NEW symlink) · `src/fabrik/scaffold.py` (SCRIPT_FILES) · `scripts/fabrik_synced_manifest.py` (VENDORED_DIRS) · `libs/health_probe/` (NEW, vendored) · `templates/scaffold/docs/DEPLOYMENT_TEMPLATE.md` + `OPERATIONS_TEMPLATE.md` · `tests/test_scaffold_deploy_contract.py` (NEW, 31) · `scripts/enforcement/check_sync_trigger_coverage.py` + `check_print_ban.py` (one exemption line each) · INDEX/CHANGELOG/plan file.
...
## Rubric (verbatim — `python scripts/review_rubric.py --changed <the Phase C diff paths>`)
...
### core/10-python.md  (hit: libs/health_probe/__init__.py, libs/health_probe/fingerprint.py, libs/health_probe/health_probe.py)
...
### core/40-documentation.md  (hit: templates/scaffold/docs/DEPLOYMENT_TEMPLATE.md, templates/scaffold/docs/OPERATIONS_TEMPLATE.md)
...
### core/45-testing-strategy.md  (hit: tests/test_scaffold_deploy_contract.py)
...
|---|---|---|
| core/35-security-auth (FLOOR) | CLEAN | no auth surface; the fleet-AI sections carry names only ("Env the hub must supply — names only, never values"); the stub reads no secret; grep `token|password|DSN` in the diff → template prose only |
| core/25-data-postgres (FLOOR) | CLEAN | no DB code; the vendored probes need psycopg2 only inside their own functions, imported lazily by the stub |
| core/30-ops (FLOOR) | CLEAN | no compose/ports; `specs/services/<id>.yaml` generation at scaffold time measured pre-existing (`create_project(generate_spec=True)`) — not re-implemented |
| 12-FACTOR (FLOOR) | CLEAN | the stub is config-free; the vendored module reads env only inside probe functions; stdout/stderr only |
| core/10-python (MATCHED) | FIXED(2) | ruff N802 refused the plan's `L0_…` row name → lowercase rows (Phase B follows); the vendored module reds the hub's ruff (6) + format → file-level `ruff: noqa` + `fmt: off` in the 3-line header, mirroring `extend-exclude` for every other vendored lib; mypy clean on the copy; `AFTER-EDIT` header added to the stub for `check_script_headers` |
| core/40-documentation (MATCHED) | FIXED(1) | the plan's `<to be filled …>` sentinel is invisible to `check_doc_stubs` (PLACEHOLDERS measured) → `{PROJECT_NAME}` embedded, seeding leaves it untouched (only `[Project Name]` is substituted, `scaffold.py:1112`); INDEX rows ×4, CHANGELOG entry, plan file markers |
| core/45-testing-strategy (MATCHED) | CLEAN | 31 tests; exit-code proven red-on-revert (2 failed with the DRAFT→2 line neutered, restored green); docusaurus assertion shown to discriminate on python-api (has DEPLOYMENT.md + the section); full scaffold suites 155 passed (8m10s) |
| core/55-observability (MATCHED: vendored) | CLEAN | vendored code unchanged below the header; its `print()`s are its CLI output → path-exempt from the print ban (the class fix, beside `scripts/`) |
| core/58-resilience (MATCHED: vendored) | CLEAN | probes carry timeouts (`:182/:226/:265` of the shipped file); the stub's `run_rows` never lets a row raise (→ UNVERIFIABLE) |
| core/self-healing (MATCHED: vendored) | REFUTED (n/a) | the stub and the module take no self-healing action; a row FAILS, the runner routes |
| Fail-open vs fail-closed on every gate/guard | CLEAN | executed battery 8/8 real probes: DRAFT never 0 · disagree 2 · raising comparator 2 · UNVERIFIABLE 2 · DOWN+disagree 1 (liveness wins) · liveness row carries no comparison key · missing vendored module → UNVERIFIABLE + exit 2, no traceback (`test_stub_without_vendored_module_fails_closed`) |
| Cost/quota/limit accounting edges | CLEAN | none — no counters |
| Boundary/sentinel/prefix collisions | FIXED(3) | predicate 3's `templates/**` fallback is orchestrator-docs-only → hub-rooted SYMLINK (measured: 4 problems → 0; live corpus 0); `libs/health_probe` distributed-but-no-trigger → DECLARED_NON_TRIGGERS (the `libs/subagents` precedent); `libs/health_probe/` print ban → SKIP_PATTERNS |
| Behavior-without-a-test | CLEAN | every Phase C Behavior Contract row has its test (stub per type · vendored byte-identity · header-only drift · manifest entry · fail-closed without the module · wordpress refused · sentinel recognised · docusaurus non-publication) |

## Disposition ledger

| # | candidate | class | disposition |
|---|---|---|---|
| 1 | the plan's ordering premise — "predicate 3 closes when the template lands" | boundary | FIXED — measured false (audit over the draft with the template on disk: 4 problems); `scripts/verify_prod_parity.py` symlink to the template (the seeded-scripts-exist-hub-rooted convention, one source); audit → 0; plan file amended |
| 2 | `libs/health_probe/` distributed with no sync trigger (gate red) | boundary | FIXED — declared in `check_sync_trigger_coverage.py::DECLARED_NON_TRIGGERS` like `libs/subagents`; 36 tests green; plan + CHANGELOG record the out-of-scope line |
| 3 | vendored CLI `print()` (8 sites) under the print ban (gate red) | boundary | FIXED — `libs/health_probe/` in `check_print_ban.py::SKIP_PATTERNS`; 7 tests green; recorded |
| 4 | `L0_…` row name refused by ruff N802 | core/10-python | FIXED — lowercase rows; the appendix's illustrative `L1_…` names are read as lowercase in Phase B |
| 5 | `<to be filled by /fabrik-deploy-checklist>` invisible to `check_doc_stubs` | core/40-documentation | FIXED — `{PROJECT_NAME}` embedded in every sentinel cell; `test_fleet_ai_sections_present_in_the_templates` asserts it is in PLACEHOLDERS |
| 6 | the stub lacked the `# AFTER-EDIT:` coupling header (`check_script_headers` WARN at the hub and in every seeded project) | core/10-python | FIXED — header added; `--header` still parses |
| 7 | probe expectation "every compare_row carries `compare_error`" | fail-open | REFUTED — fabrik-lib's `compare()` attaches `compare_error` only when the comparator raised (their docstring `:87`; executed: agree row has no key, raise row has it); the stub relays the shipped shape unchanged and Phase A reads it with `.get()` |

## Pass Ledger

| Pass | method | found | new | fixed | finders |
|---|---|---|---|---|---|
| Pass 1 | method: citation | found: 7 | new: 7 | fixed: 6 | finders: in-line (NO-POOL) — corpus predicate 3 (executed audit) · sync-trigger + print-ban gates (final_gate --check) · lint/format/mypy on the vendored copy · doc-stub sentinel registry · scaffold substitutions · docusaurus seeding buckets; dispatched: 6 partitions, returned: 6 |
| Pass 2 | method: re-derivation | found: 0 | new: 0 | fixed: 0 | finders: in-line — the stub's exit algebra re-run as an executed battery (8/8 real probes + 1 wrong expectation refuted), scaffold suites 155 + contract suite 31 re-run after the rename, red-on-revert for the exit code, the docusaurus test shown to discriminate, `git ls-files -s` proves the symlink is tracked as 120000, the gate re-run to success |
...
- (c) design tradeoff, resolved by the FIX decisions: the vendored module is exempt from three hub gates by path/header (lint, print ban, sync trigger) — the same treatment every other vendored lib receives; drift is caught by `test_hub_copy_differs_from_fabrik_lib_only_by_the_vendoring_header`, not by hub lint.
- (a) pre-existing: `sync_extensions.sh` already differs from its template — the two-copy pattern the symlink avoids for the stub.
