# T01b — the kaizen vocabulary: decision_block event and deferral cause
...
```text
$ python scripts/review_rubric.py --changed scripts/sysadmin/kaizen_collect_v2.py scripts/sysadmin/kaizen_events.py tests/test_kaizen_deferral_vocabulary.py 
...
| Class | Status |
|---|---|
| Hunt: `scripts/sysadmin/kaizen_collect_v2.py` — every changed hunk, its enclosing function or section, its callers | FIXED r1 (R1-T01b-S1: the formula, cross_reference and detail texts named two causes; now built from `sorted(PREMATURE_CAUSES)` at kaizen_collect_v2.py:127, version 3 → 4, so the definition hash follows the set; pass 2 re-executed, 3 seats NOW_FALSE) |
| Hunt: `scripts/sysadmin/kaizen_events.py` — every changed hunk, its enclosing function or section, its callers | CLEAN (one-token registration; every EVENT_TYPES consumer grepped in scripts/, tests/, .claude/hooks/ — no count, COLUMNS or schema enumerates the set) |
| Hunt: `tests/test_kaizen_deferral_vocabulary.py` — every changed hunk, its enclosing function or section, its callers | FIXED r1 (R1-T01b-O4: membership-only; now drives compute_metrics with stop_causes={'deferral': 1} → numerator 1, and emit('decision_block') with no unknown-event warning; red-on-revert shown by the coder and re-run by 3 seats) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (no guard in the diff; emit() on an unknown type warns and still writes — unchanged) |
| Recurrence: cost/quota accounting — a counter or limit at its edges | FIXED r1 (the premature_stop_rate numerator grew while its definition hash stood still — R1-T01b-S1) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (set membership is exact; the formula-text grader asserts each member as a substring of a text built from the same set) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (R1-T01b-O4) |

## Pass Ledger

| Pass | Finders | Counters | Method |
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×1 (finders sonnet, haiku, opus; refuter sonnet) | found: 10, new: 10, confirmed: 2, fixed: 2, unexecuted: 0 | method: citation — wide pass over the pinned coder commit, every confirmed claim re-executed by the orchestrator |
| Pass 2 | native opus×1 + sonnet×2 + haiku×1 (the round-1 seats over their own slice ledger; refuter sonnet) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — each ledger claim re-executed on the pinned fixup head a99e54f66 (tests run from scratch copies, the tripwire pin recomputed); 2 of 2 claims NOW_FALSE on all three seats |

## Dispositions

| ID | Disposition |
|---|---|
| R1-T01b-S1 | CONFIRMED → FIXED r1 (member lists derived from PREMATURE_CAUSES + version bump) |
| R1-T01b-S2 | REFUTED (CHANGELOG applied by the orchestrator, D3) |
| R1-T01b-H1 | CONFIRMED (dup of S1) |
| R1-T01b-H2 | CONFIRMED (dup of S1) |
| R1-T01b-O1 | CONFIRMED (dup of S1: def_hash unchanged) |
| R1-T01b-O2 | RECORDED — measured (scripts/sysadmin/kaizen_outcomes.py:1218 is outside the plan File Scope; docs/STRATEGIC_BACKLOG.md row before V4 2026-09-30) |
| R1-T01b-O3 | CONFIRMED (dup of S1) |
| R1-T01b-O4 | CONFIRMED → FIXED r1 (numerator grader + emit grader) |
| R1-T01b-O5 | RECORDED — by design (D-371) — the decision_block reader is the V4 measurement, written as the T06 backlog row |
| R1-T01b-O6 | CONFIRMED (dup of S1) |

## Gate

The ticket's own graders on its branch head (a99e54f66), run by the orchestrator: `tests/test_kaizen_deferral_vocabulary.py tests/test_kaizen_outcomes.py tests/test_kaizen_events.py` — 133 passed. The two master-red tests in `tests/test_kaizen_collect_v2.py` (test_upsert_leaves_no_tmp_residue, test_upsert_replace_failure_leaves_the_log_untouched) fail identically on master before this ticket (conftest isolation dirs land in tmp_path) and are routed separately. The whole-plan `final_gate.py` run is recorded in the plan's integration receipt (T06).

## Per-phase verdicts

### T01b — the kaizen vocabulary: PASS
