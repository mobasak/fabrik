# T10 fix — migrate-backlog runs once per repo; V2 reads the pre-adoption backlog

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 4d9095a86b08c848dd55d767f1a53c212c5a32b3; range tip 7a008bced7d7b8ddda79cb74df00230deb399857; `git diff 4d9095a86..7a008bced -- scripts/work.py tests/test_work_migrate.py docs/reference/work-tracking.md` md5 830b03074ebc93053068ceae1eca1eb6 (7199 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/work.py`, `tests/test_work_migrate.py`, `docs/reference/work-tracking.md`
...
```text
$ python scripts/review_rubric.py --changed scripts/work.py tests/test_work_migrate.py docs/reference/work-tracking.md
...
### core/45-testing-strategy.md  (hit: tests/test_work_migrate.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/work.py` — every changed hunk, its enclosing function, its callers | FIXED r0 (`cmd_migrate_backlog` returns early after `_require_store` once `migrated_at` is set, with a message naming `work.py add --kind backlog`; the digest set is still re-read inside the lock, so a racing first run cannot duplicate) |
| Hunt: `tests/test_work_migrate.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (both hub-backlog tests read the pre-adoption blob, skipped when absent; the once-per-repo test reds on revert; the module docstring matches) |
| Hunt: `docs/reference/work-tracking.md` — every changed hunk, its enclosing function, its callers | CLEAN (the verb row and the two backlog-view sentences match `cmd_migrate_backlog`) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (a store-less repo still refuses before the early return; a corrupt config raises WorkError, caught at `main`) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (no accounting on this surface) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (the guard is truthiness of `migrated_at`; an empty string reads as not migrated, the pre-existing rule) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (removing the early return reds `test_migrate_backlog_after_the_migration_creates_nothing_from_the_kept_context`) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the V2 input: 4,951 lines, 336 rows at 24fbac770; the live view gave 4 rows) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every claim executed: the mutant, both hub tests, a hand-appended row after migration) |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (A: scripts/work.py sonnet+haiku; B: tests/test_work_migrate.py + docs/reference/work-tracking.md sonnet+haiku) + refuters | found: 4, new: 4, confirmed: 2, fixed: 2, unexecuted: 0 | citation — the full partitioned pass (workflow wf_14f34ebe-379) over 4d9095a86..7a008bced; executed: the mutant without the early return (an extra ownerless item), a two-process ordering (the digest set is re-read inside the lock), a store-less repo still refused; CONFIRMED — `test_hub_copy_titles_never_start_with_an_empty_or_gapped_bold_marker` still read the live backlog, now the rendered view, 4 rows (B-S1), and the module docstring described the old V2 input (B-S2); REFUTED A-H1; RECORDED A-S1; seats: A-sonnet 0/1 · A-haiku 0/1 · B-sonnet 2/2 · B-haiku 0/0; stop: confirmed 2; fix: +7 -9 lines for 2 confirmed (588aa272a) |
| Pass 2 | native sonnet×2 + haiku×2 (A: sonnet+haiku over scripts/work.py; B: sonnet+haiku over the test file and the doc) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: the same slice owners on the pin of 588aa272a (workflow wf_4c260314-a1f); B-S1 and B-S2 re-executed NOW_FALSE (the hub-copy test runs on the 336-row pre-adoption blob, 1 passed; the docstring names the blob); A-S1 re-executed as recorded (a hand-appended row after migration creates 0 items and the message names `work.py add --kind backlog`); seats: A-sonnet 0/0 · A-haiku 0/0 · B-sonnet 0/0 · B-haiku 0/0; stop: confirmed 0 |
...
| A-S1 | RECORDED — by design (D-414) |
| A-H1 | RECORDED — measured (refuted by execution: `_existing_backlog_digests` is re-read inside the store lock, so a second racer creates nothing) |

## Per-phase verdicts

### Phase 1 — T10 fix, migrate-backlog once per repo: CONVERGED — 2 passes, confirmed 2 → 0
...
```json
$ .venv/bin/python scripts/final_gate.py --check --json   # main checkout, HEAD b6494be6e (the fix merged 43412df1a, D-414 + CHANGELOG b6494be6e; governance sync on the merge: 47 projects synced, 0 failed)
{
  "status": "success",
  "tier": 2,
  "passed": 67,
