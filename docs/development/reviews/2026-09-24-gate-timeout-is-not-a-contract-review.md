# A warn-only timeout is not a broken contract (D-382)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 5c31f796c287174e4b22cc4808a6ec2a282975c2; `git diff HEAD -- <the 17 changed files below> | md5sum` = 7a522a16fdfc9927a96ac7c80be4dd57
**Mail answered:** 01M37YR2KDNCE8V5YTRDHRB7D6
**Command:** /fabrik-review · **Changed:** `scripts/final_gate.py`, `scripts/enforcement/check_imports_resolvable.py`, `tests/test_final_gate_advisory_display.py`, `CLAUDE.md`, `templates/governance/CLAUDE.md`, `tests/test_governance_template_split.py`, `scripts/enforcement/check_stage_artifacts.py`, `scripts/enforcement/check_plan_lock_release.py`, `scripts/enforcement/check_certification_coverage.py`, `tests/enforcement/test_certification_coverage.py`, `tests/enforcement/test_check_review_hygiene.py`, `tests/enforcement/test_plan_lock_release.py`, `docs/workstation/wsl-startup-inventory.md`, `docs/reference/plan-lock-lifecycle.md`, `docs/reference/command-evaluation-checklist.md`, `docs/workflows/FINAL_GATE_WORKFLOW.md`, `docs/reference/certification-denominator.md`
...
### core/40-documentation.md  (hit: CLAUDE.md, docs/reference/certification-denominator.md, docs/reference/command-evaluation-checklist.md)
...
### core/45-testing-strategy.md  (hit: tests/enforcement/test_certification_coverage.py, tests/enforcement/test_check_review_hygiene.py, tests/enforcement/test_plan_lock_release.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/final_gate.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A3: the ⚠ rerun hint named the bare script path, dropping the interpreter and the registered args — `cmd` is built once and the ⚠ names `shlex.join(cmd)`; A-comment: two comments in `scripts/final_gate.py` still said a timeout returns 1) · CLEAN otherwise — 28 `run_cmd(` call sites examined, all treat non-zero as failure; the one that read 1 as success (`ruff check --fix`, `code in (0, 1)`) now fails a timeout, pinned by a test |
| Hunt: `scripts/enforcement/check_imports_resolvable.py` — every changed hunk, its enclosing function, its callers | CLEAN — both rewritten comments in `scripts/enforcement/check_imports_resolvable.py` are true: the check is registered BLOCKING (no `warn_only`), so a timeout fails the gate with `RC_TIMEOUT` |
| Hunt: `tests/test_final_gate_advisory_display.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A2: the `ruff --fix` timeout branch had no test — `test_a_ruff_fix_timeout_is_red_not_read_as_issues_found`, red on the mutant `code in (0, 1, RC_TIMEOUT)` in a throwaway worktree) · CLEAN otherwise — the 5 timeout tests in `tests/test_final_gate_advisory_display.py` pass and each is red on its own mutant (slice B, two mutants) |
| Hunt: `CLAUDE.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (A1: the GATE row's 11 `final_gate.py` line cites in `CLAUDE.md` pointed 1-19 lines off after the insertion) — remapped, and graded by the new test |
| Hunt: `templates/governance/CLAUDE.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (A1: the same 10 cites in `templates/governance/CLAUDE.md`) — remapped, byte-identical sentence to the hub copy (split test green) |
| Hunt: `tests/test_governance_template_split.py` — every changed hunk, its enclosing function, its callers | FIXED r2 (A5: the hub-only `:81` cite was ungraded — `_HUB_ONLY_GATE_CITES`) · CLEAN otherwise — `test_the_gate_rows_line_citations_land_on_what_they_name` in `tests/test_governance_template_split.py` is red against HEAD's gate, red on an inserted line and on an un-moved cite (slice A mutants) |
| Hunt: `scripts/enforcement/check_stage_artifacts.py` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `scripts/enforcement/check_stage_artifacts.py` |
| Hunt: `scripts/enforcement/check_plan_lock_release.py` — every changed hunk, its enclosing function, its callers | FIXED r3 (A6: two bare `:387` cites in `scripts/enforcement/check_plan_lock_release.py` missed by the remap, and `:387` was wrong at HEAD — a blank docstring line; all four ten-line-cap cites now name `:609`, `print_step`'s advisory loop); its `output[:500]` cites were false at HEAD and are RECORDED to the backlog |
| Hunt: `scripts/enforcement/check_certification_coverage.py` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `scripts/enforcement/check_certification_coverage.py` |
| Hunt: `tests/enforcement/test_certification_coverage.py` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `tests/enforcement/test_certification_coverage.py` |
| Hunt: `tests/enforcement/test_check_review_hygiene.py` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `tests/enforcement/test_check_review_hygiene.py` |
| Hunt: `tests/enforcement/test_plan_lock_release.py` — every changed hunk, its enclosing function, its callers | FIXED r2 (A4: a bare `:387` missed by the first remap) · FIXED r3 (A6: now `:609`, the real loop) — `tests/enforcement/test_plan_lock_release.py` passes |
| Hunt: `docs/workstation/wsl-startup-inventory.md` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `docs/workstation/wsl-startup-inventory.md` |
| Hunt: `docs/reference/plan-lock-lifecycle.md` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `docs/reference/plan-lock-lifecycle.md`; its `output[:500]` claim was false at HEAD and is RECORDED to the backlog |
| Hunt: `docs/reference/command-evaluation-checklist.md` — every changed hunk, its enclosing function, its callers | CLEAN — remapped HEAD-line→new-line by a difflib line map; slice A verified every remapped cite against HEAD's `scripts/final_gate.py` (25 pairs round 2, 10 round 3) — `docs/reference/command-evaluation-checklist.md` |
| Hunt: `docs/workflows/FINAL_GATE_WORKFLOW.md` — every changed hunk, its enclosing function, its callers | FIXED r2 (A4: 15 bare cites on lines 217 and 224-226 of `docs/workflows/FINAL_GATE_WORKFLOW.md` the first remap missed; :960's two cites were wrong at HEAD and now name `:1196` and `:141`) — slice A verified all |
| Hunt: `docs/reference/certification-denominator.md` — every changed hunk, its enclosing function, its callers | FIXED r3 (A7: `docs/reference/certification-denominator.md:132` was outside the first file list — `:199-209`, byte-identical to HEAD's `:198-208`) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN — a blocking check's timeout still fails in `scripts/final_gate.py::run_optional_check`; only a warn-only one becomes a named skip, visible in `skipped_checks` and `warnings` |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — no accounting code on the surface (`scripts/final_gate.py` rows, `tests/test_final_gate_advisory_display.py` checked) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — `RC_TIMEOUT` = 124 is distinct from 0, 1 and pytest's 5 in `scripts/final_gate.py`; the skip marker ` (NOT RUN` matches `_SKIP_MARKERS` (slice B executed `_summarize_skipped`) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (A2 above: the `ruff --fix` timeout) — every behaviour change in `scripts/final_gate.py` has a test seen red |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — 28 `run_cmd` call sites, 26 warn-only registrations, 25 + 10 citation pairs, each counted by the producing grep in the seats' ledgers for `scripts/final_gate.py` |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every candidate executed: mutants in worktrees/copies, live runs of `tests/test_final_gate_advisory_display.py` (46 tests incl. the human-vs-JSON gate test on a real checkout) |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×1 (fabrik-reviewer: A-sonnet and A-haiku on `scripts/final_gate.py`, B-sonnet on the tests and comments; a TRIMMED partition, SEATS 3 of 4 wanted — B had no haiku seat) | found: 6, new: 6, confirmed: 4, fixed: 4, unexecuted: 0 | citation — each seat read the pinned surface (md5-verified at start and end) and executed each candidate |
| Pass 2 | native sonnet×1 (A-sonnet, its own slice) | found: 3, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | method: re-derivation — slice A re-derived every remapped cite against HEAD's gate and mutated copies of the grader |
| Pass 3 | native sonnet×1 (A-sonnet) | found: 4, new: 4, confirmed: 3, fixed: 3, unexecuted: 0 | method: re-derivation — a live sweep of every `final_gate.py:N` cite outside frozen history (18 files); the scope-growth stop fired here (own-fix 3/3 then 2/3) |
| Pass 4 | native haiku×1 (A-haiku, its own slice) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — verify-only over the round-3 edits: the six `:609` cites, the `:199-209` byte comparison, 136 tests; slice B standing clean from pass 1 |
...
| Id | Verdict |
|---|---|
| F5 | REFUTED (A4-r1: the human and `--json` runs of the gate disagreed on `skipped` in a copy with no `.git`; on the real checkout `test_human_mode_names_its_skips_and_agrees_with_the_envelope` passed, 1 passed in 170 s) |
| F6 | RECORDED — by design (D-382) |
| F7 | RECORDED — measured (A-r3: `output[:500]` cites in `check_plan_lock_release.py`, `test_plan_lock_release.py` and `plan-lock-lifecycle.md` were false at HEAD, and dated CHANGELOG/LESSONS/DECISIONS rows cite stale lines; both are filed to docs/STRATEGIC_BACKLOG.md by the scope-growth stop) |

F6: the timed-out row is not in the `--json` `advisory` array, which is keyed on the original check name. Semgrep's timeout row behaves the same, and the skip already reaches `skipped_checks` and `warnings` (D-382 records the shape).
...
### Phase 1 — the gate timeout fix and its citation sweep: CONVERGED

confirmed 4 → 3 → 3 → 0; the scope-growth stop fired after pass 3 and the remaining citation class went to the backlog; pass 4 was verify-only over the last edits.
...
```json
$ python3 scripts/final_gate.py --json --check   # shared main tree, the full change staged (ledgers as HEAD + this change's hunks)
{"status": "success", "tier": 2, "passed": 66, "failed": 0, "skipped_checks": ["bandit", "semgrep", "pytest"]}
```

Skips: `bandit` and `semgrep` are diff-sensed (`no src/ changes`); `bandit scripts/ (HIGH only)`, `ruff` and `ruff-format (--check)` ran and passed on this surface. `pytest (NOT RUN)` is the hub's by design; the touched suites were run by hand: 252 passed (`test_governance_template_split`, `test_plan_lock_release`, `test_certification_coverage`, `test_check_review_hygiene`, `test_final_gate_advisory_display` without its slow gate test), and the slow `test_human_mode_names_its_skips_and_agrees_with_the_envelope` passed alone on this checkout (170 s).
