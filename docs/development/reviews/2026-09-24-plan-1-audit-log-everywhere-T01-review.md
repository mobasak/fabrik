# T01 acceptance review — shape.database_url_app_role flag
...
```text
$ python scripts/review_rubric.py --changed src/fabrik/spec_loader.py src/fabrik/spec_generator.py tests/test_spec_loader.py tests/test_spec_generator.py
...
### core/10-python.md  (hit: src/fabrik/spec_generator.py, src/fabrik/spec_loader.py, tests/test_spec_generator.py)
...
### core/45-testing-strategy.md  (hit: tests/test_spec_generator.py, tests/test_spec_loader.py)
...
| Class | Status |
|---|---|
| Hunt: `src/fabrik/spec_loader.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (H1 and H4 — the default holds with a database and survives a real `load_spec`; the new field and its validator at `src/fabrik/spec_loader.py` hunted by sonnet×1 + haiku×1) |
| Hunt: `src/fabrik/spec_generator.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (S1 — a database spec whose type has no `shape:` block now raises; S2 — the overlays re-validate through `_validated_shape_overlay` instead of `model_copy`) |
| Hunt: `tests/test_spec_loader.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (H1 and H4 tests added; pass 2 re-ran them green at `tests/test_spec_loader.py` against the pinned tree) |
| Hunt: `tests/test_spec_generator.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (H2 and H3 — the use_database test now asserts `needs_database` and `depends.postgres`; the S1/S2 tests were watched red first) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (S1 — the silent shape=None path in `src/fabrik/spec_generator.py` now fails loud with a ValueError naming the type) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (no counter, limit or quota in `src/fabrik/spec_loader.py` or `src/fabrik/spec_generator.py` hunks — hunted by both pass-1 seats) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (the validator is a boolean pair in `src/fabrik/spec_loader.py`; no sentinel or prefix match in either changed module — hunted both passes) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (H1–H4 — every Behavior Contract row of T01 now has its test in `tests/test_spec_loader.py` or `tests/test_spec_generator.py`) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (no count claim in the T01 hunks of `src/fabrik/spec_generator.py`; the report's 101-passed line is pytest's own total) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (pass 2 EXECUTED every ledger claim against the pinned tree at `src/fabrik/spec_generator.py`; the orchestrator re-ran the suite: 115 passed on the integrated tree) |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1 (T01 slice), one sonnet refuter | found: 7, new: 7, confirmed: 6, fixed: 6, unexecuted: 0 | method: citation — full slice pass over the T01 hunks + one hop; S3 refuted (the orchestrator applies CHANGELOG deltas at merge, D3) |
| Pass 2 | native sonnet×1 + haiku×1 (the round-1 T01 owners) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — all 6 ledger claims re-executed against the pin of 29d43e3c1 (NOW_FALSE each, with the command output); the fix hunks hunted, none found |
...
|---|---|
| T01-flag-S3 | REFUTED (the CHANGELOG entry is the orchestrator's delta, applied in the acceptance commit per /fabrik-execute-plan D3) |
...
### Phase 1 — T01 `shape.database_url_app_role`: CLEAN after r1 fixes — 101 T01 tests + 14 scaffold spec tests pass on the integrated tree
