...
|---|---|---|---|
| 1 | secret detection (both directions) | FIXED | **Settled at pass 18 by choosing a DIRECTION rather than a better split.** Three rounds tried to separate `user:pass@host` from `host:port/path@note`; they are lexically identical and every split leaked at its own seam (P16 caught the credential but ate doc links · P17 protected doc links but missed bare-hostname DSNs · P18's two-form version missed their intersection, `mysql://user:8080/api@db-host`). For the credential-bearing schemes the match is now unconditional — fail CLOSED — justified by measurement, not taste: the ambiguous shape occurs in 0 of 910 real messages, so the false positive costs nothing and is loud, while the false negative silently published a credential into a durable fleet-synced store. Non-DSN schemes keep the stricter `/`-excluding class, so ordinary doc links still send. Live-store re-verified in BOTH directions at every step. History:  P17-1 corrected the round-16 overreach: the DSN form now requires the post-`@` tail to look like a real HOST (dotted name · `name:port` · `name/`), so `postgres://internal-docs:8080/api@readme` sends while every genuine credential still refuses. Re-verified against the LIVE store in BOTH directions this time — 910 messages, **0 newly refused AND 0 detections lost** vs pre-round-16. P17-4 bound the generic pattern's `_re.I`, which round 16 added but only proved for the DSN regex (mutation-killed). Round 16's original finding:  Pass 16, a class no earlier round swept — every prior secret row audited guard ORDERING, never whether the patterns actually MATCH real credential shapes. Four bypasses scored `None` (not even LOW): `/` in a DSN password (excluded from the char class to protect doc URLs — now allowed for the credential-bearing schemes only, where no doc-link false-positive surface exists), a capitalised scheme (`_re.I` was missing on that one pattern alone), `pwd` absent from the LOW net while `PWD` sat in the HIGH one, and the underscore vendor style (`sk_live_`, `rk_`) where the existing pattern demanded a hyphen. Verified against the LIVE store: 910 real messages, **0 newly refused, 0 newly warned** — the widening costs no legitimate mail |
...
| 2 | fail-open vs fail-closed per guard | FIXED | P17-2: `read_msg` tested `is_file()` and fell through on anything else, so a stray DIRECTORY in the message slot (a bad restore) made a structurally-EXISTING parent read as MISSING — C2's fail-CLOSED HOLD inverted to a fail-open ALLOW. It now raises, which the `--auto` path already routes to HOLD. Earlier:  missing/prose parent → ALLOW (documented fail-soft); existing-but-unparseable/unreadable/quarantined → HOLD (R3/H3); unsafe repo → HOLD (P3-2); failed quarantine → still counted (P9-1) |
...
| Pass 17 | 2 native non-author finders — a LEDGER RE-SWEEP (not new angles) split across the 12 accumulated classes | 5 | 5 | 5 | The first true re-sweep of the whole ledger against current HEAD, since rounds 13-16 had rewritten `_quarantine`, `digest`, `requeue`, the exception ladder and the secret patterns underneath earlier verdicts. 10 of 12 classes reconfirmed CLEAN under fresh live probes and ~10 mutations. Two REOPENED — both by round 16's own fix: the widened DSN pattern made `scheme://host:port/path@note` parse as `user:pass@host` and **hard-refused legitimate ops mail** (the round-16 live-store check missed it because no message happened to have that shape — absence in a corpus is not proof of no false-positive surface), and `read_msg` silently skipped a non-file in the slot, reading a parent that structurally EXISTS as MISSING and inverting C2's fail-CLOSED HOLD into a fail-open ALLOW. Two mutation survivors closed the coverage gaps round 16 left (`requeue`'s `and lossless` gate at the ack-line ∩ undecodable-byte intersection; `_re.I` on the GENERIC url pattern, proved only for the DSN one). Fifth: the reference doc's secret enumeration was stale again — the same class P15 fixed, reopened by the round that widened the patterns |
| Pass 18 | 2 native non-author finders (round-17 delta + full ledger re-sweep, mutation-first) | 7 | 7 | 7 | Round 17's fix broke two more, and the split it introduced leaked at its own seam. **P18-3 REVERSES the P17-1 adjudication**: `user:pass@host` and `host:port/path@note` are LEXICALLY IDENTICAL, so no split can separate them — rounds 16, 17 and 18 each tried one and each leaked. On a guard whose purpose is that secrets never travel, the ambiguity resolves CLOSED; the doc-link shape appears in **0 of the 910** live messages, so the false positive is free and loud while the false negative silently published a credential. P18-1: the round-17 host-tail requirement missed a BARE hostname — this fleet's own convention (`postgres-main`) — so the DSN shape most likely in THIS store travelled clean. P18-2: round 17 raised on the first anomalous slot INSIDE the search loop, so a stray directory in `inbox/` shadowed a readable parent in `archive/`. Four mutation survivors closed: `MAX_RE` had zero tests at any length, both digest age thresholds were boundary-untested, the rate walk's dotfile guard was the one of three without a test, and the `_QUARANTINE_SLOTS` valve was off-by-one-untested |
| Pass 19 | 1 native non-author finder (round-18 delta) | 2 | 2 | 2 | Two of the four attack angles came back verified-correct (the deferred `anomalous` flag threads properly through all four fallback paths and both CLI verbs; the slot-bound test genuinely reads the monkeypatched global). The two real findings both landed on round 18's own reasoning: it justified the fail-closed false positive as "free" by measuring FREQUENCY (0 of 910), which says nothing about the shape of legitimate FUTURE mail — and this store's core job is carrying security findings that QUOTE evidence, so refusing a REDACTED DSN punishes exactly the behaviour to encourage. A placeholder-password exemption now lets `postgres://user:REDACTED@host` send while every real credential still refuses. Second: the round-18 doc told operators to "backtick it", which is factually wrong — a backtick is a word boundary, so the pattern still matches |
| Pass 20 | 2 native non-author finders (round-19 delta + terminal ledger re-sweep w/ completeness audit) | 9 | 9 | 9 | **The round-19 exemption was three BYPASSES.** It mixed fixed placeholder WORDS (safe — they match only if the password IS that word) with unbounded SHAPE wildcards (`<...>`, `${...}`, `YOUR...`), so any real secret merely dressed in that shape scored `None` — neither refused nor warned: `postgres://user:<9f8a7b6c5d4e>@host` published clean. An exemption on a security guard is a bypass unless it matches on CONTENT; the rule is now letters-only (no digits) AND must contain a placeholder word. **P20-1 is the round's most consequential find and is operational, not theoretical:** `_is_hub_repo()` is content-based on `Path.cwd()` and pytest always runs from the hub, so `test_cli_digest_honours_days` drove the REAL `ssh vps → docker exec apprise → Telegram` path and fired genuine operator alerts off synthetic fixtures on every suite run — while the test file's own docstring claimed that leg was stubbed. It never was. Now stubbed in the `env` fixture for EVERY test, and bound by two tests. Four more unbound guards closed (`_publish` containment, the root dot-dir filter, the operator-facing `repos` field, `_current_repo`'s fallback), and pass 18's `_QUARANTINE_SLOTS` closure was REOPENED — its test occupied more slots than the bound, so `>` and `>=` both ended in the same refusal: it proved "eventually gives up", never the boundary |
| Pass 21 | 1 native non-author finder (round-20 delta) | 1 | 1 | 1 | Three of the four attack angles came back verified-clean with evidence (no ReDoS — 7-22 ms on adversarial 60-80 KB bodies; the fixture-wide `_is_hub_repo` stub hides nothing, as that symbol has exactly one call site and the test wanting the hub branch re-patches it; no over-refusal of any documented redaction). The one finding is the same defect one level deeper: round 20's rule asked whether the password CONTAINS a placeholder word — a case-insensitive SUBSTRING test on short common tokens (`PASS`, `HERE`, `NONE`) — so ordinary letters-only passwords contain them and published clean: `CompassionateHeart`, `Nonetheless`, `Passphrase`, `therefore`. Containment is not content-matching. Split into two precise forms: BARE must BE placeholder words end to end (so `PASS` is exempt and `Passphrase` is not), WRAPPED trusts the `<…>`/`${…}` fill-this-in convention with a digit-free identifier inner |

Informal boundary rounds (pre-command, during execute-plan): 11+7+9+7+4+3 = 41, all fixed or
adjudicated-documented. Formal loop: 115 more (11+2+9+8+5+8+6+6+4+7+5+5+8+4+6+4+5+7+2+9+1). **Total 156 findings on this surface.**
