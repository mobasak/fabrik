...
| naming fossil — the test file | FIXED (1) | `tests/test_derive_cost_sidecar.py` tested `claude_p_cost.refresh()` and never touched `derive_cost`; the name was this plan's founding error wearing an explanatory docstring — the same "document the blind spot" move A3 rejects three steps earlier. Renamed to `tests/test_claude_p_cost_refresh.py`; plan File Scope, Phase A Files, the A gate command, the Shape line and the lock's `owned_paths` all repointed in the same commit; both rename halves in the pathspec (228 added / 157 deleted, verified in `git show --numstat`). `INDEX.md` needs no row — gate-verified, not assumed (`check_index_md.py` exit 0, and neither sibling test file appears among its 100 `tests/` rows). |
...
58 candidates raised across 8 rounds; **48 FIXED, 10 REFUTED with proof, 1 ROUTED, 0 unadjudicated.**
...
work: **mutation** (the suite let five mutations through as first written; 8 of 8 red now) and
...
$ python scripts/final_gate.py --check --json
  "status": "success"
  tier: 2   passed: 54   failed: 0   skipped_checks: ["pytest"]
...
`.fabrik/run-pytest`), so `skipped_checks` names it and the 43 tests above were run directly, not by
