# T08 review — both contracts carry the work-items rule
...
```text
$ python scripts/review_rubric.py --changed CLAUDE.md templates/governance/CLAUDE.md tests/test_work_contract_rule.py tests/test_governance_template_split.py
...
### core/45-testing-strategy.md  (hit: tests/test_governance_template_split.py, tests/test_work_contract_rule.py)
...
| Class | Status |
|---|---|
| Hunt: `CLAUDE.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (the paragraph after the DONE/NEXT discipline paragraph of § FINAL OUTPUT, reworded: `8 lowercase hex`, a followable commit clause, `(D-392, D-394)`; the GATE row's 11 `final_gate.py` cites re-anchored after T06's shift, each landing on its construct) |
| Hunt: `templates/governance/CLAUDE.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (both § FINAL OUTPUT copies carry the paragraph with the hub-only doc cited as `/opt/fabrik/docs/reference/work-tracking.md` and `(hub D-392, hub D-394)`; the GATE row re-anchored like the hub's) |
| Hunt: `tests/test_work_contract_rule.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (pins `_CANONICAL` and derives each template copy by exactly the two sanctioned substitutions; an identical drift in all three copies now fails 2 tests) |
| Hunt: `tests/test_governance_template_split.py` — every changed hunk, its enclosing function, its callers | CLEAN (its `_GATE_CITES` keys and 3 pinned claim strings moved with the re-anchor; the full suite passes, 39 of 39) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (prose only; the Stop harvest's write to the item a NEXT: line names is not a session-authored file (`_session_files` reads only Edit/Write tool uses), so it can never block a Stop) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (no runtime change; one paragraph per contract copy) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 — the id shape now says lowercase, matching `_ITEM_REF_RE` (an uppercase id is ignored); the hook's NEXT and final-block regexes are line-anchored, so 0 of the paragraph's lines match a closing-block key |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 — the canonical text is pinned; each Behavior-Contract row (placement, identity up to the sanctioned substitutions, the parity suite) has a failing mutation |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (3 of 3 § FINAL OUTPUT copies, 11 GATE-row cites, 23 FINAL_GATE_WORKFLOW cites, 0 cites outside the GATE row in either contract) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every clause executed: `work.py answer --help`, `_mint_id`, a harvest creating and `answer` closing an awaiting item, each cite printed against final_gate.py, the hook regexes run over the paragraph) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2 (A: CLAUDE.md + templates/governance/CLAUDE.md sonnet+haiku+opus; B: the two test files sonnet+haiku) + refuters | found: 10, new: 10, confirmed: 5, fixed: 5, unexecuted: 0 | citation — the full partitioned pass (workflow wf_954405ce-d1f) over 9503a9345..f1b5c56dc; executed: `work.py answer --help` shows `--note`; ids are `W-` + sha256[:8]; `harvest --decision-ok --repo` created an awaiting-operator item and `answer` closed it; the hook's NEXT and final-block regexes are line-anchored, so 0 of 6 paragraph lines match; all 11 re-anchored GATE-row cites land on their constructs; the paragraph sits after the DONE/NEXT paragraph in 3 of 3 copies; CONFIRMED — the template copies cite a repo-relative doc the sync never delivers (A-O2), the commit clause cannot be followed because a NEXT: line changes the item at the Stop harvest, after the commit (A-O3), the id rule omits lowercase (A-O6), D-392 alone does not carry the store design or the no-scoring wording (A-O7), the test never pins the canonical text (A-O4); RECORDED A-S1, A-H1, A-O1, B-S1 (the cited doc arrives with T09, merged first), A-O5 (the DECISION-to-item path is conditional on an allowed Stop, a store and a synced thread_anchor — true in the repos the paragraph addresses); seats: A-sonnet 0/1 · A-haiku 1/1 · A-opus 7/7 · B-sonnet 1/1 · B-haiku 0/0; stop: confirmed 5 |
| Pass 2 | native opus×1 + sonnet×2 + haiku×2 (A: sonnet+haiku+opus over the two contracts and FINAL_GATE_WORKFLOW.md; B: sonnet+haiku) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: the same slice owners on the pin of bd40160b9 (workflow wf_fd65693f-0e2); A-O2, A-O3, A-O4, A-O6, A-O7 FIXED by execution; the new surface 98f16efa8 re-derived in full — each of the 23 FINAL_GATE_WORKFLOW cites printed against the pin's final_gate.py and landing on its construct, the word-diff numbers-only; the reworded commit clause checked against `_session_files` (the harvest's write is never session-authored); seats: A-sonnet 0/0 · A-haiku 0/0 · A-opus 0/0 · B-sonnet 0/0 · B-haiku 0/0; stop: confirmed 0 |
...
| A-S1 | RECORDED — measured (the cited docs/reference/work-tracking.md arrives with T09, which merges first in the same run) |
| A-H1 | RECORDED — measured (the same T09 coupling) |
| A-O1 | RECORDED — measured (check_doc_links reds on the forward cite only until T09 lands; T09 merges before T08) |
| B-S1 | RECORDED — measured (the same T09 coupling) |
| A-O5 | RECORDED — measured (the DECISION-to-item path needs an allowed Stop, a store and a synced thread_anchor.py; the paragraph addresses repos with a store, where the hook's allowed exits all store it) |

## Per-phase verdicts

### Phase 1 — T08, both contracts carry the work-items rule: CONVERGED — 2 passes, confirmed 5 → 0; the GATE-row and FINAL_GATE_WORKFLOW citations T06 shifted re-anchored in the same run
