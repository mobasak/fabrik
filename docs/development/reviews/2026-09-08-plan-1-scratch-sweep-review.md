# Review — 2026-09-08-plan-1-scratch-sweep

**Status:** CONVERGED 2026-09-08
**Surface:** `git rev-parse HEAD` = 5dd80c2668e772769caa52912a11cea44f04146f; `git diff HEAD -- <the 14 File-Scope paths>` md5 fa81791716c72ace599cafc81aa34dc2 (40185 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/scratch_sweep.py`, `tests/test_scratch_sweep.py`, `scripts/command_run.py`, `tests/test_command_run.py`, `scripts/sysadmin/install_user_hooks.py`, `tests/test_install_user_hooks.py`, `docs/workstation/hooks-index.md`, `docs/workstation/cleanup-automation.md`, `docs/workstation/wsl-startup-inventory.md`, `docs/reference/command-run-protocol.md`, `docs/reference/multi-agent-operating-model.md`, `commands/_sources/fabrik-execute-plan.md`, `CLAUDE.md`, `templates/governance/CLAUDE.md`
...
### core/40-documentation.md  (hit: CLAUDE.md, commands/_sources/fabrik-execute-plan.md, docs/reference/command-run-protocol.md)
...
| Class | Status |
|---|---|
| Hunt: `scripts/scratch_sweep.py` — every changed hunk, its enclosing function, its callers | CLEAN — 1,888 lines read whole across 5 Phase-A rounds + 3 Phase-B rounds; every one of the 9 removal paths traced to its guard. Round-2 fix: `Row.age_s` + the brief's age sort (`scratch_sweep.py:859`), proven red by reverting line 859 to a head-slice on a copy. |
| Hunt: `tests/test_scratch_sweep.py` — every changed hunk, its enclosing function, its callers | CLEAN — 55 graders, each seen red first. Two repaired this round: the truncation grader gained its completing third leg (red on guard deletion, `1 failed in 1.51s`), and `test_the_brief_shows_the_oldest_candidates_and_counts_all_of_them` is new (red on the head-slice, `1 failed in 0.87s`). |
| Hunt: `scripts/command_run.py` — every changed hunk, its enclosing function, its callers | CLEAN — only the three scratch-advisory hunks are this plan's; the file also carries another session's D-191 seat work, reviewed by them. The advisory is queued in `_close` and printed in `main()` after the lock drops and after `finally: _flush_events(...)`; the queued tuple is read with `.get`, because a `KeyError` there returns rc 0 and would turn the mis-named-close refusal into success. |
| Hunt: `tests/test_command_run.py` — every changed hunk, its enclosing function, its callers | CLEAN — 3 scratch graders. The lock-freeness one was INERT (it timed `line`, which `main()` answers before taking the lock) and now times `step`: red at 1.45 s against the in-lock mutant, green at 0.17 s pristine, threshold 1.0 s. |
| Hunt: `scripts/sysadmin/install_user_hooks.py` — every changed hunk, its enclosing function, its callers | CLEAN — one `SessionStart` key in `ENTRIES`; `_stale`/`run` iterate `ENTRIES.items()`, so a new event key needs no loop change. `UserPromptSubmit` measured and rejected (9 of 9 live sessions fired on every prompt); the comment recording that sits beside the entry. |
| Hunt: `tests/test_install_user_hooks.py` — every changed hunk, its enclosing function, its callers | CLEAN — `test_check_finds_missing_scratch_sweep_entry_and_install_adds_it` covers both directions of `--check`/install. |
| Hunt: `docs/workstation/hooks-index.md` — every changed hunk, its enclosing function, its callers | FIXED — F2: the row closed with a live-session fire-rate census (9 of 11 / 0 of 11 / 1 of 11) that no artifact anywhere carried and that cannot be re-derived without writing stamps into other sessions' dirs. Replaced by the graded rule. F3: my replacement then cited a grader that does not exist; the three real names substituted, each confirmed by `grep -c`. |
| Hunt: `docs/workstation/cleanup-automation.md` — every changed hunk, its enclosing function, its callers | CLEAN — § F's 82 lines checked claim by claim by the docs seat: 34 claims, the cron marked NOT YET PLACED in both docs and true against `crontab -l`, `--dead --apply` for the cron against a bare `--dead` for the dry run, and the no-`flock` rationale resolving to `_Lock` at `scratch_sweep.py:884-914`. |
| Hunt: `docs/workstation/wsl-startup-inventory.md` — every changed hunk, its enclosing function, its callers | CLEAN — the cron sentence matches § F verbatim, same time, same NOT-YET-PLACED, same rationale. |
| Hunt: `docs/reference/command-run-protocol.md` — every changed hunk, its enclosing function, its callers | CLEAN — the two `FABRIK_SCRATCH_SWEEP*` rows resolve to `command_run.py:1953` and `:1955-1956`; the new section's three load-bearing properties each name their grader. Its earlier `line`/`status` claim was FALSE and is corrected. The seat-accounting hunks in the same file are another session's. |
| Hunt: `docs/reference/multi-agent-operating-model.md` — every changed hunk, its enclosing function, its callers | FIXED — F4: the pointer said `--apply` removes 'the merged + clean'; the script also requires unlocked and unheld. All four conditions now named. |
| Hunt: `commands/_sources/fabrik-execute-plan.md` — every changed hunk, its enclosing function, its callers | FIXED — F4, same clause. Corpus re-rendered from the main master checkout and `assemble_commands.py --check` reports 'check OK'. |
| Hunt: `CLAUDE.md` — every changed hunk, its enclosing function, its callers | CLEAN — the § EXIT sentence sits INSIDE item 5 after the push clause (never a new section) and the `clean-own-scratch` marker bullet was appended to the universal list. Its never-touches clause is a strict subset of the script's own refusal set as printed by `--help` — it under-promises, which is the safe direction. |
| Hunt: `templates/governance/CLAUDE.md` — every changed hunk, its enclosing function, its callers | CLEAN — byte-identical § EXIT sentence; it names the ABSOLUTE hub path, which is what makes it true from all ~46 repos and from fabrik-lib. |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN — direction named per mode and each is deliberate: the advisory fails OPEN in every direction (no script, non-zero exit, hang, exception → "" and the close is unchanged); `_holds_backup` returns True on OSError (KEEP); an unreadable sessions root makes NO sid dead and `--apply` exits rc 2; worktree provenance fails CLOSED (every worktree `wt-foreign`). Round-1 F2 was a fail-open that became a permanent SILENCE on the biggest pads — the guard now returns before the stamp write. |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — pool OFF (D-181/D-182), so nothing records to the flywheel and no `NO-POOL:` declaration is owed; seats this round were 3 native (1 Opus authoritative, 1 Opus Phase-B closing, 1 Sonnet docs-truth). Runtime cost measured, not estimated: `--brief` at close is 8 lines / 1,046 bytes against the 477 lines / 64 KB it replaced; the SessionStart walk is 3.2 s worst case on this box's largest pad (52,320 entries) against a 10 s registration timeout. |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — `age_s` defaults to None and sorts LAST under `-(r.age_s or 0.0)`, never first; `BRIEF_ROWS` slices candidates, not rows, so the count stays over the whole table; the stamp's `previous` sentinel is -1 on a missing or unparseable file, which makes the first evaluation speak. |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED — this was the round's whole yield. THREE graders did not grade: two passed with their guard deleted and one did not exist. Each repair was proven by mutation on a COPY (never in the shared tree) and restored, both halves asserted. |
| Recurrence: denominator on every count — bounded searches state their bound | FIXED — F1: the CHANGELOG said 33 graders; the re-derivable figure is 53 at Phase A (`git show f29c0463:tests/test_scratch_sweep.py | grep -c '^def test_'`) and 55 today (`pytest --collect-only -q`). Both now stated with the command that produces them. |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every claim in this receipt that could be executed was: 197 graders run, `final_gate.py --json --check` → `status: success` with `skipped_checks: ['pytest']`, `assemble_commands.py --check` → check OK, `render_doc_script_links.py --check` → 43 coupled docs current, and the three mutation proofs. The one thing NOT executed is the live-session fire-rate census — and that is exactly why F2 removed the claim rather than restating it. |
...
### Phase A — the script and its graders: CLEAN

`scripts/scratch_sweep.py` + `tests/test_scratch_sweep.py`, committed f29c0463. Five review rounds
found SIX data-loss paths by EXECUTION, each closed with a red-on-revert grader: the harness tag
swallowing a chain verdict; a dead `/proc` entry reading as unheld; an unreadable sessions root;
`--include-backups` reaching fresh and held holders; a branch ref truncated at its last slash; and a
sid-level symlink escaping the scratch root. Live proof on this session's own scratchpad: 419 stale,
40 backup-holders spared, `proc-gaps 2` naming both same-uid deniers, tree byte-identical before and
after the dry run.

### Phase B — the three triggers: CLEAN after two fix rounds

`scripts/command_run.py`, `scripts/sysadmin/install_user_hooks.py` and their graders, committed
9456380d. Round 1 raised 9 (two HIGH: a 477-line dump into every close's context fleet-wide, and a
truncation fix that silenced exactly the biggest scratchpads). Round 2 raised 2, both INERT GRADERS.
Round 3 is the closing pass. The phase's three load-bearing properties each have a grader that has
now been seen red against a mutation of the specific line it guards.

### Phase C — the contract, the pointers, the ledger: CLEAN after four fixes

Both `CLAUDE.md` files, the `clean-own-scratch` universal marker, the two pointer clauses, D-200,
Lesson 162 and this receipt. F1-F4 above were all raised and fixed inside this phase. The § EXIT
sentence is byte-identical in the hub contract and the template, names the absolute hub path, and
its never-touches clause is a strict subset of the script's own `--help` refusal set.
...
`skipped_checks: ["pytest"]` is the hub's deliberate exception — 5,913 tests under `-x` would brick
every completion gate three sessions run — so this green asserts the static tier, not the suite. The
suite is asserted separately and in the same turn:

```text
$ uv run pytest tests/test_scratch_sweep.py tests/test_command_run.py tests/test_install_user_hooks.py -q
199 passed in 78.43s (0:01:18)
```

57 scratch-sweep graders (2 added this round, both seen red against the specific mutant they guard),
130 command_run, 12 installer.

## Findings — the Finish round

Three native seats (pool OFF, D-181/D-182): one Opus authoritative seat over the whole-plan diff and
the fleet-law surface, one Sonnet docs-truth seat over the five reference/workstation files, and the
Phase-B closing seat. Every finding below was fixed in this run; none was filed as someone else's.

| # | Seat | Severity | Finding | Disposition |
|---|---|---|---|---|
| F1 | docs-truth | MED | `CHANGELOG.md` Phase A paragraph said "33 graders". The re-derivable figure is 53: `git show f29c0463:tests/test_scratch_sweep.py \| grep -c '^def test_'` → 53, and the commit's own title says 53. A transposition, asserted in the fleet's changelog. | FIXED — "53 graders at Phase A, each seen red first; 55 today." Both numbers re-derived in the same command. |
| F2 | docs-truth | MED | `docs/workstation/hooks-index.md:104` closed with "Confirmed fire rate: 9 of 11 live sessions on first sight, 0 of 11 on the next, 1 of 11 when one genuinely grew" — and NO artifact anywhere carried it: not the script's comments, not a grader, not a commit body, not the ledger. Every sibling figure in the same row is echoed verbatim in `scratch_sweep.py`; this one stood alone. A census of live sessions also cannot be re-derived later without writing stamps into other sessions' directories. | FIXED — the census is replaced by the RULE it was evidence for (speak only above the recorded count; record on every evaluation; a truncated walk neither speaks nor records) with the three graders that pin it. |
| F3 | self, while fixing F2 | MED | The replacement text I wrote for F2 cited a grader named `test_the_advisory_line_speaks_once_then_stays_silent`. **No such test exists** — I named it from memory in the same edit that removed an unverifiable claim. | FIXED — the three real names substituted and each confirmed present by `grep -c "^def <name>"` → 1. Recorded here rather than quietly corrected: it is the same defect class the round was convened to close, committed by the reviewer, one line after the finding. |
| F4 | self, pre-empting the Opus seat | LOW | Both pointer clauses (`docs/reference/multi-agent-operating-model.md`, `commands/_sources/fabrik-execute-plan.md`) said `--apply` "removes only the merged + clean". The script also requires UNLOCKED and UNHELD. A pointer that narrows the gate is still a false claim, and this one renders into the fleet-wide command corpus. | FIXED — both now say "merged + clean + unlocked + unheld"; corpus re-rendered and `--check` OK. |

| F5 | Phase-B closing seat | HIGH | The false `line`/`status` lock mechanism survived in TWO docstrings this plan ADDED (`_scratch_advisory`'s own, and the close-out grader's) — and my Phase B commit body claimed all three copies were corrected when `git show` proves only the protocol doc was. Leaving it in the advisory function's own docstring is the seed for rebuilding the belief that made the first grader inert. | FIXED — both corrected. The commit-body claim was false when written; recorded here because it cannot be amended on a shared tree. |
| F6 | Phase-B closing seat + Opus seat, independently | HIGH | `_scratch_advisory` capped its shell-out at `timeout=2`, but the sweeper's walk deadline is 8 s with an unbudgeted holder probe on top. Measured through the shipped function: the box's largest scratchpad takes 3.29 s and was KILLED at every close, returning "" — so 1 of the 19 sessions with a non-empty table, the one with the MOST residue, got no advisory at all. Verbatim the argument that raised `HOOK_DEADLINE_S` 1.0 → 4.0 for the other trigger, not carried across. | FIXED — `timeout=12`. Truncating the sweeper's own walk was tried first and REVERTED: a truncated session walk has no certain candidates, so it prints nothing either; silence sooner is not better than silence later. New grader `test_the_close_out_brief_truncates_rather_than_being_killed` pins `WALK_DEADLINE_S < ` the caller's timeout across the two files. |
| F7 | Phase-B closing seat | MED | A mutant the whole suite missed: `if budget.exhausted and previous >= 0`. Every truncation grader primes with a complete walk first, so the realistic case — the biggest pad, its FIRST SessionStart, truncated — was never exercised. Under that mutant it speaks a wobbling lower bound AND writes it down as the baseline, suppressing the true count forever. | FIXED — `test_a_first_ever_truncated_walk_speaks_nothing_and_records_nothing`, which asserts silence AND the absence of the stamp file. Red on that exact mutant (1 failed of 56), green restored. |
| F8 | Opus seat | HIGH | The § EXIT sentence's `<repo>/.tmp` clause was FALSE, on the data-protection half of a sentence that ships to ~46 repos. The script spares `.tmp` as a SCAN ROOT and says so; a worktree registered under `.tmp` is classified like any other. `/opt/fabrik-lib/.tmp/merge-check` is such a registration today, spared only because it is detached-HEAD. | FIXED — the sentence now says a worktree is a worktree wherever it sits, `.tmp` included, and that the script declines to SCAN `.tmp` without exempting it from that rule. |
| F9 | Opus seat | MED | Both pointer clauses offer the sweeper at a step whose worktree the HARNESS created (`EnterWorktree` → `<repo>/.claude/worktrees/` → `wt-harness`, not removable), and neither named `--include-harness`. Live: 15 of 15 harness worktrees across two repos classify `wt-foreign` right now, so an agent substituting the offered command for `git worktree prune` removes nothing and leaves the tree it was told to retire. | FIXED — both pointers name `--include-harness` and say it is still refused when dirty, unmerged or foreign. |
| F10 | Opus seat | LOW | Two more § EXIT overclaims: "`--worktrees` LISTS the agent worktrees you opened" (it lists EVERY registered worktree, most of them foreign — including your own after a `--resume`), and a seven-name verdict list against the ten tokens the script emits, in which "merged" is never printed at all. | FIXED — the real tokens are named and `wt-foreign` is called out as the expected common case. |
| F11 | Opus seat | LOW | `docs/DECISIONS.md` D-187's rationale says "150 of 151 fd-deniers are foreign-uid and the two same-uid ones are permanent" — 150 + 2 ≠ 151. | FIXED in a NEW row: D-200 carries the correction (466 pids, 152 denying `/fd`, 150 foreign, 2 same-real-uid). Ledger rows are immutable, so the arithmetic is not edited in place. |
| S1 | Phase-B closing seat | sub | Hook mode's `oldest` was `max(...)` over `candidates[:20]` in NAME order, printed unqualified — measured `oldest 3.52d` where the true oldest was 3.73d — and it re-walked those 20 entries AFTER the deadline the rest of the function respects. | FIXED anyway (free): every stale row now carries `age_s`, so the exact max costs nothing and 20 unbudgeted re-walks disappear. |
| S2 | Phase-B closing seat | sub | `render`'s comment claimed the brief shows the OLDEST candidates universally, but `--dead --brief` had no `age_s` on any row, so its sort degraded to directory order. Not reachable from the close-out, which only ever sees `stale`. | FIXED anyway: the `dead` row carries the `dir_age` it already computes. |
| S3 | Phase-B closing seat | sub | `hooks-index.md` said the stamp holds the count seen at EVERY evaluation; since the truncation guard returns before the write, a truncated evaluation records nothing. | FIXED — one clause. |
| S4 | Phase-B closing seat | sub | My Phase B commit body says "54 scratch-sweep graders … 13 installer". Measured: 55 and 12. The total, 197, was right. | RECORDED, not fixable — a commit body cannot be amended on a shared tree. The correct counts are here and in the plan's Evidence. |

### The universal marker, proven against the consumer's own parser

The `clean-own-scratch` bullet is only useful if fabrik-lib's `check_governance_drift.py` can read
it — that check is the whole reason the bullet exists. Its regex was applied to the hub file as
shipped, rather than eyeballed:

```text
$ python3 -c "<_MARKER_RE from /opt/fabrik-lib/scripts/enforcement/check_governance_drift.py>"
markers parsed: 13
mine: [('clean-own-scratch', 'CLEAN your own scratch')]
anchor found in the template: True
anchor found in hub CLAUDE.md body: True
```

13 markers parse where 12 did before, the label and anchor extract exactly as intended, and the
anchor string is present in both `CLAUDE.md` and `templates/governance/CLAUDE.md` — so a repo whose
contract lacks the sentence gets the warning, and this repo does not warn on itself. fabrik-lib was
mailed the notice (01M20EZM9H7451EZH64KVM2SGR, ack:no) so the new warning is expected rather than
mysterious.

**Sub-threshold, recorded not fixed:** the "87 GB" figure in `cleanup-automation.md` is not re-derivable
today (7.9 GB) and is phrased as history, which the plan review already adjudicated (P3-5).
