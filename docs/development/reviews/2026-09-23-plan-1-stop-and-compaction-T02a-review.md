...
| Class | Status |
|---|---|
| Hunt: `CLAUDE.md` — every changed hunk, its enclosing section, its callers | FIXED r1-r2 (R1-T02a-S1/O5 the 7-line template and STATE footer now point at the block; R1-T02a-H1/O2 two dropped rules restored; R1-T02a-O1 the block is written unfenced; R1-T02a-O6 the closed class list stated; R2-T02a-O3 the ad-hoc-branch disposition named `destructive`) |
| Hunt: `tests/test_governance_template_split.py` — every changed hunk, its enclosing section, its callers | FIXED r1-r2 (R1-T02a-O3/O4/O7/O8 and R2-T02a-O1/O2: the format grader scoped to the format block, the examples bound to their labels, the compact-instructions grader fence-aware for heading and body, the rules-kept grader covering every rule of the bar paragraph — each proven red by a named mutant on a copy, re-run by 3 seats in pass 3) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (R1-T02a-O3: a field deleted from the canonical format block left the grader green — fail-open grader) |
| Recurrence: cost/quota accounting — a counter or limit at its edges | CLEAN (no counter, budget or quota surface in the CLAUDE.md hunks or the graders) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1-r2 (R1-T02a-O7 / R2-T02a-O1: a fenced `# Compact instructions` matched as the real heading) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1-r2 (R1-T02a-O4, R2-T02a-O2) |

## Pass Ledger

| Pass | Finders | Counters | Method |
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×1 (finders sonnet, haiku, opus; refuter sonnet) | found: 11, new: 11, confirmed: 8, fixed: 8, unexecuted: 0 | method: citation — wide pass over the pinned coder commit, every confirmed claim re-executed by the orchestrator |
| Pass 2 | native opus×1 + sonnet×2 + haiku×1 (the round-1 seats over their own slice ledger; refuter sonnet) | found: 3, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | method: re-derivation — 8 ledger claims re-executed on 43bf9d0ea (6 NOW_FALSE, 2 STILL_TRUE); the 3 confirmed lie in round-1 fix hunks (own-fix: round 1) |
| Pass 3 | native opus×1 + sonnet×2 + haiku×1 (the round-1 seats over their own slice ledger; refuter sonnet) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the 3 round-2 claims re-executed on a769949fd, graders mutated on scratch copies; 3 of 3 NOW_FALSE on all three seats |

## Dispositions

| ID | Disposition |
|---|---|
| R1-T02a-S1 | CONFIRMED → FIXED r1 (template NEXT points at the block) |
| R1-T02a-S2 | REFUTED (no reader assumes one H1: scripts/docs_updater.py:1929 takes the first H1, fence-aware; fabrik-lib check_governance_drift.py parses the ## UNIVERSAL section only) |
| R1-T02a-H1 | CONFIRMED → FIXED r1 (restore "derive the verdict") |
| R1-T02a-O1 | CONFIRMED → FIXED r1 (block written unfenced) |
| R1-T02a-O2 | CONFIRMED → FIXED r1 (restore both dropped rules) |
| R1-T02a-O3 | CONFIRMED → FIXED r1 (format grader scoped to the format block) |
| R1-T02a-O4 | CONFIRMED → FIXED r1 (rules-kept grader) |
| R1-T02a-O5 | CONFIRMED (dup of S1 + STATE footer) |
| R1-T02a-O6 | CONFIRMED → FIXED r1 (closed class list stated) |
| R1-T02a-O7 | CONFIRMED → FIXED r1 (fence-aware compact grader) |
| R1-T02a-O8 | CONFIRMED → FIXED r1 (label-to-example binding) |
| R2-T02a-O1 | CONFIRMED → FIXED r2 (own-fix: round 1) (fence-aware heading) |
| R2-T02a-O2 | CONFIRMED → FIXED r2 (own-fix: round 1) (rules-kept grader widened) |
| R2-T02a-O3 | CONFIRMED → FIXED r2 (own-fix: round 1) (the ad-hoc-branch disposition named `destructive`) |

## Gate

On the branch head a769949fd, run by the orchestrator: `tests/test_governance_template_split.py` — 31 passed; ruff clean. Net `CLAUDE.md` growth over 5c31f796c: +1,619 bytes (the DECISION block format, two examples, the closed class list, `# Compact instructions`). The whole-plan `final_gate.py` run is in the plan's integration receipt (T06).

## Per-phase verdicts

### T02a — hub CLAUDE.md: PASS
