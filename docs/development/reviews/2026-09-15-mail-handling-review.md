...
| Hunt: `docs/reference/fabrik-mail.md` — every changed hunk, its enclosing function, its callers | CLEAN (`docs/reference/fabrik-mail.md` escalation section re-read against `scripts/sysadmin/mail_escalate.py` at HEAD; the two-leg description, `--kind finding --ack no` and the local-leg claim all verified in code) |
