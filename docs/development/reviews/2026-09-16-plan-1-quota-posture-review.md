...
| Delta 7 (resumed) | dispatched — see the verdict row below | `e9b1f8fe347c17d788e4dc201cea1d4f`, range `b989e2cb3~1..a9adc13f9` (829 lines, 8 files) | **The closing round over the RESUMED commits, on the operator's word (ground 3).** Surface: the per-window fleet band at the writer (D-275) with the three-contract re-cut and QUOTA_CLAIMS re-pin; the self-explaining QUOTA line (`_fleet_clause`) and the fleet-aware deny reason; the ledger latch for the wall advisory (D-276) with the storm reproduction; the posture invalidation on flip (B19); graders B19-B21a, C2b-C2d, C5b. Not dispatched earlier on the D-252 stop and the AMBER band, then on the operator's pause. |
