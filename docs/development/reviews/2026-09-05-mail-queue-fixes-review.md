...
| C5 | `_mask_quoted` bypass surface | FIXED(3) | Passes 1–11 as recorded. **Pass 12 (orchestrator probes, executed):** the `/dev/null` exemption was a PREFIX match — `>/dev/nullx`, `> /dev/null/../x`, `>>/dev/null.txt` allowed (pre-existing shape; non-root cannot write there, but the hold exists to stop WORK, not just file writes). Now `/dev/null` must be followed by whitespace, EOL or an operator; 10-probe table zero mismatches; graded, red on revert. Own first cut used `\b(?!/)` and let `/dev/null.txt` through — caught by its own assertion before commit |
