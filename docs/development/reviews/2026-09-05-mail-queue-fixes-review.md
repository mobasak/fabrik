...
Status: IN-PROGRESS — passes 1–11 recorded; pass 11 certified the whole surface and found ONE adjacent defect (the pre-existing redirect regex, fixed), so pass 12 (fresh sweep of that fix) runs next in this invocation.
...
| C5 | `_mask_quoted` bypass surface | FIXED(2) | Passes 1–3 as recorded. **Pass 11 (P11-A, 15 probes executed):** the masker held on every probe; the ONE finding sits beside it — the pre-existing `_FILE_REDIRECT` regex (`:72`, untouched by this review) backtracked on `>> /dev/null` and `2>>/dev/null`, refusing forms its own exemption names (fail-closed over-refusal). Fixed: `(?!>)` after the optional second `>` plus `>` in the lookbehind; graded with both exempt forms and three append forms that must still refuse; red on revert |
...
(Pass 11 raised 1 adjacent finding, fixed; Pass 12 — a fresh sweep of that fix — follows.)
