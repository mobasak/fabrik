...
Status: IN-PROGRESS — passes 1–6 recorded; pass 6 raised 4 new (all fixed), so pass 7 (fresh sweep of the pass-6 fixes) runs next in this invocation.
...
| C2 | cost/quota/limit accounting edges (standing) | FIXED(4) | Passes 1–5 as recorded. **Pass 6 (P6-A, re-derived, 27 tool uses):** the FIFTH wrong term — "one dead host is under the cap" is false for the AGGREGATE (~33s/account × 4 accounts ≈ 132s > 60s); plus an ambiguous "that is NOT inside" in the rotation doc (predating this review, inside the named range) and an ungrounded "~195s for three" legacy figure. The class was the METHOD: five rewrites each restated a number. All three copies now state the INVARIANT — the aggregate has no wall-clock bound (per-call × 2 calls × N fresh accounts, no loop budget), so any sustained slowness breaches the cap — with the two derived aggregates (132s / 260s at N=4) as illustrations; the legacy figure dropped; the backlog row's regression test asserts the aggregate under both fault shapes. P6-A independently confirmed every other number, the 4-group fleet (`assignments.json`), and that only 3 of the 4 `-k oauth` tests exercise `_oauth_get` |
...
| C7 | `GATE_OUT_OF_SURFACE` semantics | FIXED(5) | Passes 1–5 as recorded. **Pass 6 (P6-B, executed A–E fixtures):** the regex + fix-hint + `/fabrik-review` agree and behave as promised (plain / backtick / inline fence → exit 0; block fence below / hard-wrapped → exit 1 citing the hint verbatim); rendered command byte-identical to source; the fix-hint joins into one sentence. One drift: `fabrik-execute-plan.md:361`'s copy of the declaration lacked the one-line constraint — aligned and rendered. No new regex change |
...
(Pass 6 raised 4 new, so it is not the exit; Pass 7 — a fresh sweep of the pass-6 fixes — follows.)
