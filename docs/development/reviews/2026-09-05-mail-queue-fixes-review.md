...
| C5 | `_mask_quoted` bypass surface | FIXED(6) | Passes 1–13 as recorded. **Pass 14 (orchestrator, marker-proven):** `git status <(touch m)` allowed and the marker appeared — process substitution runs the inner command with no listed operator; `[<>]\(` now vetoed, quoted form stays data, red on revert. IN-DESIGN, recorded: a tool's own write flag (`git log --output=m`) is not shell syntax and the hold is default-deny on COMMANDS, not an enumeration of every tool's flags (`git commit` writes by design). Four pre-existing holes in one hold this session, every one found by EXECUTING a candidate with a side effect — a whole-line scan refuses only what it was told to look for |
...
(Pass 13 raised 1, fixed; Pass 14 — a fresh sweep of the pass-13 fixes — follows.)
