# Review — fix-project-declared-type

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 907f62f1b256f85043e9525992426b602d1c84a0; `git diff HEAD -- src/fabrik/scaffold.py src/fabrik/cli.py src/fabrik/deploy_router.py tests/test_scaffold_fix.py tests/test_scaffold.py tests/test_backfill_has_user_guide.py docs/reference/fabrik-cli-reference.md docs/workflows/SCAFFOLD_STRUCTURE.md` md5 3b8ff7a62be882215ba593f2180af478 (30795 bytes)
**Command:** /fabrik-review · **Changed:** `src/fabrik/scaffold.py`, `src/fabrik/cli.py`, `src/fabrik/deploy_router.py`, `tests/test_scaffold_fix.py`, `tests/test_scaffold.py`, `tests/test_backfill_has_user_guide.py`, `docs/reference/fabrik-cli-reference.md`, `docs/workflows/SCAFFOLD_STRUCTURE.md`
...
```text
$ python scripts/review_rubric.py --changed src/fabrik/scaffold.py src/fabrik/cli.py src/fabrik/deploy_router.py tests/test_scaffold_fix.py tests/test_scaffold.py tests/test_backfill_has_user_guide.py docs/reference/fabrik-cli-reference.md docs/workflows/SCAFFOLD_STRUCTURE.md
...
### core/10-python.md  (hit: src/fabrik/cli.py, src/fabrik/deploy_router.py, src/fabrik/scaffold.py)
...
### core/40-documentation.md  (hit: docs/reference/fabrik-cli-reference.md, docs/workflows/SCAFFOLD_STRUCTURE.md)
...
### core/45-testing-strategy.md  (hit: tests/test_backfill_has_user_guide.py, tests/test_scaffold.py, tests/test_scaffold_fix.py)
...
| Class | Status |
|---|---|
| Hunt: `src/fabrik/scaffold.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-core-S1/B-cli-S2 validate_project fallback to the shared list plus an explicit python-api-gpu entry; A-core-S3/B-cli-S1 refusal hint names the path) |
| Hunt: `src/fabrik/cli.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-core-S2 the added-count summary prints only when files were added) |
| Hunt: `src/fabrik/deploy_router.py` — every changed hunk, its enclosing function, its callers | CLEAN (src/fabrik/deploy_router.py get_project_type non-string guard and its callers cli.py apply and scaffold._declared_project_type, both A-core seats, executed) |
| Hunt: `tests/test_scaffold_fix.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (three graders added for the round-1 fixes, each watched red on the pass-1 pin in a throwaway worktree) |
| Hunt: `tests/test_scaffold.py` — every changed hunk, its enclosing function, its callers | CLEAN (tests/test_scaffold.py eight fix_project call sites pinned to python-api; the -k fix slice ran 23 passed under the A-core seats) |
| Hunt: `tests/test_backfill_has_user_guide.py` — every changed hunk, its enclosing function, its callers | CLEAN (tests/test_backfill_has_user_guide.py refusal and explicit-type backfill graders; pre-existing :123 dry-run wording recorded as A-core-H3) |
| Hunt: `docs/reference/fabrik-cli-reference.md` — every changed hunk, its enclosing function, its callers | CLEAN (docs/reference/fabrik-cli-reference.md validate and fix paragraphs checked claim by claim by the B-cli seats against cli.py) |
| Hunt: `docs/workflows/SCAFFOLD_STRUCTURE.md` — every changed hunk, its enclosing function, its callers | CLEAN (docs/workflows/SCAFFOLD_STRUCTURE.md repair paragraph checked by the B-cli seats against fix_project and cli.py) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (src/fabrik/scaffold.py _declared_project_type refuses every unresolvable type; cli.py fix and validate exit 1 on refusal and on unrepairable files) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (src/fabrik/scaffold.py and cli.py carry no quota or cost accounting; seat counts stamped via command_run dispatch) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (src/fabrik/scaffold.py validate_project missing-entry sentinel fell back to python-api; now the shared list) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (tests/test_scaffold_fix.py and tests/test_backfill_has_user_guide.py cover every changed behaviour, each grader proven red on a pin) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the /opt survey in the scoped round states 39 project.yaml files, 0 refused; tests state their pass counts) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every candidate executed by the slice refuters; every grader run red then green, src/fabrik and tests) |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (dispatched 4, returned 4) + 2 refuters | found: 8, new: 8, confirmed: 5, fixed: 5, unexecuted: 0 | method: citation; seats: A-sonnet 3/3 · A-haiku 0/3 (3 recorded pre-existing) · B-sonnet 2/2 · B-haiku 0/0; stop: confirmed 5; fix: +13 changed src lines, 3 graders, for 5 confirmed (3 distinct defects); hygiene 0 hits at start |
| Pass 2 | native sonnet×2 (the round-1 Sonnet seat of each slice; dispatched 2, returned 2) + 2 refuters | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — each seat re-executed its slice ledger over pins2 (md5 094fcc2bb1e0f772adc6c277c6faacf2, re-checked equal on the live tree after the pass) and proved every round-1 grader red by reverting its fix hunk on a copy; 5 of 5 claims NOW_FALSE; stop: closable; fix: none |
...
8 candidates → 5 FIXED + 0 REFUTED + 3 RECORDED.

| A-core-S1 | FIXED r1 (validate_project falls back to _SHARED_REQUIRED_FILES; python-api-gpu entry explicit) — test_a_type_without_its_own_list_is_held_to_the_shared_files |
| B-cli-S2 | FIXED r1 (same defect as A-core-S1, raised independently) — same grader |
| A-core-S2 | FIXED r1 (cli fix prints the added summary only when files were added) — test_cli_prints_no_added_summary_when_nothing_was_added |
| A-core-S3 | FIXED r1 (_declared_project_type RuntimeError hint names the path) — test_refusal_hint_names_the_project_path |
| B-cli-S1 | FIXED r1 (same defect as A-core-S3) — same grader |
| A-core-H1, A-core-H2, A-core-H3 | RECORDED — see Residual |

## Residual

Scoped predecessor: `/fabrik-review-scoped` round 1 over the pre-fix diff raised 16 and confirmed 15 (fixed before this run opened; ledger in the session scratch `rs-fix/pass-1.json`), O11 recorded there.

| A-core-H1 | RECORDED — measured (pre-existing dry-run wording 'opencode.json (refresh from master)', outside this diff's hunks; filed as W-152a0e3c) |
| A-core-H2 | RECORDED — measured (pre-existing dry-run wording 'project.yaml (backfill has_user_guide)', outside the hunks; W-152a0e3c) |
| A-core-H3 | RECORDED — measured (pre-existing test pin of that wording at tests/test_backfill_has_user_guide.py:123; W-152a0e3c) |
| O11 | RECORDED — measured (scoped round, kilo docs copy absent from the dry-run preview, predates this diff) |
...
Surface = 8 files of the uncommitted diff over 907f62f1b, routed up from `/fabrik-review-scoped` (>5 code files after its round-1 fixes). No prior receipt for this scope; anchor not applicable.

### Phase 1 — Independent finders: DONE

Two slices (A-core, B-cli), sonnet + haiku each, 4 of 4 returned; hygiene 0 hits over 8 files.

### Phase 2 — Execute every candidate: DONE

Slice refuters executed all 8; the orchestrator re-ran the wordpress, hint and summary probes and watched the three new graders red on the pass-1 pin.

### Phase 3 — Prove and fix: DONE

5 confirmed fixed (3 distinct defects), 3 recorded pre-existing (W-152a0e3c).

### Phase 4 — Converge: CONVERGED

Pass 2, the round-1 Sonnet seats over their own slices, confirmed 0 with every claim executed.
...
```json
{
  ...
  "advisory": [
    ...
    },
    {
      ...
      "output": "hygiene: 0 hit(s) over 2 file(s), 43 rows ungraded",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    },
    ...
    {
      "check": "Feedback duty",
      "output": "feedback duty: 11 close(s) in 14d, all carried a verdict",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    },
    ...
    },
    {
      ...
      "output": "corpus-weight: CLAUDE.md 105320 B \u00b7 baseline 98610 (+6710) \u00b7 base origin/master (\u2014)\ncorpus-weight: templates/governance/CLAUDE.md 109777 B \u00b7 baseline 91060 (+18717) \u00b7 base origin/master (\u2014)\ncorpus-weight: commands/_sources 1109195 B \u00b7 baseline 1053408 (+55787) \u00b7 base origin/master (\u2014)\ncorpus-weight: commands/_fragments 140369 B \u00b7 baseline 119777 (+20592) \u00b7 base origin/master (\u2014)\ncorpus-weight: commands/_agents 28439 B \u00b7 baseline 24465 (+3974) \u00b7 base origin/master (\u2014)\ncorpus-weight: .windsurf/rules 1569153 B \u00b7 baseline 1319893 (+249260) \u00b7 base origin/master (\u2014)\ncorpus-weight: OK \u2014 no owned surface grew vs origin/master",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    },
  ...
  "checks": [
    {
      "name": "ruff-format (--check) (SCOPE-NARROWED \u2014 3 unstaged tracked .py)",
      "outcome": "skipped"
    },
    {
      "name": "ruff (SCOPE-NARROWED \u2014 3 unstaged tracked .py)",
      "outcome": "skipped"
    },
    {
      "name": "check json",
  ...
  "warnings": [
    {
      "check": "ruff-format (--check) (SCOPE-NARROWED \u2014 3 unstaged tracked .py)",
      "output": "\u26a0 3 changed Python file(s) were NOT linted: on a shared tree an unstaged tracked edit may be a sibling's WIP, so this leg reads the writable set (staged \u222a base\u2026HEAD). `git add` yours to include them, or pass --fix-all. This green asserts nothing about: src/fabrik/cli.py, src/fabrik/deploy_router.py, src/fabrik/scaffold.py",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    },
    {
      "check": "ruff (SCOPE-NARROWED \u2014 3 unstaged tracked .py)",
      "output": "\u26a0 3 changed Python file(s) were NOT linted: on a shared tree an unstaged tracked edit may be a sibling's WIP, so this leg reads the writable set (staged \u222a base\u2026HEAD). `git add` yours to include them, or pass --fix-all. This green asserts nothing about: src/fabrik/cli.py, src/fabrik/deploy_router.py, src/fabrik/scaffold.py",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    ...
    },
    {
      ...
      "output": "\u26a0 Rule-pack reachability did not finish (Command timed out after 120s); re-run it alone: /opt/fabrik/.venv/bin/python /opt/fabrik/scripts/enforcement/check_pack_reachability.py",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    },
    ...
    },
    {
      ...
      "output": "\u26a0 6 untracked source file(s) NOT in gate scope (unstaged \u2192 unscanned): .fabrik/work/W-152a0e3c.json, .fabrik/work/W-4cc5cac3.json, .fabrik/work/W-85ac25b8.json, .fabrik/work/W-a8cd091a.json, docs/development/reviews/2026-09-25-plan-1-work-store-single-tracker-T05a-review.md, docs/development/reviews/2026-09-26-fix-project-declared-type-review.md \u2014 if yours: `git add` them and RE-RUN the gate (they ship unlinted otherwise); if a sibling's: leave them.",
      "truncated": false,
      "omitted_lines": 0,
      "rerun": null
    }
