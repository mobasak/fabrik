...
- The in-place probe inside `/opt/transdoc` (operator-approved cross-repo write, 2026-09-06 03:1x): `claude -p --worktree agent-alpha` created `/opt/transdoc/.claude/worktrees/agent-alpha` on branch `worktree-agent-alpha`; on disk and in the session's own output, CARRIED: `.mcp.json`, `.claude/hooks/skill_router.py`, `.worktreeinclude`, `.claude/settings.json`; `git config --local` inside the worktree: `rerere.enabled` true, `push.autoSetupRemote` true. ABSENT and explained by the tree, not the mechanism: `.env` (transdoc has no `.env` in its main checkout — nothing to carry) and `.fabrik/synced.lock` (not in the creation-time include set by design: T01b's sync re-copies it into every linked worktree on the next run, and `check_synced_unmodified.py` reports "not yet re-synced; skipped" until then). The prompt-blocking hook failure of the invalid probes did not occur here — the hooks were carried. The worktree and its branch were removed afterwards; transdoc's tree left as found (one worktree, its own tracked dirt untouched). Session cost $1.23.
...
- What this receipt does NOT claim: the hub's full pytest suite (the leg is off; 798 of the plan's tests ran).
