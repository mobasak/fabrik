# Review — T04 of plan 2026-09-25-plan-1-work-store-single-tracker
...
```text
$ python scripts/review_rubric.py --changed scripts/command_feedback_report.py tests/test_command_feedback_report.py commands/_sources/fabrik-command-improve.md
...
### core/40-documentation.md  (hit: commands/_sources/fabrik-command-improve.md)
...
### core/45-testing-strategy.md  (hit: tests/test_command_feedback_report.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/command_feedback_report.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (C-O1 the default ledger never read; C-S1/C-H1/C-O2 one ledger and one answered-index read; C-O3 the close guarded; C-S3/C-O9 the close retried on a no-op re-run; C-O4 take's store calls guarded; C-O7 no orphan item for an empty queue; C-O8 --rows/--commit refused; C-O11 'work store unavailable'), r2 (C-O12 the no-op close only at depth 0; C-O13 read-back failure after an id), r3 (C-O14 never 'took' when the claim is unknown) — scripts/command_feedback_report.py |
| Hunt: `tests/test_command_feedback_report.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (T-S1, T-S2 — the queue_depths guards exercised by patching _rows and the answered index; every new test proven red on the previous commit), r2, r3 — tests/test_command_feedback_report.py 178 passed |
| Hunt: `commands/_sources/fabrik-command-improve.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (C-S2, C-O10 — the --take step moved after the --queue description, every --take stdout line listed verbatim), r3 (the claim-unknown line) — commands/_sources/fabrik-command-improve.md PHASE 1 and PHASE 5 |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (C-O3, C-O4, C-O11), r3 (C-O14: the held case no longer reads as taken) — scripts/command_feedback_report.py take() and mark_answered |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (scripts/command_feedback_report.py queue_depths hunted for read counts — one ledger read and one answered-index read per call, spied in tests; no quota or pool accounting in this surface) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (C-O7 depth 0 / a typo'd command creates nothing) — scripts/command_feedback_report.py take() |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (T-S1, T-S2 plus a test per confirmed defect, each red on the prior commit) — tests/test_command_feedback_report.py |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (receipt counts carry their totals — 17 + 2 + 1 + 0 candidates over four passes; gate 178 passed over two named test files for scripts/command_feedback_report.py) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every confirmed candidate executed by its refuter on a copy of scripts/command_feedback_report.py; every fix proven by reverting it on a copy and naming the red test) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 | found: 17, new: 17, confirmed: 15, fixed: 15, unexecuted: 0 | method: citation — file partition (C: scripts/command_feedback_report.py + the command source with the Opus floor; T: the test file), workflow wf_25222cd8-027; every candidate executed by its refuter; 2 RECORDED by design; one seat edited the shared pin in place (later pins are read-only) |
| Pass 2 | native opus×1 + sonnet×2, refuters sonnet×2 (the round-1 slice owners) | found: 2, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | method: re-derivation — 15 of 15 ledger claims NOW_FALSE, each test proven red under its claim's mutation; 2 new inside pass 1's fix hunks (own-fix 2 of 2) |
| Pass 3 | native opus×1 + sonnet×1, refuter sonnet×1 | found: 1, new: 1, confirmed: 1, fixed: 1, unexecuted: 0 | method: re-derivation — C-O12, C-O13 NOW_FALSE; 1 new inside pass 2's hunk (own-fix 1 of 1); the scope-growth stop fired — new hunting suspended, the fixed set re-verified |
| Pass 4 | native opus×1 (the round-1 owner of slice C) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: C-O14 NOW_FALSE, its new test red when the 'took' line is restored on a copy; 172/178 of the file's tests green on the seat's partial copy, 178 on the worktree; standing clean since pass 1: cost-quota, denominator, proxy-evidence |
...
| P1-C-O5 | RECORDED — by design (D-419) — mark_answered closes the item whoever holds its claim: the answer is the truth, like a mail ack (spec § The delta D2) |
| P1-C-O6 | RECORDED — by design (D-419) — a partial mark closes the run's item: each /fabrik-command-improve run is one item and the next --take opens another |
| P4-R1 | RECORDED — measured (the claim-not-re-read line names the relative scripts/work.py; filed as work item W-7720269d, priority 3) |

## Per-phase verdicts

### Phase 1 — T04 feedback queues: depth and the taken item: CLEAN
...
GATE-SCOPE: out-of-surface — Doc Link Integrity (live tree); findings naming this surface: 0 of 8; measured by: `final_gate.py --check --json` in the T04 worktree at 659ea6a32 — the 8 broken refs point at gitignored cache files absent from a fresh worktree
