# T05a of plan 2026-09-25-plan-1-work-store-single-tracker

**Status:** CLEAN
**Surface:** `git rev-parse HEAD` = 3bc4e07319eaa066fc1f2bdea38e2268c8b21b74; range tip ff8a465431115898b48cac3d729203f24c8afca1; `git diff 57de025c8..ff8a46543 -- scripts/work.py tests/test_work_harvest_rules.py tests/test_work_claims.py` md5 386cd2cbf5926cfc5001c4c23f2a17ff (38917 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/work.py`, `tests/test_work_harvest_rules.py`, `tests/test_work_claims.py`
...
```text
$ python scripts/review_rubric.py --changed scripts/work.py tests/test_work_harvest_rules.py tests/test_work_claims.py
...
### core/45-testing-strategy.md  (hit: tests/test_work_claims.py, tests/test_work_harvest_rules.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/work.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (W-O1/W-O2 markdown-wrapped none and operator decision(s)/operator-decision are holds; W-O9/W-S1/T-S1 ids in paths and URLs name nothing; W-O5 rule 2's claim records the session's bound agent; W-O7 marker-closed next items are skipped; W-O8 title kept in step; W-O10/W-S3 claim written first, supersede always runs), r2 (W2-S1/W-O12/T2-S1 a truthful claimed-but-next-not-written warning; W-S1 ids judged by whitespace token), r3 (W3-O1 bracket, quote, angle, brace and dash openers restored) — scripts/work.py on_harvest and its rule helpers |
| Hunt: `tests/test_work_harvest_rules.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (W-O11/T-S2/T-S3/T-H1/T-H3 the exact 7-day boundary, a kind:next item named by id, rule-3 owner and next_at, uppercase hex), r2 (both warning paths, the token rows), r3 (six opener rows) — every row red on its named mutant; tests/test_work_harvest_rules.py 69 tests |
| Hunt: `tests/test_work_claims.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (the write-order test pins decision < claim < next, a deliberate change from the ticket's step 6 so a failed claim never leaves a rewritten next; the other-session test still asserts A's item and claim byte-identical) — tests/test_work_claims.py |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (W-O10/W-S3 the supersede runs whatever the claim write does), r2 (W2-S1 the warning names the state left behind) — scripts/work.py _apply_next_rules and _claim_named |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (scripts/work.py on_harvest timed inside the Stop hook's 5 s kill and 2 s lock: 4-43 ms on 300 items and 20 next items; 1.08 s on a first harvest closing 100 idle items in a 1000-item store, 0.06-0.13 s after) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (the 7-day boundary strictly greater, pinned with frozen time; hold word boundaries), r2 (token-anchored id match), r3 (opener set) — scripts/work.py classify_next and _next_refs |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (31 named mutants red), r2 (12), r3 (the six opener rows red on the old strip) — tests/test_work_harvest_rules.py |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the receipt's counts carry their totals: 21 + 6 + 1 + 0 candidates over four passes; 373 passed over nine named test files for scripts/work.py) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every confirmed candidate executed by its refuter on a copy of scripts/work.py; the orchestrator re-ran the 373-test gate, ruff and mypy on the worktree at ff8a46543 and proved the round-3 rows red) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 | found: 21, new: 21, confirmed: 15, fixed: 15, unexecuted: 0 | method: citation — file partition (W: scripts/work.py with the Opus floor; T: the two test files), workflow wf_0c343f19-e43; every candidate executed by its refuter; 3 refuted, 3 RECORDED measured; one seat's pin copy landed in the repo root and was removed once proven identical to the pin |
| Pass 2 | native opus×1 + sonnet×2 + haiku×2, refuters sonnet×2 (the round-1 slice owners) | found: 6, new: 6, confirmed: 4, fixed: 4, unexecuted: 0 | method: re-derivation — 14 of 15 ledger claims NOW_FALSE, W-S1 STILL_TRUE (an id after ?item= or #), rewritten as a whole-token rule; 4 of 4 confirmed inside pass 1's hunks |
| Pass 3 | native opus×1 + haiku×1 | found: 1, new: 1, confirmed: 1, fixed: 1, unexecuted: 0 | method: re-derivation — 4 of 4 ledger claims NOW_FALSE; 1 new inside pass 2's token hunk (own-fix 1 of 1); the scope-growth stop fired, so new hunting was suspended and the orchestrator fixed the one named defect |
| Pass 4 | native opus×1 + haiku×1 (the round-1 owners of slices W and T) | found: 1, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: W3-O1 and T2-S1 NOW_FALSE, the six opener rows red on the reverted strip; the one raise was a pre-existing edge RECORDED; standing clean since pass 1: cost-quota, denominator, proxy-evidence |
...
| P1-W-H1 | REFUTED (_keep_next sets next_at to the created stamp minted a line earlier in the same call, which is now) |
| P1-T-H2 | REFUTED (the identical-text harvest writes nothing: the seat's own byte comparison failed only because its probe changed the text) |
| P1-W-O6 | REFUTED (the ticket's step 4 prescribes the created fallback for an unparseable next_at) |
| P1-W-O3 | RECORDED — measured (a flush-race stale NEXT can re-claim an item released in the same turn; the claim lapses with its 2 h lease) |
| P1-W-O4 | RECORDED — measured (rule 2 never releases a session's earlier NEXT-claimed items; D3 is silent, filed as work item W-85ac25b8) |
| P1-W-S2 | RECORDED — measured (the same accumulation as P1-W-O4 through the two harvests of one Stop; W-85ac25b8) |
| P2-W-H1 | REFUTED (`- none of these apply, start W-…` is a hold by spec rule 1, a hold whatever else the line names) |
| P2-W-H2 | REFUTED (`(none yet) W-…` is a hold for the same reason as P2-W-H1) |
| P4-W-O1 | RECORDED — measured (markdown underscore emphasis `_W-…_` still reads as free text because the id regex refuses a following `_`; the same at 0f9ed4461, before this range's last hunk) |

## Per-phase verdicts

### Phase 1 — T05a the harvest's NEXT rules: CLEAN
