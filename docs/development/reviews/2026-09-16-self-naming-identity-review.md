...
### core/10-python.md  (hit: .claude/hooks/session_orient.py, scripts/command_run.py, scripts/fabrik_synced_manifest.py)
...
### core/45-testing-strategy.md  (hit: tests/conftest.py, tests/test_session_orient_hook.py, tests/test_whoami_agent.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/whoami_agent.py` — every changed hunk, its enclosing function, its callers | FIXED — 2 Opus seats read the whole file; 11 confirmed here (no-lock double-bind 30/30, the trim destroying a sibling's row, a newline name that kills the trailer block, toplevel-not-common-dir, an empty scope acting as a wildcard, a recycled pid, a pid-less row reserving nothing, argv no-ops at rc 0, a leaked tmp file, a corrupt `at` aborting bind, a FIFO hang). All closed and re-executed in round 2. |
| Hunt: `scripts/command_run.py` — every changed hunk, its enclosing function, its callers | FIXED — the guarded import leaked a sys.path entry per call (1000 calls -> 1000 entries; a failed import 30 us -> 38 ms at 10k) and `except Exception` missed SystemExit, aborting `start` with NO record written. Both closed; 31 fault shapes driven through the real CLI, 26 fail open byte-identically to the parent commit. |
| Hunt: `scripts/fabrik_synced_manifest.py` — every changed hunk, its enclosing function, its callers | FIXED — the reader synced BEFORE the writer it imports (index 11 vs 12, and the sync iterates in list order). Swapped; verified by importing the module. |
| Hunt: `.pre-commit-config.yaml` — every changed hunk, its enclosing function, its callers | CLEAN — the gate itself caught the missing trigger before commit; re-verified by reproducing the enforcer's own YAML extraction over 2,985 tracked paths: newly matched = exactly scripts/whoami_agent.py, lost = none, and whoami_agent_backup.py / .pyc / .bak / sub/ / xscripts/ all correctly miss. |
| Hunt: `tests/conftest.py` — every changed hunk, its enclosing function, its callers | CLEAN — the AGENT_IDENTITY_FILE pin is autouse for the whole suite; 257 pre-existing tests still pass with it; proven necessary by removing it on a copy. |
| Hunt: `tests/test_whoami_agent.py` — every changed hunk, its enclosing function, its callers | FIXED — 3 graders were vacuous by construction (the alphabet pin asserted raw text and passed while a mutant changed the live regex; the conftest pin asserted a token and passed with the setenv commented out; a dead-holder test asserted only rc 0). All repaired. The race grader took THREE attempts: a 24-way subprocess race passed against the unlocked build, and an in-process simulation bypassed the lock and failed against the fixed one. |
| Hunt: `tests/test_session_orient_hook.py` — every changed hunk, its enclosing function, its callers | FIXED — the prose pin asserted claims this work falsified; re-pinned to the new remedy and proven to still red under a mutant. |
| Hunt: `.claude/hooks/session_orient.py` — every changed hunk, its enclosing function, its callers | FIXED — the advisory asserted 'identity resolves from that ONE env var' and 'a live session cannot change its own environment', both falsified by this very feature in 46 distributed copies. Rewritten to name the command that works. |
| Hunt: `docs/workstation/agent-identity.md` — every changed hunk, its enclosing function, its callers | FIXED — claimed check_commit_trailers.py partially covers the shared-name case (it reads the env var directly and does not fire at all for a --as session) and claimed a backlog routing that had never happened. |
| Hunt: `docs/reference/multi-agent-operating-model.md` — every changed hunk, its enclosing function, its callers | FIXED — its § Identity was made stale by this very feature; the no-relaunch path is now documented and the fences re-balanced (6 markers). |
| Hunt: `CHANGELOG.md` — every changed hunk, its enclosing function, its callers | FIXED — one overstated mutant-kill claim corrected to what was actually proven. |
| Hunt: `docs/DECISIONS.md` — every changed hunk, its enclosing function, its callers | CLEAN — D-271 parses to 6 non-empty cells through the repo's own decisions.py::_rows; --check reports only a pre-existing dangling pointer in another repo. |
| Hunt: `INDEX.md` — every changed hunk, its enclosing function, its callers | CLEAN — both new rows verified for cell parity and alphabetical placement (the first placement was wrong and was corrected before commit). |
| Hunt: `docs/STRATEGIC_BACKLOG.md` — every changed hunk, its enclosing function, its callers | FIXED — the row a doc CLAIMED was routed did not exist; it does now, with three residue items and named owners. |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED — resolve_agent_name is imported by a fleet-synced close gate. 9 hostile store shapes could not make it raise; the one that HUNG (a FIFO) is closed. command_run's guard now also catches SystemExit, which was aborting `start` with no record. |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | FIXED — the sys.path leak was measured, not argued: 30 us -> 3,659 us -> 37,931 us per failed import at len 7 / 1,007 / 10,007. |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED — `$` matching before a trailing newline was the sharpest instance: `re.match` accepted `infra\n`, and that name makes git parse the WHOLE trailer block as nothing. Every use is now fullmatch. Also: an empty scope acting as a wildcard, and a recycled pid. |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED — 6 mutants killed nothing in round 1 (both CLI exit codes, _toplevel, _append_row, _write_rows, mode 0600). All graded; 31 graders now, each new one killed by its own mutant. |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — every number re-derived by a seat with its own tool and denominator named; the one drift found (0 of 40 -> 1 of 40) POSTDATES the commit and is the feature working. |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every candidate executed before counting. Three of this run's own graders were rewritten precisely because their names promised coverage their assertions did not have. |
...
### Phase 1 — the self-naming identity channel, built then reviewed: CLEAN

The operator ruled the plan chain out for this build (spec -> build -> one heavy review), so this
review was the only gate between the code and 46 distributed repos. It found **20 confirmed
defects** across four author-blind seats, and the verdict that matters is where they were: the READ
path — `resolve_agent_name`, the only thing the fleet-synced close gate calls — was genuinely
fail-open and could not be made to raise across nine hostile store shapes. **Every serious defect
was in the WRITE path**, and three of them compounded against the feature's own purpose:

- **No lock.** 30 of 30 barrier-synced trials had BOTH sessions take the same name, with
  `force:false` on both, so the audit field could not even distinguish a raced double-bind from a
  legitimate one. Re-measured after the `flock`: 0 of 30.
- **The 30-day trim rewrote the store from a snapshot**, destroying a sibling's row — 1 in 25
  trials, and inevitable once the store ages past the window.
- **`re.match` + `$` accepted a trailing newline.** Such a name makes git parse the WHOLE trailer
  block as nothing — reproducing the exact "0 of 40 commits carried `Agent-Name`" failure this
  feature exists to fix.

Three more are worth naming because they are about honesty rather than logic. The collision scope
was `--show-toplevel`, so a worktree and its main checkout were different scopes though one repo —
the same common-dir lesson a spec in this same session had already paid for. `session_orient.py`'s
advisory asserted "identity resolves from that ONE env var" and "a live session cannot change its
own environment", **both falsified by this very commit**, in 46 copies. And
`docs/workstation/agent-identity.md` claimed a backlog routing that had never happened.

Of the author's 31 graders, **three were vacuous by construction** and the race grader took three
attempts — a 24-way subprocess race passed against the unlocked build, and an in-process simulation
bypassed the lock and failed against the fixed one. Only a real contending sibling grades it.

Round 2 swept all 31 classes clean and CONFIRMED 0, every class re-executed rather than re-read.
