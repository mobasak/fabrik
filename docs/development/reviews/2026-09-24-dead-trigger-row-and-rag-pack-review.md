# A governance-sync trigger row that could never fire, a denominator that did not state its prune, and a stale rag-search cite

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 80d6900d942b79ecd95eea868d2dfbcf0c6d5f60; the change as `diff.patch` (the three tracked files' `git diff HEAD` plus the `.pre-commit-config.yaml` hunk, held out of the working tree until its commit) md5 = 684f6eb69177b29edaf76b4e14070079
**Mails answered:** 01M333F0C27K6N9XADKSVH4NAR · 01M382JSY7HS68Z8ZB2HF046GW
**Command:** /fabrik-review · **Changed:** `.pre-commit-config.yaml`, `scripts/enforcement/pack_layout_audit.py`, `tests/test_sync_trigger_coverage.py`, `.windsurf/rules/core/65-rag-search.md`
...
```text
$ python scripts/review_rubric.py --changed .pre-commit-config.yaml scripts/enforcement/pack_layout_audit.py tests/test_sync_trigger_coverage.py .windsurf/rules/core/65-rag-search.md
...
### core/10-python.md  (hit: scripts/enforcement/pack_layout_audit.py, tests/test_sync_trigger_coverage.py)
...
### core/40-documentation.md  (hit: .windsurf/rules/core/65-rag-search.md)
...
### core/45-testing-strategy.md  (hit: tests/test_sync_trigger_coverage.py)
...
| Class | Status |
|---|---|
| Hunt: `.pre-commit-config.yaml` — every changed hunk, its enclosing function, its callers | CLEAN — `.pre-commit-config.yaml` drops only `^scripts/kilo_47_agents_final\.json$` from the governance-sync `files:` regex; the path is gitignored (`.gitignore:189`), untracked and in no manifest list, so `check_sync_trigger_coverage.py` stays green and nothing reads the literal (A: 0 of 3 doc mentions refer to the row) |
| Hunt: `scripts/enforcement/pack_layout_audit.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A2: un-pruning `templates/` in the emission walk would falsely clear any TSX-globbing pack, because every scaffold copies `templates/saas-skeleton/` — the walk change in `scripts/enforcement/pack_layout_audit.py` and `rules_match.py` was REVERTED; only `_emitted_paths_for_type`'s docstring now states the `_EXCLUDE` prune and why) — both audits byte-identical to HEAD over 13 scaffold types |
| Hunt: `tests/test_sync_trigger_coverage.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (B1: outside a git checkout `git check-ignore` exits 128 and the test read it as clean — it now asserts rc in (0, 1) · B2: the parser skipped the grouped row naming 19 scripts — it now expands one-level groups, 29 paths) — `tests/test_sync_trigger_coverage.py` is red with the dead row restored, red-loud with no `.git`, green on the change |
| Hunt: `.windsurf/rules/core/65-rag-search.md` — every changed hunk, its enclosing function, its callers | CLEAN — `.windsurf/rules/core/65-rag-search.md:64` drops the parenthetical citing the deleted `apps/postgres-main/compose.yaml`; `infra/vps1/postgres/compose.yaml` is the only compose defining the `postgres-main` service (19 files checked) and declares `memory: 2G` |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (B1 above: `tests/test_sync_trigger_coverage.py`'s guard failed open outside git) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — no accounting on the surface (`scripts/enforcement/pack_layout_audit.py` docstring only) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — the filter's four directory-prefix rows are out of the literal check by design; every other row shape is parsed (`tests/test_sync_trigger_coverage.py`, 11 of 15 top-level branches, the other 4 prefixes) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN — the one behaviour change (`.pre-commit-config.yaml` row removal) is guarded by the new test, proven red on its restoration |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — 29 filter paths, 13 scaffold types, 19 postgres-main files, each counted by the producing command against `.pre-commit-config.yaml` and the scaffolder |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every candidate executed: both audits A/B over 13 types, the test in a git-backed pin and a no-.git copy, against `scripts/enforcement/pack_layout_audit.py` and the filter |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×1 (fabrik-reviewer: A-sonnet and A-haiku on the filter, `rules_match.py` and `pack_layout_audit.py`; B-sonnet on the tests and the rag-search pack) | found: 6, new: 6, confirmed: 3, fixed: 3, unexecuted: 0 | citation — each seat read the pinned surface (md5-verified start and end) and executed each candidate |
| Pass 2 | native sonnet×2 (the round-1 A and B seats, own slices) | found: 1, new: 1, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — a git-backed pin; A re-ran both audits over 13 types against HEAD's output, B re-ran the test three ways; slice A-haiku standing clean from pass 1 |
...
| Id | Verdict |
|---|---|
| F4 | REFUTED (A3: the unhashable-set trap in `_tree_paths(exclude=…)` is moot — the parameter was reverted with the walk change) |
| F5 | RECORDED — measured (B-r2: three grouped filter names, `kilo_code_review`, `kilo_docs_enforcer`, `update_agents_toc`, now live under `scripts/archived/`, a moved-file dead-row class outside this change's gitignore scope; routed to docs/STRATEGIC_BACKLOG.md) |
...
### Phase 1 — dead trigger row, denominator docstring, rag-search cite: CONVERGED

confirmed 3 → 0; the review reversed one part of the change (the emission walk) and kept its documentation fix.
...
```json
$ python3 scripts/final_gate.py --json --check   # shared main tree, the change staged
{"status": "success", "tier": 2, "passed": 66, "failed": 0, "skipped_checks": ["bandit", "semgrep", "pytest"]}
```

Skips: `bandit`/`semgrep` are diff-sensed (no src/); the hub pytest leg is off by design — the touched suites were run by hand (`tests/test_sync_trigger_coverage.py`, `tests/enforcement/test_pack_layout_audit.py`, `tests/test_rules_match.py`: 119 passed with the filter change applied). The filter hunk is written into the tree only in the commit step: an unstaged `.pre-commit-config.yaml` blocks every session's commits.
