# The loop's second measured run — run-1 lessons (D-350)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = f5eb13f3aeccb9f7493f5f52ceebcb7fa7895637; range tip f5eb13f3aeccb9f7493f5f52ceebcb7fa7895637; `git diff 310762e24..f5eb13f3a -- .claude/workflows/fabrik-review-loop.js commands/_sources/fabrik-review.md docs/reference/review-loop-workflow.md tests/test_review_loop_workflow.py` md5 26a9a51ee1e761d25a488b2845dcefe4 (20063 bytes)
**Command:** /fabrik-review · **Changed:** `.claude/workflows/fabrik-review-loop.js`, `commands/_sources/fabrik-review.md`, `docs/reference/review-loop-workflow.md`, `tests/test_review_loop_workflow.py`
...
```text
$ python scripts/review_rubric.py --changed .claude/workflows/fabrik-review-loop.js commands/_sources/fabrik-review.md docs/reference/review-loop-workflow.md tests/test_review_loop_workflow.py
...
| Class | Status |
|---|---|
| Hunt: `.claude/workflows/fabrik-review-loop.js` — every changed hunk, its enclosing function, its callers | FIXED(2) — S-S1/S-H1 the verify prompt lacked the StructuredOutput finish line (d53d0db2c); S-C1 the pass ≥ 2 head's candidates rule inverted by the defect-keyed statuses (0929d6e6d); the schema's ledger_status description now carries the one definition |
| Hunt: `commands/_sources/fabrik-review.md` — every changed hunk, its enclosing function, its callers | FIXED(1) — S-H3 Phase 4 defined STILL_TRUE only (→ all three, one wording, d53d0db2c); the Phase 1 wait/journal sentence re-read against Phase 1–2, consistent; rendered corpus checked |
| Hunt: `docs/reference/review-loop-workflow.md` — every changed hunk, its enclosing function, its callers | FIXED(2) — S-H2 the slices row omitted NEW; S-S3 the doc never said how the lead waits or reads a truncated result (both d53d0db2c) |
| Hunt: `tests/test_review_loop_workflow.py` — every changed hunk, its enclosing function, its callers | FIXED(2) — S-G no grader pinned the verify finish line (red on f5eb13f3a), S-C3 none pinned which rows a later pass counts (red on d53d0db2c); `_harness` keeps the two older graders' semantics (8 of 8 green); `{**_ARGS}` rebinds `slices`, no mutation leak (checked by the Sonnet seat) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED(1) — a verify seat answering in prose became a null → `unverified`; the finish line now reaches the verify prompt (S-S1) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — pool OFF; 2 seats stamped and dispatched per pass, 3 Workflow calls, 18 agents, 559 k seat tokens by notification |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — no numeric boundary on the surface; the status vocabulary is the one sentinel set and it is FIXED under mirror-definition |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED(2) — S-G and S-C3, each grader proven red on the pre-fix commit |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — every seat report carried its files-read denominator (5 of 5, 6 of 6) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every round-1 candidate re-executed by the orchestrator on the pins before the fix, and by its verify seat; every closing claim re-executed by both seats |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1 (ONE `Workflow` call — 8 agents incl. 6 verify seats) | found: 6, new: 6, confirmed: 6, fixed: 6, unexecuted: 0 | method: citation — every candidate executed by a verify seat and re-run by the orchestrator on the pins at f5eb13f3a (`probes` in the run transcript); round-zero probe — the journal claim (one `result` row per completed agent) and the Stop-hook claim (`final_gate_stop.py:859`, a `running` record blocks) executed; seats: S-sonnet 3/3 · S-haiku 3/3; one class (mirror-definition), fixed in ONE batch at d53d0db2c; own-fix: 0 |
| Pass 2 | native sonnet×1 + haiku×1 (the round-1 seats over their ledger, ONE `Workflow` call — 8 agents) | found: 6, new: 1, confirmed: 1, fixed: 1, unexecuted: 0 | method: re-derivation — 6 of 6 ledger defects re-executed as NOW_FALSE; ONE contradiction this run's own hunk created in the same sentence (the candidates rule inverted by the defect-keyed statuses, S-C1/S-C2 — the Sonnet seat obeyed it and raised the six fixed claims, which its verify seats confirmed 4 and refuted 2 as fixed), fixed at 0929d6e6d with its grader red on d53d0db2c; seats: S-sonnet 1/6 · S-haiku 0/0; own-fix: 1 |
| Pass 3 | native sonnet×1 + haiku×1 (the round-1 seats over their ledger, ONE `Workflow` call — 2 agents) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — S-C1, S-C2, S-C3 re-executed as NOW_FALSE by both seats on the pins at 0929d6e6d (8 of 8 graders green); the untouched mirror sites (doc :21, source :159/:448) checked for a stranded candidates sentence, none; seats: S-sonnet 0/0 · S-haiku 0/0; own-fix: 0 |
...
| R-1 | RECORDED — measured (lead turns: see the run close's `tok_msgs`, reported in the FEEDBACK line and D-350's successor row — the second point of the D-347 KILL) |
| R-2 | RECORDED — measured (a closing-pass Sonnet seat reported that its dispatch carried the lead session's latest operator message as a relayed 'user request' block — a Workflow-agent prompt leak outside this repo's code; it ignored it and said so under MACHINERY; routed to the doc's next revision as an observation) |

## Per-phase verdicts

### Phase 0 — Establish scope: CLEAN — record `--budget 20`, ORIENT executed, pins at f5eb13f3a, rubric, hygiene 0 hits over 4 files, headroom 2, stamped
### Phase 1 — Finders: CLEAN — one `Workflow` call per pass, both seats returned schema every pass (run 1's prose-returning seat did not recur)
### Phase 2 — Execute: FIXED(7) — 7 confirmed, each re-run on the pins before its fix
### Phase 3 — Prove and fix: FIXED — d53d0db2c, 0929d6e6d; graders red on the pre-fix commits
### Phase 4 — Converge: CLEAN — pass 3, the round-1 seats over their ledger, confirmed 0 · fixed 0
