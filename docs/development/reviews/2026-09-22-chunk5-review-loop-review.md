# Chunk 5 — the D-335 review loop as a workflow script (D-350)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = ccbab9900f7e22ac86c3a80f43c83b24816c2ea3; range tip ccbab9900f7e22ac86c3a80f43c83b24816c2ea3; `git diff 9b4b1db70..ccbab9900 -- .claude/workflows/fabrik-review-loop.js CHANGELOG.md INDEX.md commands/_agents/fabrik-reviewer.md commands/_sources/fabrik-repo-review.md commands/_sources/fabrik-review.md docs/DECISIONS.md docs/reference/review-loop-workflow.md libs/deep_research/packs/loop-research.yaml tests/test_review_loop_workflow.py` md5 7511db3da2455d8d07510bc81d505f54 (56854 bytes)
**Command:** /fabrik-review · **Changed:** `.claude/workflows/fabrik-review-loop.js`, `CHANGELOG.md`, `INDEX.md`, `commands/_agents/fabrik-reviewer.md`, `commands/_sources/fabrik-repo-review.md`, `commands/_sources/fabrik-review.md`, `docs/DECISIONS.md`, `docs/reference/review-loop-workflow.md`, `libs/deep_research/packs/loop-research.yaml`, `tests/test_review_loop_workflow.py`
...
### core/10-python.md  (hit: tests/test_review_loop_workflow.py)
...
### core/40-documentation.md  (hit: CHANGELOG.md, INDEX.md, commands/_agents/fabrik-reviewer.md)
...
| Class | Status |
|---|---|
| Hunt: `.claude/workflows/fabrik-review-loop.js` — every changed hunk, its enclosing function, its callers | FIXED(6) — A-S1 same-seat neighbour dropped by the dedupe key, A-S5 a fixed line/6 bucket split one defect (both → cross-seat near-line merge, `sameDefect`), A-S3 the verify seat's echoed id trusted (→ candidate-owned id), A-S2 `unverified` outside the seat's enum (→ script-emitted, said so), A-S7/A-H3 the pin join unstated in both prompts (→ `<pins_dir>/<path>`), B-S2/B-S4 per-seat credit and the id line — every one executed by a verify seat and re-run by the orchestrator (the node-executing grader is red on ccbab9900, green on 5a7803da2); pass 2 re-executed all as NOW_FALSE |
| Hunt: `CHANGELOG.md` — every changed hunk, its enclosing function, its callers | FIXED(1) — B-S3/B-H1 the KILL sentence read as past tense (→ 'To be measured …', 214bbdb6c); the entry sits atop `[Unreleased]` (line 7 of 5) |
| Hunt: `INDEX.md` — every changed hunk, its enclosing function, its callers | FIXED(1) — B-S1/B-H2 the doc's row sat under Root Files (→ the docs/ Files table after docs/CAPABILITIES.md, 55fca7db8); the .claude/workflows, libs and tests rows stay under Root Files beside the `.windsurf/workflows/registrar-audit.md` precedent — INDEX has no table for those paths (RECORDED below) |
| Hunt: `commands/_agents/fabrik-reviewer.md` — every changed hunk, its enclosing function, its callers | FIXED(2) — B-S4 the VERIFY sentence never asked for `id` or a `destination`, B-S5 the pass ≥ 2 `ledger_status` shape absent (both → 819e2f01a); the section's field names re-read against FINDINGS/VERDICT, identical |
| Hunt: `commands/_sources/fabrik-repo-review.md` — every changed hunk, its enclosing function, its callers | CLEAN — the per-batch launch sentence re-read against the batch-per-round mechanics (`round --seats` after each batch); both B reads found no contradiction; corpus `--check` rc 0 |
| Hunt: `commands/_sources/fabrik-review.md` — every changed hunk, its enclosing function, its callers | FIXED(1) — B-S2 the Pass-1 `seats:` cell's source unstated (→ 'read from the workflow ledger's seats[].confirmed/raised — a candidate both finders raised credits both', 5a7803da2); the three edited sentences (Phase 1 launch, Phase 2 re-run, Phase 4 relaunch) read against every standing sentence of their phases by both B reads — no contradiction; rendered corpus checked |
| Hunt: `docs/DECISIONS.md` — every changed hunk, its enclosing function, its callers | CLEAN — every count and path in D-350 executed by the round-zero probe and the B-sonnet re-dispatch (+1,737 B / +797 B by `wc -c` on both blobs, 233 lines, 5 graders, 620 k tokens, 5.9 min) |
| Hunt: `docs/reference/review-loop-workflow.md` — every changed hunk, its enclosing function, its callers | FIXED(1) — A-S4 a hand-written `## Related scripts` block, the heading the doc↔script coupling rule renders (→ `## Related surfaces`); the union and verdict sentences re-cut with the fixes; `render_doc_script_links.py --check` current |
| Hunt: `libs/deep_research/packs/loop-research.yaml` — every changed hunk, its enclosing function, its callers | CLEAN — `load_pack()` on the pin loads (3 legs, one `is_free`, `demote_rule` fields present); the header's 300 s / 4-way claim executed against `rivals_run.py:367,:504` (B-H3 refuted the finder's doubt) |
| Hunt: `tests/test_review_loop_workflow.py` — every changed hunk, its enclosing function, its callers | FIXED(2) — A-S8 the options window was a first-`})` search (→ `_call_end`, a balanced scan), A-S6 no grader executed the script (→ two node-harness graders driving the pipeline with the Workflow globals stubbed; union grader RED on the pinned pre-fix script, GREEN after; the null-finder grader pins the standing behaviour); 7 of 7 green; four earlier mutants still each kill one grader |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN — a null finder marks the slice UNVERIFIED (every unread file a logged gap, `SEAT FAILED` in the log — grader `test_a_null_finder_…`); a null verify seat is `unverified`, never dropped; the first launch's failed B-haiku seat was re-dispatched, never counted swept |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — pool OFF (D-181); seats stamped 4 before the launch, 6 native seats ran in round 1 (the re-dispatch added two, recorded as typed), 4 in round 2; 18 + 8 + 4 workflow agents, 2.61 M seat tokens over the run |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED(1) — A-S5 the fixed line/6 bucket boundary (lines 5 and 7 split) → a ±5-line window; A-S1's same-seat merge was the sentinel of the same key |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED(1) — A-S6: the union merge, the verdict-id override, the per-seat credit and the null-seat path now have executing graders; red-on-revert proven against ccbab9900's script by the orchestrator and by the A-sonnet seat in pass 2 |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — every seat report carried its files-read denominator (3 of 3, 7 of 7); the ledger's per-seat `raised`/`confirmed` are the counts the Pass rows cite |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — every candidate went through a verify seat that ran a command (14 + 6 verdicts with command and output), the orchestrator re-ran the confirmed ones' proofs (graders, greps on the pins), and pass 2 re-executed every ledger claim (22 commands, 0 re-reads) |
...
|---|---|---|---|
| Pass 1 | native sonnet×3 + haiku×3 (two finders per slice as ONE `Workflow` call — 18 agents incl. 14 verify seats; slice B's haiku seat returned no schema and the slice was re-dispatched as a second call, 8 agents) | found: 20, new: 20, confirmed: 13, fixed: 13, unexecuted: 0 | method: citation — every candidate executed by a Sonnet verify seat (command + output in the ledger) and the confirmed ones re-run by the orchestrator (node-harness grader red on ccbab9900; greps on the pins); round-zero probe — every count and path in D-350, the CHANGELOG entry, the pack header and the doc re-derived (`probes/round-zero.txt`); seats: A-sonnet 8/8 · A-haiku 1/3 · B-sonnet 3/3 + 2/2 (re-dispatch) · B-haiku 2/4; 20 raised = 17 distinct (A-S7≡A-H3, B-H1≡B-S3, B-H2≡B-S1), 13 confirmed, 4 refuted by execution (A-H1, A-H2, B-H3, B-H4); estimate_unseen 24 / 8 at shared 0 — advice only; own-fix: 0 |
| Pass 2 | native sonnet×2 + haiku×2 (the round-1 seats over their OWN slices' ledgers, ONE `Workflow` call, pass 2 — 4 agents) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — every ledger claim re-executed on the pins at 819e2f01a: slice A 11 of 11 NOW_FALSE (the A-sonnet seat swapped in the pre-fix script and reproduced the red), slice B 7 of 7 verified with the fix's own command and output; no NEW candidate inside the fix hunks or one hop out; pins byte-identical to HEAD; seats: A-sonnet 0/0 · A-haiku 0/0 · B-sonnet 0/0 · B-haiku 0/0; own-fix: 0 |
...
| A-H1 | REFUTED — executed: no `})` precedes either options object (pass 1 verify seat; the balanced scan of A-S8 makes the point moot) |
| A-H2 | REFUTED — executed: the `confidence` enum is schema-enforced at the StructuredOutput call (`FINDINGS` L54 → `schema:` L173) |
| B-H3 | REFUTED — executed: `RIVALS_LLM_TIMEOUT_S` default 300 at `scripts/rivals_run.py:367`, `RIVALS_LLM_CONCURRENCY` default 4 at `:504` |
| B-H4 | REFUTED — a non-defect raised as 'correctly specified'; nothing to fix |
| R-1 | RECORDED — measured (INDEX has no table for `.claude/workflows/`, `libs/` or `tests/` rows; the three stay under Root Files beside the `.windsurf/workflows/registrar-audit.md` precedent; makes no code or doc claim) |
| R-2 | RECORDED — measured (`parallel([])` resolving to `[]` for a zero-candidate slice is the Workflow tool's documented contract, not this repo's code; the A-sonnet seat could not execute it; the null-finder grader covers the zero-candidate path through the stub) |
| R-3 | RECORDED — measured (the pass-2 brief's `STILL_TRUE | NOW_FALSE` vocabulary keys on the DEFECT claim, while the orchestrator's ledger rows carried FIX statements: slice A reported NOW_FALSE, slice B STILL_TRUE, both meaning the fix holds; the next run's ledger rows state the defect, and the script's own pass ≥ 2 head says so — a text change deferred to the next run because the round that edits is never the last) |
| R-4 | RECORDED — measured (the lead-turn count of this run against the 15-turn target: 23 lead messages from `start` to `done`, of which 5 were in-turn waits for a workflow to land — the Stop hook forbids ending the turn mid-command — 1 a failed parse of the task output, 1 the re-dispatch of a failed seat; the first measured run of the D-347 KILL, read with its causes in D-350's successor row) |

## Per-phase verdicts

### Phase 0 — Establish scope: CLEAN — record opened with `--budget 30`, ORIENT executed (mcp 14/15 live, 4 packs, infra and manifesto sections named), rubric injected by path, hygiene 0 hits over 5 files, pins at ccbab9900, `dispatch_headroom.py --slices sonnet=2,haiku=2` → SEATS 4, stamped
### Phase 1 — Independent finders: FIXED — one `Workflow` call per pass; the failed B-haiku seat re-dispatched (a seat that returns nothing is a failed seat, never a clean round)
### Phase 2 — Execute every candidate: FIXED(13) — 13 confirmed, 4 refuted, every verdict carrying its command and output; the orchestrator re-ran the confirmed proofs before each fix
### Phase 3 — Prove and fix: FIXED — commits 5a7803da2, 55fca7db8, 214bbdb6c, 819e2f01a; graders red-first (the union grader against the pinned pre-fix script) and mutation-proven
### Phase 4 — Converge: CLEAN — round 2 (the round-1 seats over their own ledgers) confirmed 0, fixed 0, unexecuted 0; every class swept; the terminal condition met inside the budget
...
```json
{"status": "success", "tier": 2, "passed": 62, "failed": 0, "skipped_checks": ["bandit", "bandit scripts/", "semgrep", "pytest"]}
```
