# T07a of plan 2026-09-25-plan-1-work-store-single-tracker
...
| Class | Status |
|---|---|
| Hunt: `CLAUDE.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (K-S1/K-S2/K-H1/K-O1/K-O2/K-O3/K-H5 the sentence states the first named item that is open, ready and held by no other session, and the mailbox's own repo; K-H3 `NEXT: none — terminal` back on one line), r2 (K2-O1 a `next` item is never claimed) — CLAUDE.md Work items paragraph |
| Hunt: `templates/governance/CLAUDE.md` — every changed hunk, its enclosing function, its callers | FIXED r1, r2 (the same sentence in both § FINAL OUTPUT copies, equal to the hub after the two sanctioned substitutions) — templates/governance/CLAUDE.md |
| Hunt: `tests/test_work_contract_rule.py` — every changed hunk, its enclosing function, its callers | FIXED r1, r2 (_CANONICAL follows the sentence; reverting it turns both equality tests red) — tests/test_work_contract_rule.py |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (CLAUDE.md and templates/governance/CLAUDE.md are prose; the contract test compares all three copies, each copy's drift proven red by a seat's mutation) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (templates/governance/CLAUDE.md and CLAUDE.md growth measured by check_corpus_weight.py: one sentence per copy, recorded in the plan's merge D-row) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (K-H3 the code span no longer splits across a line) — CLAUDE.md, templates/governance/CLAUDE.md |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (tests/test_work_contract_rule.py: reverting _CANONICAL turns 2 of 4 red, drifting one template copy turns 1 of 4 red) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the receipt's counts carry their totals: 13 + 1 + 0 candidates over three passes; 43 passed over tests/test_work_contract_rule.py and tests/test_governance_template_split.py) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (each sentence claim executed against scripts/work.py _claim_named and scripts/mail.py _mail_store by the seats; the readers of `NEXT: none — terminal` grepped across tests/, scripts/ and .claude/hooks/) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×1 + haiku×1, refuter sonnet×1 | found: 13, new: 13, confirmed: 8, fixed: 8, unexecuted: 0 | method: citation — one slice (the three copies and the pin), workflow wf_ef0b467f-4c9; every claim of the sentence executed against the merged code; 1 refuted, 4 RECORDED |
| Pass 2 | native opus×1 (the round-1 slice owner), refuter sonnet×1 | found: 1, new: 1, confirmed: 1, fixed: 1, unexecuted: 0 | method: re-derivation — 7 of 8 ledger rows NOW_FALSE; K-H1's kind:next exclusion STILL_TRUE as K2-O1 (a residue, not own-fix) |
| Pass 3 | native opus×1 (the round-1 slice owner) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: K2-O1 NOW_FALSE against _claim_named, the contract test green on the copy; standing clean since pass 1: fail-open, cost-quota, denominator, proxy-evidence |
...
| P1-K-H2 | REFUTED (mail.py's CLI claim calls _note_mail_claim after the rename, which opens the item; the refuter's run created it) |
| P1-K-H4 | RECORDED — measured (the paragraph opens with "A repo with a `.fabrik/work/` store", which scopes every later sentence; a repo without a store gets no behaviour) |
| P1-K-O4 | RECORDED — measured (the same scope as P1-K-H4 in the ~46 synced repos, most without a store today) |
| P1-K-S3 | RECORDED — measured (no reader keys on the single-line phrase in CLAUDE.md; every reader of `NEXT: none — terminal` reads transcripts) |
| P1-K-O5 | RECORDED — measured (the same as P1-K-S3; the phrase is on one line again after round 1) |

## Per-phase verdicts

### Phase 1 — T07a the contract sentence in all three copies: CLEAN
