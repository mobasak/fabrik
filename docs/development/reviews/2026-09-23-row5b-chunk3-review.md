# Row 5b chunk 3 — stages per change, seat tokens from transcripts, lead-inline rejected

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = b169cbf53e138373797fedd49259620c42963a8c; `git diff HEAD -- scripts/command_feedback_report.py tests/test_feedback_stages.py docs/reference/command-run-protocol.md scripts/review_loop_ledger.py tests/test_review_loop_ledger.py commands/_sources/fabrik-review.md docs/reference/review-loop-workflow.md docs/reference/command-loop-performance.md` md5 b1ed7d64d5865b68ace4ef674806352b (44942 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/command_feedback_report.py`, `tests/test_feedback_stages.py`, `docs/reference/command-run-protocol.md`, `scripts/review_loop_ledger.py`, `tests/test_review_loop_ledger.py`, `commands/_sources/fabrik-review.md`, `docs/reference/review-loop-workflow.md`, `docs/reference/command-loop-performance.md`
...
### core/40-documentation.md  (hit: commands/_sources/fabrik-review.md, docs/reference/command-loop-performance.md, docs/reference/command-run-protocol.md)
...
### core/45-testing-strategy.md  (hit: tests/test_feedback_stages.py, tests/test_review_loop_ledger.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/command_feedback_report.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S1 negative or corrupt figures bent a change negative — `_pos`; B-S1 hours re-rounded per row drifted — seconds summed, rounded once; A-S2 the text omitted the repo — a repo column) — B-S4 and B-S5 refuted below; the orchestrator's own pre-pin probe fixed the `-review` receipt key before pass 1 |
| Hunt: `tests/test_feedback_stages.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S4/B-S3 the docstring said 101 of 238 against the report's 239) — 4 graders in the ledger's REAL shape (per-round `confirmed` lists); the list-sum and the repo column each proven red on a mutant |
| Hunt: `docs/reference/command-run-protocol.md` — every changed hunk, its enclosing function, its callers | CLEAN (docs/reference/command-run-protocol.md's `--stages` sentence matches the mode as built; the three `{{…}}` hygiene hits at lines 171-172 are pre-existing text describing the template syntax — RECORDED below) |
| Hunt: `scripts/review_loop_ledger.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S3 last-write-wins let a trailing all-zero line erase an id's usage — the per-field maximum, command_run.py's rule; B-S2 an id-less message was dropped — counted on its own) |
| Hunt: `tests/test_review_loop_ledger.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (a grader for the trailing zero line and the id-less message, watched red) — 8 graders |
| Hunt: `commands/_sources/fabrik-review.md` — every changed hunk, its enclosing function, its callers | CLEAN (commands/_sources/fabrik-review.md's added words — the helper prints minutes AND tokens — match the helper's output; slice B seats) |
| Hunt: `docs/reference/review-loop-workflow.md` — every changed hunk, its enclosing function, its callers | CLEAN (docs/reference/review-loop-workflow.md's token sentence matches `_tokens()` as fixed; slice B seats) |
| Hunt: `docs/reference/command-loop-performance.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (ORCH-1 finding 27's refuter figures were wider than the pinned run supports — re-derived: 555,503 over 18 calls and 305,682 over 10, about 31 k per call, 0.31–0.56 M each, 13–25×; both slice-B seats re-derived the same counts in pass 2) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (a dropped id-less message and an erased usage line read as a smaller seat in scripts/review_loop_ledger.py — both now counted) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (pool OFF; 6 and 4 seats stamped for .claude/workflows/fabrik-review-loop.js passes 1-2; every seat timed and its tokens totalled by scripts/review_loop_ledger.py, none over its box) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1 (the plan key's `-review` suffix and the D-row width in scripts/command_feedback_report.py — the first fixed, the second refuted) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (every behaviour added to scripts/command_feedback_report.py and scripts/review_loop_ledger.py has a grader; each watched red first) |
| Recurrence: denominator on every count — bounded searches state their bound | FIXED r1 (the coverage line of scripts/command_feedback_report.py --stages leads every report; the docstring's 238 corrected to 239) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (--stages run on the real feedback ledger and scripts/review_loop_ledger.py on three real runs; every candidate reproduced before its fix) |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (two finders per slice, ONE `Workflow` call on the committed loop — 6 agents incl. 2 refuters) | found: 10, new: 10, confirmed: 7, fixed: 7, unexecuted: 0 | method: citation — 9 seat candidates plus the orchestrator's own re-derivation of finding 27's figures (ORCH-1); the orchestrator's pre-pin probe had already fixed the `-review` receipt key; B-S4 and B-S5 refuted; read through the helper (97,124 out, 7.6 M cache read over 6 seats, none over box); stop: confirmed 7; fix: +132 -16 lines for 7 confirmed, graders included (git diff --numstat, pass-1 pins → pass-2 pins); seats: A-sonnet 4/4 · A-haiku 0/0 · B-sonnet 4/4 · B-haiku 1/1 |
| Pass 2 | native sonnet×2 + haiku×2 (the round-1 seats over their own ledgers, ONE `Workflow` call — 4 agents) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — 7 of 7 ledger claims re-executed NOW_FALSE, the finding-27 figures re-derived by both slice-B seats to the same counts; 166 of 166 slice graders green; stop: closable (every claim executed, zero confirmed); fix: none |
...
| B-S4 | REFUTED (scripts/decisions.py lines 509, 521 and 552 mint every id as `D-{nid:03d}`, and 358 of 358 hub rows carry three or more digits, so `D-\d{3,}` matches every real decision id while keeping `D8`-style spec-clause references out of the key) |
| B-S5 | REFUTED (scripts/command_run.py line 4814 writes `confirmed` as a list of `_count` results — integers or None — so a scalar numeric string never reaches the ledger; 285 of 285 real rows carry a list) |
| H-1 | RECORDED — hygiene false positive (docs/reference/command-run-protocol.md lines 171-172 describe the `{{include:run-record}}`, `{{COMMAND}}` and `{{PHASES}}` template syntax inside backticks; pre-existing since 7224e8eff) |

## Per-phase verdicts

### Phase 0 — Establish scope: CLEAN — record opened, the real feedback ledger and a real run pinned, hygiene 3 hits over 8 files all RECORDED false positives, the orchestrator's own mirror probe fixed the receipt key before any seat
### Phase 1 — Finders: CLEAN — one `Workflow` call per pass on the committed loop, every seat returned schema, none wrote under the repo, none over its box
### Phase 2 — Execute: FIXED(7) — 7 confirmed (6 from seats, 1 the orchestrator's), each reproduced before its fix; 2 refuted with the writer's code
### Phase 3 — Prove and fix: FIXED — 4 graders added or tightened, each watched red first
### Phase 4 — Converge: CLEAN — pass 2 confirmed 0 · fixed 0, every slice verified
...
The ruff rows read SCOPE-NARROWED (a sibling's unstaged tracked .py); `ruff check` over the four touched .py files passes by hand.
