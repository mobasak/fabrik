# A NEXT naming a work item says what the item is — review of the contract wording change

**Status:** CLEAN
**Surface:** base `76ad8774c`; `git diff HEAD -- CLAUDE.md templates/governance/CLAUDE.md tests/test_work_contract_rule.py` (md5 `c25638adc388` with the ledgers)
**Command:** /fabrik-review · **Changed:** `CLAUDE.md`, `templates/governance/CLAUDE.md`, `tests/test_work_contract_rule.py`
**Decision:** D-426 (operator: "yes")
...
```text
$ python scripts/review_rubric.py --changed CLAUDE.md templates/governance/CLAUDE.md tests/test_work_contract_rule.py
...
### core/45-testing-strategy.md  (hit: tests/test_work_contract_rule.py)
...
| Class | Status |
|---|---|
| Hunt: `CLAUDE.md` — the hub's Work items paragraph | CLEAN (CLAUDE.md asks for `NEXT: <item id> — <what it is>` with the example `NEXT: W-1a2b3c4d — fix the flaky login test`; the paragraph still follows the DONE:/NEXT: discipline paragraph, tests/test_work_contract_rule.py 4 passed) |
| Hunt: `templates/governance/CLAUDE.md` — both Work items copies | CLEAN (templates/governance/CLAUDE.md carries the same text in both § FINAL OUTPUT copies, equal to the hub copy under the two sanctioned substitutions; test_each_template_copy_equals_canonical_after_the_two_sanctioned_substitutions passes) |
| Hunt: `tests/test_work_contract_rule.py` — the pinned canonical text | CLEAN (tests/test_work_contract_rule.py's `_CANONICAL` was changed first and watched red, 2 failed, before the three copies changed; 43 passed with the template-split suite after) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (scripts/work.py unchanged; `classify_next` and `_next_refs` give `names-item` and the id for `W-1a2b3c4d — fix the flaky login test`, `W-1a2b3c4d: fix it` and `W-67f668f1 — triage the 8 stale CONVERGED plans (intel)`, executed) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (CLAUDE.md grows 155 bytes and templates/governance/CLAUDE.md 310 bytes (two copies), measured with wc -c against HEAD; no pool units, 5 native seats across the two passes) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (scripts/work.py `_next_refs` strips the em dash as a NEXT opener and its id pattern needs a non-alphanumeric boundary, so `W-1a2b3c4d —` yields exactly one ref; executed by both round-1 seats) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (tests/test_work_contract_rule.py pins all three copies byte for byte after substitution; red when only the pinned text changed) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the receipt's counts carry their totals: 2 + 0 candidates over two passes; the example id matches 0 of 371 item files) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (scripts/work.py's classifier run on the new line forms, and the pinned grader run on the pinned tree) |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1, refuter sonnet×1 | found: 2, new: 2, confirmed: 0, fixed: 0, unexecuted: 0 | method: citation — one slice (A: the three contract copies and the pinned grader), workflow wf_4f5de194-273; seats: A-sonnet 0/2 · A-haiku 0/0; both candidates executed and RECORDED; stop: closable |
| Pass 2 | native sonnet×1 + haiku×1 (the round-1 slice owners) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass, workflow wf_766a9a60-73a: A-L1 and A-L2 NOW_FALSE by execution on the re-pinned tree (md5 c25638adc388), the pinned grader 4 passed, one hop around the diff raised nothing; stop: closable |

## Residual

| P1-A-S1 | RECORDED — by design (D-417) — a description carrying "operator decision" makes the whole NEXT line a rule-1 hold that claims nothing; D-426 now says so |
| P1-A-S2 | RECORDED — measured (the example id `W-1a2b3c4d` matches 0 of 371 item files, so a verbatim copy claims nothing and changes nothing, as a literal `<item id>` would) |

## Per-phase verdicts

### Phase 1 — the NEXT wording rule: CLEAN

The operator could not read `NEXT: W-67f668f1`; the contract asked only for the id. All three copies now ask for the id plus what the item is, and the harvest's claim mechanism is unchanged.

## Gate

`final_gate.py --check --json`, run in the throwaway worktree holding this change on `76ad8774c` (check_convergence reads the fenced block):
