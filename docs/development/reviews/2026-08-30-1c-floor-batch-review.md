# Review — the 1c approach-floor batch (spec gates + check + archive)

**Status:** CONVERGED
**Surface:** HEAD (3 commits: 21f37809 gate texts + check · 1c27ba2c archive mv · the review-fix) — batch diff md5 recomputed at close
**Scope:** commands/_sources/fabrik-spec.md + fabrik-spec-review.md (1c floor texts) · scripts/enforcement/check_spec_convergence.py (APPROACH-FLOOR, date-gated) · tests/enforcement/test_spec_convergence.py (3 red-first) · the rejected spec's archive mv. NO-POOL — solo native; escalated from scoped per the enforcement-surface rule.
**Anchor:** no prior report for this scope → full WIDE pass 1.

Rubric: ran `python scripts/review_rubric.py --changed <the 4 surfaces>` this run — verbatim output:
...
### core/40-documentation.md  (hit: commands/_sources/fabrik-spec-review.md, commands/_sources/fabrik-spec.md)
...
|---|---|---|---|
| 1 | fail-open/fail-closed | CLEAN | the floor only ADDS findings on post-cutoff CONVERGED specs; DRAFT stays ungraded by design (being incomplete is what DRAFT means); advisory path unchanged |
| 2 | cost/quota accounting | CLEAN | no metered surface |
| 3 | boundary/sentinel/prefix | CLEAN | date gate probed both sides (pre-cutoff fixture silent, post-cutoff fires); mv verified both-paths-in-pathspec (numstat R100); archive dirs confirmed in check_doc_index EXCLUDE_PREFIXES |
| 4 | behavior-without-a-test | CLEAN | 3 red-first tests (self-exemption fires · 2-source satisfies · pre-cutoff immune); 23/23 |
| 5 | enforcement-overclaim | FIXED(1) | pass 1 caught the command claiming the CHECK enforces the full floor (tools + search leg) it cannot see — corrected to name the countable subset + review-owned remainder (the oasdiff class, caught before distribution) |
| 6 | test quality | CLEAN | fixtures vary only the probed dimension (intake table + residuals held constant); mirror guards present |
| 7 | 12-Factor | CLEAN | N/A surface |

## Per-finding dispositions (1 raised → 1 FIXED)

1. **enforcement overclaim in fabrik-spec 1c** — CONFIRMED → FIXED (review-fix commit; re-rendered; corpus check green at commit).

## Pass Ledger

- Pass 1 (WIDE) — finders: native solo (NO-POOL) — method: citation+probe — found: 1, new: 1, fixed: 1
- Pass 2 (CLOSING WIDE) — finders: native solo — method: re-derivation — re-ran the suite fresh (23/23 — count read from the run), re-counted the CONVERGED denominator from the tree (20, matching the measurement), re-ran the live check (0 APPROACH-FLOOR retro-fires), re-read the whole post-fix diff — found: 0, new: 0, fixed: 0

Terminal: pass 2 `new: 0`, all adjudicated; the fixing pass was not the last look.
