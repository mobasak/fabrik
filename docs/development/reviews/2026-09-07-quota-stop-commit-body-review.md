# Review — 2026-09-07-quota-stop-commit-body

**Status:** CONVERGED 2026-09-07 (round 2 no-op: found 0 / new 0 / fixed 0; gate success at 082d7500)
**Surface:** `git rev-parse HEAD` = 3e0f5cfcb35978725c2685e4d035dd9a5f3249b8; `git diff HEAD -- .claude/hooks/quota_stop.py tests/test_quota_stop_hook.py` md5 684ee837419e96dc209b89960fc08a81 (3949 bytes)
**Command:** /fabrik-review · **Changed:** `.claude/hooks/quota_stop.py`, `tests/test_quota_stop_hook.py`
...
```text
$ python scripts/review_rubric.py --changed .claude/hooks/quota_stop.py tests/test_quota_stop_hook.py
...
### core/10-python.md  (hit: .claude/hooks/quota_stop.py, tests/test_quota_stop_hook.py)
...
| Class | Status |
|---|---|
| Hunt: `.claude/hooks/quota_stop.py` — every changed hunk, its enclosing function, its callers | CLEAN (.claude/hooks/quota_stop.py:392 `_mask_quoted(command, commit_body=False)`, the quoted-newline branch ~:448, decide's call ~:530; 17 executed shapes: the trailered double- and single-quoted bodies ALLOWED; `$(`/backtick/line-continuation in the body, a newline outside quotes, `-a`/`--amend` with a body, env-prefixed and double-spaced `git commit` all still DENIED (fail-closed); a quoted newline in a pathspec or -F path is data the shell never acts on) |
| Hunt: `tests/test_quota_stop_hook.py` — every changed hunk, its enclosing function, its callers | CLEAN (tests/test_quota_stop_hook.py — `test_a_trailered_commit_passes_the_hold_and_a_multiline_command_does_not` seen red (1 failed) then 36 passed; pins two allowed shapes and eleven refused ones incl. `$(` in the body, the escaped newline, other git verbs and the hyphenated verb cousins; the :511 grader pins the new message literal; ruff clean) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (every miss of the leading-word test — `GIT_X=1 git commit`, `git  commit`, `/usr/bin/git commit` — stays on the REFUSE side; the exception is opened only for a line that starts with `git commit ` and only for a quoted newline, which the shell treats as data; unbalanced quotes still return the raw line to the vetoes) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (pool OFF by operator directive D-181 — the breadth layer is unavailable and recorded as such, 0 units dispatched; native finder ×1 + own executed probes ×17) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (`startswith("git commit ")` with the trailing space excludes `git commit-tree`/`commit-graph`; `lstrip()` admits leading whitespace only; the mask keeps `$(`, a backtick and a span-opening `$` visible on the masked line exactly as before) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (the grader kills the mask — reverting the `commit_body` branch reds the ALLOW assertion (seen red first) — and pins five refusals; the pre-existing masker graders (~:500-520 in the suite) still pass: 36/36) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (17 of 17 probe shapes listed with verdicts; 36 of 36 tests; 4/4 + 22/0 numstat; 46 project copies receive the identical bytes on the post-commit sync) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every verdict above is `decide()` executed under `stamp_exists=True`, not read from the code; the infra report's DENY was reproduced before the fix and the grader watched red) |
...
|---|---|---|---|
| Pass 1 | pool UNAVAILABLE (OpenRouter pool OFF by operator directive D-181 — 0 units; recorded, not scored) + native general-purpose×1 (48 probe shapes, 4 mutants, a scratch git repo) + own executed probes ×17 | found: 5, fixed: 4, noted: 1 | F1 grader gap (escaped-newline branch unpinned on a commit line — mutant M1 survived) → two refused shapes added; F2 grader gap (gate not pinned against other git verbs — M2 survived) → `git push`/`git add` quoted-newline shapes added, docstring corrected; F3 the hold's message named a shape the masker refuses (heredoc / backtick bodies) → `_reason` now prescribes a single-quoted message and names what stays refused (the :511 grader pins the literal); F5 pre-existing: `commit\b` admitted `commit-tree`/`commit-graph` (an object write under the hold, proven in a scratch repo) → `(?!-)` on the verb group, four cousins now deny, the read verbs still allow; F4 (three parsers decide "commit line"; two-space `git  commit` stays refused, fail-closed) accepted as a note |
| Pass 2 | pool UNAVAILABLE (D-181) + native general-purpose×1 closing (81 probe shapes, 3 mutants in scratch copies all killed, HEAD-vs-tree non-str comparison) | found: 0, fixed: 0 | method: re-derivation — every round-1 fix re-executed (F1/F2/F5 by mutant kill, F3 by the :511 literal + heredoc/backtick denials); 27 legitimate git shapes still allow, 10 hyphenated cousins deny; `_reason` 904 chars, JSON-safe; no repo-specific bytes; NEW DEFECTS: 0 |
...
### Phase 1 — round 1 (native finder + own probes; pool off per D-181): FIXED (4 of 5 rows fixed with grader lines, 1 accepted note)

### Phase 2 — closing round (native; pool off per D-181): CLEAN (found: 0, new: 0, fixed: 0 — the no-op round)
...
```json
{
  ...
  "stamped_at": "082d7500",
  "note": "final_gate.py --check --json re-run on the tree after round 1's fixes (HEAD 082d7500 + this change); the pytest leg is the hub's deliberate opt-out - tests/test_quota_stop_hook.py by hand: 36 passed"
}
```
