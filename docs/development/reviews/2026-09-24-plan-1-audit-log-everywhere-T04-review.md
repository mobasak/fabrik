...
| T04-registrar-O6 | RECORDED — by design (D-408) — an unparseable yaml beside the specs refuses the cutover under the fail-closed rule; the operator removes or fixes the stray file |
| T04-registrar-O5-crossdir | RECORDED — by design (D-408) — the refusal is scoped to the spec's own directory; all 22 needs_database specs live in specs/services/ (measured) |
