# T04 acceptance review — the app-role registrar step
...
```text
$ python scripts/review_rubric.py --changed src/fabrik/orchestrator/infrastructure.py src/fabrik/orchestrator/deployer_ssh.py src/fabrik/drivers/ssh.py tests/conftest.py tests/test_app_role_provision.py tests/orchestrator/test_deployer_ssh.py tests/drivers/test_ssh.py docs/operations/fabrik-lifecycle.md docs/reference/modules/drivers.md
...
### core/10-python.md  (hit: src/fabrik/drivers/ssh.py, src/fabrik/orchestrator/deployer_ssh.py, src/fabrik/orchestrator/infrastructure.py)
...
### core/40-documentation.md  (hit: docs/operations/fabrik-lifecycle.md, docs/reference/modules/drivers.md)
...
### core/45-testing-strategy.md  (hit: tests/conftest.py, tests/drivers/test_ssh.py, tests/orchestrator/test_deployer_ssh.py)
...
| Class | Status |
|---|---|
| Hunt: `src/fabrik/orchestrator/infrastructure.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (S2/S3/O5 sibling scan, O3 strict boolean flag, O4 rollback owner check, O7 pinned DATABASE_URL), FIXED r2 (O10 a placeholder is not a pin) — every hunk of `src/fabrik/orchestrator/infrastructure.py` hunted by sonnet×1 + haiku×1 + opus×1 |
| Hunt: `src/fabrik/orchestrator/deployer_ssh.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (O2 a refused sudo no longer reads as absent, O9 `export ` keys), FIXED r2 (O9 a quoted value with an inline comment) in `src/fabrik/orchestrator/deployer_ssh.py` |
| Hunt: `src/fabrik/drivers/ssh.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (O1 a timed-out ssh re-raises with a redacted argv and no chained original) in `src/fabrik/drivers/ssh.py` — a declared Touches deviation, the root of the leak |
| Hunt: `tests/conftest.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (S1/O8 an opted-in module meets fail-loud defaults on `_run_sql` and every ssh binding) in `tests/conftest.py` — a declared Touches deviation |
| Hunt: `tests/test_app_role_provision.py` — every changed hunk, its enclosing function, its callers | FIXED r1–r2 (a test per confirmed item in `tests/test_app_role_provision.py`, each watched red on the prior head) |
| Hunt: `tests/orchestrator/test_deployer_ssh.py` — every changed hunk, its enclosing function, its callers | FIXED r1–r2 (refused sudo, `export ` keys and quoted-comment values covered in `tests/orchestrator/test_deployer_ssh.py`) |
| Hunt: `tests/drivers/test_ssh.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (the timeout test asserts no password or payload in str, repr, cause or context, in `tests/drivers/test_ssh.py`) |
| Hunt: `docs/operations/fabrik-lifecycle.md` — every changed hunk, its enclosing function, its callers | FIXED r1–r2 (the pin sentence, the rollback row, the shared-database paragraph and runbook step 1 in `docs/operations/fabrik-lifecycle.md` match the code) |
| Hunt: `docs/reference/modules/drivers.md` — every changed hunk, its enclosing function, its callers | CLEAN (the role-model table in `docs/reference/modules/drivers.md` matches the driver; swept clean in pass 1) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1–r2 (O2, O3, O4, S3, O9 each read as success or a silent `unmanaged` before; now named failures) in `src/fabrik/orchestrator/infrastructure.py` |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (no counter or quota in the `src/fabrik/orchestrator/infrastructure.py` hunks; S4's double `ensure_app_role` call is the ticket's contract) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r2 (O10 the placeholder sentinel, reusing `_is_placeholder` in `src/fabrik/orchestrator/infrastructure.py`) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (every confirmed item has a test in `tests/test_app_role_provision.py` watched red first; the db_before_boot exemption proven red-on-revert) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the sibling scan's scope was measured: 22 needs_database specs, all in specs/services/, 0 `.yml`; test totals from pytest's summary in `tests/test_app_role_provision.py` runs) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every claim was executed on a pinned copy of `src/fabrik/orchestrator/infrastructure.py` with ssh and `_run_sql` patched to raise; the orchestrator re-ran the suites each round: 133, 166, 171, 405 passed) |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1 + opus×1 (T04 slice), one sonnet refuter | found: 14, new: 14, confirmed: 11, fixed: 11, unexecuted: 0 | method: citation — full slice pass over the T04 hunks + one hop; 2 recorded by design, 1 refuted |
| Pass 2 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T04 owners) | found: 3, new: 3, confirmed: 2, fixed: 2, unexecuted: 0 | method: re-derivation — 11 ledger claims re-executed (all fixed); O10 new inside the r1 O7 hunk (own-fix: round 1); O5 recorded |
| Pass 3 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T04 owners) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — O9 and O10 re-executed by all three finders (fixed) |
...
|---|---|
| T04-registrar-H1 | REFUTED (create_database passwords are CSPRNG [a-zA-Z0-9], so the unencoded owner DSN cannot carry @ : / ? #) |
| T04-registrar-S4 | RECORDED — by design (D-390) — the ticket calls ensure_app_role on every apply and again with reset_password=True at a cutover; the second round trip is the contract |
| T04-registrar-O6 | RECORDED — by design (D-407) — an unparseable yaml beside the specs refuses the cutover under the fail-closed rule; the operator removes or fixes the stray file |
| T04-registrar-O5-crossdir | RECORDED — by design (D-407) — the refusal is scoped to the spec's own directory; all 22 needs_database specs live in specs/services/ (measured) |
| T04-refuter-live-ssh | RECORDED — measured (a pass-1 refuter probe with a mis-named mock made one real ssh to postgres-main; `\connect mydb` failed with database "mydb" does not exist; a read-only check found 49 roles and none ending in _app, so nothing was created; later briefs made ssh and _run_sql raise first) |
| T04-orchestrator-suites-live | RECORDED — measured (the existing orchestrator suites reach production over ssh; filed as mail 01M39XVS and a STRATEGIC_BACKLOG row, outside this plan) |
...
### Phase 1 — T04 the app-role registrar step: CLEAN after r1–r2 fixes — 405 tests pass on the integrated tree (T01–T04 suites plus the real-PG suite); the production-reaching orchestrator suites were deliberately not run
