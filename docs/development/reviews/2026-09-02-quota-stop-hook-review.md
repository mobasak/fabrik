# Review — the fleet-wide graceful-stop hook (`.claude/hooks/quota_stop.py`) — 2026-09-02

Status: CONVERGED
Surface: f2370abdaf9a8a2ec166c3a87b8ffe2e163413d6 b841492a15490ddafa8f9807cac51b1a
Scope: `.claude/hooks/quota_stop.py` (NEW PreToolUse hook) · `.claude/settings.json` (PreToolUse entry, FLEET-SYNCED) · `scripts/fabrik_synced_manifest.py` (hook added to AGENT_HOOK_FILES, FLEET-SYNCED) · `tests/test_quota_stop_hook.py` · `docs/workstation/hooks-index.md` · `docs/workstation/claude-account-rotation.md` · CHANGELOG entry · D-083.
Anchor: no prior review for this scope (`ls docs/development/reviews/ | grep -i quota` → none) → full WIDE pass 1.
NO-POOL: standing session directive — no Agent tool, no pool dispatch; every finder partition was run by the reviewing session in-line, with executed probes, and the ledger says so per row.

## Rubric (verbatim — `python scripts/review_rubric.py --changed .claude/hooks/quota_stop.py .claude/settings.json scripts/fabrik_synced_manifest.py tests/test_quota_stop_hook.py docs/workstation/hooks-index.md docs/workstation/claude-account-rotation.md`)
...
### core/10-python.md  (hit: .claude/hooks/quota_stop.py, scripts/fabrik_synced_manifest.py, tests/test_quota_stop_hook.py)
...
### core/40-documentation.md  (hit: docs/workstation/claude-account-rotation.md, docs/workstation/hooks-index.md)
...
### core/45-testing-strategy.md  (hit: tests/test_quota_stop_hook.py)
...
|---|---|---|
| core/35-security-auth (FLOOR) | CLEAN | no auth surface; the hook reads a local stamp and a log mtime, writes nothing, emits no secret — hunted `quota_stop.py` for env/secret reads: only ROTATE_STATE_DIR / QUOTA_STOP_* names |
| core/25-data-postgres (FLOOR) | CLEAN | no database touched — hunted for `psycopg|asyncpg|DATABASE_URL` in the diff: 0 |
| core/30-ops (FLOOR) | CLEAN | no compose/ports/deploy surface; the sync path is the existing AGENT_HOOK_FILES consumer (`sync_enforcement_to_projects.py:627`) — no new mechanism |
| 12-FACTOR (FLOOR) | CLEAN | config via env (`ROTATE_STATE_DIR`, `QUOTA_STOP_TICK_LOG`, `QUOTA_STOP_TICK_STALE_S`), no state held in-process, stdout/stderr only |
| core/10-python (MATCHED: quota_stop.py, fabrik_synced_manifest.py, the test) | CLEAN | stdlib only, ruff clean, exits enumerated (`grep -nE 'return|raise|except'` → 15 sites, every non-deny path returns 0); `Path.exists()` swallows the unreadable-dir case, proven end-to-end (chmod 0 state dir → allow) |
| core/40-documentation (MATCHED: hooks-index, claude-account-rotation) | FIXED(1) | hooks-index row + rotation § Rotation paragraph written; session-pickup fact and the ≤5-min stamp lag added after pass 1 (finding #6/#7) |
| core/45-testing-strategy (MATCHED: the test) | FIXED(5) | 15 tests; every pass-1/2 finding has a test seen red first (chain bypass, destructive git/sed, MCP editors, legacy key, non-object payload); deny branch red-on-revert |
| Fail-open vs fail-closed on every gate/guard | FIXED(1) | fail-open on: no stamp · unreadable payload · non-object payload (FIXED — it raised) · unreadable state dir · stale/missing tick log (warn); fail-CLOSED only while the stamp stands, for tools/commands that cannot be proven a checkpoint (malformed tool_input held — test) |
| Cost/quota/limit accounting edges (unknown≠0) | CLEAN | the hook computes no quota; the tick owns the stamp; an unknown tick age (log missing) is treated as stale → allow_warn, never as fresh |
| Boundary/sentinel/prefix collisions | FIXED(2) | allow-list was a PREFIX match on the first segment (chains passed) and `git reset`/`sed -n` admitted `--hard`/`-i` — whole-line single-command rule + narrowed forms; 47-probe battery 0 mismatches |
| Behavior-without-a-test | FIXED(1) | MCP editors bypassed the matcher untested → matcher `.*` + default-deny + `test_mcp_edit_tools_are_held_and_mcp_reads_pass` |

## Disposition ledger (every candidate ever raised → FIXED or REFUTED)

| # | candidate | class | disposition |
|---|---|---|---|
| 1 | chained/piped/redirected Bash passed on a first-segment match (`git push && python3 evil.py`) | boundary/prefix | FIXED — `_UNSAFE_SHELL` + `_FILE_REDIRECT` + whole-line match; `test_chained_or_redirected_bash_is_held_whole` (red first) |
| 2 | `git reset --hard` and `sed -n -i` admitted by the allow-list | boundary/prefix | FIXED — `git reset [-q] HEAD` form only, `sed -n` with a `-i` lookahead refusal; `test_destructive_git_and_in_place_sed_are_held` (red first) |
| 3 | MCP editors (serena replace/insert/rename, browser clicks), Agent/Workflow outside the matcher | behavior-without-a-test / fail-open | FIXED — matcher `.*`, default-deny with a read allow-list; `test_mcp_edit_tools_are_held_and_mcp_reads_pass` (red first) |
| 4 | legacy `decision: block` key emitted alongside `hookSpecificOutput` | output contract | FIXED — the installed CLI 2.1.258 carries the string "PreToolUse, use hookSpecificOutput.permissionDecision instead" (grep -a on `~/.local/share/claude/versions/2.1.258`); only the current form is emitted; `test_deny_output_uses_only_the_current_pretooluse_contract` |
| 5 | a non-object JSON payload raised AttributeError → exit 1 + traceback (pass-2 probe) | fail-open | FIXED — `isinstance(payload, dict)` guard; `test_a_non_object_payload_never_blocks` (red first) |
| 6 | sessions already running do not get the hook until restart | session pickup | REFUTED as a defect — hooks load at session start by the CLI's design; documented in hooks-index, CHANGELOG and the rotation doc |
| 7 | the stamp can outlive relief / be left by a crashed tick | stamp races | REFUTED — the tick re-evaluates walled∧no-successor every 5 min and unlinks on relief (`claude_rotate.py:3868-3897`: write after `_drain_mail`, before the ledger append; a crash in between is re-judged next tick); the ≤5-min lag is documented |
| 8 | the manifest entry might not reach the sync | manifest consumer | REFUTED — `AGENT_HOOK_FILES` is consumed by `sync_enforcement_to_projects.py:627` and by the scaffolder (`scaffold.py:1214`); `settings.json` is in the governance-sync filter (`.pre-commit-config.yaml:134`) |
| 9 | a defect in the hook could freeze the fleet | fail-open completeness | REFUTED — every exit enumerated (15 return/except sites); malformed `tool_input` (a list) is HELD while stamped by design (a command we cannot inspect is not a checkpoint) — `test_malformed_tool_input_is_held_while_the_stamp_stands` |

## Pass Ledger

| Pass | method | found | new | fixed | finders |
|---|---|---|---|---|---|
| Pass 1 | method: citation | found: 8 | new: 8 | fixed: 4 | finders: in-line (NO-POOL) — fail-open · allow-list · matcher/MCP · output contract (CLI string search) · session pickup · stamp races · manifest consumer · tests/docs; dispatched: 8 partitions, returned: 8 |
| Pass 2 | method: citation | found: 1 | new: 1 | fixed: 1 | finders: in-line, SCOPED to the fix diff + callers — exit enumeration, malformed-input end-to-end (list tool_input · missing tool_name · string payload · unreadable state dir · missing tick log) |
| Pass 3 | method: re-derivation | found: 0 | new: 0 | fixed: 0 | finders: in-line, FULL fresh read of the final hook + the 47-probe battery re-run (0 mismatches) + test count re-derived (15) + gate re-run; standing rows cited, none counted |
...
## Residual risks (pre-existing / escalated / documented tradeoffs only)

- (c) design tradeoff, resolved by the FIX decisions above: while the stamp stands a session cannot run its own test suite or a multi-command checkpoint line — one simple command per call. That is the stop working as intended; the hold lifts with the stamp.
- (a) pre-existing: the tick's own refresh ping (`claude -p ping`) is what keeps idle readings fresh; when it fails under cron PATH the stamp logic still runs on cached rows — unchanged by this review.
