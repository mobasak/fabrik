# Chunk 7 — the producing commands delegate reads, rank by a judge panel, close in four calls (D-363)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` before the change = 71c82a864b9d5ac1a4bff58b1526e97dd3382dbd; the change landed as `325e6d8ae` (8 files, +148 −3): `git show 325e6d8ae`
**Command:** /fabrik-review · **Changed:** `commands/_fragments/delegated-reads.md`, `commands/_fragments/judge-panel.md`, `commands/_fragments/close-chain.md`, `commands/_sources/fabrik-spec.md`, `commands/_sources/fabrik-plan-after-chat.md`, `commands/assemble_commands.py`, `tests/test_producing_command_fragments.py`, `docs/reference/command-loop-performance.md`
...
```text
$ python scripts/review_rubric.py --changed commands/_fragments/delegated-reads.md commands/_fragments/judge-panel.md commands/_fragments/close-chain.md commands/_sources/fabrik-spec.md commands/_sources/fabrik-plan-after-chat.md commands/assemble_commands.py tests/test_producing_command_fragments.py docs/reference/command-loop-performance.md
...
### core/40-documentation.md  (hit: commands/_fragments/close-chain.md, commands/_fragments/delegated-reads.md, commands/_fragments/judge-panel.md)
...
| Class | Status |
|---|---|
| Hunt: `commands/_fragments/delegated-reads.md` — every changed hunk, its enclosing function, its callers | CLEAN (commands/_fragments/delegated-reads.md's ~30 k / ~10 k figures match finding 4 of the program doc; anchored claims executed by the lead; slice A seats, passes 1-3) |
| Hunt: `commands/_fragments/judge-panel.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S1 the panel's brief carried the lead's pick — now alphabetical, equal depth, no marker, recommendation formed after the panel returns) |
| Hunt: `commands/_fragments/close-chain.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S3 no push) · FIXED r2 (A-NEW2 the push had no rejection path — it now points at CLAUDE.md § EXIT's ladder) |
| Hunt: `commands/_sources/fabrik-spec.md` — every changed hunk, its enclosing function, its callers | FIXED r2 (A-NEW1/B-NEW1 Phase 3 opened by leading with the recommendation — the panel now ranks first) |
| Hunt: `commands/_sources/fabrik-plan-after-chat.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S2/B-S3 the one-line commit rule lost its reason — restored: the Stop hook blocks it, parked WIP on a shared tree can be silently destroyed) |
| Hunt: `commands/assemble_commands.py` — every changed hunk, its enclosing function, its callers | CLEAN (commands/assemble_commands.py's two close-chain PARAMS name scripts and flags that exist, now graded by executing each --help) |
| Hunt: `tests/test_producing_command_fragments.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S4 no grader for the named checkers) · FIXED r2 (A-NEW3 the flag regex truncated at an uppercase letter or digit; B-S1 a stale id in the docstring); 6 graders, each watched red |
| Hunt: `docs/reference/command-loop-performance.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (B-S1 the row cited D-362, which a sibling had minted — now D-363, minted by the tool) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (tests/test_producing_command_fragments.py runs each named checker's --help and requires every flag as a whole word) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (pool OFF; 6, 6, 4 seats stamped across .claude/workflows/fabrik-review-loop.js passes 1-3; the panel's three seats are the floor and stamped by the fragment) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r2 (the flag regex's truncation in tests/test_producing_command_fragments.py) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (every fragment rule has a grader in tests/test_producing_command_fragments.py, each red on HEAD or on a mutant) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (6 of 6 graders in tests/test_producing_command_fragments.py; 38 of 38 commands rendered with --check OK) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every candidate executed by its slice refuter; the flag mutants run by the orchestrator in scratch copies of commands/assemble_commands.py) |

## Pass Ledger

| Pass | Finders | Counters | Method |
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (two finders per slice over two slices, ONE `Workflow` call — 6 agents incl. 2 refuters) | found: 8, new: 8, confirmed: 5, fixed: 5, unexecuted: 0 | method: citation — 8 candidates, 6 confirmed as 5 defects, 3 refuted; stop: confirmed 5; fix: the panel brief, the push, the restored reason, a checker grader, the re-minted id |
| Pass 2 | native sonnet×2 + haiku×2 (the round-1 seats over their own ledgers, ONE `Workflow` call — 6 agents) | found: 7, new: 7, confirmed: 4, fixed: 4, unexecuted: 0 | method: re-derivation — 5 of 6 ledger defects NOW_FALSE, B-S1 STILL_TRUE in the test's docstring; A-NEW1 Phase 3's opening order, A-NEW2 the push's rejection path, A-NEW3 the flag regex; own-fix 4; stop: confirmed 4 |
| Pass 3 | native sonnet×2 + haiku×2 (the round-1 seats over their own ledgers, ONE `Workflow` call — 4 agents, no candidate to refute) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — 5 of 5 ledger defects re-executed NOW_FALSE; the rendered Phase 3 read in order; a `--plan-dirV2` mutant red; stop: closable |

## Residual

| A-H1 | RECORDED — measured (refuted at pass 1: the "either shape" wording cannot change which command runs) |
| B-S2 | RECORDED — measured (refuted at pass 1: three panel seats are the documented floor, which needs no headroom script) |
| B-S4 | RECORDED — measured (refuted at pass 1: the Doc Sync check is not a commit-time hook, so it cannot block the first commit) |

## Per-phase verdicts

### Phase 0 — Establish scope: CLEAN — record opened, the eight files pinned with HEAD copies, the diff and the rendered commands
### Phase 1 — Finders: CLEAN — one `Workflow` call per pass, every seat returned, no gap
### Phase 2 — Execute: FIXED(9) — 9 confirmed across two passes, each re-run by the orchestrator before its fix
### Phase 3 — Prove and fix: FIXED — 6 graders, each watched red on HEAD, the pass-1 state or a mutant
### Phase 4 — Converge: CLEAN — pass 3 confirmed 0 · fixed 0, every slice verified

## Gate

`final_gate.py --check --json` over this receipt (the change landed in `325e6d8ae`, the ledgers in `4dc4e10f6`), pasted verbatim:

```json
{"status": "success", "tier": 2, "passed": 64, "failed": 0, "skipped_checks": ["bandit", "bandit scripts/", "semgrep", "pytest"]}
```

The hub's pytest leg is off by design; the touched test file was run by hand, 6 of 6 green, and the corpus renders with `--check` OK.
