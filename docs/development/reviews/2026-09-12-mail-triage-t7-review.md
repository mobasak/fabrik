# Review — 2026-09-12-mail-triage-t7
...
```text
$ python scripts/review_rubric.py --changed scripts/final_gate.py docs/workflows/FINAL_GATE_WORKFLOW.md tests/test_final_gate_advisory_display.py docs/superpowers/specs/2026-09-10-kaizen-feedback-loop-design.md docs/development/plans/2026-09-12-plan-1-kaizen-corpus-weight-and-tokens-per-round.md
...
### core/10-python.md  (hit: scripts/final_gate.py, tests/test_final_gate_advisory_display.py)
...
### core/40-documentation.md  (hit: docs/development/plans/2026-09-12-plan-1-kaizen-corpus-weight-and-tokens-per-round.md, docs/superpowers/specs/2026-09-10-kaizen-feedback-loop-design.md, docs/workflows/FINAL_GATE_WORKFLOW.md)
...
### core/45-testing-strategy.md  (hit: tests/test_final_gate_advisory_display.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/final_gate.py` — every changed hunk, its enclosing function, its callers | FIXED r1 r2 (scripts/final_gate.py — the corpus-weight registration beside the lint ratchet; r1 rewrote the comment's false causal claim (warn_only does not stop a non-zero exit from redding — executed against run_optional_check's `if code != 0` branch) and dropped the flag word from the display name; r2 replaced a wrong numeric cite (:1849 pointed at the Ticket Breadth rationale) with the note's own text; the missing-optional-check skip-accounting shape is the pre-existing class, RECORDED F1) |
| Hunt: `docs/workflows/FINAL_GATE_WORKFLOW.md` — every changed hunk, its enclosing function, its callers | FIXED r1 r7 (docs/workflows/FINAL_GATE_WORKFLOW.md:181 — the Corpus Weight bullet: r1 aligned the tag to the doc's convention and stated the warn_only contract; r7 corrected the surface list from the requester's five to the plan's six (commands/_agents, D-241) — executed: a63bda73's bullet grep 'five governance' → 1, 052c32d8 adds commands/_agents) |
| Hunt: `tests/test_final_gate_advisory_display.py` — every changed hunk, its enclosing function, its callers | FIXED r1 r2 r3 (tests/test_final_gate_advisory_display.py — the AST grader: r1 pinned the cw_args definition (a `cw_args = ()` mutant had stayed green), r2 admitted an annotated definition (a correct split form had redded), r3 ignored a bare annotation (PEP 526) so a value-less mutant fails on the clean message; mutants M0/M7/M8a pass, M3/M5/M6/M8b fail by name on fresh copies) |
| Hunt: `docs/superpowers/specs/2026-09-10-kaizen-feedback-loop-design.md` — every changed hunk, its enclosing function, its callers | FIXED r1 r2 (docs/superpowers/specs/2026-09-10-kaizen-feedback-loop-design.md — the nine amendment residues C1–C9 and round 23's residue written by a second session; r1 corrected my own breaker-window count to four (rounds 12–14 was the firing the unchanged text names), 'no round 16 ran', and the no-fix md5-cell recipe; r2 placed the 1,052 figure in § D4; the 15-value series verified against the events file 15 of 15) |
| Hunt: `docs/development/plans/2026-09-12-plan-1-kaizen-corpus-weight-and-tokens-per-round.md` — every changed hunk, its enclosing function, its callers | FIXED r2 r3 r5 r6 r7 r8 (docs/development/plans/2026-09-12-plan-1-kaizen-corpus-weight-and-tokens-per-round.md — the plan owner's request paragraph :105 and its mirrors :107 :120 :438-439 brought to the landed state in one tense, the sixth surface attributed to 052c32d8, the landed item moved to Resolved; r5 found the round-4 rewrite had never reached its commit (the owner's editor rewrote the file between my write and my hash) and reapplied it under a hunk-level guard executed at hashing time) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (scripts/final_gate.py's registration comment no longer claims warn_only keeps a first fire off ~46 repos — run_optional_check reds the gate on ANY non-zero exit; the precondition on the shipping check (accept --check, exit 0 on every non-strict path) is stated in the comment, the grader docstring and the workflow bullet) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (scripts/final_gate.py — the row is warn_only and skipped while the check is absent; every round of this review sized by dispatch_headroom.py --delta and stamped first: 2 · 2 · 2 · 1 seats in the BLOCKED record, 1 · 1 · 1 · 1 · 1 in the resumed one) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r7 (docs/workflows/FINAL_GATE_WORKFLOW.md:181 — five surfaces vs the six of the plan's SURFACES tuple and D-241; the kaizen spec's breaker windows counted by the spec's own rule: four, not three) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 r2 r3 (tests/test_final_gate_advisory_display.py — the grader pins the cw_args VALUE (IfExp on check_only with ("--check",)) for both plain and annotated definitions; seven mutants executed on fresh copies, four dead by name, three legitimate forms green) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (the kaizen spec's series re-derived from the events file: 15 round events, confirmed [17,5,5,4,3,1,2,4,26,23,13,3,6,7,7]; the breaker windows enumerated by hand — 4 of 13 windows fire; the sweep for pending-voice mirrors over the 439-line plan lists every hit with its verdict) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every mechanism claim executed by a seat: run_optional_check's non-zero branch, the --json advisory/warnings buckets, the skip markers, the display prefix, the shas and display names at a63bda73/418f86b0/052c32d8, D-241's row, the events file) |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×1 | found: 10, new: 10, confirmed: 8, fixed: 8, unexecuted: 0 | citation — partition by file: Opus on final_gate.py + the grader, Sonnet on the spec residues + the workflow bullet; every mechanism claim executed on a copy (run_optional_check's branches, the --json buckets, the events file 15 of 15); 2 RECORDED (F1, F2) |
| Pass 2 | native opus×1 + sonnet×1 | found: 6, new: 4, confirmed: 4, fixed: 4, unexecuted: 0 | delta over the r1 fix (48 lines / 4 files by numstat of the pins): the :1849 cite off by five, an annotated definition redded, the kaizen plan's requested block named the retired display string, the § Reproduce mislocation; 2 RECORDED (F3, F4) · mirrors: 2 read |
| Pass 3 | native opus×1 + sonnet×1 | found: 3, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | delta over the r2 fix (46 lines): a bare annotation counted as a definition (M8b crashed in ast.dump(None)); the plan's request paragraph still framed the registration as pending; 1 RECORDED (F5) · mirrors: 3 read |
| Pass 4 | native opus×1 | found: 1, new: 1, confirmed: 1, fixed: 0, unexecuted: 0 | delta over the r3 fix (6 lines): the landed note contradicted the clause after it; the fix was written but did NOT reach bd69e10e (the plan owner's editor rewrote the file between write and hash) — discovered at pass 5; the record then closed BLOCKED on the fleet-quota hold · mirrors: 2 read |
| Pass 5 | native opus×1 | found: 4, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | delta over b43d679f..bd69e10e (resumed record): the missing round-4 fix reapplied under a hunk-level guard executed at hashing time (exactly the hunks asserted), :439's mirror fixed · mirrors: 3 read |
| Pass 6 | native opus×1 | found: 5, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | delta over the r5 fix (4 lines): the landed item moved from Still open to Resolved, the :105 tail no longer narrows step 5, the appositive → by a63bda73's heavy review — a class rewrite of the two sentences (D-231) · mirrors: 4 read |
| Pass 7 | native opus×1 | found: 2, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | delta over the r6 fix (6 lines): the workflow bullet's five surfaces vs the plan's six (D-241) — executed at a63bda73 and 052c32d8; :120's pending voice; the whole plan swept for the landed fact's mirrors in one pass (:107, :120) · mirrors: 6 read |
| Pass 8 | native opus×1 | found: 2, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | delta over the r7 fix (6 lines): :120 attributed six surfaces to a63bda73 (it had five); :107's antecedent · mirrors: 2 read |
| Pass 9 | native opus×1 | found: 2, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — delta over the r8 fix (4 lines): a63bda73's bullet grep 'five governance' → 1, 052c32d8 adds commands/_agents (one + line, no − line), D-241's row read, the registration at a63bda73 grep → 1, the commit order a63bda73 → 418f86b0 → 052c32d8, the four sha-bearing lines of the 439-line plan all reconciled; 1 RECORDED (F6) · mirrors: 4 read |
...
| F1 | RECORDED — measured (a registered check that is absent on disk is a green never-run row that `skipped_checks` does not report — the pre-existing missing-optional-check shape of `_SKIP_MARKERS`; `run_optional_check` also adds the name to WARN_ONLY_CHECKS before the existence guard — mail-triage plan T4/T10 backlog) |
| F2 | RECORDED — measured (`test_every_green_not_run_row_the_gate_emits_is_summarized` reads the runtime-mutated WARN_ONLY_CHECKS, so a gate run in the same interpreter would red it on the new row — same backlog row) |
| F3 | RECORDED — measured (no RUNTIME grader asserts the argv the ratchets receive under --check; the lint ratchet's own threading has no grader either — same backlog row) |
| F4 | RECORDED — measured (numeric cross-line cites inside a 2,700-line concurrently edited final_gate.py are a class; one instance retired at r2 by citing the note's text — backlog, measure the fire rate before a check) |
| F5 | RECORDED — measured (the kaizen plan's :175 keeps a deliberate conditional on the registration — the probe, not the prose, is the evidence — plan owner informed) |
| F6 | RECORDED — measured (the kaizen plan's Resolved entry at :438 lists a63bda73/418f86b0 and not 052c32d8 where the sixth surface reached the bullet — a one-token completeness edit for the plan owner) |
| F7 | RECORDED — measured (my commits 4662aad1, b43d679f and bd69e10e carried the plan owner's uncommitted hunks — 9, 4 and 2 — because the blob was hashed from a working tree not re-checked at that moment; disclosed by mail 01M2AW3QTB4X204GS4H34YV5P0 + correction 01M2B41EK5KCAZY5C8EBT775QC; the per-commit guard was executed at every later commit) |
...
### Phase 0 — mail-triage T7 (the registration block + grader, the kaizen spec residues, the plan's request paragraph): CONVERGED

Nine passes across two records (the fleet-quota hold split them after pass 4): 8 · 4 · 2 · 1 · 3 · 3 · 2 · 2 · 0 confirmed, every one inside the previous fix's hunks after pass 1 — the residue chain lived in one plan paragraph and its mirrors, and closed once the whole plan was swept for the landed fact in one pass (pass 7).
...
```json
{
  ...
  "run_at": "2026-09-12 · HEAD 0a1fe8bc · after the closing round"
}
```
