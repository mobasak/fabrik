# T03 — check_convergence.py QUIET_PASS follows the new grammar — per-ticket review

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 0c1f2d5005f779baf8ea0641d366b631507a8fb6; range tip c37386e11eb55a7d576efc874d62ded3788e7842; `git diff 0c1f2d50..c37386e1 -- scripts/enforcement/check_convergence.py tests/test_check_convergence.py` md5 123328f200d9fc4f5cb07d3e341446a8 (11282 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/enforcement/check_convergence.py`, `tests/test_check_convergence.py`
...
```text
$ python scripts/review_rubric.py --changed scripts/enforcement/check_convergence.py tests/test_check_convergence.py
...
### core/10-python.md  (hit: scripts/enforcement/check_convergence.py, tests/test_check_convergence.py)
...
### core/45-testing-strategy.md  (hit: tests/test_check_convergence.py)
...
| Class | Status |
|---|---|
| Hunt: `scripts/enforcement/check_convergence.py` — every changed hunk, its enclosing function, its callers | FIXED (1) — the rewritten comment block asserted order-independence the forward-only lookaheads lack (`\| 18 \| confirmed 3 \| found 0 \| fixed 0 \|` still matches branch 1 — executed by the Opus seat and by me); reworded at 375ca554 to the truth (guards look right of the anchor; the DEPTH gate refuses a displaced counter; 0 of 275 receipts carry one). The regex itself is the spec's V13 literal, 185 chars IDENTICAL (`cmp` by the Opus and Haiku seats); the ordering false-accept on a reordered row is `RECORDED — by design (D-205)` — this gate is a PRESENCE test whose depth is `check_review_coverage.py`, the spec freezes the regex verbatim, exposure 0 of 275 — booked to the backlog beside the `0+` symmetry. Untouched regexes, the six-name import and the call site byte-identical to base (Opus seat check 2–3) |
| Hunt: `tests/test_check_convergence.py` — every changed hunk, its enclosing function, its callers | FIXED (1) — the corpus-parity test asserted `len(new_quiet) == 166` over the LIVE growing corpus (`git ls-files`), a red on the next converged receipt any sibling commits; at 375ca554 the denominator is pinned to the tree object at 8092e8a8 (`git ls-tree` + `git show`): 275 receipts / 166 / 166 / 0 flips, stable. The five-row V13 test and the prose-label guard are discriminating (3 of 5 rows red on the old regex, the digit-anchor mutation reds the prose test while parity stays 166/166 — executed by both seats) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN — the two new branches are each guarded by a red-first test (branch 1 by the pinned parity test, the digit anchor by the prose-label test); the only fail-open shape found (a counter LEFT of its anchor) is masked by the coverage gate's order refusal and is RECORDED by design above, not a swallowed error |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN — no seat, quota or limit code on this surface; the round's seats are stamped (`dispatch --seats 3`) and closed on the run record |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN — `(?<![\w-])` holds (`unconfirmed:`/`non-confirmed:` keep an honest row quiet — executed, Opus seat A9/A16); `0+` reads `confirmed 00` as quiet at both gates; `confirmed 0.5` reading quiet is `RECORDED — measured (fractional counters are not a receipt shape; 0 of 275)` |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN — Behavior Contract row 1 → `test_quiet_pass_grades_the_five_v13_rows` (red on rows 2, 3, 5 under the old regex), row 2 → the pinned parity test (red on neutering branch 1: 166 → 0), plus the digit-anchor guard (red on removing `\d`); all three mutations asserted on copies with byte-identical restores |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN — every count in this receipt carries its denominator (275 receipts, 166 quiet, 78 tests, 5 rows, 3 seats); the parity test prints counts and the first 3 flips of each direction beside its assertion |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN — both Gate lines run by the coder AND by the orchestrator in the worktree (78 passed; `check_convergence.py` exit 0), the regex identity proven by `cmp` not by eye, the corpus parity re-derived by two seats independently, the ordering candidates executed on the pinned module before adjudication |
...
|---|---|---|---|
| Pass 1 | native opus×1 + sonnet×1 + haiku×1 (3 seats stamped) | found: 8, new: 8, confirmed: 2, fixed: 2, unexecuted: 0 | citation — the 168-line diff at c37386e1 pinned by md5; the Opus seat executed 23 rows (5 contract + 18 adversarial) and the corpus parity (275 / 166 / 166 / 0), the Sonnet seat the discrimination of every new test and ruff with the repo config, the Haiku seat the byte identity of the regex and the stale-phrase sweep (13 hits, 0 live); 2 CONFIRMED — the comment block's order-independence claim and the live-corpus exact count — both fixed by the coder at 375ca554; RECORDED, never counted: the reordered-row false-accept (by design, D-205 — a presence gate under a spec-frozen regex, 0 of 275), the corpus docs that still teach `found 0` in `fabrik-execute-plan.md`/`fabrik-plan-review.md` (by design, D-205 — the review-family backlog row), `confirmed 0.5` (measured), the docstring's carried-forward word "final" and the commit body's "five mentions" count (below the bar) |
| Pass 2 | native opus×1 + haiku×1 (2 seats stamped — the delta is one comment block and two tests: one risky slice, one mechanical) | found: 8, new: 8, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — delta over the fix diff c37386e1..375ca554 plus one hop — 23 ledger shapes executed against the pinned `QUIET_PASS` (every sentence of the reworded comment probed in its stated direction, incl. the RECORDED ordering hole), the parity denominator re-derived (`git ls-tree 8092e8a8` → 275 receipts, 166 quiet under both regexes, 0 flips, cwd-independent), the prose-label test and the pinned parity test executed on the pin (78 passed) and on the pre-fix module, `QUIET_PASS`'s one live consumer (:804, any-line search) read against the comment, the mechanical sweep (stale phrases, round labels, ruff, dead symbols: 0). 0 CONFIRMED — quiet under D-206: the review closes. RECORDED, never counted: (a) the spec-frozen regex's branch-2 anchor `confirmed\s*:\s*0+\b` carries no `(?<![\w-])` guard, so `\\| 19 \\| found 12 \\| confirmed 4 \\| unconfirmed: 0 \\| fixed 0 \\|` satisfies this PRESENCE gate (0 of 275 committed receipts carry a word-prefixed `confirmed 0`) — the same class as the ordering hole D-205 recorded: the DEPTH gate is the control, and T02's token rule refuses exactly that row (spec V2, "displaced between": a `confirmed` token `_MEGA_ROW` did not capture is refused by name) — verified at T02's merge gate, and the regex character itself is filed for the plan-review adoption backlog row rather than edited under the D-205 freeze; (b) the two tests added by the fix are characterization tests (the module change is comment-only, so no test can be red on the pre-fix module — by design); (c) `fixed:` carries no `\s*` before its colon while the comment's `\s*` sentence is about the two lookaheads (true as written; spec-verbatim regex); (d) the comment's compensation sentence about `check_review_coverage.py` is asserted, not executed by the seat — executed by the orchestrator: on the T01 pin the row reads `(12, 4, 0, None)` (not quiet), and the displaced-cell shapes are T02's token-rule fixtures |
...
### Phase 1 — `check_convergence.py` QUIET_PASS follows the D-206 grammar (T03): CLEAN — two executed review rounds (8 candidates → 8, confirmed 1 → 0): the reworded comment block probed sentence by sentence against the regex, the parity denominator pinned to the tree at 8092e8a8 (275 receipts, 166 quiet under both regexes, 0 flips), the presence gate's residual (a word-prefixed `confirmed: 0` anchor; a counter left of its anchor) RECORDED as compensated by the depth gate's token rule (T02) and filed for the plan-review adoption; every finding reproduced by the orchestrator before routing.
...
`final_gate.py --lean --check --json` on a shared clone of master (5559fdcf) carrying the T03 merge content, 2026-09-09.
