# Row 5b chunk 1 — the refuter per slice, closable, ledger rows

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 846fe2450c25b33f1b2dfae7d66e7036cf05eec9; `git diff HEAD -- .claude/workflows/fabrik-review-loop.js tests/test_review_loop_workflow.py commands/_agents/fabrik-reviewer.md commands/_sources/fabrik-review.md docs/reference/review-loop-workflow.md` md5 f2ed54a03f869e8e7e76d1e45f68618c (47589 bytes)
**Command:** /fabrik-review · **Changed:** `.claude/workflows/fabrik-review-loop.js`, `tests/test_review_loop_workflow.py`, `commands/_agents/fabrik-reviewer.md`, `commands/_sources/fabrik-review.md`, `docs/reference/review-loop-workflow.md`
...
```text
$ python scripts/review_rubric.py --changed .claude/workflows/fabrik-review-loop.js tests/test_review_loop_workflow.py commands/_agents/fabrik-reviewer.md commands/_sources/fabrik-review.md docs/reference/review-loop-workflow.md
...
### core/40-documentation.md  (hit: commands/_agents/fabrik-reviewer.md, commands/_sources/fabrik-review.md, docs/reference/review-loop-workflow.md)
...
| Class | Status |
|---|---|
| Hunt: `.claude/workflows/fabrik-review-loop.js` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S1 a placeholder refutation passed the guard; A-S2 two candidates sharing an id shared one verdict, and two verdict rows kept the first; A-S5 a case-slipped ledger id held a slice open) · FIXED r2 (A-S7 the case fold merged two ids; A-S8 the placeholder list rejected a real output) — exact id wins, a fold counts only onto one claim, placeholders read on the COMMAND |
| Hunt: `tests/test_review_loop_workflow.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S4 the harness rejected on a throw where the runtime resolves null and drops the item, so the dropped-slice path was untested) — faithful parallel/pipeline, 8 → 19 graders, every new one watched red before its fix |
| Hunt: `commands/_agents/fabrik-reviewer.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (A-S6 the frontmatter was invalid YAML — a plain scalar holding `): a` — so the nested experimental.cacheTtl could be dropped by a strict parser; the description is now single-quoted and a grader parses it with yaml.safe_load) |
| Hunt: `commands/_sources/fabrik-review.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (B-S1 the Phase 2 sentence said the refuter ran a command per candidate; it runs one per candidate it REACHED) — the Phase 1 launch and Phase 4 closable sentences re-read by both B seats in passes 1-3 |
| Hunt: `docs/reference/review-loop-workflow.md` — every changed hunk, its enclosing function, its callers | FIXED r1 (B-S2 the D-330 grounds and the growing refuter box; B-S3 the per-seat shape lacked confirmed; B-S4 the cacheTtl caveat) · FIXED r2 (B-L1 the refutation clause restated after the A-S8 fix) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (a bare or placeholder refutation read as a closed candidate — fail-open; now unverified, and unverified never closes a slice) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (pool OFF; 6, 6, 4 seats stamped per pass in .claude/workflows/fabrik-review-loop.js runs; 17 + 6 + 4 workflow agents, about 1.34 M seat tokens by the notifications) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r2 (A-S7 a prefix-free but case-folded id match; exact id now wins, the fold counts onto exactly one claim) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (every behaviour the change adds has a grader in tests/test_review_loop_workflow.py; the 11 new or changed graders were each watched red before the fix) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (every seat stated its denominator; 19 of 19 graders, 5 of 5 and 3 of 3 files read per slice in docs/development/reviews/2026-09-23-row5b-chunk1-review.md's passes) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every candidate executed by a refuter or a seat under node, then re-run by the orchestrator in .claude/workflows/fabrik-review-loop.js's harness before each fix) |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (two finders per slice on HEAD's copy of the loop, never the working-tree subject; ONE `Workflow` call — 17 agents incl. 13 verify seats) | found: 13, new: 13, confirmed: 9, fixed: 9, unexecuted: 0 | method: citation — every candidate executed by a verify seat and re-run by the orchestrator on the live tree; A-S3 recorded, A-H1/A-H2/A-H3 refuted; seats: A-sonnet 6/6 · A-haiku 1/3 · B-sonnet 4/4 · B-haiku 0/0 |
| Pass 2 | native sonnet×2 + haiku×2 (the round-1 seats over their own ledgers, ONE `Workflow` call — 6 agents) | found: 2, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | method: re-derivation — 9 of 9 ledger defects re-executed as fixed (the A-haiku seat wrote STILL_TRUE beside outputs reading PASSED; the orchestrator re-ran all 17 graders green); A-S7 and A-S8 NEW inside the round-1 fix hunks; own-fix 2; seats: A-sonnet 2/2 · A-haiku 0/0 · B-sonnet 0/0 · B-haiku 0/0 |
| Pass 3 | native sonnet×2 + haiku×2 (the round-1 seats over their own ledgers, ONE `Workflow` call — 4 agents) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — A-S7, A-S8, B-L1 re-executed NOW_FALSE on 19 of 19 graders plus four mirror probes each; no NEW candidate in the round-2 fix hunks; seats: A-sonnet 0/0 · A-haiku 0/0 · B-sonnet 0/0 · B-haiku 0/0 |
...
| A-S3 | RECORDED — measured (row 5b's "checks under ~10 calls run by the lead inline" is lead behaviour the script cannot see; it is chunk 2 of row 5b, in `commands/_sources/fabrik-review.md`, named in the row's status cell) |
...
### Phase 0 — Establish scope: CLEAN — record opened, pins materialised with HEAD's loop copy so the subject never graded itself, hygiene 0 hits over 5 files, receipt, headroom 4 finders, 6 stamped
### Phase 1 — Finders: CLEAN — one `Workflow` call per pass, every seat returned schema, no gap
### Phase 2 — Execute: FIXED(11) — 11 confirmed across three passes, each reproduced by the orchestrator before its fix
### Phase 3 — Prove and fix: FIXED — 11 graders added or changed, each watched red first
### Phase 4 — Converge: CLEAN — pass 3 confirmed 0 · fixed 0, both slices verified
...
The two ruff rows read SCOPE-NARROWED (a sibling's unstaged tracked .py in the tree); `ruff check` and `ruff format --check` were run by hand on every touched .py and pass, and `tests/test_review_loop_workflow.py` was reformatted after the closing pass (layout only — 19 of 19 graders re-run green). An earlier run of this gate was red only on a sibling's untracked research ledger without its INDEX row (theirs, added in their tree) and on a stray `pins_copy/` a pass-1 seat wrote into the repo root (the orchestrator's pins, md5-verified and removed).
