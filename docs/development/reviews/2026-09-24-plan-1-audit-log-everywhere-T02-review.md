# T02 acceptance review — the <db>_app role driver
...
```text
$ python scripts/review_rubric.py --changed src/fabrik/drivers/postgres.py tests/test_app_role_driver.py tests/test_app_role_real_pg.py
...
### core/10-python.md  (hit: src/fabrik/drivers/postgres.py, tests/test_app_role_driver.py, tests/test_app_role_real_pg.py)
...
### core/45-testing-strategy.md  (hit: tests/test_app_role_driver.py, tests/test_app_role_real_pg.py)
...
| Class | Status |
|---|---|
| Hunt: `src/fabrik/drivers/postgres.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (O1–O8, O10, S3 and the PUBLIC TRIGGER/REFERENCES revoke), FIXED r2 (O13 revoke-as-grantor ACL walk, O14 probe/ensure membership match, O15 hex-encoded probe fields), FIXED r3 (O16 superuser grantor fails fast) — every hunk of `src/fabrik/drivers/postgres.py` hunted by sonnet×1 + haiku×1 + opus×1 |
| Hunt: `tests/test_app_role_driver.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (fake-SQL rows for the fresh-role drop, the probe parse and the membership flags added beside the SQL-shape rows in `tests/test_app_role_driver.py`) |
| Hunt: `tests/test_app_role_real_pg.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (O11 — pulls postgres:16 on a miss and fails under FABRIK_REQUIRE_REAL_PG=1; O12 — every finding has a real-PG row in `tests/test_app_role_real_pg.py` seeding its bad state, watched red first) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | FIXED r1 (the probe's missing sentinel, CREATE via group roles and the stale membership all passed silently; now each is a named failure in `src/fabrik/drivers/postgres.py`) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (no counter or quota in the `src/fabrik/drivers/postgres.py` hunks; the ACL walk's 1000-revoke bound raises, verified in pass 3) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r2 (O15 — separator bytes inside an identifier could forge the `probe|done` sentinel; fields are hex-encoded in `src/fabrik/drivers/postgres.py`) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r1 (O12 — the untested states O1–O7 each got a seeded real-PG row in `tests/test_app_role_real_pg.py`, proven red-on-revert in a throwaway worktree) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (no count claim in the `src/fabrik/drivers/postgres.py` hunks; test totals quoted from pytest's own summary lines) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every pass executed its claims on a throwaway postgres:16 from `tests/test_app_role_real_pg.py`-style harnesses; the orchestrator re-ran the suite each round: 57, 74, 78, 79, 79 passed) |
...
|---|---|---|---|
| Pass 1 | native sonnet×1 + haiku×1 + opus×1 (T02 slice), one sonnet refuter | found: 16, new: 16, confirmed: 12, fixed: 12, unexecuted: 0 | method: citation — full slice pass over the T02 hunks + one hop; 3 recorded by design, 1 refuted |
| Pass 2 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T02 owners) | found: 3, new: 3, confirmed: 3, fixed: 3, unexecuted: 0 | method: re-derivation — 12 ledger claims re-executed on postgres:16 (all fixed); 3 new inside the r1 fix hunks (own-fix: round 1) |
| Pass 3 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T02 owners) | found: 1, new: 1, confirmed: 1, fixed: 1, unexecuted: 0 | method: re-derivation — 3 ledger claims re-executed (all fixed); 1 new inside the r2 hunk (own-fix: round 2); scope-growth stop fired |
| Pass 4 | native sonnet×1 + haiku×1 + opus×1 (the round-1 T02 owners), remainder round | found: 1, new: 1, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — O16 re-executed by all three finders on postgres:16 (fixed); O17 recorded below per the scope-growth rule |
...
|---|---|
| T02-driver-O9 | RECORDED — by design (D-386) — spec § 1 names the schema's same-transaction revokes, emitted by T05, as the guard for a table created after ensure_app_role |
| T02-driver-S1 | RECORDED — by design (D-386) — the same default-privilege window as O9 |
| T02-driver-S2 | RECORDED — by design (D-390) — T04 runs ensure_app_role after the watchdog step on every apply |
| T02-driver-H1 | REFUTED (__all__ restricts only wildcard imports; an explicit `from fabrik.drivers.postgres import ensure_app_role` resolves) |
| T02-driver-O17 | RECORDED — measured (own-fix residue raised after the scope-growth stop; the O16 message named a remedy that does not work on PG16, so the orchestrator corrected the string inline to the measured one — ALTER ROLE grantor NOSUPERUSER then re-apply — and the O16 real-PG test now asserts NOSUPERUSER; re-read at D7) |
| T02-remainder-ledger | RECORDED — measured (the refuter did not re-run O16 itself; all three finders executed it on postgres:16 and reported it fixed with output) |
| full-suite-readiness | RECORDED — measured (one real-PG run hit the 90 s container-readiness bound while four review containers ran concurrently; it failed closed, and the clean re-run passed 79) |
...
### Phase 1 — T02 the `<db>_app` role driver: CLEAN after r1–r3 fixes — 79 T02 tests pass on the integrated tree (180 with T01's); bandit B608 on identifier-validated SQL annotated per the file's existing convention
