# T10 V5 fix — archived plans still carry their spec (classes 1 and 4)

**Status:** CONVERGED
**Surface:** `git rev-parse HEAD` = 24fbac770621f43caeda33c1dee3fe766d8beae2; range tip 9467bed4245b692abe5f5f58f3edd1cd0cdf52f1; `git diff 24fbac770..9467bed42 -- scripts/work.py tests/test_work_sync.py docs/reference/work-tracking.md` md5 005f045db1983e6aa69c9e5c11b94488 (7460 bytes)
**Command:** /fabrik-review · **Changed:** `scripts/work.py`, `tests/test_work_sync.py`, `docs/reference/work-tracking.md`
...
```text
$ python scripts/review_rubric.py --changed scripts/work.py tests/test_work_sync.py docs/reference/work-tracking.md
...
### core/40-documentation.md  (hit: docs/reference/work-tracking.md)
...
| Class | Status |
|---|---|
| Hunt: `scripts/work.py` — every changed hunk, its enclosing function, its callers | FIXED r1 (`_iter_plan_spines(archived=True)` reads `plans/archived/`; `_drift_report` adds archived spines' designated citations to class 1 and checks archived EXECUTED plans for class 4; the redundant `report[4].sort()` removed, the return already sorts; the status PLAN board and classes 2, 3, 8 stay live-only) |
| Hunt: `tests/test_work_sync.py` — every changed hunk, its enclosing function, its callers | CLEAN (3 tests: two red on revert of `work.py`, the third pins the live-only rule for classes 2, 3, 8 and passes both ways by design; 53 passed) |
| Hunt: `docs/reference/work-tracking.md` — every changed hunk, its enclosing function, its callers | CLEAN (the class table's rows 1 and 4 and the live-only sentence match `_drift_report`) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (a missing `archived/` yields nothing, as the live reader does; an unreadable spine reads as empty text, the existing pattern) |
| Recurrence: cost/quota accounting — pool units scored, native seats counted, a limit at its edges | CLEAN (`work.py status` over the hub's 95 archived spines plus 150 synthetic ones ran in 0.22 s against the gate row's 30 s budget) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | CLEAN (archived dir and file names match by the same anchored `_PLAN_DIR_NAME_RE`/`_PLAN_FILE_NAME_RE` as live plans) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | CLEAN (reverting `work.py` to 24fbac770 reds the class-1 and class-4 tests) |
| Recurrence: denominator on every count — bounded searches state their bound | CLEAN (hub class 1: 28 lines before, 24 carried by an archived plan, 4 after; 95 archived spines read by `_iter_plan_spines(archived=True)`, 7 odd-named collection dirs holding 58 md files) |
| Recurrence: proxy-as-evidence — the real check EXECUTED, not read | CLEAN (every claim executed: the revert, the timing, the hub drift report before and after, `command grep -rl` for the 4 remaining specs under `archived/`) |
...
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (A: scripts/work.py sonnet+haiku; B: tests/test_work_sync.py + docs/reference/work-tracking.md sonnet+haiku) + refuters | found: 4, new: 4, confirmed: 1, fixed: 1, unexecuted: 0 | citation — the full partitioned pass (workflow wf_3449161c-b06) over 24fbac770..9467bed42; executed: the 3 new tests on a copy of the pin, every caller of `_iter_plan_spines` (the status PLAN board stays live-only), an archived plan-set directory link through `_normalize_plan_ref`, `work.py status` over 95 archived spines plus 150 synthetic ones in 0.22 s, the class-1 count on the real tree 28 to 4; CONFIRMED — the added `report[4].sort()` is dead, the return already builds `sorted(set(paths))` (A-S1); RECORDED A-S2, B-S1, B-S2; seats: A-sonnet 1/2 · A-haiku 0/0 · B-sonnet 0/1 · B-haiku 0/1; stop: confirmed 1; fix: +0 -1 lines for 1 confirmed (243053def) |
| Pass 2 | native sonnet×2 + haiku×2 (A: sonnet+haiku over scripts/work.py; B: sonnet+haiku over the tests and the doc) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the closing pass: the same slice owners on the pin of 243053def (workflow wf_edd7b44f-a85); A-S1 re-executed NOW_FALSE (no `report[4].sort()` in the file, the return sorts every class); B-L1 re-executed NOW_FALSE (with `work.py` at 24fbac770 the class-1 and class-4 tests fail, 2 failed 1 passed; with the fix both pass); the doc table re-read against `_drift_report`; seats: A-sonnet 0/0 · A-haiku 0/0 · B-sonnet 0/0 · B-haiku 0/0; stop: confirmed 0 |
...
| A-S2 | RECORDED — measured (legacy archived collection dirs whose names do not match the plan-set pattern are not read; on the hub 7 such dirs hold 58 md files from 2025-01 to 2026-06, and 0 of the 4 remaining class-1 specs is named anywhere under `archived/`) |
| B-S1 | RECORDED — measured (a seat's statement that the live-only rule for classes 2, 3, 8 matches the code; makes no defect claim) |
| B-S2 | RECORDED — measured (refuted as worded: two of the three tests red on revert, the third pins the live-only rule and passes both ways by design) |

## Per-phase verdicts

### Phase 1 — T10 V5 fix, archived plans still carry their spec: CONVERGED — 2 passes, confirmed 1 → 0
...
```json
$ .venv/bin/python scripts/final_gate.py --check --json   # main checkout, HEAD 64721143 (the fix merged e4c578608, ledgers 64721143; governance sync on the merge: 47 projects synced, 0 failed)
{
  "status": "success",
  "tier": 2,
  "passed": 67,
