# Stop hook: a `BLOCKED:` header exempts a DEFERRAL only in its format (D-391)

**Status:** CONVERGED — pass 4 confirmed 0 (B-EXTRA1 NOW_FALSE by execution); slice A's ledger closed at pass 3 on code byte-identical since (verified: the hook and test hunks of the pass-3 and pass-4 diffs are equal, and the live files equal the pin)
**Surface:** `git rev-parse HEAD` = 8dff134ffdd923c22bf6bc2cff8d64eafe264dbc; working diff over .claude/hooks/final_gate_stop.py tests/test_final_gate_stop_deferral.py docs/workstation/hooks-index.md, md5 a41a467916f4c9c6906bc70d545a5f56
**Command:** /fabrik-review · **Changed:** .claude/hooks/final_gate_stop.py tests/test_final_gate_stop_deferral.py docs/workstation/hooks-index.md docs/STRATEGIC_BACKLOG.md
**Ruling:** operator, 2026-09-24, answering a DECISION NEEDED (ground: gate — design approval) block: "Recommendation: A … good" (D-391)
...
```text
$ python scripts/review_rubric.py --changed .claude/hooks/final_gate_stop.py tests/test_final_gate_stop_deferral.py docs/workstation/hooks-index.md
...
### core/45-testing-strategy.md  (hit: tests/test_final_gate_stop_deferral.py)
...
| Class | Status |
|---|---|
| Hunt: `.claude/hooks/final_gate_stop.py` — every changed hunk, its enclosing function or section, its callers | FIXED r1-r2 (`_blocked_header`: fields in a quoting fence; a trailing example fence taken as the agent's own footer; a fenced footer line cutting the window) |
| Hunt: `tests/test_final_gate_stop_deferral.py` — every changed hunk, its enclosing function or section, its callers | FIXED r0-r2 (10 new cases, each new one seen red on the draft it guards; 5 fixtures moved to the format) |
| Hunt: `docs/workstation/hooks-index.md` — every changed hunk, its enclosing function or section, its callers | FIXED r1-r3 (line cites shifted twice by the fixes; the window wording; the stale "exempts globally" sentence) |
| Hunt: `scripts/sysadmin/stop_mine.py` (caller, via `deferral_shape`) — every changed hunk, its enclosing function or section, its callers | CLEAN (imports `deferral_shape` from the hook; parity by construction, executed by two seats) |
| Recurrence: fail-open/fail-closed — a swallowed error or an absent check that reads as success | CLEAN (a refused escalation still releases at CAP=3 — executed; `_blocked_header` is pure over the text) |
| Recurrence: cost/quota accounting — a counter or limit at its edges | CLEAN (no counter touched; the per-character fence filter timed at 11 ms on an 81 KB message and 88 ms with 500 fences) |
| Recurrence: boundary/sentinel/prefix — an off-by-one, a sentinel value, a prefix-vs-exact match | FIXED r1-r2 (fence span edges; the trailing-fence one-key vs two-key rule; the window end outside fences) |
| Recurrence: behavior-without-a-test — a contract row no test kills (mutation asserted) | FIXED r0-r2 (every rule has a case; the 5 new r0 tests red on HEAD's hook, each later case red on the draft before it) |

## Pass Ledger

| Pass | Finders | Counters | Method |
|---|---|---|---|
| Pass 1 | native sonnet×2 + haiku×2 (slice A sonnet, haiku; slice B sonnet, haiku) + sonnet×2 refuters | found: 6, new: 6, confirmed: 3, fixed: 3, unexecuted: 0 | method: execution — the pinned hook imported and driven on each scenario; a 81 KB timing |
| Pass 2 | native sonnet×2 + haiku×2 (the round-1 seats over their own slice ledgers) + sonnet×2 refuters | found: 2, new: 2, confirmed: 2, fixed: 2, unexecuted: 0 | method: re-derivation — 3 ledger claims NOW_FALSE; 2 new inside the fix |
| Pass 3 | native sonnet×2 + haiku×2 (the same seats) + sonnet×2 refuters | found: 2, new: 2, confirmed: 2, fixed: 1, unexecuted: 0 | method: re-derivation — slice A ledger closed; A-S3 recorded (scope-growth stop), B-EXTRA1 fixed |
| Pass 4 | native sonnet×1 (the slice B seat; B-EXTRA1 only) | found: 0, new: 0, confirmed: 0, fixed: 0, unexecuted: 0 | method: re-derivation — the row and every cite executed on the pin |

## Dispositions

| ID | Disposition |
|---|---|
| R0-fence | CONFIRMED → FIXED r0 (own probe before dispatch: fields inside a code fence counted) |
| R1-A-S1 | CONFIRMED → FIXED r1 (a trailing example fence was taken as the agent's own footer) |
| R1-A-S2 | RECORDED — by design (D-391) — a `>`-styled header counts: the T03 design admits `> ` as markdown styling; 0 of the 9 exempted corpus turns use it |
| R1-A-H1 | RECORDED — by design (D-391) — the same `>` styling rule applied to the fields |
| R1-B-S1 | CONFIRMED → FIXED r1 (three Stop-row cites 5 lines off) |
| R1-B-H1 | CONFIRMED (dup of B-S1) |
| R1-B-H2 | CONFIRMED → FIXED r1 (the window ends at the first footer line, not "the closing footer") |
| R2-A-S-NEW1 | CONFIRMED → FIXED r2 (a trailing example fence holding a `NEXT:` line still counted as own; now two footer lines are required) |
| R2-B-S3 | CONFIRMED → FIXED r2 (a footer line inside an example fence cut the window) |
| R3-A-S3 | RECORDED — backlog (D-391) — a `>`-quoted footer line inside an escalation ends its window early; 0 of 9 corpus turns; releases at CAP=3; row in docs/STRATEGIC_BACKLOG.md |
| R3-B-EXTRA1 | CONFIRMED → FIXED r3 ("`BLOCKED:` exempts globally" now names the five pattern shapes) |

## Measurement

On 16,278 interactive turn ends (the V1 corpus): 21 deferrals were exempted by a `BLOCKED:` header under the old rule; 9 remain exempt under the final rule, 12 are refused (one an operator question relabelled `BLOCKED: operator decision — …`). An evidence requirement on `searched:` (the DECISION block's `_SEARCHED_EVIDENCE_RE`) was measured and rejected: it refused 15 of 42 honest escalations.

## Gate

`tests/test_final_gate_stop_deferral.py tests/test_final_gate_stop_hook.py tests/test_stop_hook_deferral_exemption.py tests/test_stop_mine.py tests/test_kaizen_hook_emitters.py tests/test_gate_decision_blocks.py` — 399 passed, 2 skipped; ruff check + format clean; mypy clean; `render_doc_script_links.py --check` current; `final_gate.py --check --json` in the closing commit's run.

## Per-phase verdicts

### D-391 BLOCKED format rule: PASS
