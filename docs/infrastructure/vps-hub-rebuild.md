...
- `docker compose up -d` × 16 stacks + image pulls: ~15–25 min
...
| 15b | install boot-resilience unit | `fabrik-compose-boot.service` — root oneshot (`After=docker.service`) running `docker compose up -d` for every `/opt/*/compose.yaml` on boot | Closes the `restart: unless-stopped` reboot race (a container that already exited non-zero when dockerd stopped is NOT resumed — the 2026-07-08 `alertmanager` 4-day outage). Without this a DR-restored hub carries the race. See `scripts/systemd/README.md` |
