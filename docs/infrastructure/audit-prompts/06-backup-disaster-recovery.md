...
**Last Updated:** 2026-06-02 (rewritten — each VPS now has its own Backrest; was hub-only pre-W11 — patched 2026-06-02 evening after live-validation: corrected B2 repo name from `vps1-ocoron-backups` plural to `vps1-ocoron-backup` singular. Prompt itself runs clean; the live state surfaced 3 real defects — hub `restic forget` failing nightly on 3 plans due to lock contention, hub `.restic-password` mode 711 instead of 600, and 1 failed `host-state` op on vps2 — tracked outside this prompt-validation work)
