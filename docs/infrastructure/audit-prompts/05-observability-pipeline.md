...
- 18 Gatus config files, 31 endpoint definitions (was 33 until the `coolify` (core) + `coolify-public` (external) endpoints were removed 2026-06-17; `core` and `external` each dropped by 1). Endpoint count is the stable signal; file count drifts as endpoints are split. Count live with `grep -rcE '^\s*- name:' configs/gatus/`.
