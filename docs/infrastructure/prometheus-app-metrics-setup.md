...
| ~~pushgateway~~ | `http://pushgateway:9091/metrics` | none | **NOT in `prometheus.yml`** (re-verified 2026-07-12: 16 job_names configured — `fabrik-services` is a null-target placeholder, so 15 are active and 20 targets are up — none of them `pushgateway`). The `pushgateway` *container* is still running on vps1, but it is not scraped (no scrape job points at it). The `rules/fabrik-drift.yml` header comment still references "prometheus's `pushgateway` job" as the source of `fabrik_audit_drift_total` (pushed by `audit_all_registrars.py` cron) — a known minor inconsistency: the container runs but isn't scraped, so reconcile the two before relying on the drift alert. |
