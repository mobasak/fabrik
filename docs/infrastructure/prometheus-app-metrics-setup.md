...
**Last Updated:** 2026-07-20 (**pushgateway scrape RESTORED 2026-07-19** — `job_name: pushgateway` live at `configs/prometheus/prometheus.yml:126` scraping `pushgateway:9091`, honor_labels on; **17 `job_name`s configured → 16 active** / the registrar-drift chain (`fabrik_audit_drift_total`, 710 series) is live end-to-end. Supersedes the 2026-07-12 16/15 totals.) **Prior 2026-07-12** (**spoke federation is LIVE** — re-verified `/api/v1/targets`: **16 `job_name`s configured → 15 active jobs / 20 targets / 20 up**, including `node-spokes` / `cadvisor-spokes` / `promtail-spokes` at 2 targets each. This supersedes the 2026-06-07/06-16 notes below that recorded those jobs as absent and the totals as 12 jobs / 14 targets.) **Prior 2026-06-16** (job count corrected to **13 job_names configured** — `fabrik-services` is a null-target placeholder, so 12 active jobs / 14 targets up; clarified that the `pushgateway` *container* is still running on vps1 but is NOT scraped, and that `fabrik-drift.yml`'s header still references the absent `pushgateway` job — a known minor inconsistency. **Prior 2026-06-15:** network rename `coolify`→`fabrik` corrected in cross-mesh scrape path; `pushgateway` scrape job marked NOT-present after verifying live `prometheus.yml` has no such job — see "What's scraped" table. **Prior 2026-06-07:** aro-wake SLI scrape job covers full fleet of vps1+vps2+vps3 via the same `aro-wake` job; cross-mesh container→host NAT path documented below as a worked example. **Two updates 2026-06-07**: `aro_wake_requests_total` counter now has `status="rate_limited"` value alongside `success` + `failure`; `AroWakeLowSuccessRate` alert rule denominator updated from `aro_wake_requests_total` to `aro_wake_requests_total{status!="rate_limited"}` so refused-at-the-gate drops don't unfairly lower the LLM success-rate SLI. The stale `netdata` scrape job is REMOVED from `prometheus.yml` — caused a 24× Telegram flood overnight before removal via the Phase 4 wire's `repeat_interval: 30m`.)
...
| pushgateway | `http://pushgateway:9091/metrics` | none | **Scraped** (`prometheus.yml:126`, `honor_labels: true` — restored 2026-07-19 after a gap; carries `fabrik_audit_drift_total` pushed by the hourly `audit_all_registrars.py` cron, feeding the `FabrikRegistrarDrift` alert in `rules/fabrik-drift.yml`). |
...
> **✅ Truth check RE-VERIFIED 2026-07-12:** the spoke-specific scrape jobs (`node-spokes`, `cadvisor-spokes`, `promtail-spokes`) **ARE live** in `prometheus.yml` (`:46,:58,:70`) — 2 targets each, all up. Live totals (2026-07-20): **17 `job_name`s configured → 16 active** (`fabrik-services` is a null-target placeholder; `pushgateway` restored 2026-07-19). Spoke federation shipped in `8342ef1`, superseding the 2026-06-07 note below that said these jobs were absent. The `aro-wake` job (3 targets across vps1+vps2+vps3 over mesh) adds push-trigger SLI counters on top of the node/container exporters.
...
- `AroWakeLowSuccessRate` — `sum(rate(aro_wake_requests_total{status="success"}[10m])) by (host) / sum(rate(aro_wake_requests_total{status!="rate_limited"}[10m])) by (host) < 0.90` for 15m (warning) — denominator excludes gate-refused drops (`alerts.yml:135-137`)
...
When deployed, these route via **Alertmanager → Telegram (native `telegram_configs`)** + a webhook to `aro-wake` — **not** through Apprise (`configs/alertmanager/alertmanager.yml:58-84`).
