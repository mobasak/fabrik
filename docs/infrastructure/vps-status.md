...
**T-P2 watchdog ships (mostly in `/opt/fabrik-lib/watchdog/`, on `mobasak/fabrik-lib`; rule pack + orchestrator wiring + driver in `/opt/fabrik`):** artifacts 1-13 of 15 — WatchdogConfig + 22 tests (artifact 1), `claude-settings.json.template` (2), `hooks/PreToolUse.sh` (3), `Dockerfile` (4), `requirements.txt` + `llm_client.py` (5), `actions.py` (6), `state.py` (7), `agent.py` + vendored `cost_budget.py` (8), SQLite schema inline in state.py (9), emitter library at `fabrik-lib/watchdog/emitter/` (10), rule pack `.windsurf/rules/core/60-watchdog.md` (11), `_register_watchdog()` registrar wiring in `src/fabrik/orchestrator/infrastructure.py` (12), `src/fabrik/drivers/watchdog.py` image-build + compose-overlay + bring-up driver (13). Sidecar code is ~2,134 lines; driver adds another 387; orchestrator wiring +63. Cross-process emitter→sidecar contract verified by integration smoke (emit 5, read 5, mark processed, next read returns 0); driver verified through 3 dry-run paths (default → dry-run, opt-out → skipped, missing id → WatchdogProvisionError). 40/40 orchestrator tests green after wiring. Remaining 2 artifacts: 14 — fabrik-lib README modules-table row; 15 — test spec opt-in to prove end-to-end.
