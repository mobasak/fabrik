...
**T-P2 watchdog ships (mostly in `/opt/fabrik-lib/watchdog/`, on `mobasak/fabrik-lib`; rule pack in `/opt/fabrik`):** artifacts 1-11 of 15 — WatchdogConfig + tests (artifact 1), `claude-settings.json.template` (2), `hooks/PreToolUse.sh` (3), `Dockerfile` (4), `requirements.txt` + `llm_client.py` (5), `actions.py` (6), `state.py` (7), `agent.py` + vendored `cost_budget.py` (8), SQLite schema inline in state.py (9), emitter library at `fabrik-lib/watchdog/emitter/` (10), and rule pack `.windsurf/rules/core/60-watchdog.md` (11). Sidecar code is at ~2,134 lines total; runs through to a working `python3 agent.py` once env vars are set by the driver (artifact 13, not yet shipped). Cross-process emitter→sidecar contract verified by integration smoke (emit 5, read 5, mark processed, next read returns 0).
