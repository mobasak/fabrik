...
| **Claude quota governance** (2026-08-30) | Every host runs ONE `claude` as **`ob@ocoron.com`** — the sole entrypoint for host crons and containers alike; **the VPS does not rotate accounts** (rotation is WSL-only, and the WSL→VPS credential sync is disabled). `quota_governor.py` routes each call `ob@`/`pool`/`pool-diagnose` from live headroom, `claude-run.sh` is the gate (a shed routine call exits `75`), `claude_broker.py` serves containers completion-only with no creds, and `incident_context.py` escalates a capped incident to a pool diagnosis so **the fix is never dropped**. `QUOTA_RESERVE_PCT=80` on all 3 hosts. Runbook: [`docs/workstation/vps-claude-quota-governance.md`](../workstation/vps-claude-quota-governance.md). |
...
| Per-project watchdog driver | Deployed via `fabrik apply` for any spec with `watchdog.enabled: true` (enabled-by-default for service/worker/wordpress via the dispatcher; the `WatchdogConfig` field defaults `False`). Driver at [`src/fabrik/drivers/watchdog.py`](../../src/fabrik/drivers/watchdog.py); detects host docker.sock GID, mounts `~/.claude` + `~/.claude.json` from `FABRIK_VPS_CLAUDE_HOME`, builds per-project image from `/opt/fabrik-lib/watchdog/watchdog_sidecar/`, writes `compose.watchdog.yaml` overlay. |
...
| iptables | `DOCKER-USER` chain persisted via **`iptables-docker-user.service`** — `enabled` + `active` on both spokes (re-verified live 2026-07-12). Two rules (idempotent, re-applied at boot): `ACCEPT -i wg0` (the mesh is trusted) and `DROP -i ens3 -p tcp --dports 5432,6379,9090,9091,9100,8080,3100,7700,8000` (`FABRIK_MESH_ONLY_PORTS`) — so mesh-only services can never be published to the internet. 22/80/443 are NOT in the drop list, so SSH + Traefik are unaffected. This matters because **Docker bypasses UFW**: a published container port is internet-reachable even when UFW would deny it. Installed by `bootstrap-vps.sh` step_10 (G5) / `bootstrap-spoke-restore.sh` step_07 (G5b); for an already-bootstrapped host use `scripts/sysadmin/install-docker-user-rules.sh <host>`. **History:** a `/fabrik-docs-review` on 2026-07-12 found both spokes had NO such unit and an EMPTY chain (not an exposure then — only Traefik 80/443 were Docker-published — but any future published port would have been unguarded); fixed the same day. No OpenVPN on spokes. UFW remains the host-level firewall. |
...
### 6. AI sysadmin oversight (two AIs — host sysadmin + per-project watchdog; aro-wake is a push *trigger* of the host sysadmin, NOT a third AI — all Phases 1+2+3+4 LIVE on the FULL FLEET as of 2026-06-06)
...
- **Layer 2 — Per-project watchdog sidecars (live on vps1; default-on for new specs):**
  - One sidecar container per opted-in project (`watchdog.enabled: true` in spec — enabled-by-default for service/worker/wordpress specs, computed by the dispatcher from `shape.kind`; the `WatchdogConfig` class field itself defaults `False`).
...
- **A second trigger of the host sysadmin (Layer 1), NOT a third AI — aro-wake push-trigger endpoint** (it spawns the same Claude with the same `system-prompt.txt` as the Telegram bot; trio Phase 3 LIVE on the FULL FLEET since 2026-06-06; Phase 4 wire LIVE on vps1):
