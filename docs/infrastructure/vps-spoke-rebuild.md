...
> - **DR drill (throwaway):** `fabrik vultr drill spoke` — creates a Vultr droplet, runs `bootstrap-vps.sh --skip-mesh --skip-dns` (hermetic — no prod mesh/DNS touch), runs `bootstrap-vps.sh --verify` as the end-state contract, then **always auto-destroys** (even on failure). Live run 2026-06-08: bootstrap_rc=0 + verify_rc=0, 483s, 0 orphans, 0 `vps4` peers left on vps1 wg0. **Re-validated 2026-06-13/14 post-G5/G5b**: bootstrap_rc=0 + verify_rc=0, 528s, 0 orphans — confirms the new `iptables-docker-user.service` (G5) bootstraps cleanly on a fresh droplet.
>   - **`--g0-smoke` (opt-in):** also copies the hub's `~/.claude/.credentials.json` to the throwaway and runs one `claude -p`. 2026-06-13/14 result: `immediate_auth_ok=True`, no token rotation on first use — copying Claude OAuth creds to a fresh host authenticates *immediately* (single-session rejection ruled out). Does NOT exercise the ~4-day refresh-token rotation race.
...
| 09b | verify restored configs (drill) | `step_09b_verify_restored_configs` — runs ONLY in drill mode (`--skip-services` OR `--skip-mesh`); dry-validates the configs the drill skipped, with 4 c-dry checks: **[1]** `/etc/wireguard/wg0.conf` parses (`wg-quick strip`); **[2]** the 3 service `compose.yaml` files (monitoring-agent / traefik / backrest) parse + resolve (`docker compose config -q`); **[3]** `/opt/backrest/.env` (AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY) + `.restic-password` present + non-empty; **[4]** WG identity self-consistency — privkey derives a pubkey that matches the stored `/etc/wireguard/spoke.publickey`, and the hub-peer entry carries PublicKey + Endpoint + AllowedIPs. Exercises restored state without touching live infra. |
| 09c | start core services (drill) | `step_09c_start_core_services_drill` — under `--skip-services` with `--drill-start-core-only`, boots **monitoring-agent only** (`docker compose up -d`) for a runtime check that the restored config actually starts; stays isolated from the live mesh. |
...
- DOCKER-USER chain regenerated by `iptables-docker-user.service` (step_07), **not** restored from `rules.v4` — G5b moved spoke DOCKER-USER persistence off `iptables-persistent`/`rules.v4` to the systemd unit (Ubuntu 24.04 `iptables-persistent` `Conflicts: ufw`). The chain comes back regardless of backup shape.
...
### DR / provisioning validation status (updated 2026-06-15)

What's been exercised on real infra vs. what's deferred-by-design. **All three DR paths are now live-proven.**

- **G5 (fresh-spoke `iptables-docker-user.service`)** — ✅ **live-proven** (drill spoke 2026-06-13/14: bootstrap_rc=0, verify_rc=0, 0 orphans).
- **G5b (`bootstrap-spoke-restore.sh` restore path)** — ✅ **live-proven 2026-06-15** via the new `fabrik vultr drill spoke-restore` (4 drills, 3 bugs, first green: `dr-drill-spoke-restore-20260615-125146`). The drill restores under **vps2's** identity but runs `bootstrap-spoke-restore.sh --skip-mesh --skip-services --skip-local-b2-check`, so the throwaway never handshakes with vps1 (would steal vps2's peer endpoint) and never writes to `spokes/vps2` on B2 — the live spoke is untouched. (Was previously deferred as operator-initiated; the peer AI built the isolated drill.)
- **Hub DR (G9, `bootstrap-hub.sh`)** — ✅ **live-proven 2026-06-15** (drill #6 sweep, 9 drills, 6 bugs, first green: `dr-drill-hub-20260615-111639`). Same isolation flags. Details in [`vps-hub-rebuild.md`](vps-hub-rebuild.md).
