...
**Last Updated:** 2026-06-04 evening (Trio Phase 1+2+3 SHIPPED — three veteran-sysadmin model deployable; canonical prompt restored to watchdog sidecar; bootstrap step_14 + step_15 ship sysadmin pack + aro-wake push-trigger to spokes; port 8201 allocated for aro-wake)
...
| 7 | **aro-wake push-trigger endpoint (trio Phase 3 — code shipped 2026-06-04)** | systemd-managed FastAPI on each host. `POST /wake` accepts `source=consult` (peer asks "what do you see?") or `source=manual` (operator curl); spawns Claude with this host's veteran-sysadmin prompt; returns the result. Bound to wg0 mesh IP `:8201` only — never `0.0.0.0`. Thread-safe rate limiter (20/h per `(source, topic)`) + per-(source,topic) Claude session reuse for warm prompt cache (effective `session_id` stored from envelope, not local sid, so future `--resume` targets the right session). Defensive envelope parse handles both single-dict and event-list Claude output shapes. Pending-queue at `/var/lib/aro-wake/pending.jsonl` (24h TTL, 1000-entry cap) for failed cross-host forwards; mesh-recovery hook drains. `ARO_WAKE_PEER_HOSTS` env var uses CSV format (`vps1:10.99.0.1,vps3:10.99.0.3`) NOT JSON — systemd's bare `Environment=KEY={"...":"..."}` strips embedded double-quotes, which would have crashed uvicorn at startup; verified via systemd round-trip 2026-06-04 batch-3. **Response shape (consult):** `{ok, from_host, trace_id, seen_by, view, correlation, no_action}` — matches peer-protocol.md §2.1 spec; `[<host>]` Telegram-prefix the canonical prompt instructs is stripped from `view` before return so peer consumers see clean text. **Response shape (manual/future-alertmanager):** `{ok, from_host, trace_id, seen_by, result, no_action: false}` — unstructured `result` field for non-consult sources. | On push (consult / future Alertmanager webhook) | **Code shipped, not yet enabled live.** Each host gets its own aro-wake via `bootstrap-vps.sh step_15_install_aro_wake`. Operator-gated: `claude auth login` on the host + `sudo systemctl enable --now aro-wake.service`. Smoke: `curl http://10.99.0.<N>:8201/health → {"ok":true,"host":"vpsN",...}`. **Known limitation (Phase 4 work):** Claude-only — no OpenRouter fallback (the watchdog sidecar's fallback chain in `fabrik-lib/watchdog/sidecar/llm_client.py` is not shared with aro-wake). If Claude primary returns non-zero, `/wake` returns 503. Acceptable for Phase 3 consult-only (caller's 5s timeout falls through to annotated "peer unreachable" report); Phase 4 will lift the chain to aro-wake since push-trigger reliability matters more once Alertmanager wires in. |
| 8 | **Per-host AI sysadmin (trio Phase 2 — code shipped 2026-06-04, deploy operator-gated)** | Same `vps-sysadmin-bot.service` + `/etc/cron.d/vps-sysadmin` pack vps1 has, deployed to vps2 + vps3 via `bootstrap-vps.sh step_14_install_sysadmin_pack`. Hash-stable cron-minute slots per host (sha1sum of HOST_NAME mod 30 + mod 60) so digest + keepalive don't fire concurrently across the fleet. Each host's bot reads `/opt/fabrik/.env.sysadmin` for its own Telegram bot token (three @BotFather bots — operator action) and the canonical system prompt with `{{ HOST_NAME }}` etc. rendered for THAT host. | Telegram-driven Q&A + hash-staggered cron | **Code shipped, not yet enabled live.** Same operator gates as #7. |
...
> **Today (live): AI auto-action covers (a) ALL Prometheus signals across vps1+vps2+vps3 via the 15-min `proactive-check.sh` cron on vps1 — limited to vps1-side `sudo docker` actions; (b) one project's container-state + log triggers on vps1 via the per-project watchdog sidecar (T-P5 dogfood, default-on for future specs); the watchdog now runs the canonical 353-line veteran-sysadmin prompt + dispatch contract (trio Phase 1.3). Code-shipped, operator-gated for deploy: (c) per-host AI sysadmin pack on vps2 + vps3 via `bootstrap-vps.sh step_14`; (d) aro-wake push-trigger endpoint on every host at `10.99.0.<N>:8201` via `step_15`, supporting `consult` verb with thread-safe rate limiter + 24h pending queue + mesh-recovery drain. Remaining gaps: Alertmanager → aro-wake wire (Phase 4), Apprise/Loki ruler/propose-ack (Phase 5).**
