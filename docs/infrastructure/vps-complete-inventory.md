...
| Backrest failure → AI | Webhook URL stale (Known Issue 1); even if fixed, only routes to Telegram. **`bootstrap-vps.sh step_14` now includes the one-shot sed fix during install — applies on next spoke bootstrap.** | After step_14 lands the fix on a host, point the Apprise hook at aro-wake instead of telegram. Phase 5 deferred. |
