...
**Last Updated:** 2026-06-05 (Trio Phase 1+3+4 LIVE on vps1 — aro-wake systemd service active, peer-protocol consult verified end-to-end via synthetic vps3→vps1 call, Alertmanager → aro-wake wire applied + reloaded clean, two REAL alerts caught through the new path within seconds. Phase 2 spokes still operator-gated on bot tokens + claude auth.)
...
> **Today (LIVE on vps1, 2026-06-05): AI auto-action covers (a) ALL Prometheus signals across vps1+vps2+vps3 via the 15-min `proactive-check.sh` cron on vps1; (b) one project's container-state + log triggers on vps1 via the per-project watchdog sidecar; the watchdog now runs the canonical 354-line veteran-sysadmin prompt + dispatch contract (trio Phase 1.3); (c) aro-wake push-trigger endpoint on vps1 at `10.0.1.1:8201` / `10.99.0.1:8201` accepts peer `consult` + Alertmanager webhooks + manual ops — verified end-to-end with synthetic vps3→vps1 consult + REAL ContainerHighMemory alert caught within seconds (trio Phase 3 + 4); (d) Alertmanager config wired: every firing alert with severity=critical|warning routes to aro-wake-routed receiver FIRST (continue: true preserves the existing telegram fallback). Code-shipped, operator-gated for spoke deploy: (e) per-host AI sysadmin pack on vps2 + vps3 via `bootstrap-vps.sh step_14`; (f) aro-wake on spokes via `step_15`. Remaining Phase 5 work: Apprise pre-route / Loki ruler integration / `propose`+`ack` peer-protocol verbs. Per-host vps2+vps3 aro-wake needs `claude auth login` + `bootstrap-vps.sh --spoke vpsN` + `systemctl enable --now aro-wake.service`.**
