...
**Last Updated:** 2026-07-13 (`/fabrik-docs-review` — watchdog + aro-wake reconcile: `WatchdogConfig.enabled` class-default is **False** (dispatcher computes True for service/worker/wordpress), not True; `deadman_timeout_seconds` default **300s**, not 120s; **18** WatchdogConfig fields (not 13); driver **~1284 lines** (not 387); `watchdog/sidecar/` → `watchdog_sidecar/`; aro-wake **5** HTTP routes (not 3) + **9** metrics (not 8); Alertmanager→aro-wake Phase 4 **shipped on vps1** (was "not wired yet"). Prior same-day: health/security re-audit: site-provisioner flap noted; spoke UFW rule-count clarified — the live 8201 + mesh-allow rules are mesh-scoped, no public exposure.) **Prior:** 2026-07-12 (`/fabrik-docs-review` reconciliation vs live fleet: Alertmanager sends to Telegram **natively** — it does NOT route via Apprise; Prometheus is **15 active jobs / 20 targets / 20 up** with spoke federation LIVE; Backrest hook + Apprise `alerts` config **fixed** (Apprise was 204-ing every alert); `fabrik-compose-boot.service` reboot-race unit added fleet-wide; spoke `DOCKER-USER` drift recorded.) **Prior:** 2026-06-16 (Trio Phase 1+2+3+4 LIVE across the FULL FLEET since 2026-06-06; **Phase 5.1.a operator-reversal cron LIVE on full fleet 2026-06-07** via `detect_reversals.py` + `*/5 min` cron; **rate-limited 429 wakes now tracked** via `aro_wake_requests_total{status="rate_limited"}` + `AroWakeLowSuccessRate` denominator excludes them; **stale netdata scrape job removed** (caused 24× Telegram flood overnight); **6 bootstrap defenses shipped** including preflight SSH-user-transition trap detection; **fresh-spoke install MEASURED 2026-06-07**: `bootstrap-vps.sh --skip-mesh --skip-dns` on Vultr → 3m 13s wall-clock, 15/15 substantive end-state checks — this measures the FRESH-INSTALL path, NOT DR restore; **fresh-spoke drill proven live 2026-06-13/14**; **the actual DR-restore paths — `bootstrap-spoke-restore.sh` (spoke) + `bootstrap-hub.sh` (hub) — were validated live across hub + spoke + spoke-restore drills 2026-06-15/16** (all green; LE/DNS cutover exercised via the `tojlo.com` sandbox domain). See the `fabrik vultr drill` subsystem below.)
...
- **DNS state (post-residue-cleanup, evening; updated 2026-06-17 after the 2 vps4 orphans were deleted):** 18 A records in CF zone `ocoron.com`. vps1 has 13 live subdomains (auth, auto, backup, browser, errors, monitor, notify, pdf, **provision** [new — was never in CF; site-provisioner had a Traefik router but no public DNS — created during the cleanup], search, status, **watchdog-test** [live router], vps1 apex). vps2 + vps3 each have apex + wildcard. **6 stale subdomains deleted** (`coolify`, `control`, `dns`, `fabrik-e2e-timing`, `images`, `netdata`).
...
`site-provisioner` is running on vps1 at `provision.vps1.ocoron.com` (container up, alembic migrations applied, Cloudflare + Postgres connectivity verified; `/health` returns 200 but **flaps** — its live CF+PG+Namecheap probe occasionally exceeds the 10s healthcheck timeout and self-recovers; app-level `RuntimeError: No response returned` (~36×/24h) is a `mobasak/site-provisioner`-repo concern) — but **do not treat it as a production-deployable service yet**. It was brought up manually this afternoon to unblock the spoke DNS step. The proper `fabrik apply` pipeline for it has known gaps that must close before redeploy via fabrik is safe:
...
| `apprise` | 768m | Notification dispatcher — fed by **Gatus + the sysadmin scripts + the watchdog driver** via `POST /notify/alerts`, **not** by Alertmanager — gunicorn workers 2 |
...
├── watchdog-test/   — T-P5 dogfood stack (nginx + watchdog sidecar; compose.yaml + compose.watchdog.yaml)
...
UFW was installed + enabled by W1 of the fleet-hardening plan on 2026-05-31 evening. Package status `ii` (installed, version `0.36.2-6`), service active, default policy `deny (incoming) / allow (outgoing) / deny (routed)`. That probe recorded 8 ALLOW rules (4 IPv4 + 4 IPv6); the live spokes now also carry 2× `8201` mesh/bridge allows + a `from 10.99.0.0/24` mesh-allow (added 2026-06-05/06 for aro-wake) — all mesh-scoped, no public exposure (re-verified live 2026-07-13). Verified via [`probe-reports/infra-probe-2026-06-07T20-20Z.yaml`](probe-reports/infra-probe-2026-06-07T20-20Z.yaml) (W6) plus W5's rule-by-rule IPv4↔IPv6 mirror check (2026-06-01T00:43Z).
...
- Apprise webhook used for custom routes — e.g. Backrest failure hooks target `http://apprise:8000/notify/alerts` (**working**: live configs use the stable `apprise` hostname, and the Apprise `alerts` stateful config was created 2026-07-12 — before that Apprise answered `204` and silently dropped every alert)
...
**RESOLVED 2026-07-12 (was Issue 1):** plan failure hooks in `/opt/backrest/config/config.json` now use the stable `apprise` hostname (`http://apprise:8000/notify/alerts`) — the Coolify-era UUID-suffix name is gone from every live config. Separately, Apprise had no `alerts` stateful config until 2026-07-12, so it answered `204` and silently discarded these hooks; that is fixed and the path now delivers.
...
| `image-broker` | yes (regenerated `5caaa23b`) | no (was stale) | no | no | id=66 (orphaned; can be deleted in GlitchTip UI) |
| `translator` | yes | no | no | no | yes (id=67) |
| `proxy` (fabrik-proxy) | yes | no | no | no | — |
| `emailgateway` | no (deleted `a1749dd5`) | no | no | no | yes (id=68) |
| `file-api` | no (deleted `a1749dd5`) | no | no | no | yes (id=69) |
| `file-worker` | no (deleted `a1749dd5`) | no | no | no | yes (id=70) |
...
| 2 | Stand up `site-provisioner` on vps1 so spoke DNS work could proceed | High | Was blocked on #1 | ⚠ partial — interim manual stand-up running (healthcheck **flaps** — self-recovering 10s-timeout on the live `/health` probe; see "site-provisioner status" earlier); `fabrik apply` pipeline NOT yet ready |
...
### Issue 1: Backrest failure-notification webhook — RESOLVED 2026-07-12

Live `/opt/backrest/config/config.json` hooks now emit `http://apprise:8000/notify/alerts` (stable hostname; the Coolify-era `apprise-<uuid>` name is gone fleet-wide — a live probe on 2026-06-04 already found zero `apprise*` UUID refs). A **second, deeper** break was found and fixed on 2026-07-12: Apprise had no `alerts` **stateful** config, so it answered every `POST /notify/alerts` with `204 No Content` — accepted, nothing sent — silently discarding Backrest hooks, Gatus alerts and the sysadmin scripts alike. The `alerts` config now exists and the path returns `200`. See `docs/TROUBLESHOOTING.md` § "Alerts are silently never delivered".
...
### Issue 3: `/opt/prometheus/compose.yaml` stale leftover — RESOLVED

`/opt/prometheus/` has been removed (verified live 2026-07-12: the directory no longer exists). Real Prometheus runs from `/opt/monitoring/compose.yaml`.
...
| 4 | **Per-project watchdog sidecar — poll loop** | `<project>-watchdog` container running `agent.py` on a 60s loop; rule-based `detect_anomalies()` (container-state + log triggers) → Claude Code Opus diagnose → Tier A action (`restart_container` / `clear_redis_cache` / `rotate_logs`) | Every 60s | **One project (`watchdog-test`) on vps1.** The dispatcher computes watchdog-ON for `service`/`worker`/`wordpress` kinds (`WatchdogConfig.enabled` class-default is False; `spec_loader.py:379-386`), so those kinds get a sidecar unless opted out. Per-project `state.db` isolates blast radius. |
...
| :--- | :--- | :--- |
| Alertmanager → aro-wake (trio Phase 4) | **SHIPPED on vps1 2026-06-05.** The `aro-wake-routed` `webhook_configs` receiver is wired in `alertmanager.yml` (route matches `severity=~"critical\|warning"`, `continue: true` so the native Telegram path stays); a critical/warning alert reaches aro-wake within seconds. Alertmanager runs hub-only, so this is a vps1 wire (spokes have no local Alertmanager). | Done on vps1 (verified via `amtool` synthetic + a real `ContainerHighMemory` alert). |
...
A **second** AI layer distinct from the host-level sysadmin above. The host sysadmin watches the platform; per-project watchdog sidecars watch one tenant each. With T-P2 complete, every service/worker/wordpress spec (enabled-by-default via the dispatcher — the `WatchdogConfig` field itself defaults `False`; opt out with `watchdog: { enabled: false }`) will gain a `<project_id>-watchdog` container at `fabrik apply` time — the registrar resolves the applicability, the driver builds the per-project image from `/opt/fabrik-lib/watchdog/watchdog_sidecar/`, writes `compose.watchdog.yaml` next to the spec's `compose.yaml`, and brings the sidecar up via `docker compose -f compose.yaml -f compose.watchdog.yaml up -d watchdog`.

| Layer | Artifacts | Where | Lines |
| :--- | :--- | :--- | :--- |
| **Sidecar code** (in-container) | claude-settings template + PreToolUse hook + Dockerfile + llm_client + actions + state + agent + vendored cost_budget + (inline) SQLite schema | [`/opt/fabrik-lib/watchdog/watchdog_sidecar/`](../../../fabrik-lib/watchdog/watchdog_sidecar/) | ~2,134 |
| **Orchestrator wire-up** | `_register_watchdog` resolver + dispatch in [`infrastructure.py`](../../src/fabrik/orchestrator/infrastructure.py) + ~1284-line driver at [`drivers/watchdog.py`](../../src/fabrik/drivers/watchdog.py) (grew with Tier-D bootstrap, governance mount, deploy-key auto-push) | `/opt/fabrik/src/fabrik/` | ~1284 + 63 |
| **Spec field** | `WatchdogConfig` Pydantic class with 18 fields + 22 tests | [`src/fabrik/spec_loader.py`](../../src/fabrik/spec_loader.py) + [`tests/test_spec_loader.py`](../../tests/test_spec_loader.py) | ~290 |
...
Default applicability: the dispatcher enables the sidecar for `service`/`worker`/`wordpress` kinds (class-default False in `spec_loader.py:379-386`; kind-computed at dispatch) unless the operator sets `watchdog: { enabled: false }`. Shape-kind-driven recommendation (operator discipline, NOT encoded in the registrar) lives in [`.windsurf/rules/core/60-watchdog.md`](../../.windsurf/rules/core/60-watchdog.md) — on for `service`/`worker`/`wordpress`; off for `static-site`/`docusaurus`. Hub-side P1 plumbing (`fabrik_analytics` DB + `cost_ledger` table) is live and consumed by the sidecar's vendored `cost_budget.py`.

**Phases shipped 2026-06-03:** T-P3 (`core/self-healing.md` rule pack — 8-row escalation ladder + 5 anti-patterns + signup-flood worked example) and T-P4 (universal-coverage overlay into `02-epic-decomposition-command.md` (then under docs/traycer/, since superseded by docs/orchestrator/mega-epic-breakdown/02-epic-decomposition-fabrik.md) via 12 surgical edits totaling ~84 lines, plus 1-line sync to `03-expand-epic-files-command.md`'s Metadata block). Both subplans archived.
...
- Gatus: **31 endpoints across 18 config files** (counted from source-controlled `configs/gatus/` 2026-06-17 — Gatus configs are now in git, so this is repo-verifiable, not a live-only probe). Top-level breakdown: `apps/` 13 files / 16 endpoints; `core/` 1 file / 4 endpoints; `external/` 1 file / 4 endpoints; `observability/` 1 file / 4 endpoints; `data/` 1 file / 3 endpoints; `_base.yaml` + `apps/services.yaml` carry 0 endpoints. (Was 33 before the `coolify` (core) + `coolify-public` (external) endpoints were removed 2026-06-17.) Count it any time with `grep -rcE '^\s*- name:' configs/gatus/`. Some `apps/` files probe services no longer deployed — see "Stale/residue" section below.
...
- Prometheus: **17 `job_name`s configured / 16 active** (`fabrik-services` null-target placeholder; `pushgateway` restored 2026-07-19). Repo re-verified 2026-07-20; prior live probe 2026-07-12 showed 20/20 targets up.
...
| :--- | :--- | :--- |
| DNS | Deleted 6 stale subdomains via CF API (`coolify`, `control`, `dns`, `fabrik-e2e-timing`, `images`, `netdata`) | 17 A records remain (was 22 + 1 added = 23 then –6 = 17) (now 18 after the 2026-06-17 vps4-orphan deletion) |
...
- Lessons learnt (latest = 81): `docs/LESSONS_LEARNT.md`
