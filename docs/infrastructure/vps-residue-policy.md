...
2. **Orphan DNS `*.vps4.ocoron.com` + `vps4.ocoron.com`** — both → `45.77.68.63`, leftover DR-drill residue with **no live vps4 droplet** (all Vultr instances destroyed). These are the "records pointing to destroyed services" the DNS scanner (#7) flags; delete both A records via the Cloudflare API.
