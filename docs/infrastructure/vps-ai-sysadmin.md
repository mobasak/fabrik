...
| **Per-project watchdog sidecar** (T-P2 watchdog platform) | One sidecar **container** injected next to every project app at `fabrik apply` time | One project's health: its own containers, its own DB, its own cost-budget | Per-project; lives as long as the project's compose stack does | Inherits host OAuth via a mount of the VPS user's `~/.claude/` (path configurable via `FABRIK_VPS_CLAUDE_HOME`, defaults `/home/ozgur/.claude`); can also fall back to OpenRouter API key | ✅ **T-P2 COMPLETE — 15 / 15 artifacts shipped 2026-06-03.** Sidecar (`/opt/fabrik-lib/watchdog/sidecar/` ~2,134 lines) + orchestrator wiring (`infrastructure.py` registrar + `drivers/watchdog.py` 387-line driver) + operator surface (emitter library, rule pack `60-watchdog.md`, fabrik-lib README row, test spec `specs/services/watchdog-test.yaml`). End-to-end verified through the spec_loader → resolver → driver chain. Next: T-P3 (self-healing synthesis, 1 day, no subplan needed per parent plan). Subplan: [`docs/development/plans/2026-05-30-ai-watchdog-platform-P2-subplan.md`](../development/plans/2026-05-30-ai-watchdog-platform-P2-subplan.md). |
