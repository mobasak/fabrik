...
Per-project enablement is controlled by the `watchdog:` block in each spec. ⚠️ **The default differs by code path, and they disagree.** `WatchdogConfig.enabled` defaults to **False** (`spec_loader.py:428`), but the apply path never constructs that class — it reads raw YAML and falls back to **True** (`orchestrator/infrastructure.py:344`, `watchdog_cfg.get("enabled", True)`). So `fabrik apply` provisions a sidecar for a spec that omits the block, while `fabrik plan` / `audit` / the destroyer go through `model_dump()` and see `enabled: false`. There is NO shape-kind defaulting anywhere — the `_register_watchdog` dispatcher that `spec_loader.py` cites three times (:414, :417, :812) does not exist, and the shape-kind matrix is operator discipline, not encoded behaviour. Filed as a code defect; the comment at `infrastructure.py:337` asserting "WatchdogConfig.enabled defaults to True" is itself wrong and is how this propagated.
