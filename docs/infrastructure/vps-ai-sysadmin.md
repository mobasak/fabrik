...
Per-project enablement is controlled by the `watchdog:` block in each spec. **A spec that OMITS the block gets a sidecar** — the default is `True` on both code paths as of 2026-09-03 (D-108). It was not always: `WatchdogConfig.enabled` defaulted to **False** while the apply path, which never constructs that class, read raw YAML and fell back to **True**, so `fabrik apply` provisioned a sidecar for a spec that `fabrik plan` / `audit` / the destroyer called not-applicable — and the destroyer replays on the model-dump side, so a sidecar apply created could outlive its stack. 34 of 72 fleet specs omit the block and were all in that gap. The operator's ruling (D-108) made the MODEL match apply: no provisioning changed, and plan/audit/destroy now report what is actually created. There is still NO shape-kind defaulting anywhere — the `_register_watchdog` dispatcher `spec_loader.py` cites three times was never built, and the shape-kind matrix in `core/60-watchdog.md` remains operator discipline. Filed as mail 01M1G851DWCX35T5NSQXADQW0H; the comment at the watchdog gate that asserted the wrong default (and is how this propagated here) was corrected the same day.
...
When deployed, Prometheus rule alerts route via **Alertmanager → Telegram (native `telegram_configs`)** + a webhook to `aro-wake` — **not** through Apprise (`configs/alertmanager/alertmanager.yml:58-84`). Apprise serves Gatus, the sysadmin scripts and the watchdog (`POST /notify/alerts`).
