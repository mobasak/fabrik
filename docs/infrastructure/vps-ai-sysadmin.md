...
| **Per-project watchdog sidecar** (T-P2 watchdog platform) | One sidecar **container** injected next to every project app at `fabrik apply` time | One project's health: its own containers, its own DB, its own cost-budget | Per-project; lives as long as the project's compose stack does | Inherits host OAuth via a mount of the VPS user's `~/.claude/` (path configurable via `FABRIK_VPS_CLAUDE_HOME`, defaults `/home/ozgur/.claude`); can also fall back to OpenRouter API key | ⏳ **T-P2 in progress — 13 / 15 artifacts shipped (last bump 2026-06-03).** Shipped: WatchdogConfig spec field + 22 tests (1), claude-settings template + 10-capability v1 lock (2), PreToolUse hook (3), Dockerfile (4), llm_client + requirements (5), actions.py with 6 Tier A + 3 Tier B + 1 Tier C handlers (6), state.py with 4 tables (7), agent.py + vendored cost_budget (8), schema inline in state.py (9), emitter library at `fabrik-lib/watchdog/emitter/` (10), `.windsurf/rules/core/60-watchdog.md` rule pack (11), `_register_watchdog()` registrar wiring + 3 docstring matrix rows (12), `drivers/watchdog.py` 387-line image-build + compose-overlay + bring-up driver (13). Remaining 2: `fabrik-lib/README.md` modules-table row (14), test spec opt-in (15). Subplan: [`docs/development/plans/2026-05-30-ai-watchdog-platform-P2-subplan.md`](../development/plans/2026-05-30-ai-watchdog-platform-P2-subplan.md). |
