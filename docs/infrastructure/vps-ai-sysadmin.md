...
**Last Updated:** 2026-06-04 evening (Trio Phase 1+2+3 SHIPPED code-side — canonical prompt restored to watchdog sidecar; bootstrap step_14 + step_15 ship the sysadmin pack + aro-wake push-trigger to spokes; three-veteran-sysadmin model deployable, operator-gated on Telegram tokens + spoke claude auth)
...
| **aro-wake push-trigger endpoint** (trio Phase 3, 2026-06-04) | systemd-managed FastAPI on each fleet host, bound to wg0 mesh IP `:8201`. Sleeps when idle (~60MB RAM); spawns Claude on POST `/wake`. | Push-triggered: peer consult (LIVE) + future Alertmanager webhook (Phase 4) + manual ops curl | One systemd unit per host; spawns Claude Code subprocess per wake | Same per-host OAuth as host-level sysadmin | ✅ Code shipped 2026-06-04. Operator-gated for live enable. Smoke: `curl http://10.99.0.<N>:8201/health`. Peer protocol: `consult` is the only LIVE verb; `propose`/`ack` deferred to Phase 5 (cross-host destructive bridges via operator Telegram `reply "go"` until then). |
