...
| **Per-project watchdog sidecar** (T-P2 watchdog platform) | One sidecar **container** injected next to every project app at `fabrik apply` time | One project's health: its own containers, its own DB, its own cost-budget | Per-project; lives as long as the project's compose stack does | Inherits host OAuth via a mount of the VPS user's `~/.claude/` (path configurable via `FABRIK_VPS_CLAUDE_HOME`, defaults `/home/ozgur/.claude`); can also fall back to OpenRouter API key | ✅ **T-P1+T-P2+T-P3 complete (2026-06-03).** Sidecar (`/opt/fabrik-lib/watchdog/sidecar/` ~2,134 lines) + orchestrator wiring (`infrastructure.py` registrar + `drivers/watchdog.py` 387-line driver) + operator surface (emitter library, rule pack `60-watchdog.md`, fabrik-lib README row, test spec `specs/services/watchdog-test.yaml`, self-healing rule pack `core/self-healing.md`). End-to-end verified. ⏳ **T-P4 sub-plan staged 2026-06-03** at [`docs/development/plans/2026-06-03-watchdog-P4-subplan.md`](../development/plans/2026-06-03-watchdog-P4-subplan.md) — owner-confirmed; 14 edits to `02-epic-decomposition-command.md` (~143 inserted lines) + 1 line to `03-expand-epic-files-command.md`. Code edits not yet performed (one-edit-per-turn discipline). Next: T-P5 (dogfood E2E) after P4. |
