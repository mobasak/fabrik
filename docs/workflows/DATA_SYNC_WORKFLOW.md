...
| **Steps** | `scripts/gather_envs.py --apply` (reads every `/opt/*/.env` except the hub's; writes `secrets/all-envs.env`, chmod 600, gitignored, `#svc`-annotated per provider) → `scripts/classify_services.py --apply --tombstone-unresolved --max-per-run 10` (web-grounds the uncatalogued providers, bounded) → `gather_envs.py --apply` again → `scripts/registry_sync.py --fetch-credits` (upserts `services` + `api_keys` into the local Postgres registry, SHA-256 of each secret, never the value) → `scripts/gen_dashboard.py` → `external-services-dashboard.html`. Every later step runs only while `core_failed` is still 0 — set by a failing `gather_envs`, `gather_envs_reconsolidate` or `registry_sync` — never by `classify_services`, and never by a `registry_sync` exit 3 (registry written, only the credit fetch failed: that alerts and the dashboard still runs): a failed first scan skips classify, the reconsolidate pass, `registry_sync` and the dashboard in one shot; a failed reconsolidate or registry sync skips only the steps still ahead of it |
| **Heartbeat** | the stamp file `.tmp/external-services/chain-heartbeat` — the `evidence.path` of the `external-services-chain` entry in `.fabrik/liveness-registry.json`, aged by `scripts/sysadmin/liveness_audit.py` — written only after every data step succeeded (a `registry_sync` exit 3 counts as succeeded: the registry is committed, only the credit fetch failed) AND the `gen_dashboard` step itself exited 0 — the write sits inside that step's success branch (30 h budget) |
...
| **Heartbeat** | `scripts/kilo-benchmarks/cache/daily_refresh_last_success.txt`, read by `check_daily_refresh_freshness.py` (alert when >36 h stale) — a different stamp from the external-services chain's (§ 1.3) | same |
