...
| `docs/PROJECT_CATALOG.md`, `PORTS.md`, `docs/reference/technology-stack-decision-guide.md` — 3 of the 7 REFERENCE_DOCS — plus `docs/reference/prebuilt-app-containers.md` (in no manifest list) | `docs/reference/opt-project-catalog.md`, `PORTS.md`, the same two names. The `PORTS.md` copy is overwritten at once by the inline-generated one, and the sync thereafter WARN-skips it as destination-newer; the remaining 4 REFERENCE_DOCS arrive with the sync |
