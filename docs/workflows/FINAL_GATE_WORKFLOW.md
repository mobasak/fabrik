...
|---|---|---|
| `⚠ this green SKIPPED n test(s) [and DESELECTED m test(s)] and asserts NOTHING about them …` (`skip_advisory`, `final_gate.py:196-236`) | a GREEN run whose pytest summary shows skips or deselections — the LAST summary line is read, since inner pytest runs printed by graders carry their own counts | the green is bounded: n + m tests did not execute. The counts exist ONLY in this ⚠ text tail (they reach `--json` `warnings` because of the prefix); no structured key carries them — a consumer cannot machine-assert "this green skipped n" today (recorded, owner infra) |
| `⚠ this suite STOPPED at the first failure (\`-x\`) … see the whole list with python -m pytest tests/ -q (no -x)` (`final_gate.py:1317-1327`) | a RED run that pytest ended with `stopping after 1 failures` | the failure you see is the FIRST, not the only one — `-x` is hardcoded (`:1273`, a deliberate cost decision recorded at `:1317-1319`); run the suite without `-x` before reasoning about "the" failure |
| `this repo's CI does not invoke pytest, so the gate does not either — PERMANENT … THIS GREEN ASSERTS NOTHING ABOUT THE TEST SUITE …` or `no src/, tests/ or scripts/ changes in this diff` (`final_gate.py:1338-1350`) | the leg did not run | the row is `pytest (NOT RUN)`, a warn-only name (`WARN_ONLY_CHECKS`, `:327-340`): its NAME lands in `skipped_checks`, its whole TEXT lands in `advisory` (`:3023-3027`), and NOTHING lands in `warnings` (that list takes only ⚠-prefixed output, `:3058-3062`) — read `advisory` for the reason |
| (no line) | the interpreter | the gate resolves its OWN interpreter — `.venv/bin/python` when it exists, else `sys.executable` (`final_gate.py:70,79`) — and never inherits the shell's; a dependency missing from THAT interpreter is a collection ERROR at rc 2 that aborts the run (never a silent green), unless the import is guarded (`pytest.importorskip`, a conftest `except → skip`), which yields skips the first row above reports |
