...
- **Work items (sync)** - `scripts/work.py sync --check` *(registered `advisory=True`, not `warn_only` — the row CAN fail)*
  - Prints the spec/plan ↔ work-store drift (`DRIFT <class> (blocking|advisory)  <path>`) and appends one reading to `fabrik-work/readings.jsonl` under the git common dir (outside the tree); exits 1 on drift classes 2-6 only once the repo shows 7 consecutive clean daily readings after its `migrated_at` — before that the row is green and carries the drift text
  - No `scripts/work.py` → the `⚠ check not present, skipping` row; no `.fabrik/work/` store → the one-line `no work store in <repo> — nothing to sync`, green
...
- `scripts/work.py sync --check` — Work items (sync) (Tier 2; ADVISORY row)
