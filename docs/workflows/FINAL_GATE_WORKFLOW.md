...
- **Work items (sync)** - `scripts/work.py sync --check` *(appended by `_work_sync_row()`, 30 s timeout `TIMEOUTS["work_sync"]` — the row CAN fail)*
  ...
  - Fails open, visibly: no `scripts/work.py` → the `⚠ check not present, skipping` row; a timeout, an old work.py without `sync`, a work.py error or a traceback → `⚠ Work items (sync) could not run (<exit N | timed out>): <first line>`, green; no `.fabrik/work/` store → the one-line `no work store in <repo> — nothing to sync`, green
