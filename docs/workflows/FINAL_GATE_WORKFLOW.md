...
one of the three `_why` reasons (`:1336-1350`) → `pytest (NOT RUN)`; exit 5 → the separate `pytest (NO TESTS COLLECTED)` row (`:1280-1283`). Via the CLI a missing pytest never reaches these rows — the `REQUIRED_TOOLS` probe aborts at `status: "setup-error"` (`:2877-2893`) first (pytest must import under the selected interpreter, ruff must resolve as a binary, `:80`), which leaves the `pytest (NOT RUN) — pytest is not installed` branch at `:1276-1278` unreachable.
...
|---|---|---|
| `⚠ this green SKIPPED n test(s) [and DESELECTED m test(s)] and asserts NOTHING about them …` (`skip_advisory`, `final_gate.py:196-240`) | a GREEN run whose pytest summary shows skips or deselections — the LAST summary line is read, since inner pytest runs printed by graders carry their own counts | the green is bounded: n + m tests did not execute. The counts exist ONLY in this ⚠ text tail (they reach `--json` `warnings` because of the prefix); no structured key carries them — a consumer cannot machine-assert "this green skipped n" today (recorded, owner infra) |
| ``⚠ this suite STOPPED at the first failure (`-x`) … see the whole list with python -m pytest tests/ -q (no -x)`` (`final_gate.py:1317-1327`) | a RED run whose pytest output carries the early-stop marker `stopping after ` (`:289`) — a 900s timeout or an exit-4 `SUITE REFUSED` red carries no such line | the failure you see is the FIRST, not the only one — `-x` is hardcoded (`:1273`, a deliberate cost decision recorded at `:1317-1319`); run the suite without `-x` before reasoning about "the" failure |
| one of three reasons — `no tests/ directory in this repo`, `this repo's CI does not invoke pytest, so the gate does not either — PERMANENT … THIS GREEN ASSERTS NOTHING ABOUT THE TEST SUITE …`, or `no src/, tests/ or scripts/ changes in this diff` (`final_gate.py:1336-1350`) | the leg did not run (a `--lean`, `--systemic` or docs-only Tier-2 run emits NO pytest row at all — `:990`, `:1106`, `:1007`) | the row is `pytest (NOT RUN)`, a warn-only name (`WARN_ONLY_CHECKS`, `:327-340`): its NAME lands in `skipped_checks`, its whole TEXT lands in `advisory` (`:3023-3027`), and NOTHING lands in `warnings` (that list takes only ⚠-prefixed output, `:3058-3062`) — read `advisory` for the reason |
| (no line) | the interpreter | the gate resolves its OWN interpreter — `.venv/bin/python` when it exists, else `sys.executable` (`final_gate.py:70,79`) — and never inherits the shell's; a dependency missing from THAT interpreter is a collection ERROR at rc 2 that aborts the run (never a silent green), unless the import is guarded (`pytest.importorskip`, a conftest `except → skip`), which yields the ⚠ skip row above when other tests still collect — or, when the guard empties collection, pytest exit 5 and the separate GREEN `pytest (NO TESTS COLLECTED)` row (`:1280-1283`), a warn-only shape with no ⚠ text that reduces to the bare name `pytest` in `skipped_checks` |

The arming condition is a conjunction — `tests/` exists AND (`.fabrik/run-pytest` OR a workflow names pytest) AND (the sentinel OR an empty diff OR a `src/`/`tests/`/`scripts/` change), `final_gate.py:1261-1271` — cite it, never paraphrase it (a paraphrase drifted once, D-284).
