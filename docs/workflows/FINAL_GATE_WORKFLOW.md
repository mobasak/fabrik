...
**Phase 3: Repo Consistency (36 checks)** — the results list of `run_consistency_checks(tier=1, changed_files=set())` (stub `run_optional_check` and `run_cmd`, count the rows; at tier 1 the call count is the same 36); fourteen run unconditionally outside the `if tier in (1, 2):` block — among them `check_convergence.py`, `check_review_coverage.py`, `check_review_hygiene.py` and `check_plan_lock_release.py`. **The 20 CHECK ROWS bulleted below carry the Tier-1 notes (a 21st bullet, the flip-gate invocation matrix, documents a command-side gate battery, not a `run_consistency_checks` row); four of them (Convergence Evidence, Coverage Checklist, Plan-lock release, Review hygiene) are unconditional rows also bulleted under Tier 3, where the remaining ten are named.** Four Tier-1/2-only rows carry no note here and are listed only under § Enforcement Scripts: Frozen Chain (`check_frozen_chain.py`), Decision Ledger (`check_decisions_unique.py`), Doc-Script Links and Doc-Script Coverage (`render_doc_script_links.py`)
- **Convergence Evidence (plans + reviews)** - `check_convergence.py` — runs every tier, unconditionally; the CLOSING-ROW rule (D-206): once a NON-ARCHIVED plan-set spine carries any `| Pass …` / `| Round …` table row with a `confirmed:` counter — anywhere in the file, no `## Pass Ledger` heading required — a CONVERGED or EXECUTED flip whose LAST such row does not read `confirmed: 0` is refused (`_check_spine_set` is reached from both claim paths; an `archived/` spine returns unchecked; `_PASS_ROW` — indented rows included, blockquoted excluded; fences stripped, code spans masked, HTML comments blanked, in that order; the row's last `confirmed:` token counts; `_closing_row_fail` is the single function the flip check calls and the one any fleet census must call). The canonical closing row is `term-coverage`'s five-counter run `found: F, new: N, confirmed: C, fixed: X, unexecuted: U`; the rule reads only the LAST `confirmed:` token — `new:` is prose it never parses
- **Coverage Checklist (reviews)** - `check_review_coverage.py` — runs every tier, unconditionally; what may ENTER a delta round's `confirmed:` is the fragments' bounded-hop rule (`term-edit`/`term-coverage`, D-230: inside the previous round's hunks or a live contradiction they created — pre-existing one hop out is RECORDED with a destination), and the grader reads the counter the fragments define
...
- **The flip-gate invocation matrix** — the gates a `term-edit` review loop (`/fabrik-spec-review`, `/fabrik-plan-review` and the eleven other consumers of `commands/_fragments/term-edit.md`) runs on the artifact at pass 1 and again at the close. A gate run COUNTS only when the runner can show the artifact was in the gate's examined set. The rows below are `tests/enforcement/test_flip_gate_matrix.py::MATRIX`, each pinned by a test that plants a fixture and asserts the gate's own examined marker names it; one row per gate: invocation · tree · the examined marker · the fail-open shape it avoids.
  ...
  - `check_convergence.py --project-root <scratch>` · a scratch GIT root where the FLIPPED spine is UNTRACKED or TRACKED-AND-UNCOMMITTED (`git add -N`); the target list is `_converged_targets(root)` — it reads `git status --porcelain` and takes every `M`/`A` path, plus a `??` path ONLY when the running run record of the session (`CLAUDE_SESSION_ID` + `COMMAND_RUN_DIR`) names that plan in its `--surface` (a plan flipped and committed in one motion would otherwise reach HEAD ungraded, while a sibling session's untracked draft must never red another session's gate) · marker: the spine path in that list, then the gate's refusal text when its Evidence is removed · avoids: a committed-clean copy (settled at HEAD) and an untracked copy no running record names are both invisible, and the gate prints nothing and exits 0.
...
- **Imports Resolvable (clean checkout)** - `check_imports_resolvable.py` *(advisory)*
  ...
  - **Three verdicts, not two.** `ERROR_AREAS` is `src`/`app`/`tests`; `WARN_AREAS` is `scripts`. A repo with a **library layout** (modules at `<name>/<pkg>/`, none of the three dirs) prints `NOT APPLICABLE: none of src/app/tests exist … NOT reporting a clean result` and exits 0 — it is not a defect, but it is not a pass either. The `OK:` line names only the areas actually walked. `WARN_AREAS` are unaffected — a `scripts/` phantom still surfaces under NOT APPLICABLE.
- **Lint Ratchet (repo-wide, no new debt)** - `check_lint_ratchet.py` *(advisory)* — repo-wide ruff count may only go down
- **Corpus Weight (byte ratchet)** - `check_corpus_weight.py` *(advisory — registered `warn_only`, so a non-zero exit is a broken contract and reds the gate; the check must accept `--check` and exit 0 on every non-`--strict` path)* — the hub's six governance surfaces (`CLAUDE.md`, `templates/governance/CLAUDE.md`, `commands/_sources`, `commands/_fragments`, `commands/_agents`, `.windsurf/rules` — D-241 added `commands/_agents` to D-240's five) may only grow in a change that cites a D-row; a synced project owns none of them and the check says so (D-234)
...
**Phase 2: Static Analysis** — Tier 1 also runs `ruff` (only if changed `.py` files), `check json`, `check yaml`; mypy/bandit/semgrep/sqlfluff/vulture are Tier-2-only (`final_gate.py:580-581`).
...
one of the three `_why` reasons (`:1355-1369`) → `pytest (NOT RUN)`; exit 5 → the separate `pytest (NO TESTS COLLECTED)` row (`:1299-1302`). Via the CLI a missing pytest never reaches these rows — the `REQUIRED_TOOLS` probe aborts at `status: "setup-error"` (`:2896-2912`) first (pytest must import under the selected interpreter, ruff must resolve as a binary, `:81`), which leaves the `pytest (NOT RUN) — pytest is not installed` branch at `:1295-1297` unreachable.

**What the pytest leg PRINTS, and what each line means** (read these before writing prose about the mechanism):

| The gate prints | When | It means |
|---|---|---|
| `⚠ this green SKIPPED n test(s) [and DESELECTED m test(s)] and asserts NOTHING about them …` (`skip_advisory`, `final_gate.py:197-241`) | a GREEN run whose pytest summary shows skips or deselections — the LAST summary line is read, since inner pytest runs printed by graders carry their own counts | the green is bounded: n + m tests did not execute. The counts exist ONLY in this ⚠ text tail (they reach `--json` `warnings` because of the prefix); no structured key carries them — a consumer cannot machine-assert "this green skipped n" today (recorded, owner infra) |
| ``⚠ this suite STOPPED at the first failure (`-x`) … see the whole list with python -m pytest tests/ -q (no -x)`` (`final_gate.py:1336-1346`) | a RED run whose pytest output carries the early-stop marker `stopping after ` (`:290`) — a 900s timeout or an exit-4 `SUITE REFUSED` red carries no such line | the failure you see is the FIRST, not the only one — `-x` is hardcoded (`:1292`, a deliberate cost decision recorded at `:1336-1338`); run the suite without `-x` before reasoning about "the" failure |
| one of three reasons — `no tests/ directory in this repo`, `this repo's CI does not invoke pytest, so the gate does not either — PERMANENT … THIS GREEN ASSERTS NOTHING ABOUT THE TEST SUITE …`, or `no src/, tests/ or scripts/ changes in this diff` (`final_gate.py:1355-1369`) | the leg did not run (a `--lean`, `--systemic` or docs-only Tier-2 run emits NO pytest row at all — `:1009`, `:1125`, `:1026`) | the row is `pytest (NOT RUN)`, a warn-only name (`WARN_ONLY_CHECKS`, `:335-348`): its NAME lands in `skipped_checks`, its whole TEXT lands in `advisory` (`:3042-3046`), and NOTHING lands in `warnings` (that list takes only ⚠-prefixed output, `:3077-3081`) — read `advisory` for the reason |
| (no line) | the interpreter | the gate resolves its OWN interpreter — `.venv/bin/python` when it exists, else `sys.executable` (`final_gate.py:71,79`) — and never inherits the shell's; a dependency missing from THAT interpreter is a collection ERROR at rc 2 that aborts the run (never a silent green), unless the import is guarded (`pytest.importorskip`, a conftest `except → skip`), which yields the ⚠ skip row above when other tests still collect — or, when the guard empties collection, pytest exit 5 and the separate GREEN `pytest (NO TESTS COLLECTED)` row (`:1299-1302`), a warn-only shape with no ⚠ text that reduces to the bare name `pytest` in `skipped_checks` |

The arming condition is a conjunction — `tests/` exists AND (`.fabrik/run-pytest` OR a workflow names pytest) AND (the sentinel OR an empty diff OR a `src/`/`tests/`/`scripts/` change), `final_gate.py:1280-1290` — cite it, never paraphrase it (a paraphrase drifted once, D-284).
...
- **Doc Link Integrity (live tree)** - `check_doc_links.py` (every repo-path reference in the live knowledge tree must resolve; archives/pipeline artifacts/LESSONS ledger/scaffold `*_TEMPLATE.md` + `scaffold-templates/` exempt — templates carry intentional placeholder refs; blocking)
- **INDEX.md ↔ docs tree drift** - `check_doc_index.py` (INDEX targets exist + every live doc indexed by path/basename; blocking; the lean tier additionally runs `--untracked-only` as a warn_only row that reports ONLY untracked live docs and always exits 0, so the run that creates a doc hears about its INDEX row while it still holds the file; an UNTRACKED doc counts as live — except the hub-seeded `docs/DECISIONS.md`, exempt while its bytes are still the distributed seed and owed the moment the repo writes its first decision, D-227 superseding D-225; a missing or unreadable INDEX.md is a named finding, not a traceback)
- **Retired-Tech Tripwire** - `check_retired_terms.py` *(ADVISORY row)* (WARN-only — the script always exits 0; flags unmarked Kilo CLI / Windsurf Cascade / Coolify / Supabase live-framing; it advises rather than blocks on the measured basis in § Advisory rows)
- **Stage-Skip Artifact Gate** - `check_stage_artifacts.py`
  - Spec freshness + FROZEN header shape for a stage that was skipped
- **Hooks Index Fresh** - `check_hooks_index.py` (the required set is every hook the settings/config files register PLUS every hook file tracked under `.claude/hooks/` and present in the working tree — deleting a registration does not shrink the requirement)
...
- **.env.example Completeness** - `check_env_example.py` *(ADVISORY row)*
  ...
  - No other check covers code → `.env.example` (`check_doc_sync` only fires `.env.example` → `CONFIGURATION.md`); it advises rather than blocks on the measured basis in § Advisory rows
...
- **Work items (sync)** - `scripts/work.py sync --check` *(appended by `_work_sync_row()`, 30 s timeout `TIMEOUTS["work_sync"]` — the row CAN fail)*
  - Reads the spec/plan ↔ work-store drift (`DRIFT <class> (blocking|advisory)  <path>`) and appends one reading to `fabrik-work/readings.jsonl` under the git common dir (outside the tree). The ONE red is exit 1 with a `DRIFT <n> (blocking)` line — the verb's verdict once the repo shows 7 consecutive clean daily readings after its `migrated_at`. Before that, drift passes as a ⚠ row (so `--json` `warnings` carries it)
  - Fails open, visibly: no `scripts/work.py` → the `⚠ check not present, skipping` row; a timeout, an old work.py without `sync`, a work.py error or a traceback (even after a `(blocking)` line — work.py labels classes 2-6 that way in the advisory window too) → a green SKIP row named `Work items (sync) (NOT RUN — <exit N | timed out>)`, listed in `skipped_checks`, reading `⚠ Work items (sync) could not run (…): <the last output line that is not an import-fallback notice>`; no `.fabrik/work/` store → the one-line `no work store in <repo> — nothing to sync`, green
...
**Phase 3: Repo Consistency (22 checks)** — `run_consistency_checks` (the `tier >= 2` / Tier-3 selections; the results list under an empty changed set — no Tier-3 row is changed-set-gated, and 20 is the `run_optional_check` call count). **Twelve rows are bulleted below — seven Tier-3-specific, the Kilo CLI Health Check row shared with Tier 2, plus the four unconditional every-tier rows repeated from Tier 1 (Convergence Evidence, Coverage Checklist, Plan-lock release, Review hygiene). The remaining ten are the rest of the fourteen unconditional rows** (Vendored Drift, Routing Policy, Governance Tables, Certification Coverage, Rivals dossier, Spec convergence, Rule grounding, Citations resolve, Feedback duty, Trigger routing)
- **Convergence Evidence (plans + reviews)** - `check_convergence.py` — runs every tier, unconditionally; the closing-row rule is stated once under § Tier 1
- **Coverage Checklist (reviews)** - `check_review_coverage.py` — runs every tier, unconditionally; what may ENTER a delta round's `confirmed:` is the fragments' bounded-hop rule (`term-edit`/`term-coverage`, D-230: inside the previous round's hunks or a live contradiction they created — pre-existing one hop out is RECORDED with a destination), and the grader reads the counter the fragments define
- **Plan-lock release** - `check_plan_lock_release.py` — runs every tier, unconditionally (advisory `warn_only=True`): reports a `.fabrik/plan-locks/<id>.json` left NON-TERMINAL (`active`/`paused`/`blocked`) after its plan finished. Every-tier ON PURPOSE — `--lean` is the mode agents run while a lock is live. Doc: [plan-lock-lifecycle.md](../reference/plan-lock-lifecycle.md)
- **Review hygiene (advisory)** - `check_review_hygiene.py` — runs every tier, unconditionally (advisory `warn_only=True`); its classes, arguments and fire rate are stated once under § Tier 1
...
- **Documentation Sprawl** - `check_doc_sprawl.py` (new-file detection is HEAD-or-staged-rename — a merely-staged new .md no longer bypasses the allowlist)
...
- **VPS Docs Freshness** - `check_vps_docs.py` *(ADVISORY row)*
  - Checks VPS-facing docs haven't gone stale against the live fleet
    - Every finding it can construct is `Severity.WARN`; exit follows severity (ERROR fails; `--strict` promotes WARN), matching `_check_runner.run_as_main`
...
**`check_doc_index.py` has a THIRD direction: added CODE paths owe an INDEX row, ADVISORY.** Directions (a) and (b) cover `docs/` both ways; the Doc Sync Matrix's *File added/removed/renamed → INDEX.md* had no enforcement on the code half at all — `check_index_md.py` asserts six headings and seven hardcoded root filenames, never a tree↔row comparison, and it is UNWIRED besides. Direction (c) reads only paths ADDED in the STAGED set under `scripts/`, `tests/`, `.claude/hooks/`, `.fabrik/`. ⚠️ **Fire rate measured before arming:** whole-tree, **584 of 865** tracked files under those roots carried no INDEX mention — wallpaper as a blocking row; staged-scope and added-only, **37 of the 113** files added in the two weeks before it landed (33 %) were unindexed — true positives, but too many to BLOCK on day one, so it never changes the exit code and rides `--json` under `code_index_advisory`. ⚠️ **Its cobra path is written down where the next reader finds it** (in the check's own comment): the cheapest way to satisfy it without the outcome is to paste the bare basename into INDEX.md, so the membership test demands the PATH — which makes the cheap edit a real row, though nothing here reads the description, and that residue is review's job rather than a longer regex.

**The WRITE scope is narrower than the READ scope.** `get_changed_files()` is what the gate READS: committed-but-unpushed, staged, and unstaged modifications to TRACKED files — the static tier should red on all of it. `get_writable_files()` is what the gate may MUTATE: staged plus `base...HEAD`, and nothing else. ⚠️ The two differ because an unstaged edit to a tracked file, on a tree three sessions share, is a sibling's uncommitted work as surely as an untracked file is, and git cannot tell their unstaged edit from yours. Otherwise one bare `final_gate.py` run lets `ruff format` rewrite a sibling's in-progress file with no red and no warning. The rule is the one this gate already applied to untracked files, extended one layer: **authorship = staging**. Files dropped from the fix scope are NAMED in the run output with both ways out (`git add` yours, or `--fix-all`); `--fix-all` opts back into the whole change set for a solo tree. The auto-STAGE half was already safe — it re-stages only what was staged when the gate started.

**The per-check roster (`checks`).** Without it `--json` carried counts plus the names of the FAILURES and the skips only, so a consumer could not ask the question a roster exists for — *did check X run in this tier?* A check that was never registered and a check that ran and passed were equally invisible. `checks` is a list of `{name, outcome}` over every row the run built, in order, with `outcome` one of `pass` · `fail` · `skipped` · `advisory`. Precedence, because two of the rows are both: **`skipped` wins over `advisory`** — it is the more informative half (the check did not run at all) and it is what `skipped_checks` already counts, so the roster and the counts beside it cannot disagree. The kaizen `gate_run` event reads the SAME builder (`_check_roster`), so the two consumers cannot drift into two answers about one run.

⚠️ **Every semgrep not-run path — not installed, not authenticated, timed out, no `src/` changes — is a NAMED skip row**, because the skip roster matches on the ROW NAME: a plain `semgrep` row for a run where semgrep never executed reported `skipped: 0`.

**Why this exists.** A canary sweep gave eight registered checks a real violation each; each printed it and exited 0 — a row identical to a check that genuinely blocks — and four were not even `advisory=True`, so their stdout was discarded. Each was decided on measured evidence — the reasoning is written at each registration in `final_gate.py`:
...
| `check_reusable_modules` | UNWIRED | universe is empty — no repo carries a `src/utils/` or a `src/lib/` holding a single `.py` file |
...
- `check_corpus_weight.py` — Corpus Weight (byte ratchet) (Tier 1/2; advisory — `warn_only`, never blocks)
...
- `check_doc_index.py --untracked-only` — INDEX.md ↔ new docs (untracked, this run) (Tier 1; advisory — `warn_only`, never blocks)
...
- `check_ticket_breadth.py` — Ticket Breadth (plan sets) (Tier 2; advisory — `warn_only`, never blocks in the gate's no-argument form; an explicit undated `--plan-dir` is REFUSED with exit 2, T4.9)
...
- `scripts/work.py sync --check` — Work items (sync) (Tier 2; appended outside `run_optional_check` by `_work_sync_row()` — reds ONLY on exit 1 with a `DRIFT <n> (blocking)` line; a missing script is the ⚠ `check not present` row, and a timeout or any other failure is a ⚠ pass)

**On disk but NOT gate-wired as their own row** (no `run_optional_check` call site in `final_gate.py`; NOT all dead — five are reached through another row's import and a sixth only through an explicit `--surface` run, see each bullet; `final_gate.py`'s eight `# UNWIRED —` markers — the four retired from the Tier-2 block (`check_env_updates.py`, `check_test_coverage.py`, `check_compose_services.py`, `check_reusable_modules.py`) and the four from the Tier-3 block (`check_ports.py`, `check_deps_sync.py`, `check_watchdog.py`, `check_health.py`); each carries the measurement that retired it beside its marker). Their intent is now covered by `check_doc_sync.py` (Doc Sync Matrix) + `docs_updater.py --check`, not by these files:
...
- `check_ports.py` — was: PORTS.md registration ; LIVE via `validate_conventions.py` on every changed Dockerfile and `.py/.ts/.tsx/.js/.yaml/.yml` file
- `check_deps_sync.py` — was: dependencies documented ; LIVE via `validate_conventions.py` on a changed `requirements.txt`
- `check_watchdog.py` — was: watchdog scripts present ; LIVE via `validate_conventions.py` on every changed compose file it dispatches (`check_watchdog.py` itself silently skips `docker-compose.yml` and any case-variant — its guard matches the un-lowercased name while the dispatcher lowercases first)
- `check_health.py` — was: /health endpoint validation ; LIVE via `validate_conventions.py` on every changed `.py` file (the check itself skips any file with `test` in its name)
- `check_env_updates.py` — was: .env Updates (Secrets) (`# UNWIRED —` marker: could not fail)
- `check_test_coverage.py` — was: Test Coverage (New Code) (`# UNWIRED —` marker)
- `check_compose_services.py` — was: Compose Services Docs (`# UNWIRED —` marker)
- `check_reusable_modules.py` — was: Reusable Module Tagging (`# UNWIRED —` marker: its universe is empty)
...
**⚠️ NOT gate-wired** — exists on disk, no call site in `final_gate.py` . Covered instead by `check_doc_sync.py` (Doc Sync Matrix).
...
**⚠️ NOT gate-wired** — exists on disk, no call site in `final_gate.py` . Covered instead by `check_doc_sync.py` (Doc Sync Matrix), which fires on new env vars per the Doc Sync Matrix.
...
#### check_env_updates.py — UNWIRED

**⚠️ NOT gate-wired** — `final_gate.py`'s own `# UNWIRED — check_env_updates.py` ; still EXECUTED as a vacuity canary by `scripts/sysadmin/liveness_audit.py` (`CANARIES`), so it is not dead code.
...
part of the change under gate and legitimately omits every optional variable. It also shared the display name ".env Updates (Secrets)" with the blocking
...
**⚠️ NOT gate-wired as its own row — imported behind a guard** — no `run_optional_check` call site in `final_gate.py` ; `check_review_hygiene.py`'s `changelog-entry` class imports and calls its `check_changelog_quality` behind a `try/except ImportError` (a missing file degrades to an advisory hit), and only a `--surface` run whose expansion contains a file named `CHANGELOG.md` (a directory surface counts) reaches that class — the gate's argument-less row never does. The CHANGELOG-on-change rule itself is `check_doc_sync.py`'s (Doc Sync Matrix).
...
**⚠️ NOT gate-wired** — exists on disk, no call site in `final_gate.py` . Covered instead by `check_doc_sync.py` (Doc Sync Matrix), which folds in API/SDK/CLI-change → `docs/QUICKSTART.md`.
...
#### check_test_coverage.py — UNWIRED

**⚠️ NOT gate-wired** — `final_gate.py`'s own `# UNWIRED — check_test_coverage.py` ; still EXECUTED as a vacuity canary by `scripts/sysadmin/liveness_audit.py` (`CANARIES`), so it is not dead code.
...
**Why it is no longer a gate row:** its rule — a test per public symbol — is the 100%-coverage dogma the Behavior Contract explicitly rejects.
...
#### check_compose_services.py — UNWIRED

**⚠️ NOT gate-wired** — `final_gate.py`'s own `# UNWIRED — check_compose_services.py` ; still EXECUTED as a vacuity canary by `scripts/sysadmin/liveness_audit.py` (`CANARIES`), so it is not dead code.
...
**⚠️ NOT gate-wired** — `final_gate.py`'s own `# UNWIRED — check_reusable_modules.py` marker retired it (its universe is empty: no repo carries a `src/utils/` or a `src/lib/` holding a single `.py` file).
...
**⚠️ NOT gate-wired as its own row — LIVE via import** — `final_gate.py`'s `# UNWIRED — check_deps_sync.py` marker retired the row ; `validate_conventions.py` imports `check_file` from it and runs it on a changed `requirements.txt` under the Tier-3 row, so the file is not dead.
...
**⚠️ NOT gate-wired as its own row — LIVE via import** — `final_gate.py`'s `# UNWIRED — check_watchdog.py` marker retired the row ; `validate_conventions.py` imports `check_file` from it and runs it on every changed compose file it dispatches under the Tier-3 row (the check itself silently skips `docker-compose.yml` and any case-variant — its own guard matches `file_path.name` un-lowercased against `compose.yaml`, `compose.yml`, `docker-compose.yaml`, while the dispatcher lowercases first), so the file is not dead.
...
**⚠️ NOT gate-wired as its own row — LIVE via import** — `final_gate.py`'s `# UNWIRED — check_health.py` marker retired the row ; `validate_conventions.py` imports `check_file` from it and runs it on every changed `.py` file under the Tier-3 Fabrik Convention Validator row (the check itself skips any file with `test` in its name), so the file is not dead.
...
**⚠️ ARCHIVED, NOT gate-wired** — the script lives at `scripts/archived/update_agents_toc.py`; no call site in `final_gate.py` .
...
| sqlfluff | 180 |
| ruff | 120 |
| semgrep | 30 (hardcoded at the call site, `final_gate.py:1196`; the `TIMEOUTS["semgrep"]=300` dict entry at `:141` is dead — never read) |
