...
**Phase 3: Repo Consistency** — inherits all **17** Tier-1 checks above (16 in the `tier in (1, 2)` block; `check_convergence.py` runs unconditionally outside it), **plus 19 Tier-2-only checks** (the `if tier == 2:` block — incl. the 3 docs-truth durability gates: Doc Link Integrity, INDEX↔tree drift, Retired-Tech Tripwire [advisory] — plus the Plan-Set Contract, Hooks Index Fresh, Sync Trigger Coverage, and Phase Tests [advisory, plan-window]), **plus the Kilo CLI Health Check** (shared with Tier 3, `tier >= 2`) — **37 checks total**. (Counts verified 2026-08-10 by enumerating the `run_optional_check` calls in that block; the line-number ranges that used to be cited here are deliberately dropped — they drifted on every insertion and a wrong `path:line` is worse than none.)
