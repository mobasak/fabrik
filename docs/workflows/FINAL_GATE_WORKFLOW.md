...
**Phase 2: Static Analysis** — Tier 1 also runs `ruff` (only if changed `.py` files), `check json`, `check yaml`; mypy/bandit/semgrep/sqlfluff/vulture are Tier-2-only (`final_gate.py:1124-1126`).
...
|---|---|---|
| `⚠ this green SKIPPED n test(s) [and DESELECTED m test(s)] and asserts NOTHING about them …` (`skip_advisory`, `final_gate.py:198-242`) | a GREEN run whose pytest summary shows skips or deselections — the LAST summary line is read, since inner pytest runs printed by graders carry their own counts | the green is bounded: n + m tests did not execute. The counts exist ONLY in this ⚠ text tail (they reach `--json` `warnings` because of the prefix); no structured key carries them — a consumer cannot machine-assert "this green skipped n" today (recorded, owner infra) |
| ``⚠ this suite STOPPED at the first failure (`-x`) … see the whole list with python -m pytest tests/ -q (no -x)`` (`final_gate.py:1337-1347`) | a RED run whose pytest output carries the early-stop marker `stopping after ` (`:291`) — a 900s timeout or an exit-4 `SUITE REFUSED` red carries no such line | the failure you see is the FIRST, not the only one — `-x` is hardcoded (`:1293`, a deliberate cost decision recorded at `:1337-1339`); run the suite without `-x` before reasoning about "the" failure |
| one of three reasons — `no tests/ directory in this repo`, `this repo's CI does not invoke pytest, so the gate does not either — PERMANENT … THIS GREEN ASSERTS NOTHING ABOUT THE TEST SUITE …`, or `no src/, tests/ or scripts/ changes in this diff` (`final_gate.py:1356-1370`) | the leg did not run (a `--lean`, `--systemic` or docs-only Tier-2 run emits NO pytest row at all — `:1010`, `:1126`, `:1027`) | the row is `pytest (NOT RUN)`, a warn-only name (`WARN_ONLY_CHECKS`, `:336-349`): its NAME lands in `skipped_checks`, its whole TEXT lands in `advisory` (`:3090-3094`), and NOTHING lands in `warnings` (that list takes only ⚠-prefixed output, `:3125-3129`) — read `advisory` for the reason |
| (no line) | the interpreter | the gate resolves its OWN interpreter — `.venv/bin/python` when it exists, else `sys.executable` (`final_gate.py:71,79`) — and never inherits the shell's; a dependency missing from THAT interpreter is a collection ERROR at rc 2 that aborts the run (never a silent green), unless the import is guarded (`pytest.importorskip`, a conftest `except → skip`), which yields the ⚠ skip row above when other tests still collect — or, when the guard empties collection, pytest exit 5 and the separate GREEN `pytest (NO TESTS COLLECTED)` row (`:1300-1303`), a warn-only shape with no ⚠ text that reduces to the bare name `pytest` in `skipped_checks` |
...
| sqlfluff | 180 |
| ruff | 120 |
| semgrep | 30 (hardcoded at the call site, `final_gate.py:1197`; the `TIMEOUTS["semgrep"]=300` dict entry at `:141` is dead — never read) |
