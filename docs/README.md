...
| `workstation/` | Local dev-box setup: WSL fixes, MCP transports, editor tooling, the full Claude config inventory, the fabrik-mail AI message channel, the 4-account Claude quota-rotation pool, the liveness layer that proves the scheduled machinery is actually running | anything that runs on the fleet |
