...
3. **`/opt/ai-model-catalog/engine/derive_quality_v2.py`** (modify, ~10 LOC delta) — extend the `SERVICE_TYPES_KNOWN` set (or equivalent gate) to include `'3d_gen'` and `'moderation'` so quality-tier derivation doesn't skip WaveSpeed's 30 new-category rows.

4. **`/opt/ai-model-catalog/engine/export_models_browser.py`** (modify, ~30 LOC delta) — no new columns; ensure the payload exposes `service_type` in `('3d_gen', 'moderation')` values so the browser JS can filter them. Add a `via_wavespeed` badge on the model card (matching the existing `via_siliconflow`/`via_modelscope` badge pattern in `models_browser.html`).
