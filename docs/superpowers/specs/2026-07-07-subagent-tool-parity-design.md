...
Hub-side governance + config only — no scaffold type, no deployed service, **no `shape.*` flags**. Deliverables: two new agent types `~/.claude/agents/fabrik-researcher.md` + `fabrik-gui.md` (**built**); the `.windsurf/rules/core/62-using-subagents.md` pack (**built**); **`/opt/fabrik/mcp.json`** the MCP source-of-truth (**built**); key provisioning into `/opt/fabrik/.env` (**done**); and edits to the 10 dispatching command briefs to name the right `subagent_type` (`fabrik-researcher` on the 4 web-research commands · reviewer-class on the 5 repo-grounded commands · a general builder type on `/fabrik-execute-plan` implementers · `fabrik-gui`/`design-review` on GUI phases — **pending**). The v1 "propose the Brave provider" item is **done** (fabrik-lib shipped it).
