...
- **No window after the table is created:** the default privileges above hand the app role `UPDATE` and `DELETE` on any table the owner creates, `audit_log` included, the moment it is created. So the audit block folded into the project's schema ends with its own revokes, in the same transaction as the `CREATE TABLE`: `REVOKE UPDATE, DELETE, TRUNCATE ON audit_log FROM "<db>_app"`, and the same for `<db>_wd_rw` and each Pattern A group role, each guarded by the role existing. The scaffolder writes the role names in from the registrar's own database-name rule (the spec id with `-` → `_`, or `depends.postgres` when set — `src/fabrik/orchestrator/infrastructure.py:574-579`); in a migration wave the project agent writes them. The registrar step below and the weekly check are the backstops, never the only guard.
...
| Pass 2 | opus×1 + sonnet×1 + sonnet×1 fabrik-researcher (the round-1 slice owners) + refuters · the pass-1 fix diff plus one hop | found: 2, new: 2, confirmed: 2, fixed: 2, unexecuted: 0, edits: 2 | method: re-derivation — slice R re-executed its 13 rows (all NOW_FALSE; D0 and D1 re-run identical; the PG 16 grant forms and the USAGE-only refusal probed on a local PostgreSQL 16.14 inside a rolled-back transaction), slices P and X their 6 (all NOW_FALSE, the three changed URLs re-fetched); 2 new, both own-fix: round 1 — R-O15 (an Open-unknowns sentence still named a project declaration) and R-O16 (default privileges grant the app UPDATE/DELETE on a freshly created audit_log until the next apply — the schema block now revokes in the same transaction); round-zero probe — the orchestrator's check of PG 16's GRANT role syntax against the raw docs, superseded by the seat's executed probe | d4374088… → 88bdab81… |
| Pass 3 | opus×1 (the round-1 owner of slice R; P and X had no open claim) + refuter · slice R's two open claims | found: 0, new: 0, **confirmed: 0**, fixed: 0, unexecuted: 0, edits: 0 | method: re-derivation — R-O15 re-grepped (0 'declaration' hits; the code scan it names exists in § 2); R-O16 EXECUTED on a local PostgreSQL 16.14 inside BEGIN…ROLLBACK: after the same-transaction guarded revokes the app and watchdog roles hold only INSERT and SELECT on audit_log, the existence guard runs clean on missing roles and an unguarded revoke errors, 0 roles left behind; RECORDED — measured (wording; changes no behaviour): the db-name cite `infrastructure.py:574-579` ends one line before its assignment on `:580` — for the plan; standing clean since pass 2: every class of slices P and X | 54795cba… → 54795cba… ✓ → **CONVERGED** |
