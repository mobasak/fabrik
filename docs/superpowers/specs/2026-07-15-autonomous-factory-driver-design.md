...
| **Gate 1** | **format FIXED (2026-07-19 reshape):** BIG = the mega `00` Vision confirm, persisted to `docs/superpowers/specs/YYYY-MM-DD-<project>-vision.md` **with its own `Status: LOCKED <date>` header** (the vision IS the project's decisions lock); SMALL = the operator confirm at **ettw `01R`**, which writes the **`Status: LOCKED <date>`** line into `docs/superpowers/specs/YYYY-MM-DD-<slug>-decisions.md` — the driver's Gate-1 predicate greps that line. Per-epic ettw runs inside a BIG flow **auto-lock** at `01R`'s no-op (vision already operator-locked; no per-epic gate) |
