...
**Status:** CONVERGED — re-reviewed and re-converged 2026-08-12; **D5 resolved by operator decision (WSL-first, flywheel does not move)**.
**Scope of this migration:** **WSL only.** The engine relocates and keeps running on the WSL box; VPS deployment is a separate, later spec (§7 D5). The engine's SQLite store is consolidated into **Postgres** in the new repo.
**Binding invariant:** the **flywheel must not break** — see §7 § *Flywheel-safety invariant* for the three gates that make that testable.
...
> **⚠️ Inventory is DERIVED AT EXECUTION TIME, never copied from this spec (re-verified 2026-08-12).** An earlier revision hard-coded "176 first-party files, ~51k LOC, 52 tests". That count is now stale: **52 files were added after this spec froze** (the whole `dispatcher-bench/` benchmark suite, `dispatcher_bench.py`, `build_lcb_difficulty_manifest.py`, `microbench_vision_describe.py`), and the flat top-level script count moved **98 → 101**. ⚠️ **Test counts: use the PLAN's figures, not loose globs.** An earlier revision of this note said 53 in-tree / 15 repo-root; those came from globs that swept the `dispatcher-bench/` fixture tree and `__init__.py`. Measured strictly, the engine test set is **50 in-tree + 14 repo-root = 64**, exactly as the execution plan states — three of its gates assert 64, so do NOT 'reconcile' the plan up to a looser number. The engine is under active development; any hand-enumerated file list ages within days. The plan MUST derive the move-set mechanically (import-graph audit) and treat every count here as illustrative.
>
...
**Consequence: "provision a network-reachable read-only DSN to that instance" is NOT ACHIEVABLE as written.** There is no network-reachable instance to point at, and no route from vps1 to that host. The flywheel works today *only* because the reader and its **5 configured writers** all sit on this one box and share one Unix socket (they resolve the same host-less `SUBAGENT_RUNS_DSN`, `pg_ledger.py:152`). Moving the reader off-box breaks the read; moving the DB to reach it re-points those writers *and* requires a WSL→`postgres-main` route that does not exist today (§7 D5).

**RESOLVED for this migration (D5, §7 — operator decision 2026-08-12): the flywheel does not move.** This migration is **WSL-only**, so the reader keeps hitting the same local socket on the same box as the same user, and all 5 writers are untouched. The probe evidence above is therefore not a blocker here — it is the **starting evidence for the separate VPS-deployment spec**, where the three end-states (migrate to `postgres-main` / ranking stays hub-side / catalog stays hub-side) get decided. §7 retains them.

**What DOES bind this migration** is that the reader *changes repos*, and it authenticates through `sudo -n -u postgres psql` — a property of the invoking context, not of the file's path. See §7 § *Flywheel-safety invariant* for the three gates that make "we should not break the flywheel" testable. **Fail-safe unchanged:** the reader's fail-open (`("error",[])` stub → `_TABLE` floor, `rank_task_subagents.py:179`) bounds any mis-wiring to *staleness, never an outage*. Fabrik stays tenant-zero: its private telemetry drives its own selection doc, produced by the catalog.
...
**Phase 5 — ⛔ OUT OF SCOPE for this migration (deferred to the VPS-deployment spec).** Per D5 (§7, operator decision 2026-08-12) the engine stays on WSL, so there is no deploy phase here and **the migration ends at Phase 4**. Retained as the hand-off brief for the later spec, which inherits three seams and the probe evidence that sizes them: (1) the **flywheel** — `fabrik_analytics` is on the WSL workstation's local socket, `listen_addresses=localhost`, not on `postgres-main`, and the box is not on the fleet mesh, so "provision a network-reachable read-only DSN" has no target until one of D5's three end-states is chosen; (2) the **catalog store** — moot once the store is Postgres per §7, but the deployed instance still needs choosing; (3) the **`claude -p` credentials** — either declare `shape.uses_claude_cli` + `claude_cli_home` in `specs/services/ai-model-catalog.yaml` (the deployer then requires the `~/.claude:ro` mount, `deployer_ssh.py:540,625-643`) or scope those microbenches hub-only. *Gate for that later spec: each seam demonstrably works from the DEPLOYED container (a real `SELECT` against the flywheel; a non-empty catalog read; one live claude-`-p` call) or is explicitly scoped hub-only.*
...
### ✅ D5 — Flywheel end-state: RESOLVED 2026-08-12 → **WSL-first; VPS deferred; the flywheel does not move**

**Operator decision (2026-08-12):** *"we will extract it, it will be in wsl first, we should not break flywheel."*

**Scope of THIS migration is WSL-only.** The engine relocates to `/opt/ai-model-catalog` and keeps running on the WSL box, delivering to fabrik by local-filesystem copy exactly as today. **`fabrik_analytics` stays exactly where it is** — the WSL host's local PostgreSQL, reached over the same Unix socket, by the same reader, as the same user. Nothing about the flywheel's storage, DSN, writers, or auth changes. Options (a)/(b)/(c) below are **not chosen now**; they are the decision surface for the *separate, later* VPS-deployment spec, at which point the probe evidence in §4 is the starting point.

**Consequently the three host-only seams found on 2026-08-12 are OUT OF SCOPE here and must not be "solved" in this migration** — they are deployment concerns and all three are inert while the engine runs on WSL: the flywheel socket (this decision), the `claude -p` credentials (§4 — the host's `~/.claude` is present on WSL), and the catalog-store transport (§4 Data stores — no git-sourced container is involved; the store simply becomes Postgres per the operator's decision below). A plan that tries to fix them here has mis-scoped the work.

**Operator decision on the catalog store (2026-08-12):** *"catalog system have a postgres and also sqlitedb and we will put all into postgres after this extraction inside its new repo."* → the engine's SQLite `kilo_agents.db` is **consolidated into Postgres inside the new repo**, which independently closes the gitignored-DB gap (§4 Data stores) since a Postgres store has no untracked-file problem. On WSL that Postgres is the local instance; the deployed target is the later spec's concern.

---

### 🔒 Flywheel-safety invariant (BINDING — the operator's "we should not break flywheel", made concrete)

The flywheel is `fabrik_analytics.subagent_runs`: **5 configured writers** (`fabrik`, `fabrik-lib`, `iterative_image_editor`, `trade-intelligence`, `tryton-crm`) and **one reader** — `rank_task_subagents.py`, which **moves with the engine**. The writers do not move and are not touched. The reader changing repos is the entire risk surface.

**Why this is not automatically safe, even on one box:** the reader queries through `sudo -n -u postgres psql` (`:187-191`), so it depends on the *invoking user's* passwordless-sudo context, not on its own path. Moving the file is fine; moving the **invocation context** (a different user, a different shell, a container even in dev, a cron with a different environment) silently is not.

**And the existing tripwire is currently muted.** A prior `/fabrik-review` (the "A6 fix") deliberately made `_query_rows()` return `(state, rows)` with `state ∈ {"ok","error"}` so that a *broken* read is distinguishable from a *genuinely empty* one — on `error` it emits a distinct stub and `main()` **returns exit 1** (`:1114`, `:1374`). The docstring records the reason: the old behaviour "returned `[]` for both cases and the 'Last refresh' date advanced daily on the stub, making broken indistinguishable from healthy-but-empty." **But `daily_refresh.sh:422-423` invokes it as `_step … || echo "[daily_refresh] rank_task_subagents failed (non-fatal)"`, which swallows that exit code into a log line.** `_step` itself propagates `rc` correctly (`:115-122`); the `|| echo` at the call site is what mutes it. So today a broken flywheel read publishes a stub selection doc, fleet-syncs it, and raises no alert.

**Three gates the plan MUST carry — all three, or the migration is not flywheel-safe:**

1. **Positive proof, not absence of error.** After the reader moves, a gate asserts `_query_rows()` returns `state == "ok"` **and a non-empty row set**, run from the *relocated* repo under the *real* invocation context (the same user and shell the cron uses). "It didn't crash" is not evidence — fail-open guarantees it won't.
2. **Preserve the tripwire.** The `state == "error"` → distinct stub → **exit 1** behaviour must survive the move byte-for-byte in intent. A regression test asserts a simulated read failure still yields exit 1 and the distinct stub, never a silently-advanced "Last refresh".
3. **Un-mute it.** The relocated invocation must **not** swallow the ranker's exit code. Either drop the `|| echo … (non-fatal)`, or route the non-zero through `send_alert` — a broken flywheel read is precisely the condition the operator asked not to miss, and it is currently invisible.

**Rollback:** the fail-open floor still bounds the worst case to *staleness, never an outage* (`select.py:479-483`); and until the engine is deleted from fabrik (Phase 4), reverting is "stop the relocated engine, fabrik self-produces again." Neither is a substitute for the three gates — they bound the damage, they do not detect it.

---

### The three end-state options (deferred to the VPS-deployment spec, retained as evidence)

Grounded by live probe (§4): `fabrik_analytics` (~6.3k rows and growing) lives on the **WSL workstation's local PostgreSQL**, `127.0.0.1:5432`, `listen_addresses=localhost`, **not** on `postgres-main`, and that box is **not on the WireGuard fleet mesh**. Today the flywheel works only because the reader and its 5 configured writers share one Unix socket on one box. Pick one:

| Option | What it buys | What it costs |
|---|---|---|
| **(a) Migrate `fabrik_analytics` → `postgres-main`** | The only option where the designed end-state (deployed worker reads the flywheel) actually works; also the natural home for the catalog store, collapsing two seams into one | **Re-points 5 writers, not 49** (measured 2026-08-12 — corrected below). Plus a data migration of the ~6.3k existing rows, **and** solving WSL→`postgres-main` reachability (see the note below). |
...
**⚠️ Correction to the blast radius (measured 2026-08-12).** An earlier revision of this section said option (a) "re-points ~49 fleet writers". **That was wrong** — 49 is the count of *vendored `select.py` copies*, not of configured writers. Only **5** `/opt` projects actually set `SUBAGENT_RUNS_DSN` and therefore write to the flywheel: `fabrik`, `fabrik-lib`, `iterative_image_editor`, `trade-intelligence`, `tryton-crm`. The other 17 leave it unset, so their vendored `pg_ledger` **no-ops to JSONL** by design (`pg_ledger.py:11`). Option (a)'s writer-side cost is therefore **five env-var changes**, not a fleet migration.

**The real cost of (a) is elsewhere: WSL cannot currently reach `postgres-main`.** `postgres-main` is hub-only, on the `fabrik` Docker network with no host `ports:`; spokes reach it at `10.99.0.1:<port>` over WireGuard, and **this WSL box is not on that mesh** (probed §4). It also has **no `fabrik*` database today**. So option (a) additionally requires a route — WSL joins the mesh, an SSH tunnel, or the writers stay local and the data is replicated/exported to `postgres-main`. This is the substance of the decision, not the writer count.

**Recommendation: (a)**, because it is the only option that leaves the spec's architecture intact and it also dissolves the §4 catalog-store transport problem. But the WSL→`postgres-main` route must be chosen first, so it is the operator's decision, not the reviewer's.
...
✅ **CONVERGED and ready for `/fabrik-plan-after-chat`.** D1–D4 signed off 2026-07-26; **D5 resolved 2026-08-12** by operator decision (WSL-first; the flywheel does not move; VPS deferred to a later spec). No execution-blocking question remains.

**What the plan must carry, in priority order:**
1. **The flywheel-safety invariant (§7)** — its three gates are the operator's hard constraint made testable: positive proof of a real read from the relocated repo, preservation of the `error → exit 1` tripwire, and **un-muting the swallowed exit code at the call site**.
2. **Scope discipline** — WSL only. The three host-only seams (flywheel socket, `claude -p` credentials, store transport) are **deployment concerns, inert on WSL, and out of scope**; a plan that "fixes" them here has mis-scoped the work.
3. **SQLite → Postgres** consolidation inside the new repo (operator decision, §7).
4. **Mechanically-derived inventory** (§3a) — the import-graph audit, never a hand-enumerated file list.
5. **The `daily_refresh.sh` split** (§5 Phase 3) — hub-only steps enumerated there do not move.
