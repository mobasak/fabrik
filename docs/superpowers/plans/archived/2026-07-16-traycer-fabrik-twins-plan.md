...
- **Rename** the file to `~/.claude/commands/fabrik-workflow-review.md` + update the frontmatter `description` to cover BOTH folders. Then update the **8 callers — in two DIFFERENT ways** (they are not all mechanical):
  - **7 ettw twin footers** (`00`–`06`) — a **mechanical** name swap. Verify: `grep -rn fabrik-ettw-review docs/traycer/epic-to-ticket-workflow/` → **0**.
  - **`docs/orchestrator/00-autonomous-factory-north-star.md` (lines ~116/155/182) — a SEMANTIC rewrite, not a find-replace.** Those lines are *status prose* that is **correct until B1 lands and false after it**: they say the remaining work is to "**extend** `/fabrik-ettw-review` → `/fabrik-workflow-review`", and :155 separately asserts "the shared `/fabrik-ettw-review` skill exists". On B1 completion, rewrite them to past tense — "`/fabrik-ettw-review` **extended + renamed** → `/fabrik-workflow-review` (the shared folder-neutral skill)" — and **drop :155's now-false "the shared `/fabrik-ettw-review` skill exists"** clause. ⚠️ Without this, B1 finishes with the canonical doc asserting a skill file that no longer exists.
  - ⚠️ Do **not** assert `grep -rn fabrik-ettw-review docs/traycer/` → 0 as the gate — the north star legitimately names the old skill while describing the transition; the spec/plan likewise retain historical mentions. The `epic-to-ticket-workflow/` grep above is the real caller check.
- **Add a hollow-citation item to the mega checklist** (the ettw item-132 equivalent — verified absent: ettw has 1, mega has 0) so the mega twins have the CC2 audit as an explicit yardstick item.
- Converge the skill itself to a no-op (pool + native Opus on its own prose), gate, commit.
...
The Fabrik-managed front-end: a **Zed extension speaking ACP** (the analog of Traycer-in-VS-Code) `[canonical: docs/orchestrator/00-autonomous-factory-north-star.md — the D-Zed decision]`. **Not built by Phases A–C** — it starts only once both folders' commands (the `-fabrik` twins **and** the Traycer `-command` sources) are finalized. Recorded here so the arc is visible; it gets its **own** `/fabrik-spec` → `/fabrik-plan-after-chat` cycle when it begins (it is a code project, not a command twin).
