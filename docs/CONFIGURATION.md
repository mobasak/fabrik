...
`shape.database_url_app_role` (boolean, default `false`, requires `needs_database`) is the per-project switch. `fabrik scaffold --db` emits it `true`, so a new project is born on the app role; an existing project flips it after `fabrik app-role-check` passes, and unsetting it rolls back. The runbook and the converge table are in [`docs/operations/fabrik-lifecycle.md`](operations/fabrik-lifecycle.md) § App-role cutover and rollback.
