...
| `lint-ratchet — ruff changed X → Y, so the stored count was measured under a DIFFERENT ruleset` | The linter moved under the baseline. An absolute stored count is only comparable within one ruleset, so the comparison is unfounded in BOTH directions — a widened rule reads as fresh debt, a removed one silently lowers the floor | Re-seed explicitly: `python3 scripts/enforcement/check_lint_ratchet.py --reseed`, in a commit that says the linter moved. ⚠️ This used to re-seed itself and PASS, which meant real debt landing in the same change as a ruff bump was absorbed into the new floor and never seen (01M1RE497). Measured when the change landed: 17 of 49 repos carry a baseline, 15 of those have no version field (unaffected), and exactly one — site-provisioner, stored 0.14.10 against a live 0.15.10 — reds until re-seeded |
