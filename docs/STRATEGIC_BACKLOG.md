...
**Follow-up surfaced during this work (NOT yet on a tier):** the same git-vs-live drift exists for `configs/prometheus/prometheus.yml` (md5 mismatch verified 2026-06-13). Same root cause — `drivers/prometheus.py::add_scrape_target` writes to vps1 live, never to git. A prometheus sync is harder than gatus's snapshot-replace because scrape jobs accumulate; design call deferred to its own session.
