...
- [ ] **[infra]** **Telegram legacy-Markdown parity at the alerting boundary** (external-services review pass 33, CW4): `libs/alerting/telegram.py` sends `*{title}*\n{body}` with `parse_mode: Markdown` and escapes nothing, so any caller whose body carries an odd number of `_` (or a stray `*`/`` ` ``/`[`) gets HTTP 400 and the alert is lost with only a warning logged — measured on the chain: 1 of 346 triage names (`bridge_internal`) can reach the classify INFO alert's provider list; the chain's step alert is parity-safe by construction (graded). The class fix is escaping at the boundary (every one of the 8 `send_alert` call sites is exposed); mirror: a caller passing intentional Markdown loses its formatting. Blocked by: measuring which callers pass Markdown on purpose.
