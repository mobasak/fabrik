...
- **[infra] 10 confirmed defects in the enforcement git-decoder spec, left unfixed by the D-278 stop (2026-09-20)** — `/fabrik-spec-review` closed at DRAFT on the scope-growth stop (confirmed/own-fix `13/4 → 12/12 → 10/9`, 2 of the last 3 rounds inside the review's own fix prose). The 10 are enumerated with executed evidence in `docs/superpowers/specs/2026-09-19-enforcement-git-decoder-design.md` § OPEN DEFECTS: 5 UNBUILDABLE (the MULTI-USE wrapper deletion contradicts its own caller staging; `check_script_headers.py:206` declared single-shape is polymorphic across three AND its `env=LANGUAGE=C` has nowhere to go in the mandated signature; the second cobra rule names Python-2 `commands` and misses `subprocess.getoutput`; the `GitUnavailableError(OSError)` MIRROR names 4 sites where 5 exist and none catches the timeout it describes; Stage 3's bullet was not updated with Stage 2's) and 5 stale satellite counts. **Destination:** one fix pass over that spec in which the FINDER and the FIXER are different agents — the whole reason these stand open is that the reviewer fixing its own findings regenerated the surface it was correcting.
