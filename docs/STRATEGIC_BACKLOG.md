...
| :--- | :--- | :--- | :--- |
| **M** | **DR drill on a throwaway VPS** — run [`scripts/bootstrap/bootstrap-hub.sh`](../scripts/bootstrap/bootstrap-hub.sh) against a fresh VPS, measure wall-clock against the ≤90 min target in [`vps-hub-rebuild.md`](infrastructure/vps-hub-rebuild.md). Same for spoke via [`bootstrap-vps.sh`](../scripts/bootstrap/bootstrap-vps.sh) (≤30 min target). **Partial drill ran 2026-06-07 on Vultr — caught 2 real bugs (step_14b quote escape + SSH user transition trap), wall-clock 2m 39s to crash, both bugs fixed + 6 defenses shipped (commits `ae5f20f` + `11efe1c`).** Full re-drill blocked on next operator window OR on `fabrik vultr drill spoke` shipping. | "Scripted end-to-end" today is target-not-measured; until drilled to completion, the DR-in-hours claim is aspirational. | Either: (a) operator provisions a Vultr droplet via dashboard + we run the drill manually (~$0.05), OR (b) `fabrik vultr drill spoke` ships per [`docs/development/plans/2026-06-07-fabrik-vultr-provisioning.md`](development/plans/2026-06-07-fabrik-vultr-provisioning.md). |
| **L** | **`fabrik vultr` — on-demand VPS provisioning** (permanent + disposable). Full plan at [`docs/development/plans/2026-06-07-fabrik-vultr-provisioning.md`](development/plans/2026-06-07-fabrik-vultr-provisioning.md). Two modes: `fabrik vultr provision <name>` (becomes a permanent fleet member, mesh+DNS+Backrest+observability registration auto-runs) AND `fabrik vultr drill <kind>` (throwaway, auto-destroyed, drill report written to `logs/dr-drill-history.jsonl`). Eliminates the operator-discipline traps from the 2026-06-07 drill by making the whole flow one well-typed call. | Today's DR-drill operator workflow is 15+ manual steps each of which can fail. Plan also adds cost guardrails + monthly cost report + quarterly hub drill cron. | Operator generates Vultr API key (see plan §"How to get the Vultr API key") + commits ~4-5 days of focused work to implementation Phases 1-6. |
