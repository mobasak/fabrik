...
- [ ] **[infra]** **The /fabrik-review convergence bar reopens the loop on ANY receipt row — the D-191 review took 11 rounds / ~8 h on an 85-file surface because RECORDED and REFUTED rows count as findings** (operator ask 2026-09-08: "all my day is passing while waiting reviews"). Proposed ruling for the operator: the closing round is one with zero CONFIRMED code/doc defects (RECORDED/REFUTED rows do not reopen it); every finder seat carries a hard time box (15 min — the Opus seat overran twice by 20–30 min); a review surface is capped near 25 files (a change that grows across nine fix commits is re-cut per range). Seat COUNT is not the lever — a round's wall-clock is the slowest seat + the adjudicate/fix/commit cycle, ~40 min whatever the count (D-194). Owner: infra; needs a D-row.
