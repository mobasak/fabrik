...
- **[infra] Stop hook: a `BLOCKED:` line exempts every stall shape whatever its cause or format (2026-09-24)** — `.claude/hooks/final_gate_stop.py` (`_GATE_EXEMPT_GLOBAL_RE`, `_DEFER_BLOCKED_RE`) exempts any line-start `BLOCKED:`, as the approved spec (`docs/superpowers/specs/2026-09-23-stop-and-compaction-enforcement-design.md` § C1, "otherwise only `BLOCKED:`") chose; CLAUDE.md says BLOCKED has three named causes and a required format (`BLOCKED: <what> — searched: <sources> — missing: <need>`). Measured on the V1 corpus (16,278 interactive turn ends): 115 carry a line-start `BLOCKED:`, 34 of them with `searched:` and `missing:` on that line. With DEFERRAL now blocking, an unformatted `BLOCKED:` is the remaining unchecked exit. Tightening it changes an approved spec decision — awaiting the operator's design call (asked 2026-09-24). Owner: infra.
