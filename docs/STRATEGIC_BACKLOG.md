...
- **[infra] The Stop hook's run-record cause reads a legitimate WAIT on a dispatched background Workflow or Agent as abandonment (2026-09-23)** — while `/fabrik-spec-review` round 1 waited on its review-loop workflow, `final_gate_stop.py`'s fifth cause blocked the turn three times ("COMMAND STILL RUNNING"). The only in-turn wait that passed was a foreground `until` loop. The kaizen stream holds 1,379 `stop_block cause=run-record outcome=warned_through` events in its 35 days (snapshot `docs/reference/research/2026-09-23-stop-compaction/stop-events-2026-09-23.json`), and some unknown share of them is this. It is the mirror of the premature-stop problem: a false BLOCK, not a false stop. The fix belongs to the run-record cause: treat a same-session background task still running (the Stop input's `background_tasks[]`, E1 of the spec below) as a legitimate wait. Out of scope of `docs/superpowers/specs/2026-09-23-stop-and-compaction-enforcement-design.md` (its Intake I17). Owner: infra.
- **[infra] Four own-fix defects the stop-and-compaction spec review RECORDED under its scope-growth stop — mandatory inputs for `/fabrik-plan-after-chat` of that spec (2026-09-23)** — `docs/superpowers/specs/2026-09-23-stop-and-compaction-enforcement-design.md` converged with them routed here, not fixed (D-278's own-fix exit). (A-O40) C1's D4 frame fires on factual prose: "The fix takes effect in a new session", "Hooks load when you start a new session", "Verified in a fresh session", "The new session will inherit the synced hooks" (executed on `d4_probe.py`'s regex: 5 of 5 fire); the plan narrows D4 to a subject that is the agent's own remaining work and adds these as green fixtures. (A-O41) the DECISION clear skips every `/`-prefixed prompt, so an operator who answers by running a command leaves the block open; the plan clears on a `/`-prefixed prompt that is not a built-in (`/compact`, `/context`, `/cost`, `/model`, `/autocompact`). (A-O42) V3's fixture must pin the slash-command clear rule. (A-O43) `asked:`'s 12-character substring check accepts an expanded command body; the plan requires the quote to come from an operator entry that is not a command expansion and to end in `?`. Also recorded: `draw.py`'s docstring says "run mine.py first", which re-mines rather than re-draws; and `scope:` quotes are not verified against the operator's request. Owner: infra.
- **[infra] `scripts/sysadmin/kaizen_outcomes.py`'s session-level `premature_stop` (version 9) still names two causes in its formula at `:1218`, while it imports the three-member `PREMATURE_CAUSES` (2026-09-24)** — found by the T01b review of `docs/development/plans/archived/2026-09-23-plan-1-stop-and-compaction` (finding R1-T01b-O2); outside that plan's File Scope. Once the Stop hook emits `cause=deferral`, the metric's numerator grows while its definition hash does not, splicing the series. Fix: build the formula text from `sorted(PREMATURE_CAUSES)` as `kaizen_collect_v2.py` now does, bump the version, re-pin the tripwire. Due before the V4 reading on 2026-09-30. Owner: infra.
- **[infra] `templates/governance/CLAUDE.md` cites hub decision ids bare in places (`:37`, `:70`, `:444`, `:471` at 537f359ab) with no written rule for the `hub D-NNN` prefix (2026-09-24)** — found by the T02b review of `docs/development/plans/archived/2026-09-23-plan-1-stop-and-compaction` (R1-T02b-note). D-054 resolves to a DIFFERENT decision in two project ledgers (youtube, web-ecommerce-factory), so a bare hub id in the fleet contract can send a project agent to the wrong row. Fix: state the rule once in the template (hub rows are written `hub D-NNN`), prefix the remaining bare ids, and add a grader that no bare `D-NNN` in the template resolves to a project-local row. Owner: infra.
- **[infra] Stop hook: three recorded residues of the T03 DEFERRAL review (2026-09-24)** — from `docs/development/reviews/2026-09-23-plan-1-stop-and-compaction-T03-review.md`, recorded under the scope-growth stop, not fixed: (1) `_decision_quote` still absorbs a later quote that opens after `(`/`,` or onto `...` (a genuine `asked:`/`scope:` so quoted is refused; the agent rewrites it, CAP-bounded); (2) a negated gate class on the Why line (`not a deploy`) still passes `ground: gate` — the V6 sampled audit is the counter-measure (D-371); (3) the pre-existing `_PROMISE_VERB_RE` lookahead `run(?!\s+through)` fires on "run it through X". Also `scripts/enforcement/check_certification_coverage.py:24,258` still names `_midrun_marker` as a plan-lock reader, which T03 made dead code in the hook. Due with the V4 reading, 2026-09-30. Owner: infra.
- **[infra] `scripts/thread_anchor.py`: three recorded residues of the T04 review (2026-09-24)** — from `docs/development/reviews/2026-09-23-plan-1-stop-and-compaction-T04-review.md`, recorded, not fixed: (1) the prompt-time re-harvest re-reads the previous final message on every prompt, so it re-adds an anchor `done` just closed and re-stamps its time, and an anchor echoed in every message never ages into the 72 h fold (pre-existing); (2) `_anchor_key` masks every digit, so `plan-1.md` and `plan-2.md` collapse to one anchor key (pre-existing); (3) a word-for-word identical WHOLE message re-emitted after the operator answered its DECISION block is refused as a stale echo (by design). Due with the V4 reading, 2026-09-30. Owner: infra.
- **[infra] Stop-and-compaction V4–V6 readings due 2026-09-30 and 2026-10-07 (2026-09-24)** — plan `docs/development/plans/archived/2026-09-23-plan-1-stop-and-compaction` shipped the Stop hook's DEFERRAL check, the DECISION block and WHERE YOU ARE fleet-wide on 2026-09-24. Spec `docs/superpowers/specs/2026-09-23-stop-and-compaction-enforcement-design.md` § Validation: **V4 (2026-09-30, 2026-10-07)** — re-judge a uniform random 80 of the turn ends that still end deferring (`python3 scripts/sysadmin/stop_mine.py --root ~/.claude-fleet/active/projects --since 2026-09-24 --backtest --out <scratch>/v4.json`, then draw 80 and judge with three native seats; target false stops 32.5% → ≤10%), and 25 compactions with no live command run (drift 9/25 → ≤3/25; read each WHERE YOU ARE block against the next turn). **V5** — whether an earlier auto-compact window pays, before any `autoCompactWindow` is set (D-376). **V6 (2026-10-07)** — a sampled audit of accepted DECISION blocks by native judge seats (`kaizen_collect_v2` `decision_block` events by ground; a fake `gate`/`owned` block is the cobra path). Also observed at the V3 probe on 2026-09-24: the WHERE block's `NEXT (before the compaction)` line carries no age, so a NEXT harvested hours earlier reads as current — decide at V4 whether to print its age. And the template carries § FINAL OUTPUT twice (`templates/governance/CLAUDE.md`, both copies graded byte-identical by `tests/test_governance_template_split.py`) — fold the duplicate into one. Owner: infra.
- **[infra] Stop hook: a `>`-quoted footer line inside a `BLOCKED:` escalation ends its field window early (2026-09-24)** — `.claude/hooks/final_gate_stop.py::_blocked_header` (D-391) ends the `searched:`/`missing:` window at the first footer line outside a quoting FENCE, but `_FOOTER_KEY_RE` also matches a `>`-quoted `> NEXT:`, so `BLOCKED: x —\n> NEXT: example\nsearched: a — missing: b` is refused although formatted (the D-391 review's pass 3, A-S3, recorded under the scope-growth stop). Measured: 0 of the 9 exempted corpus turns use a `>` line; a wrong refusal still releases after 3 attempts (CAP). Revisit at the V4 reading (2026-09-30): if `>`-styled escalations appear, treat `>` lines as the escalation's own for BOTH the header and the window end. Owner: infra.
...
## [infra] `work.py migrate-backlog` resolved-rule edges seen only on synthetic titles (2026-09-24)

Recorded by the T03 review's closing pass; no hub row shows either today, so neither was fixed. (1) A table cell that starts with a hyphenated status word (`| infra | DONE-ish follow-up | open |`) resolves the row, because `\bDONE\b` matches before the `-` and `_STATUS_AFTER_PIPE_RE` accepts it. (2) Only the FIRST stays-open phrase after a status marker is weighed, so `CLOSED 2026-09-20 … was still open and still open today` resolves because that first phrase is past tense. Fix when a real backlog row exhibits either; destination `scripts/work.py` `_row_is_resolved`. Owner: infra.

## [infra] `thread_anchor.py cmd_line` anchor lines can exceed the 300-char cap (2026-09-24)

Recorded by the T04 review (A-H1, A-H2): the pre-existing anchor lines `cmd_line` prints are not bounded the way T04's new `prompt_block` lines are (300 chars), so one long thread title widens every prompt. Untouched by T04's diff. Destination `scripts/thread_anchor.py` `cmd_line`. Owner: infra.

## [infra] `check_convergence._STATUS_LINE` misses `Status: **EXECUTED**` (2026-09-24)
...
## [infra] Long-running hub shell scripts execute whatever an in-place edit leaves at their byte offset (2026-09-23)

bash reads a script incrementally, so a script still running when its file is rewritten in place resumes at its old byte offset in the NEW content. `scripts/sysadmin/selfwatch_arm.sh` (waits up to an hour, run by every session) is now one function called on its last line and cannot be hit (mail 01M36FPTT9YS7ADH3CYQ5MQ1P3). Still exposed, by runtime: `scripts/governance_sync_postcommit.sh` (~60 s per trigger commit; docs cite its line numbers, so a wrap must move those citations with it), `scripts/kilo-benchmarks/daily_refresh.sh` (cron), `scripts/wsl_startup_hook.sh`, and the box-local `~/.claude/bin/claude-selfwatch.sh` (hours; mesh, read-only without an operator go-ahead). Fix at the class: the same main-function shape, or atomic replacement for every writer. Owner: infra.

## [infra] Doc↔script coupling reads `.py` headers only — a shell script's `# AFTER-EDIT:` is decorative (2026-09-23)
...
## [infra] ~40 `final_gate.py:N` line citations outside the CLAUDE.md GATE row have no grader (2026-09-24)

D-382 remapped every live `final_gate.py:N` citation when its edit moved the file's lines, and graded only the CLAUDE.md GATE row's cites (`tests/test_governance_template_split.py`). The rest stay ungraded: three enforcement scripts (`check_plan_lock_release.py`, `check_stage_artifacts.py`, `check_certification_coverage.py`), four tests, `docs/workflows/FINAL_GATE_WORKFLOW.md`, `docs/reference/plan-lock-lifecycle.md`, `docs/reference/command-evaluation-checklist.md`, `docs/workstation/wsl-startup-inventory.md`, `INDEX.md`, and dated rows in this file. The remap kept each one pointing at the code it pointed at in HEAD; whether that code was the RIGHT code at HEAD was never checked. `scripts/enforcement/check_citations_resolve.py` scans only specs, plans, reviews and `docs/reference/`, and proves only that a target line is non-blank. Fix at the class: cite a symbol (`final_gate.py::run_cmd`) where the reader needs the construct, not the line, or extend the resolver to assert a named construct. Known wrong at HEAD already (the D-382 review found them and left their claims alone): `check_plan_lock_release.py` (:123, :245, :568, :620), `tests/enforcement/test_plan_lock_release.py` (:452, :732, :918) and `docs/reference/plan-lock-lifecycle.md:86` cite `final_gate.py:2133`/`:2111` as an `output[:500]` cut that no longer exists (`clip_output` replaced it, `final_gate.py:249`), so the claim is false, not only the line. `docs/LESSONS_LEARNT.md` and `CHANGELOG.md` cite `final_gate.py:1111` for the Kilo health check (it was never there), but those are dated rows. Measure how many of the rest were wrong before fixing the class. Owner: infra.

## [infra] kaizen terminator_spam counts a final block emitted with no open run in its numerator (2026-09-24)
...
## [infra] Adopt the markdown table-termination detector? The measured fire rate is in (2026-09-24)

iterative_image_editor ran its markdown-it table detector over the hub's 1,464 tracked `*.md` (mail 01M2Z7E6PXZ7ZFGJM4Y71VCZD7). It found 46 files and 491 orphan lines: 3.1% of files and 1.0% of pipe-leading lines. They verified 7 of the 46 against rendered HTML, with 0 false positives. The heaviest clusters are review receipts and ledgers, where a table cut short by a blank line renders the audit trail as prose. One proven hit, the README production-infrastructure table, is fixed; the review of that fix also found two false cells that had been hidden as prose. The decision still owed is whether to adopt it as a gate check, warn-only first, over `docs/development/reviews/**` and the ledgers. The remaining 39 files are candidates, not confirmed defects: render them before counting. Their report: iterative_image_editor `/opt/iterative_image_editor/docs/reference/2026-09-20-fabrik-table-detector-sweep.md` (f203d6b). Owner: infra.
...
**FILE 31 — `core/ocoron-design-system.md` (2026-09-24): the pack splits into `core/design-system-template.md` (structure, token slots, computed-contrast contract, the `docs/design-system.md` contract) and the Ocoron brand; its colours now pass AA in both modes (D-406).**
- **brand-identiy-creator (mailed 01M39HKMJN6YS03VYQAAR3NKY6, and the #10 mail below):** build gap #1 (semantic colours from the brand palette, both modes) and #4 (a computed contrast table that fails closed), then emit `For-Your-Developer/design-system.md` in the template's contract shape (#10). Operator-approved 2026-09-24.
- **Fleet (mailed — see D-406):** the saas-skeleton's bundled `docs-site/src/css/custom.css` and `templates/mobile-app/src/global.css` ship the Ocoron brand by default (D-051); the mobile file's light `--color-text-muted: #999999` fails AA and it still emits `--color-secondary`/`--color-purple` and `#2980B9`/`#9B59B6`; `_scaffold_chrome_extension` labels `#4f46e5` as "Ocoron Design System (Compact) tokens"; the i18n kit's Level 2/3 call the retired Kilo CLI; `office-extension` has no rules pack and no design-system matrix row.
- **Cross-pack, for their own turns:** `core/tojlo-design-system.md` still restates the pre-split state colours (`:522-526`, `:539-540`, `:1160-1161`, `:1908-1909`, `:1957-1961`), a light `--text-muted: #999999` (`:2015`) that fails AA, and a `#F5A623` on `#0A0A0A` row stating 9.31:1 (`:1842`, computes 9.77:1); `mobile-app/tojlo-mobile-design-system.md:72` names `--color-purple`/`--color-secondary`; `mobile-app/80-mobile.md:174-182` (§ Ocoron Design System (Mobile)) mandates Ocoron's colours, fonts, dark default and spacing for EVERY mobile project with no house-identity gate (D-051), contradicting its own Related Rule Packs line, and says `react-native-unistyles` while the mobile template compiles its theme with Uniwind; `youtube` (the one project `docs/design-system.md` on the box) names ocoron as the parent of its spacing, density, motion and states — now the template's.
- **Own-fix residue from the scoped review's remainder round (routed here by the scope-growth exit, not re-reviewed):** `core/design-system-template.md` § Mapping to the scaffold's variables — the Tailwind CSS-first alias `--color-surface-0: var(--surface-0)` should sit in `@theme inline` (a plain `@theme` alias resolves on `:root`, so a mode hook below `<html>` never reaches `bg-surface-0`); and with shadcn `--muted` mapped to `--surface-2`, the saas-skeleton's `hover:bg-muted` sites (AppShell.tsx, StateBlocks.tsx, settings/page.tsx and 8 more) and its `bg-muted` progress track land on `--surface-2`, not the hover/track slot `--surface-3` — state it under the table or move those sites to `hover:bg-accent`.
...
- **Cross-pack, for their own turns:** `core/self-healing.md:74` still says the deadman restart does not re-check the protected list — now stale (the deadman routes through `execute("A", "restart_container")`, D-276); `ai/00-ai-model-selection.md` pins the `opus` alias to a dated id while Anthropic's model-config page resolves it to the recommended Opus for the provider.

**FILE 28 — `saas/88-saas-launch-checklist.md` (2026-09-24): the launch checklist states what Paddle, CCPA, KVKK and Turkish VAT actually require, and names the module behind each item (D-400).**
- **Cross-pack, for their own turns:** `saas/87-abuse-detection.md` still collects the FingerprintJS hash on every registration with no ePrivacy Art. 5(3) consent gate (88 now gates it); `mobile-app/81-mobile-billing.md` § Teknokent states a flat 0% export / 20% domestic split that KDV Geçici 20/1 overrides for TGB-produced software; `saas/60-saas-ui.md` § Legal lists /terms, /privacy, /cookies and the banner but not the Refund, Pricing and TRY-lane pages 88 now requires; `core/25-data-postgres.md` should state that PgBouncer (current release) defaults `max_prepared_statements` to 200 (ledger se-07, se-08). Paddle's domain approval and Acceptable Use Policy are not yet in 88's gate (ledger op-16).
- **fabrik-lib (mailed 01M39AJE6NDFAPKV5JMTCYRXWQ):** docs-site `terms.md` promises a 5-day Paddle window and `cookies.md` demands the CCPA link unconditionally; neither terms template carries Paddle's mandatory reseller clause or the TRY-lane pages (mesafeli satış sözleşmesi, teslimat-iade, hakkımızda, iletişim); gdpr-data-rights documents `/api/users/…` while mounting `/api/v1/users`, and its one-click unsubscribe has no route.
- **Scaffold (fleet, mailed 01M39AJE8MAJJKX0N1YYD0J56D):** saas-skeleton's `app/(marketing)/terms` grants "personal, non-commercial transitory viewing only" and neither page carries billing, refund or governing-law content — render them from `legal-pages`.
...
- ~~**Approval-window conflict (fleet mail 01M37F6Z1PC7Y5R92EA5CN3EPH)**~~ ✅ **RESOLVED 2026-09-23 (D-378)**: the hub default is 300 s again; the explicit spec values are the open item below. The original entry follows. The hub spec defaults `code_fix_window_sec` to 1800 and the driver passes it through, against fabrik-lib's operator ruling of 300 (fabrik-lib D-236); `calendar-orchestration-engine` runs Tier D at 1800 today. The hub-side decision is owed.
- **[operator] Do the 33 explicit `code_fix_window_sec: 1800` lines follow D-378's 300 s default? (2026-09-23)** — 33 of the 72 live specs (`specs/services/*.yaml`) set it explicitly (19 from the 2026-07-05 sweep 2c5b56e1a, 14 written later while 1800 was the default; 4 `.draft`/`.superseded` files carry it too), so the new default reaches none of them. Only `calendar-orchestration-engine` also sets `auto_code_fix: true`; it is the one live Tier-D window, and it stays at 30 minutes until its line changes. Options: drop the explicit lines (they inherit 300), or keep calendar's as a deliberate per-project choice. Owner: fleet, on the operator's word.
- **Cross-pack drift, for 60-watchdog's re-visit:** its Tier A/C tables describe the parameter-bearing actions as working, its Tier-D window default was 1800 (re-cut to 300 on 2026-09-24, mail 01M3806TBQ86A96X9K9N7RVR4P), and its § Worked example — OOM diagnosis narrates the kernel oom-killer line reaching `docker logs`, which it does not (the restart-count pass fires).
...
password cannot be reset for one of them without breaking the others; the registrar refuses (D-408). A cutover for
...
is rebuilt from spec env + secrets alone — dropping `DATABASE_URL_OWNER` after an app-role cutover, the case D-410
