...
| :--- | :--- | :--- | :--- |
| **M** | **Spoke DR — end-to-end measured recovery (`provision` drill against a real billed test spoke)**. `fabrik vultr drill spoke` is shipped + hermetic (`bootstrap-vps.sh --skip-mesh --skip-dns + --verify`, ~8m, zero orphans live-proven). What's still NOT measured: the mesh + DNS + monitoring registration path that `fabrik vultr provision <name>` performs against a real billed instance, and B2 restore in the disaster path. To close: run `fabrik vultr provision vps4` (or similar test name), measure wall-clock to "vps4 green in Gatus", then `fabrik vultr destroy vps4 --reverse-fleet-add` to unwind. Costs a few dollars + briefly mutates vps1 wg0/DNS/Prometheus/Backrest — operator-approval required, can't be drilled hermetically. | Drilled OS+bootstrap recovery proves nothing about the mesh/DNS/monitoring fleet-add path that the real disaster would hit. | Operator schedules a focus window + accepts brief mutation of vps1 state. |
