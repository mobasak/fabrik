...
- **[infra] Stop hook: four DECISION/BLOCKED enforcement gaps the T06 docs review found (2026-09-24)** — executed on `.claude/hooks/final_gate_stop.py` at 7136d51cf, none a doc error, each a place where the contract (CLAUDE.md § FINAL OUTPUT) is stricter than the hook: (1) the `(a)/(b)` menu ban is not checked — a menu block under `ground: owned` with a real operator question passes `parse_decision_block` (`:2294`); (2) any line-start `BLOCKED:` exempts every stall shape including DEFERRAL whatever its cause (`_GATE_EXEMPT_GLOBAL_RE`, `_DEFER_BLOCKED_RE`), so a context-budget stop wrapped in `BLOCKED:` passes although `# Compact instructions` says context is never a reason to stop (D-374); (3) the `owned` ground fails OPEN on an unreadable transcript (`:2376-2380`), and "still unanswered" is never checked; (4) `scripts/thread_anchor.py:67` `_NEXT_RE` harvests only a plain `^NEXT:` while the hook's `_NEXT_LINE_RE` (`:1527`) also treats `**NEXT:**`, `- NEXT:` and `> NEXT:` as the footer, so WHERE YOU ARE shows a stale NEXT after a bold one. Measure each against the V4 sample (2026-09-30) before closing it: a check that fires on legitimate turns is wallpaper. Owner: infra.
