...
## Phase 4 Registrar Coverage Status (audited 2026-05-09)

The 9-registrar architecture for the Fabrik orchestrator (`fabrik apply <project>`) defines automated platform-side actions per shape attribute. **The orchestrator implementation is partial — most cross-service registrations today are still done manually.** Status snapshot:

| # | Registrar | Driver implemented? | Manual fallback today | Affected services |
|---|---|---|---|---|
| 1 | postgres | NO — manual `CREATE DATABASE` + user via `psql` on `postgres-main` | Yes | 4 DBs created so far (glitchtip, proxy_management, translator_service, site_provisioner) |
| 2 | redis | NO — DB index assigned manually, no central registry | Yes | 2 DBs in use (db3=authelia, db4=glitchtip-web); 14 free indexes |
| 3 | gatus | PARTIAL — config files dropped under `/opt/monitoring/configs/gatus/apps/<app>.yaml` manually | Yes | 28 endpoints across 14 files |
| 4 | backrest | NO — plans + repos created via Backrest UI manually | Yes | 1 repo (b2-vps1), 4 plans |
| 5 | glitchtip | NO — projects created via GT API by hand; DSN injected to Coolify env via `gt_provision_helper.py` (semi-automated) | Mostly | 7 GT projects, 5 of which actively flowing |
| 6 | grafana | NO — dashboards generated by `configs/grafana/build_dashboards.py` and dropped via existing bind mount; per-service templates not yet implemented | Yes | 9 dashboards (5 Fabrik + 4 community) |
| 7 | authelia | NO — rules added by hand to `configuration.yml`; sync hook propagates to volume + restarts container | Sync hook only | 8 rules (1 deny default + 7 bypass + 1 two_factor) |
| 8 | meilisearch | NO — no service currently uses meilisearch indexes | N/A (zero indexes in use) | 0 indexes |
| 9 | prometheus | PARTIAL — 13 jobs in `prometheus.yml` added by hand; SIGHUP reload pattern documented but not automated | Yes | 12/12 targets up |

**What this means for `fabrik apply`:** the orchestrator's Phase 4 codepath does NOT yet make any of these registrar calls. Currently when scaffolding a new service:
- The orchestrator handles Coolify Application creation, env injection, network attachment, deploy.
- Postgres DB / Redis DB index / Gatus endpoint / Backrest plan / GlitchTip project / Grafana dashboard / Authelia rule / Meilisearch index / Prometheus scrape job all need to be configured **manually** afterward.
- The 9-registrar table represents the **target architecture**, not current behavior.

**Priority for implementation** (highest leverage first):
1. **postgres + glitchtip** — every new service needs both. Manual provisioning is the biggest friction.
2. **prometheus** — adding scrape jobs is cookie-cutter; high automation ROI.
3. **gatus** — same pattern as prometheus, file-drop-and-reload.
4. **authelia** — rules are domain-specific; the sync hook already handles the volume copy + restart side, so the registrar only needs to compose the rule.
5. **backrest, redis** — needed less often.
6. **grafana** — per-service template can come from `configs/grafana/build_dashboards.py` extension.
7. **meilisearch** — zero current consumers; defer until a service needs it.
