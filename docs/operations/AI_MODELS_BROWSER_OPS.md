...
**17 rows actively scraped daily** across 7 vendors. Use `/opt/ai-model-catalog/engine/audit_direct_vendor_freshness.py` (ai-model-catalog) for the live count.
...
.venv/bin/python /opt/ai-model-catalog/engine/backfill_unknown_providers.py            # dry-run
.venv/bin/python /opt/ai-model-catalog/engine/backfill_unknown_providers.py --apply    # write

# Dry-run (no DB writes; shows what would happen)
.venv/bin/python /opt/ai-model-catalog/engine/fetch_direct_vendor_prices.py

# Apply to DB
.venv/bin/python /opt/ai-model-catalog/engine/fetch_direct_vendor_prices.py --apply

# Subset of vendors
.venv/bin/python /opt/ai-model-catalog/engine/fetch_direct_vendor_prices.py --vendors soniox,deepgram
...
.venv/bin/python /opt/ai-model-catalog/engine/audit_direct_vendor_freshness.py
.venv/bin/python /opt/ai-model-catalog/engine/audit_direct_vendor_freshness.py --status seed-only
.venv/bin/python /opt/ai-model-catalog/engine/audit_direct_vendor_freshness.py --csv audit.csv
```
...
6. Add tests at `/opt/ai-model-catalog/engine/test_direct_vendor_parsers.py`.
...
The end-to-end onboarding pipeline (registry edit → orchestrator fetch → audit MD generation → cleanup) is validated by `/opt/ai-model-catalog/engine/test_add_vendor_roundtrip.sh` (ai-model-catalog). Run it before shipping a new-vendor PR to confirm the workflow still works:

```bash
time bash /opt/ai-model-catalog/engine/test_add_vendor_roundtrip.sh
...
- If the diff is real (vendor changed prices), update the seed in `/opt/ai-model-catalog/engine/seed_direct_vendors.py` (ai-model-catalog) so the next daily run can write cleanly.
