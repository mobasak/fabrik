...
- step_11 — backrest compose-up + spoke's own backup chain restoration (config dry-validated by step_09b c-dry/2 + c-dry/3, but not started). **However `step_11b` DOES close the B2-write loop in drill mode:** it inits a sandbox B2 repo with the restored spoke creds, writes a test snapshot, reads it back, and cleans up ("spoke can WRITE to B2 with restored creds — loop closure VALIDATED"). Only the backrest *container* compose-up under the live prefix remains unstarted.
- `step_11c` installs `fabrik-compose-boot.service` + `/usr/local/bin/fabrik-compose-boot.sh` fresh from the repo (reboot-race safety net) — generated by bootstrap, never restored from restic.

Real DR confidence on the still-unstarted paths (mesh handshake, traefik, backrest container runtime) needs a non-drill exercise on isolated hardware (probably an isolated WireGuard mesh with throwaway peer pubkeys + sandbox B2 prefix). Not done yet (2026-06-15).
