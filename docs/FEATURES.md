...
+ `core/25-data-postgres` + `core/30-ops` + all twelve 12-Factor axes, always injected regardless of glob (in the HUB a partition whose every voting path sits under one of `review_rubric.TOOLING_PREFIXES` — `.claude/hooks/`, `scripts/`, `commands/`, `.windsurf/`, `templates/governance/`, `tests/`, `docs/reference/`, `docs/workstation/` — gets the TOOLING floor instead: `core/10-python` for code, `core/40-documentation` for an all-prose partition; code paths vote first, then the tooling prose, while root-level docs (the ledgers and governance prose — CHANGELOG, INDEX, AGENTS, CLAUDE, README and the like: any file with no `/`) and `docs/` outside `docs/reference/` and `docs/workstation/` never vote; in a project the service floor is unconditional), so
