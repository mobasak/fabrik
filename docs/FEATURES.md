...
+ `core/25-data-postgres` + `core/30-ops` + all twelve 12-Factor axes, always injected regardless of glob (in the HUB a partition whose every path sits under one of `review_rubric.TOOLING_PREFIXES` — `.claude/hooks/`, `scripts/`, `commands/`, `.windsurf/`, `templates/governance/`, `tests/` — gets the TOOLING floor, `core/10-python`, instead, doc paths voting only when they are the whole partition; in a project the service floor is unconditional), so
