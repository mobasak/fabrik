...
hub-only check owes (a) a hardcoded hub root, (b) an explicit `running_on_hub()` skip that also
refuses to reach ACROSS from a project into `/opt/fabrik` on the dev box, and (c) a test that RUNS the
script from a fake project dir and asserts rc=0 with no traceback. And the general form: a gate's
blast radius is the set of repos it lands in, which for this directory is never one.
