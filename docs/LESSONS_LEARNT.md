...
- **What happened.** To learn whether a CLI refresh extends the OAuth refresh chain, a session ran
  `claude -p` on a TEMP COPY of mob@'s fleet dir with `expiresAt` forced past. The copy refreshed,
  received the ROTATED refresh token, and was deleted. The real dir kept the spent token; the tick's
  next refresh (15:50) was refused and the CLI wrote the logged-out shape (`accessToken: ""`,
  `expiresAt: 0`). mob@ was the only account with weekly headroom, so `--switch mob` was refused
  ("credentials … carry no access token") while the fleet was on HOLD. One `/login` in the real dir
  revived it. Same day, ob@ lapsed at its stored `refreshTokenExpiresAt` while it was the ACTIVE
  account serving a session every ten minutes: use does not extend the chain, and the weekly
  `--keepalive` had pinged nothing in either of its logged runs (2026-08-31, 2026-09-07) because its
  idle gate read a credential mtime the tick renews daily.
- **The rules.** (1) A credential file is read-only for every probe: read the expiry fields, call
  the profile/usage endpoints with the ACCESS token (they rotate nothing), never force a refresh,
  never run `claude` in a copied dir — "on a copy" IS the consumption. (2) The chain is ~30 days
  from the last `/login` and nothing extends it; the recurring duty is a monthly `/login` per
  account in its own dir, and the tool's job is to say so in time (the 5-day warning carries the
  exact re-login line; the tick pushes once per chain inside 3 days). (3) A remedy printed by a tool
  is a claim: "run one claude turn" was printed for weeks and was false — test the remedy, not the
  warning.
