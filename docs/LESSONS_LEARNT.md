...
**Why it survived.** The change felt like presentation, and presentation feels untested — but a page IS this program's output, and half a dozen tests read it as a string. The signal that should have fired is mechanical and was available: `grep -rl "#external" tests/` before deleting the literal, or simply running the suite for the file I edited rather than the suites for the subsystem I was thinking about. Instead I chose the test scope from my mental model of the change ("this is about cost") rather than from the FILES the diff touched. The same session had already published the rule it broke: a green gate on the files you thought about asserts nothing about the files you edited.

**Lesson.** Pick the test scope from `git diff --name-only`, not from what the change is ABOUT. And when a diff deletes a literal from generated output, grep the test tree for that literal before deleting it — a rendered string is an interface. The repair is never to delete the assertion: re-point it at the mechanism (choosing a tab writes its hash; any existing pane restores from it), which survives the next implementation and still fails when the behaviour goes.

**Guard:** `tests/test_quota_dashboard.py::test_page_has_an_external_services_tab_with_a_lazy_iframe_and_freshness` now pins the restore mechanism, proven red by replacing the restore with `showTab("quota")`. The same review found the sibling class in `PIPELINE_ORDER` (three corpus sources committed with no order slot) and a fail-silent the calendar change had sharpened, both fixed in the same run.

# Lesson 154: an enforcement mechanism that a HARDENING made stricter must be re-read against the rules it is supposed to permit — the mirror lands on the permitted case, not the blocked one

**What happened.** `quota_stop.py`'s vetoes were hardened to scan the WHOLE command line, closing a real chained-command bypass (`git push && python3 evil.py` had passed a first-segment match). The mirror was never named: a PERMITTED command carrying shell punctuation inside a quoted argument now reads as a control operator. CLAUDE.md mandates `BLOCKED: <what> — searched: <sources> — missing: <need>`, and any operator listing two sources writes a semicolon there — so the hold structurally refused the very `command_run.py blocked` call its own message orders a held session to make, while the Stop hook blocked the turn for the record that could not be closed. Two repos deadlocked (trade-intelligence lost a day with a converged review uncommitted; this session hit it three times with three different wordings, concluding the WORDING was the variable).

**Why it survived.** Both halves were individually correct — the veto blocks what it should, the allow-list permits what it should — and the defect lives only at their SEAM, in the order they run. Reading either predicate alone shows nothing. My own filed finding named the wrong mechanism ("the allow-list is dead code") because I reasoned about the ORDER of two predicates instead of executing `decide()` against the commands I had actually been denied; one probe would have settled it in seconds. A peer's executed reproduction, feeding the verbatim denied commands from my transcript, produced the real cause.

**Lesson.** When you tighten a check, enumerate the cases it must STILL PERMIT and run them — the mirror of a hardening lands on the permitted set, and a permitted case that starts failing is invisible to every test written for the blocked one. And a mechanism whose own instructions are executable text (a mandated message format, a required command) must be tested against ITS OWN prescribed output: this hold refused the exact string its message told agents to produce.

**Guard:** `tests/test_quota_stop_hook.py::test_shell_punctuation_inside_a_quoted_argument_is_data_not_an_operator` runs the peer's eight probe commands in both directions, plus `test_the_masker_never_invents_or_hides_an_operator` for the masking invariants. Red-on-revert proven; the grader also caught a hole in the fix itself before it shipped (masking the `(` broke the `$(` pair the veto keys on, briefly allowing a double-quoted command substitution).

# Lesson 155: a gate that PASSES silently and a gate that never ran are byte-identical — and a flag the tool itself recommends can be the one that disables it

**What happened.** Three plan-set gates, all correct, all unreadable. (1) `check_plan_tickets --plan-dir` exited 0 with ZERO bytes on success, while two commands instructed agents to cite that silence as sizing evidence — an un-denominated claim by construction. (2) `check_ticket_breadth` printed its count FIRST and its payload after, so a `| tail` dropped the denominator; a reviewer read "16 of 24" off a tailed run when the figure was 20 of 33. (3) `--allow-external` — the flag the tool's OWN error message recommends for a scratch copy ("checking a scratch copy? pass --allow-external") — did not merely skip the read budget as reported: `check_plan_dir` returned `[]` for any dir outside the plans layout, so EVERY check was skipped. Proven by deleting the spine from a copy of a live 33-ticket set: exit 0, zero output, where the same set in place raises a hard ERROR.

**Why it matters.** (3) is a PASS standing in for a check that never ran — the fail-open the enforcement corpus exists to prevent — and it was reachable by following the tool's own advice. The reporter had it as "the budget is disabled"; reading the code found the wider shape. A finding accepted at its reported scope would have shipped a one-line skip notice over a function that was returning nothing at all.

**Lesson.** A green run must say what it graded and against what. Adopt the denominator at BOTH ends of any long advisory, since output ORDER is itself a bound. And when a report names a specific disabled check, read the enclosing function before fixing the named one — the reported instance is a sample of the population, not the population.

**Guard:** `tests/enforcement/test_check_plan_tickets.py` — `test_an_external_scratch_copy_is_still_checked_not_silently_skipped`, `test_the_read_budget_measures_a_scratch_copy_against_the_given_root`, `test_a_clean_run_says_what_it_graded_and_against_what`; `tests/test_check_ticket_breadth.py::test_a_tailed_read_still_carries_the_denominator`. All red on revert.
...
that could not function on its only deployment target. The plan put `claude_rotate.py` out of scope
("the governor is a reader"), which silently froze an assumption: that the VPS emits the same
`--status --json` fleet payload as the WSL dev box. It does not (single-key, no fleet scaffold) —
and pulling that thread on the live fleet exposed FOUR nested causes, none visible from the repo:
(1) the fleet-payload gap (parked snapshots, null telemetry); (2) no quota-free token freshness
(the direct oauth refresh is Cloudflare-403'd → an on-demand probe cannot work; "unknown → shed"
then WEDGED the routine loop — the canary caught it, rolled back); (3) ob@'s subscription had
lapsed at the payment layer (server-side entitlement refusal predating the deploy — the "dead
chain" was a symptom); (4) the datacenter vantage itself (api.anthropic.com 429s VPS IPs;
platform.claude.com 403s python's default UA). Every one was found only by EXECUTING the real
check on the real host — cold probe, then a REAL completion, then the ladder re-run after each fix.
The class rule: when a plan declares a dependency out of scope, the review must still verify the
dependency's CONTRACT ON THE DEPLOY TARGET (one ssh + one command here); and a deploy is validated
only by the full ladder on the target, never by the dev box's shape. (2026-08-30, fleet)

**Last Updated:** 2026-08-30 (Lesson 144 — **an unfollowable reading duty teaches skimming, and self-asserted reading proves nothing — compute the duty's scope and demand a greppable proof.** The rule-grounding gate said "open every ACTIVE pack, fresh reads only": 26 ACTIVE packs at the hub with ~3 relevant to a given scope, no grader anywhere. The honest answer "partially, not the full contract" drew the operator ruling that partial rule-reading IS the drift mechanism. Fixed same-day (plan-2, the third countable floor of the day): the MUST-READ set is COMPUTED — FLOOR + `review_rubric.py --changed <the touched surfaces>` MATCHED packs — and the Constraints Digest carries a verbatim quote + `file:line` per pack; `check_rule_grounding.py` greps exactly that on CONVERGED plans (you cannot quote a line from a pack you did not open). Rules: (1) a duty sized beyond its relevance teaches the violation it forbids — right-size the duty before adding a grader; (2) wherever a grader can grep, proof-of-work replaces assertion-of-work; (3) the honest "partially" is what made the fix possible — grade the gap, never the honesty.) Previously 2026-08-30 (Lesson 143 — **a fix that leaves a stale duplicate of the instruction it corrects has not shipped — the stale twin keeps teaching the defeated behavior, and in a HOOK it re-teaches it to every session.** Commit 50675991 made self-watch arming Monitor-based (the only form whose wake line can reach the pane), but a second, static copy of the same mandate in `session_orient.py`'s unconditional ORIENT text kept ordering the old `nohup … >/dev/null` form for two more weeks — so sessions obediently armed watches that consumed death markers and printed their one wake into /dev/null. web-ecommerce-factory's `86cf0e31` was revived 13 times, re-armed nohup-form per the stale paragraph on 08-20/08-22, delivered zero wakes after, and its 08-29 death went unrevived; the discriminator tests that would have caught the duplicate were among the accepted-red suite backlog, red since the paragraph was added. Rules: (1) fixing an instruction means grepping for its RESTATEMENTS and killing every copy in the same change — CLAUDE.md's READ-BEFORE-YOU-EDIT clause, but the search direction is outward from the fix; (2) an accepted-red test backlog is a place where regressions hide with zero signal — a discriminator test that goes red carries information even when 24 other reds are noise; (3) prose branded "operator-mandated" is exactly the prose agents obey without checking the mechanism, so a stale copy of a mandate is worse than a stale comment.) Previously 2026-08-29 (Lesson 142 — **a root-cause theory that fits all the evidence can still be wrong — the durable fix in a low-observability failure is the TRACE, which turns the next failure into a measurement.** The register blackout was attributed to a drifted post-compact payload cwd; the theory fit every observed fact, the `__file__`-fallback fix shipped with red-first tests, and it was WRONG — the `anchor_harvest` event shipped alongside it measured the real cause on its first day: `tp=True, chars=0` at a turn-final Stop vs `chars=3749` mid-turn, i.e. the harness fires Stop before the final text entry is flushed, so the extractor was reading the closing tool_use entry as an empty message. The class fix (skip textless entries + a race-free prompt-time second harvest) was then a measurement away. Rules: (1) when a failure is intermittent and its inputs are not directly observable, ship the instrument WITH the plausible fix and say which is which — the instrument is the part guaranteed to pay off; (2) a wrong-but-plausible fix that hardens a real adjacent weakness is not wasted, but never let it close the investigation: state what the trace must show before you call the cause proven; (3) for producer/consumer races you do not control, prefer a second read at a moment the data MUST exist (prompt-time) over retrying harder at the moment it may not (Stop-time).) Previously 2026-08-29 (Lesson 141 — **two consumers going dark TOGETHER points at their shared input seam, and telemetry proves nothing about a component until you read its producer field.** The thread-anchor register went stale for 26 minutes post-compaction while `gate_run` events kept landing under the session's sid — which read as "the Stop hook ran at 13:50" until the events' own `sid_source: 'env'` showed they came from background `final_gate.py` tasks, a different producer entirely; there was ZERO hook-attributable trace because the harvest sat below an unrelated `final_gate.py` eligibility early-return and every one of its failure paths was silent. Rules: (1) when two independent consumers of one input fail simultaneously, suspect the seam they share before either consumer; (2) before treating an event stream as proof a component ran, read the producer/attribution field — same sid ≠ same producer; (3) a best-effort block placed below an early-return it does not depend on inherits that return's failure modes for free — order memory/telemetry above eligibility gates, and give every silently-skipped path a trace event (`anchor_harvest`) so "never ran" and "ran, found nothing" stop being byte-identical.) Previously 2026-08-29 (Lesson 140 — **a probe is only as good as its two ends: an ungated endpoint proves nothing about the key, and a silently-empty env var proves nothing about the endpoint.** Six API keys were "verified" with `set -a; source .env` + curl on `/v1/models`: the NVIDIA "200 ×3" was a PUBLIC endpoint answering 200 to bogus and no-auth alike, and the Mistral "401 ×4" was curl sending `Bearer ` with EMPTY vars — because `.env` had three unquoted values (a `|` in a token, parens in a query, `<placeholders>`) that ABORTED `source` at line ~100, so every var after it read empty while the probe loop printed confident verdicts. Both false claims shipped into CONFIGURATION.md and a review artifact before a payload screen's KeyError exposed the chain; re-verified against GATED endpoints with grep-read values: NVIDIA genuinely live (chat 200 ×3), Mistral 402 (credits exhausted — the operator's ground truth, which the empty-key 401 had accidentally imitated). Rules: (1) a key-liveness probe must use an endpoint that FAILS on a bad key — prove the gate with a bogus-key control before trusting any 200; (2) after sourcing an env file, assert the specific vars are non-empty before using them — `${#VAR}` costs nothing and a KeyError-style hard read beats a silent empty; (3) `.env` values containing `| ( ) < > ,` or spaces must be quoted at write time — the file is consumed by BOTH dotenv parsers and shell `source`, and the shell path fails at the FIRST bad line, poisoning everything below it.) Previously 2026-08-29 (Lesson 139 — **an aggregation that cites a contract must implement EVERY clause of it — implementing the convenient half flipped a production rank 1.** The canary-grounding Phase B rewrote the flywheel ranking QUERY "per set_quality's reconcile contract" and implemented reconcile-per-agent with `MAX(quality_score)` — but the contract's own words are "the non-NULL / **latest** score," and MAX is only the wins-over-NULL half. A native-Opus finder proved it live: two `glm-4.5-air` research agents had been re-scored DOWNWARD by a human (4→1, 4→2); MAX resurrected both 4s, pushed shrunk_q from 2.46 (gated out) to 2.56 (past the 2.5 gate), and the shipped doc crowned the model rank 1 of `research` fleet-wide — on the strength of exactly the verdicts a human had already withdrawn. The same review found the mirror error in the same commit: an inner HAVING that dropped orphan scored deltas the contract explicitly promises to PRESERVE — the two queries in one commit adopting opposite orphan policies, neither chosen. Rules: (1) when code claims to implement a documented contract, diff the implementation against the contract's every clause — "latest" ≠ MAX, "count runs" ≠ COUNT(*), "preserved" ≠ dropped; the citation makes the gap harder to see, not smaller; (2) an aggregation change over production data owes an old-vs-new diff on the REAL data at the same instant (the review's 78-row isolation harness separated 3 semantic changes from 10 days of window decay in one query); (3) a "deferred edge" note ("MAX == latest for today's binary judge, revisit if downward re-scoring ever exists") must be probed, not assumed — downward re-scoring already existed, live, in the very rows the query ranked.) Previously 2026-08-28 (Lesson 138 — **an absence claim about a config must enumerate every layer of the consumer's resolution chain — a partial-layer read shipped a false fleet-wide directive and induced two downstream misfilings.** The flywheel advisory diagnosed "no SUBAGENT_RUNS_DSN — unrecordable, provision it" by reading process env + repo `.env` — but the consumer (`load_env`) resolves a THIRD layer, `~/.config/fabrik/subagents.env` (`_dotenv.py:146-172`, "set it ONCE and every project's pool picks it up"), and a `.env`-less repo (transdoc) recorded 64 rows the same day through it. The wrong message induced job-agent's wrong filing, then MY wrong fleet survey (a grep of the same single layer), then a wrong provisioning request to infra — three artifacts, one unread resolution chain, each citing the previous as evidence; infra refuted with executed proof and fixed the check to resolve all three layers (3dfd3e70, red-on-revert). My adversarial review (2 pool rounds + closing sweeps) confirmed the defective diagnosis because every finder judged the message against the code that produced it, never against the CONSUMER it made claims about. Rules: (1) before shipping any "X is absent/unprovisioned" enforcement message, open the consumer's full resolution chain and enumerate every layer in the check — a subset read is Lesson-123's broken instrument wearing a green suite; (2) a diagnosis that survives review by finders armed with the same partial model is not verified — one `record_agent_run` probe would have refuted the whole finding in thirty seconds; ground the consumer's BEHAVIOR before asserting facts about its inputs; (3) a refutation with executed evidence fans out to every artifact the wrong claim touched — the downstream filer got a superseding mail, not silence.) Previously 2026-08-28 (Lesson 137 — **distribution is not enforcement.** A duty asked for three times was written into both constitutions, auto-appended to 30/30 commands and 4/4 agent definitions, given a flag, a persisted verdict, a kaizen cell and a grader — and produced 13 closes in 14 days, 12 with no verdict and ZERO filings. Distribution was flawless; nothing BOUND, because `done` returned 0 without the flag and the grader is warn_only. Two things change agent behaviour on this box: a command that REFUSES, and the Stop hook. The fix reuses the second — a refused close leaves the record `running`, which the Stop hook already blocks on, so it needed no new check and cannot redden 46 repos. It also nearly shipped inert: the cutoff was a local date rounded up past the current UTC instant, caught only by the end-to-end test.) Previously 2026-08-27 (Lesson 136 — **fixing the authoring command leaves the REVIEW rubric still armed, and a rubric can contradict itself two axes apart.** A filed conflict in `/fabrik-flows` was fixed in the authoring command within the hour; the four false CRITICALs that motivated the filing stayed fully reproducible, because an author-blind reviewer reads the REVIEW rubric — where the same unscoped law also lived. A law stated in a producer/reviewer twin is TWO copies; grep the twin before closing. Sharper: axis F of that rubric banned file paths while axis G twenty lines below REQUIRED the section that names them — one rubric, opposite verdicts on one section, the third instance of contract-vs-grader in two days. And the filing's throwaway aside — that the contract calls `[PRIMARY PATH]` "the certification denominator" — was the real finding: the hub said that in three places and it had been false since the gauntlets moved to a live registry, with their own grader flagging any doc-sourced cert board. The aside in a filing is often the part the filer could not evaluate.) Previously 2026-08-27 (Lesson 135 — **a convergence loop is only a loop if the round can ask the question again.** `/fabrik-rivals` Phase 2 told the agent to re-run discovery each round and terminate on two consecutive DRY rounds, but the engine discovers ONCE per `job_id` (`orchestrator.py:566`, flag persisted) and the driver derives `job_id` deterministically from the market — so round 2 onward could not surface a new rival BY CONSTRUCTION and the terminal condition auto-satisfied. "No new X was found" and "the round could not have found a new X" produce byte-identical evidence. Prove the re-run is not a replay. And the obvious fix was wrong: un-skipping discovery alone would have DROPPED rivals, because the engine REPLACES the competitor list rather than merging it — accumulate-across-rounds needs an explicit union, never a side effect of re-running. Four of the command's five defects were *stale-companion*: the source was fixed days earlier and its reference doc, INDEX rows, router entry and grader did not move with it.) Previously 2026-08-26 (Lesson 134 — **a test that iterates a constant cannot see a value REMOVED from it, and a mutation that fails to apply is not evidence.** Two vacuity defects found in the CLOSING rounds of a review whose earlier rounds had already fixed three of the same kind. (1) `test_terminal_set_and_token_list_are_complete` asserted `for tok in FINISHED_TOKENS: finished_token(tok) == tok` — a tautology over the very tuple it was pinning. The whole suite passed with **7 of the 9 tokens deleted**, while the test's own docstring claimed to close exactly that mutation. A collection is pinned only by asserting it LITERALLY, from outside the module, plus the values that must stay ABSENT. (2) A red-on-revert harness reported a test VACUOUS when the real cause was a regex that never matched a multi-line `_REMEDY = (` — the source was never mutated, so the green was the ORIGINAL code passing its own test. Always assert the mutation applied (substring found, or md5 changed) before reading its verdict; a no-op mutation reads exactly like a worthless test and will make you delete a good one. Corollary observed twice this run: the round that FIXES anything is never the last — rounds 3-8 each fixed something, including an off-by-one inside a round-4 fix that its own new test exposed one command later.) Previously 2026-08-26 (Lesson 133 — **a rule's activation condition must address the NON-COMPLIANT population; globs that describe the mandated end-state arm only for projects already compliant.** Three confirmed instances of one class (transdoc's inert-glob triplet): rule-pack globs written for the layout a COMPLIANT project would have — `emails/`+`*.mjml` (the pack's own mandated pipeline output), `workers/`/`jobs/` directory-per-concern, `routes/`/`api/` backend dirs — while scaffolded projects emit FILE-per-concern (`worker.py`, `notify.py`, `lib/api.ts`), so the packs were silently inert exactly where they were needed. The subtlest form is SELF-FULFILLING addressing: core/86's globs matched precisely what its own MJML mandate produces, so it reached the compliant and missed the non-compliant — the exact population it exists to correct. Measured cost, all under green gates: 19 phantom frontend calls, an empty beat loop, three `str.format` security-notice emails against an MJML mandate. Rules: (1) write activation globs for the code as it exists BEFORE compliance and ground them in a real fleet inventory of names (`find` over `/opt`), never in the pack's own prescribed layout; (2) prefix-shape file globs so synced infra (`mail.py`, `mail_notify.py`) can never arm a pack as noise; (3) where a scaffold type emits a matching path, add `applies_to:` so `check_pack_reachability` guards the claim mechanically — where none does (email senders and API clients are post-scaffold additions), an annotation is an unsatisfiable claim and grounded glob breadth is the only guard.) Previously 2026-08-26 (Lesson 132 — **a fix that RELOCATES a defect without changing its KIND recurs, and a fix shipped with no test cannot detect its own recurrence — but the deeper error was fixing the mechanism instead of questioning the premise.** The fleet quota advisory spammed trade-intelligence with 10 near-identical mails in one evening, 8 of them a dead-steady mob@ 95%, two days AFTER commit `51181918` "fixed the dedup". That fix moved the dedup key off the fast-sliding 5-hour reset onto the weekly reset — but kept it a **raw sliding epoch** (`str(int(weekly_reset))`), which jitters across the `:59↔:00` minute boundary and churned the key every tick: the SAME defect class (a sliding value as a dedup key), relocated, not killed. It survived because `51181918` shipped with **zero test coverage** — no watched-fail-first test asserted suppression across ticks, so nothing could catch the regression. The real fix was not a better dedup key: under one-active-pointer-for-all-projects an individual account hitting 95% is a **non-event** — the flip leg re-points to a sibling with headroom and every agent keeps working — so the per-account advisory was both spam AND a false alarm. Reframing it to fire ONE fleet-wide advisory only when the ACTIVE account is walled with no auto-relief made the dedup **epoch-free by construction** (a presence-only latch, re-armed when relief arrives) — the churning-key problem simply ceased to exist. Rules: (1) when a fix relocates a key/value rather than changing its type, ask whether the new site inherits the old defect class — "fix the class, not the location" applies to your OWN fixes; (2) a fix with no red-proven test cannot detect its regression, and a class marked "fixed" that recurs is evidence the test was missing, not that the bug was hard; (3) a recurring symptom is a prompt to question the PREMISE (why does this fire at all?), not just harden the mechanism — the operator saw in one sentence that the advisory should never fire while the fleet has headroom. Previously 2026-08-23 (Lesson 131 — **a gate that punishes the habit its own skeleton teaches will be worked around, and the workaround destroys the signal**. `check_plan_tickets._norm_behavior` compared Behavior-Contract rows by set equality while forgiving bullets, bold, case, whitespace, trailing `(N)` and periods — but not a trailing `(path:line)`. The plan skeleton teaches exactly that citation, so an author who cited the ticket's own line in the ticket and the primary path in the spine scored TWO errors for ONE behavior; a downstream project measured ~30 on a 16-ticket set and their workaround was to strip every citation to a bare `Grounding:` line. **A gate whose cheapest path to green is deleting evidence is worse than no gate** — it converted grounded plans into ungrounded ones and reported success. Fix: peel the citation, built from the existing `PROOF` pattern so it forgives exactly the extension set the rest of the gate calls a citation and cannot drift; 96 errors → 0 on the reporter's real plan set. **Corollary, same run, and the sharper lesson: I scanned every plan dir on the box with `check_plan_tickets.py <dir>`, counted zero findings, and nearly reported "no fleet impact" — the script takes `--plan-dir`, so it had printed a usage error and exited without ever running.** Counting zero from a check that never executed is the identical failure the whole sweep exists to hunt, committed while hunting it. **Rule: a zero-finding result must be proven to be a result — assert the check actually ran (non-empty parse, known-bad canary, or exit-code discipline) before any claim rests on it.** Previously 2026-08-23 (Lesson 130 — **a regex classifier on a security boundary is not a winnable problem; five consecutive attempts each shipped a real bypass**. `mail.py`'s secret guard refuses `scheme://user:pass@host`. To let a security finding QUOTE a redacted DSN, rounds 19-23 each added an exemption for "obviously placeholder" passwords — and every one leaked a live credential to all ~46 synced repos, each scoring `None`, not even a warning: shape wildcards (`<any>`, `${any}`) → a secret in brackets; substring matching on PASS/HERE/NONE → `CompassionateHeart`; the wrapper trusted on shape → `<CorrectHorseBatteryStaple>`; the `@` unanchored → `placeholder@REAL_SECRET@host`. Deciding whether a string IS a secret or a LABEL for one, from the string alone, is undecidable in practice, and each round spent a real bypass to relearn it. The fix was DELETION, not a sixth version: fail closed, no exceptions, and give the workflow a one-keystroke escape (break the shape) plus a do-not-reintroduce note carrying the five-leak history. **Rule: when consecutive rounds each find a new bypass in the same exemption, the exemption is the defect — remove it rather than refine it.** Corollary from the same run: **a test that fires a real operator alert can hide in a green suite for days** — `_is_hub_repo()` is content-based on `Path.cwd()`, pytest always runs from the hub, so the digest CLI test drove the live `ssh → apprise → Telegram` path on every run while the test file's own docstring claimed that leg was stubbed; it never was. Stub side-effecting seams in the shared FIXTURE so the claim is true by construction, and bind it with a test that asserts no subprocess spawns. Wall-clock was the tell: the suite dropped ~5s → ~1.5s once it stopped blocking on real SSH connects. Previously 2026-08-21 (Lesson 129 — **a no-ceiling review loop needs a PROPORTIONALITY escalation, not just a stall breaker**. The mega-enforcement /fabrik-review loop ran 51 closing sweeps; rounds 1–86 caught real whole-grammar fail-opens (every finding red-proven), but rounds 87–103 narrowed into GFM table-parsing minutiae — each fix breeding the adjacent variant, the finders themselves marking most "latent, zero corpus hits" — and the loop kept dispatching because its contract only stops on a quiet round or a same-finding-3x BLOCKED. The stall circuit-breaker fires on non-decreasing counts in ONE class, but a converging-yet-endless chain of one-finding rounds in an ever-narrower class trips nothing. The missing rule: when N consecutive rounds each find only latent/zero-corpus-incidence variants of one shrinking class, the loop owes the OPERATOR a proportionality checkpoint — "the class is now microscopic relative to the gate's purpose; adjudicate it as a class residual or authorize continuing" — instead of burning rounds until the operator interrupts with "this is bullshit" (which is what happened, at round 103). Corollary: fidelity-to-a-spec sub-chases (GFM here) should be bounded up front by naming which spec dimensions the gate actually needs, not discovered one divergence at a time. Previously Lesson 128 — **an installer that WRITES the whole crontab destroys every entry it did not know about, and nothing notices until a consumer goes stale**. The M0 wake-proof cutover installed its 3 entries by replacing the crontab wholesale; 32 other entries died silently — the */5 rotation tick, the */15 WIP backup, DR mirrors, the CI-fix dispatcher, the liveness audit. Detection took 44h and arrived as a SYMPTOM (a stale quota dashboard the operator had already reported twice, each time 'fixed' at the wrong layer — the refresh mechanism was correct, its delivery vehicle was dead). Corollaries: (a) crontab edits must be read-modify-write over the LIVE table with an entry-count delta check, never a rendered-from-scratch install; (b) when a fix is claimed for the Nth time, verify the DELIVERY CHAIN end-to-end (code → schedule → last-run timestamp), not the code alone — `rotate-tick.log` mtime dated the wipe to the minute; (c) a reconstruction after such a loss must be grounded in the last backup PLUS every documented interim change (docs named the retired drift-check, the added dashboard --ensure and keepalive lines), never the backup alone. Recovery: `~/backups/crontab.backup.*` + the docs rebuilt a 39-entry crontab pending one operator install. Previously Lesson 127 — **a usage-evidence census lies at its JOINS: every artifact class has its OWN usage channel, and measuring a wrong-but-adjacent one produces a confident deletion list**. The M0 shrink audit's first live run named 17 deletion candidates; 8 were false, each from a wrong channel or a broken join — fragments measured on ledger mentions when their real channel is `{{include:}}` at render time; cron/hook/core-script liveness never joined because the liveness findings key on registry surface ids (`cron_match`/`evidence.command_contains`), never script paths; immune keys drifted between absolute/relative/stem forms so the SAME file rendered immune in one class and tickable-for-deletion in another. Corollaries: (a) three pool finder rounds missed every one of these — ONE grounded native reviewer found 14, incl. a phantom census artifact (`PYTHONPATH=/opt/fabrik/src` keyed as a cron) and a 54% corpus blind spot (subagent transcripts) — pool is breadth, native grounded review is the authority, again; (b) an audit that greps transcripts CONTAMINATES its own next measurement (discussing a candidate writes the literal marker into the transcript and flips it to keep) — evidence extraction must be structure-keyed to the producing event, never raw text; (c) a dedupe or guard built on string containment/splitting dies on content that contains the separator — track sets/lists, never re-parse joined strings. `scripts/sysadmin/kaizen_shrink_audit.py` + its 41-test suite encode all of it. Previously Lesson 126 — **the instruction set was the last thing anyone audited, and a wrong NAMESPACE degrades silently instead of failing**: four commands passed PROVIDER names as `web_tools`, so the loop advertised zero tool schemas and every "grounded" research fan-out ran blind while still returning confident prose. Fixed and guarded by a BLOCKING `check_command_corpus.py` that reads the live registry rather than a copy. Previously Lesson 125 — **"advisory" is a DECISION with a fleet blast radius, not a severity, and the audit that finds inert checks must not punish the honest fix.** Resolving Lesson 124's eight inert registrations: a uniform answer was wrong both ways, and only measuring each rule across this repo AND all 44 `/opt` repos decided it — 4 declared `warn_only=True` (`check_env_example`, `check_script_headers`, `check_retired_terms`, `check_doc_stubs`), 4 unwired with the measurement written where the registration was. **An empty UNIVERSE and a clean result are indistinguishable from a "0 findings" run**: `check_reusable_modules` scored 0 fleet-wide because not one of 44 repos has the `src/utils/`+`src/lib/` layout it hardcodes, while the `[reusable]` tag it enforces is in live use elsewhere. **Volume is the weakest reason to retire a check; STRUCTURE is the strongest** — `check_compose_services` had the SMALLEST blast radius (30 findings) and the clearest unwire, because it only detects services when `services:` is itself an added diff line, i.e. it is blind to a service added to an existing compose. The inverse defect is the same disease: `check_vps_docs` emits only `Severity.WARN` yet exited 1 on any finding, redding a blocking row over two un-regenerated ops docs — a gate that reds on nothing is ignored as fast as one that greens on everything, and the fix is the exit code, never generating the docs. `advisory=` (preserve stdout) and "cannot fail" had been ONE flag, which is why none of it was visible — five checks carrying `advisory=True` genuinely block; the separate `warn_only=True` declaration prints `[ADVISORY]`, feeds `--json`'s `advisory`/`blocking` split, and still FAILS loudly if the check ever exits non-zero. Last: the audit's own verdict rule had to fork — a blocking row must go RED, a declared-advisory or unwired one must SPEAK (output diffed against the clean control) — or declaring a row honestly would read INERT forever; vacuity went LIVE 36 / DEAD 8 / UNKNOWN 3 → **LIVE 44 / DEAD 0 / UNKNOWN 3** with no check weakened. Two ratchets, not one: a warn-only check may never be an ordinary blocking row, AND a `warn_only=True` must be backed by a written contract — the second immediately caught that `check_ticket_breadth`'s CHECK can fail while its ROW cannot, a distinction that had no vocabulary) (Lesson 124 — **a gate row that can only WARN can never fail, and "no canary" hides that exactly as well as a bug would.** Completing the canary corpus for all 43 checks `final_gate.py` registers took vacuity from LIVE 7 / DEAD 0 / UNPROVEN 40 to **LIVE 36 / DEAD 8 / UNKNOWN 3** — and the 8 DEADs are the real finding: `check_compose_services`, `check_env_example`, `check_env_updates`, `check_script_headers`, `check_test_coverage`, `check_reusable_modules`, `check_retired_terms`, `check_doc_stubs` have **no non-zero exit path anywhere in their source**, yet sit in the gate beside checks that can red it and produce an identical green row (four are not even `advisory=True`, so their WARN text is discarded on exit 0 — a fully silent row). **A check's severity contract and its gate registration are two separate facts, and only the second is visible in the gate's output.** Three rules follow. (1) The 40 did not arrive in one commit — they accumulated one unexamined `run_optional_check(...)` at a time, so the fix is a RATCHET test that fails on a new registration with neither a canary nor a written-down obstruction, never prose telling future agents to author one. (2) **A canary that survives a broken check proves nothing about a working one** — `NEUTERS` deletes one check's core rule in a throwaway copy and asserts the canary then goes GREEN; two fixtures failed exactly this way while being written (`check_no_host_ports` only flags a `ports:` binding on a service that ALSO declares `traefik.enable=true`, and neutering `check_user_guide`'s missing-dir branch still exited 1 via its empty-dir branch). (3) An UNREACHABLE check is a finding about the CHECK: `check_vps_docs` reads a hardcoded absolute `FABRIK_ROOT = Path("/opt/fabrik")` with no `--root`, no env var and no cwd sensitivity — unfixturable, unreviewable, and **RED on the hub right now** while every one of its findings is `Severity.WARN`) (Lesson 123 — **absence of evidence is not evidence of absence: a probe must validate its own INSTRUMENT against a positive control before it may report an absence, and the verdict space needs a THIRD state.** Three of one session's own probes lied in a single morning and every lie was reported to the operator as fact: `grep -c` on a log carrying one invalid UTF-8 byte printed nothing (binary-suppressed; `grep -a` showed 21 events, and the suppression proved INTERMITTENT as the file grew — worse than a stable bug), `ls specs/services/ | grep apprise` searched ONE directory and was generalised to "never existed" (the spec is in `specs/infrastructure/`), and `ssh vps docker ps` returned EMPTY because the ssh user is not in the `docker` group — 31 containers were running. In all three a BROKEN INSTRUMENT was read as a TRUE NEGATIVE. The fix is structural, not vigilance: LIVE / DEAD / **UNKNOWN**, with a single constructor that collapses any unproven instrument to UNKNOWN with the fault NAMED, in BOTH directions (a broken instrument cannot prove presence either). Neuter that one function and 6 tests go red. Corollaries that each cost a false finding: read evidence as BYTES with `errors="replace"` and never shell out to `grep`; a missing file whose parent is absent or unreadable is UNKNOWN, not DEAD; a `/tmp` path is VOLATILE so its absence proves nothing; a systemd unit `not-found` at system level must be re-queried with `systemctl --user` before absence is claimed. Second half: **the same rule must survive the hand-off to a metric.** The kaizen `Missed crons` column is now fed by this audit, and an UNKNOWN is excluded from BOTH halves of the fraction — folding it into the denominator reports a healthier box than was measured, folding it into the numerator invents defects, and either is the honesty rule broken by a number that looks fine. Third: **a monitoring layer that blocks work gets disabled, and then it monitors nothing** — the audit exits 0 by default and never raises; `--strict` is opt-in, UNKNOWN never fails it, but a CRASHED PROOF does, because a silently skipped proof is how a monitor learns to say all-clear) (Lesson 122 — **a check that cannot fail buys the same green as one that works, activating a check is a MEASUREMENT job, and the tool you gather liveness evidence WITH can lie.** Six Tier-3 checks were registered in `final_gate.py` and reported PASS while asserting nothing — each defined only `check_file()` with no `__main__`, so running them exited 0 with EMPTY output; their logic was live only through Cascade's dormant `post_write_code` path. Running them for the first time against this repo AND all 45 `/opt` repos changed nearly every decision: `check_env_contract` treated a MISSING `.env.example` as "every compose var violates the contract" and parsed YAML COMMENTS as references (the scaffolder's template `${VAR:?}` made 17 of 45 repos "require" a variable named `VAR`; blast radius 17/45 → 2/45 on those two fixes), while four checks were UNWIRED with the measurement recorded in the gate because their models are too coarse to gate on — **an honest missing check beats a fake green one**, and `advisory=True` output is not a substitute for a failure mode. Activation paid immediately: `check_docker`'s first real run found **5 of 45 repos shipping `platform: linux/arm64` in a production compose deployed to an x86_64 VPS**, while one of its ERROR rules had to be demoted (vendor `redis:7-alpine` images run in production; the No-Alpine invariant targets BUILT images). Separately, alert delivery had been dead because `/opt/fabrik/.env` stores only the SECRET HALF in `TELEGRAM_BOT_TOKEN` — posting to `/bot<secret>/sendMessage` returns HTTP **404**, which reads as "dead endpoint" not "malformed credential"; a sibling script already composed the halves, so **when two consumers read the same credential and only one works, diff their READ, not their transport** — and a delivery layer must report per-method causes, not one line naming neither. Last and sharpest: the `@reboot` sweep was reported as having NEVER run, and that was false — one **invalid UTF-8 byte** at offset 1313 makes GNU grep treat the log as binary and **silently suppress matching lines**, while the corroborating stale mtime proved nothing because `>>` never updates mtime when a process writes no stdout. **`grep -a` when grepping a log you did not write; a file's mtime is not a run record; reproduce a system's execution before "fixing" it dead**) (Lesson 121 — **a metric whose components encode advice must be audited for what the ADVICE would teach, not just for whether the number correlates; a heuristic gate earns its place by PUBLISHING where it disagrees with reality, at the point of use; and `lstrip("./")` strips a character SET, which silently blinds every dotfile detector.** Building the ticket-breadth advisory, the score counted `tests/` as an independent risk area — statistically unremarkable, but it made the tool EMIT *"keep `scripts/` and peel off `tests/` into separate tickets"*, an anti-pattern about to sync to ~46 repos: tests ship WITH the behaviour they prove (the Behavior Contract requires the test in the same ticket; watched-fail-first requires both in one changeset), so a ticket whose tests live elsewhere cannot be red-on-revert proven and its `Gate:` passes while proving nothing. **Every generated suggestion is a rule you are shipping — review the tool's OUTPUT STRINGS as governance, not as formatting.** Removing the test component then forced a full re-derivation and the numbers got worse, honestly: the 8-round anchor fell from score 6 to **exactly 5**, precision 0.50, recall 2/3 (it misses a 4-round ticket), Spearman **ρ=0.45** — so the threshold survived only because 5 is the LARGEST value that still catches the anchor (at 6 recall is 0/3, a cliff). Rules: (1) **a scoring change invalidates the calibration — re-pair against the receipts in the same change**, or the threshold is fitted to nothing; (2) the advisory must state its own hit rate IN the warning ("2 of 4 flags matched a genuinely expensive ticket; a prompt to LOOK, not a verdict"), because an advisory that overstates itself gets ignored wholesale, which is worse than not warning at all; (3) the "1–2 rounds per risk class" prose was measured against HAND-counted classes and the mechanical proxy has a different scale — **the constants follow the data, not the sentence that motivated them**, or the tool ships a doubled prediction on day one. Process failure worth its own line: I read the review-receipt directory with `ls | head -40` and concluded only three per-ticket receipts existed; T03 and T04 were on lines 41–42, so the first calibration ran on n=12 instead of n=14 and was corrected only because the reviewer independently checked. **A truncated listing is not an enumeration** — never derive a denominator from a paged read. Last: `path.lstrip("./")`, intended as "drop a leading `./`", strips any leading run of `.` and `/`, so `.claude/hooks/x` became `claude/hooks/x` and every dotfile governance surface was invisible; `removeprefix` is the operation that was meant) (Lesson 120 — **a rule declared "binding" whose trigger is left in the spec's Open items ships a rule that never runs — and the fix must split the cheap half from the expensive half or it still won't.** The roles spec made a weekly kaizen pass binding for infra and fleet, shipped both charters and both log tables, and left "Kaizen cadence day/trigger (suggest: Monday…)" as an open item; four days later the operator asked "did it run, what did we learn?" and the honest answer was **never once** — both logs still held only their baseline row of em-dashes. Three rules: (1) an execution plan that implements a spec must close the spec's Open items that the implemented rule DEPENDS ON, or mark the rule not-yet-live — shipping the charter without the trigger is the documentation-not-enforcement class (Lesson 116's sibling, one layer up: 116 is prose that can't bind a running session, this is prose that has no runner at all); (2) when the recurring work is part mechanical and part judgement, **split it** — here a stdlib cron measures and mails, the agent only analyses, because a pass that costs a Claude session on a box where three of four accounts sit near their weekly wall is a pass that will not run, and quota was never the reason it hadn't; (3) **`—` beats a plausible number**: only 2 of the 5 pinned metrics had a real source, so the other 3 are written as an em-dash *with the reason recorded*, because a wrong metric silently ends the investigation it should start — and a review ledger with no round marker shrinks the denominator (`4.4 (n=13/16)`) instead of averaging in as zero rounds, which is how "are reviews still taking 30 rounds?" would have been answered with a comfortable lie. Counting only `## Round N` headings would have scored the 16-round stalled-midstream ledger as 0: when a corpus has two dialects, read both or your metric measures the dialect, not the thing) (Lesson 119 — **the adjudicator's own acceptance fixes are unreviewed prose until a NON-AUTHOR pass covers them, and a doc's remedy instruction must be traced through the consumer's actual keying field.** Closing the login-once plan's docs-review: pass 2's only two REAL findings were precisely the two sentences the orchestrator itself wrote as pass-1 acceptance fixes — an rc claim asserted without running the command (`--sync-mcp` exits 1 on an absent root, not the "0" the fix stated) and a recovery instruction ("edit the row's `account`, then `/login`") that edits a field the consumer never reads: fleet grouping keys on the PINNED `identity`, which is only re-probed while `pending-login`, so the documented recovery leaves the dir mis-grouped forever. Rules: (1) an acceptance-fix batch by the adjudicator carries the same fix-residue risk as any coder fixup — the convergence loop's next pass must target the FIXES, not re-verify the original claim table; (2) every runnable claim in an operator doc (an rc, a printed string, a fallback) is verified by RUNNING it, exactly like code review's probe duty; (3) a remedy that mutates state must be traced to the field the consuming reader actually keys on — same class as "test the real invariant, not a proxy", surfacing in prose. Also validated: the per-ticket loop held — the D7 whole-plan fresh sweep (3 pool lenses + native seat, requirements table + live box probes + ledger-honesty spot-checks) closed at found: 0 after 34 per-ticket rounds, and the one cross-site class of the plan (unclamped stamp-age reads) was killed at all three sites in ONE round because the in-file `_CLOCK_SKEW_TOLERANCE_S` precedent was named in the fix brief) (Lesson 118 — **an instruction that names the ANCHOR is blind to the inlined walk, and a string-matching invariant cannot see a computed path.** A relocation step said "repoint the `FABRIK_ROOT`/`FABRIK_DIR` anchor to `OUTPUT_ROOT`"; two producers had no named anchor — they inlined `Path(__file__).parent.parent.parent` — so a commit claiming "7 producers no longer walk out of `engine/`" was false for exactly the two whose output the whole migration exists to preserve, and from the new location that walk resolved to **`/opt`**, with `mkdir(parents=True)` on the write path. Sweeping for the IDIOM instead of the two named instances found **20** modules in five shapes. Three compounding rules: (1) when a fix is described by the SYMBOL it edits, re-derive the target set from the BEHAVIOUR (a path that escapes its root) — the anchor-shaped instruction defines the blind spot; (2) the sibling invariant `test_no_fabrik_paths.py` passed throughout because every instance is *computed*, so the guard must be an AST hop-counter that resolves module-level names (`SCRIPT_DIR = Path(__file__).parent` then `SCRIPT_DIR.parent.parent` = 3 hops), not a grep — and it must be committed RED against the real tree (20) before green (0); (3) **check whether the escape already fired before assuming it is theoretical** — it had: the target repo's own `.windsurf/rules/ai/*.md` and nine `docs/reference/kilo/` files already carried engine-generated content, discoverable only by looking at the working tree, since no gate covered it. Also: redirecting output into a new root breaks every writer whose parent directory previously always existed — 13 tests went `FileNotFoundError` until `mkdir(parents=True)` was added; and prove "no regressions" against a BASELINE WORKTREE at the prior commit, because a tree with 16 pre-existing failures makes "16 failed" meaningless without the before-picture) (Lesson 117 — **"why did it give up?" must be reported by the decider — per-thread, probed deterministically, and with the fail direction chosen per consequence.** A pause gate returned a bare `None` and the caller re-read the marker to decide "withheld vs exhausted": on a host with nothing to rotate to the re-read invented a withholding and silenced a true "credentials are dead" alert; it raced the operator resuming; and the obvious fix — a module-level flag — broke under the real caller (the aro-wake twin drives the same function from `asyncio.to_thread` plus an unlocked background task), one thread's verdict suppressing a sibling's genuine alert. Rules: a function whose `None` has several operational meanings owes the caller which one; that report lives in thread-local state unless every caller is provably single-threaded (check the OTHER copy of a vendored twin before assuming); a fail-closed refusal must not inherit the silence of a deliberate one (an outage that mutes the alert channel is the worst of both); and the DIRECTION of every fail-soft matters as much as its presence — widening an `except` without asking what the fallback VALUE means turned a dwell guard fail-OPEN (corrupt ledger read as "never switched" → a fresh credential install every 5 minutes), while "unknown" must map to the safe answer and the genuinely-empty case (no ledger yet) must be told apart from the broken one or the safe answer wedges the feature shut. On probing: a STATISTICAL concurrency test is not a guard — it stayed green while a shared-object mutant survived 21/30 runs; force the interleaving with events so the mutant fails every time, and never key a per-thread seam on `threading.get_ident()` (idents recycle the moment a thread exits). Eight finder rounds, 34 fixups, every fix mutant-proven — the pattern held: each round's fix opened the next round's surface, and the loop's cost was repaid by catching a fail-open credential thrash and a silenced drain broadcast that no green suite would ever have shown) (Lesson 116 — **prose cannot enforce a machine-readable format, because an agent reads its contract ONCE at session start and commits for hours afterwards.** 200/200 hub commits carried `Agent-Role:` and only 10 parsed, caused by one malformed example in CLAUDE.md. The example was corrected fleet-wide — and the next **50 commits still parsed 0/50**, including one written by the very agent that made the correction, which followed the pre-fix snapshot loaded into its own context window rather than the file it had just fixed on disk. A documentation fix is therefore invisible to every session already running, and cannot be verified by re-reading the doc; it must be verified by measuring the ARTIFACTS produced after it (`git log --format='%(trailers:key=…)'`). When a governance rule has a mechanically-checkable output, the fix is a check at the moment the output is still editable — here `commit-msg`, since a pushed bad trailer block is only fixable by a force-push, a HARD STOP. Corollary for the checker itself: delegate to the real tool (`git interpret-trailers --parse`), never reimplement its rules, and test that the hook is WIRED at the right stage AND that `.git/hooks/commit-msg` exists — `pre-commit install` alone installs only `pre-commit`, so a correct guard sits inert. Same round: a block-scope redirect `{ … } >> "$LOG_FILE"` skips the ENTIRE compound body on a failed redirection, and an unguarded `mkdir -p` on the line above fails in the identical scenario — the line-scope guard test that existed skipped any line without `pipeline_alert.sh` in it, so it green-lit the block-scope instance of the exact symptom it was named for) (Lesson 115 — detection and ARMING are two independent legs of the resume mesh: shipping a new death-class detector (connection-failure/ENOTIMP) does nothing for a session whose self-watch was never armed — arming is ADVISORY (session_orient.py INSTRUCTS the agent to call Monitor as its first action; a non-complying or pre-mandate session, like the live 35204643 incident, has zero watches to consume the death record). A mechanical arm (a hook that spawns the Monitor directly, if the harness allowed it) would close the gap; until then, 'auto-resume implemented' is only half-true for any session that skipped the arm. Verify BOTH legs before claiming a class is covered. Also: key a death-class detector on the STRUCTURAL invariant (retryAttempt == maxRetries = retries exhausted), never a string allowlist of error codes — the codes are an open set that rots, the exhausted-retries signal is the actual death condition) (Lesson 114 — a choreographed race test must run the REAL contended operation: a monkeypatch that raises on the actor's behalf proves only that exceptions propagate (the fake-raise version stayed green with the guard reverted — closer-caught, E2); and file-store concurrency converges when every transition shares ONE lock discipline (rename) with timestamps that TRAVEL WITH the atomic op (utime-then-rename), not stamped after it — five designs collapsed to that invariant across the mail resolve-window rounds) (Lesson 113 — an ORG retirement decision that doesn't propagate to the machine-readable layer the same day becomes a stale-`ok` advertisement: the capability catalog listed the whole dead Kilo-CLI family and the retired supabase driver as `ok` a month after retirement, because "ok" = "the probe passes" and the `# RETIRED` marker mechanism (built for exactly this) was never applied when the decision landed — third instance of the registry-vs-decision drift class (captcha "stays live", supabase driver, kilo family). Rule: a retirement decision's landing checklist includes marking every affected file/driver the catalog reads, and org markers must BEAT mechanical probes in every enumerator — `_enum_drivers` only learned that today) (Lesson 112 — creating a new `/opt/<dir>` silently ENROLLS it in governance-sync: `sync_enforcement_to_projects.py` discovers targets by `is_dir()` under `/opt` with NO `.git` requirement, so the fabrik-mail DATA store — provisioned by `mkdir -p /opt/fabrik-mail` — was adopted as a "project" and filled with the whole governance corpus (CLAUDE.md, AGENTS*, `.claude/`, `.windsurf/`, `scripts/`, `libs/`, `docs/`). The spec FROZE "neutral path, no governance-sync races" but that was prose, never enforced — the plan provisioned the root and never added the `exclude_folders` entry, and EIGHT review passes (4 native + 4 pool + the whole-plan cross-phase) all verified code distributing OUT to projects while NONE asked "does this new `/opt` dir get adopted IN by the sync's `is_dir()` walk". Sync-consciousness is BIDIRECTIONAL: when a change creates or leaves ANY new `/opt/<dir>` that is not a real project (a data store, a socket dir, a cache), assert it lands in `exclude_folders` in the SAME change. Caught only by the operator, post-EXECUTED; fixed by the exclusion + a fail-safe mailbox-preserving cleanup) (Lesson 111 — a defensive-guard fix must close the anti-pattern's whole CLASS at every STRUCTURAL LEVEL, or the review loop re-finds it level by level: `X or default` narrows falsy-but-not-truthy-wrong-type, and fixing it for the text LEAVES (round 8) still left the CONTAINERS (round 10: content=99, input=42, message=42) and then the SCALARS (rounds 12-15: bool timeoutMs/delaySeconds — `float(True)=1.0` silently defeats the garbage default without raising). Three finder rounds paid for one grep-able class; on the first instance, sweep every `.get(...)` the same walk touches at ALL type levels (leaf, container, scalar-coercion) and close them in ONE wave. The cost dual: on a small feature over a fail-open production surface, distinguish "defect in the new code" from "pre-existing fragility the new walk exposes" — the loop contract as written iterates to no-op over BOTH, which is defensible for a crash=false-ring surface but is the compounding tail that turns a 45-minute feature into 4 hours; adjudicate-and-batch the pre-existing class, don't round-trip it) (Lesson 110 — a behavioral sandbox test cannot catch a SELF-CONSISTENT defect: a plan authored with the literal `<plan-stem>` passes every ownership case because the guard greps for exactly what the open wrote — and works at deploy time too, while colliding with every other plan that kept the literal (mutual window theft). Execution proves behavior against ITS OWN writes; identity/collision classes need an explicit static check (a grep for surviving placeholders) beside the execution harness. Same round proved the dual: an INNER-layer-only quoting test is a parse check in disguise — the outer ssh argv-join silently drops a stem guard's trailing space, so a bare-quote defect passes inner-only testing and deletes a sibling's window live; always push authored remote one-liners through BOTH quoting layers, asserting output+rc+file-mutation per case) (Lesson 109 — an evidence artifact can be wrong three ways while its claim stays true: a fenced quote was doctored (lines silently dropped), its correction went stale when the quoted file was edited the same day, and a code-block line-label was off-by-one TWICE — the second time in the very edit that claimed to correct it. The paste is never the evidence; re-run the probe. And the label errors survived three author passes to be caught only by NON-AUTHOR rounds — an independent grounder + a wave-verifier confirmed 7 findings the author could no longer see; probe-duty + role-separation are now plan-2's shipped rules, not advice) (Lesson 108 — an enforcement script in `scripts/enforcement/` is FLEET code: it syncs to ~48 projects that lack the hub's inputs, so a `__file__`-derived root + an unhandled `FileNotFoundError` breaks every project's completion gate while the hub stays green — but HARDCODING the hub path over-corrects into the opposite vacuous green, silently skipping in any hub worktree. Derive the root, then accept it only if the input it needs is actually there; test both directions) (Lesson 107 — a green harness on a shipped system hid 48 defects: recall comes from adversarial finders + LIVE artifacts (the production log, empirical repro, fix-seam audits), never from re-running green gates; every fix round is a new defect surface; the quiet exit round must include an authoritative native pass) (Lesson 106 — a canonical LIST edit must audit every DERIVED consumer by OPENING each deriving fn: the .gitignore block consumed GOVERNANCE_FILES as a name list, my residual asserted dest-rel derivation unopened, and the split shipped a fleet-wide dropped ignore line the review caught post-execution) (Lesson 105 — a rule's PRESCRIBED REMEDY is a claim like any other: EXECUTE it against the canonical config before writing it into governance. The banned-row fix "validate via `model_validate` so a missing key RAISES" was disproven by a live pydantic probe — under `FabrikBaseModel` (`populate_by_name=True`) an optional missing key still yields `None` and wrong casing is silently accepted, so the remedy would have reproduced the exact false-defect it existed to kill; the corrected rule asserts the KEY SET, not just schema validity. Same class as "verify a flag's real effect by reading the fn" — but for remedies, reading isn't enough: run it) (Lesson 104 — first live dispatcher run: (a) NEVER render a shared install target from a worktree — the renderer's prune deletes master-only artifacts box-wide; render only from merged master at each merge commit; (b) a Stop-hook that attributes gate failures by CHECK NAME pins a sibling's shared-tree dirt on the session and traps it against its own contract — attribute by FILE (failing-check outputs ∩ transcript-authored files); (c) description rewrites drop load-bearing semantic hooks reproducibly (T06a and T06b, same class, both caught only by exemplar-grounded review) — a description sweep needs a substance-preservation diff pass, not just style review; (d) a squash-apply onto a moved master can silently clobber merged content on 3-way fallback — hand-apply small deltas after checking the target region, and never run git file mutations without pinning cwd) (Lesson 103 — a byte-identical shared regex can still be fail-open at ONE consumer: the four-module Status regex's `>` tolerance was fail-closed at the ban/claim consumers but fail-OPEN at spine-status determination (a quoted `> Status: DRAFT` example downgraded the whole gate to advisory); fix the FAIL DIRECTION at the consumer — strip quoted lines from that one scan — never by forking the regex, so parity survives; and prove each `>` with a mutation-killing test, or the next "harmonizer" reverts it green) (Lesson 102 — `fabrik apply` REBUILDS the .env and will overwrite a registrar-injected real value (DATABASE_URL) with the spec's PLACEHOLDER → `compose up --wait` fails on the placeholder DSN before the registrar re-injects = self-inflicted outage; `redeploy` never re-syncs env at all. Fix: `_build_env_content` is now placeholder-aware. And: the deployer agent HAS the keys/tools — recover in place, don't defer to the operator) (Lesson 101 — a re-triage/re-bill dedup test must assert the CONSUMER's real bucketing invariant, not a proxy function in isolation; and "exempt on error" must split transient-transport from completed-but-unusable) (Lesson 100 — a gate check is not live until its PASS line appears in the target tier's output; a path pin is a one-line edit, not a placement argument) (Lesson 99 — vendor a heavy external eval tool GRADING-ONLY: its declared torch/vllm deps are for generation you don't do; `--no-deps` + call the eval function directly, don't shell the CLI wrapper that imports the GPU stack) (Lesson 98 — a twice-converged plan still carried a false premise only visible at the cross-phase seam; the confirming round exists for the FIXER too) (Lesson 97 — a production work-dispatcher is the wrong instrument to measure the models it dispatches; direct-dispatch a benchmark, and an errored call is a non-result not a wrong answer) (Lesson 96 — the gate counted a sibling's untracked WIP as this session's debt; "CI-parity" that includes untracked files isn't parity) (Lesson 95 — arithmetic cannot settle a domain question; assume your own fix is defective))
...
# Lesson 133: a rule's activation condition must address the NON-COMPLIANT population — globs that describe the mandated end-state arm only for the already-compliant

**Date:** 2026-08-26 · **Context:** transdoc filed three instances of one class (`01M0WN9R`,
`01M0WNVK`, `01M0Z2E979`): rule packs whose frontmatter globs could not reach the code their
mandates bind. core/15-api-contracts banned hand-written TS API types with all-backend globs
(`lib/api.ts` unreachable); core/75-workers-jobs had directory-shaped globs (`workers/`, `jobs/`)
while the entire job queue lived in `worker.py`; core/86-email-templates globbed on `emails/` +
`*.mjml` — the layout its own mandated pipeline PRODUCES — so it armed exactly when a project had
already adopted MJML and stayed silent for every project it exists to correct (self-fulfilling
addressing). Measured cost, all under green gates: 19 frontend calls to nonexistent routes, an
empty beat loop with retention purge never running, three `str.format` security-notice emails
against an MJML mandate.

**The class:** globs were written for a directory-per-concern layout a compliant project would
have, while fabrik scaffolds emit file-per-concern (`worker.py`, `notify.py`, `billing_routes.py`,
`lib/api.ts`). A pack whose activation condition describes the prescribed end-state instead of the
violating starting-state is inert by construction, and nobody finds out until a conformance review
several plans later.

**Rules:**
1. Write activation globs for the code as it exists BEFORE compliance — the trigger population is
   the violators, not the adopters. Ground the glob set in a real fleet inventory of names
   (`find` over `/opt` for the concern's file names), never in the pack's own prescribed layout.
2. Prefix-shape file globs (`**/notify*`, not `**/*notify*`) so synced infra (`mail.py`,
   `mail_notify.py`) can never arm a pack as noise.
3. Where a scaffold type genuinely emits a matching path, add `applies_to:` frontmatter so
   `check_pack_reachability` guards the claim mechanically forever. Where none does (email senders
   and API clients are post-scaffold additions), the annotation would be an unsatisfiable claim the
   auditor rightly flags — grounded glob breadth is the only guard, which is why it must be
   measured, not guessed.

---

# Lesson 132: a fix that relocates a defect without changing its KIND recurs; a fix with no test cannot catch its own recurrence; and the deeper error was fixing the mechanism instead of questioning the premise

**Date:** 2026-08-26 · **Context:** trade-intelligence filed friction finding `01M0YAB2` — the fleet
quota advisory sent it 10 near-identical `finding` mails between 16:50 and 17:55 UTC on 2026-08-25,
9 byte-identical "mob@ at 95%". The rotate ledger showed mob@ fired the advisory **8 times at a
dead-steady 95%** — two days after commit `51181918` was logged as the fix for exactly this spam.

**The defect, three layers deep.**

1. **Relocated, not killed (the immediate cause).** `51181918` moved the per-account dedup key off
   the fast-sliding 5-hour reset onto the weekly reset, on the assumption the weekly reset is "fixed
   within a quota week". It is not — the reported value jitters across the `:59↔:00` minute boundary
   (the live stamp read `95|1787990399` = `20:59:59`), so `cycle = str(int(weekly_reset))` changed
   tick-to-tick and the suppression never matched. **The same defect class — a raw sliding value as a
   dedup key — simply moved from the 5h reset to the weekly reset.** I fixed the location and missed
   the class, which is the precise failure `feedback_first_unattended_run_is_the_proof` warns against,
   committed while carrying that lesson.

2. **No test (why it recurred silently).** `51181918` shipped with **zero test coverage** — no
   watched-fail-first test asserted "steady band across ticks ⇒ one message". A class marked fixed
   with no red-proven test cannot detect its own regression; the recurrence sailed straight through
   the gate because nothing was watching.

3. **Wrong premise (the real fix).** The operator broke the frame in one sentence: *"we don't have to
   fire 95% warnings if we have an account with active quota — only when we have none left."* Under
   one-active-pointer-for-all-projects, an individual account crossing 95% is a **non-event**: the
   flip leg re-points to a sibling with headroom and every agent keeps working. So the per-account
   advisory was not merely mis-deduped — it was **spam AND a false alarm**. Reframing it to fire ONE
   fleet-wide advisory only when the post-flip ACTIVE account is walled with no auto-relief (no
   successor, or the operator's pause held the flip) made the dedup **epoch-free by construction** — a
   presence-only latch, re-armed the instant relief arrives. The churning-key problem didn't get a
   better key; it ceased to exist.

**The generalisable rules.** (1) When a fix relocates a key or value rather than changing its KIND,
ask whether the new site inherits the old defect class — "fix the class, not the location" binds your
OWN fixes hardest. (2) A fix with no red-proven test cannot catch its regression; a "fixed" class that
recurs is evidence the test was missing, not that the bug was subtle. (3) A recurring symptom is a
prompt to question the PREMISE ("why does this fire at all?"), not only to harden the mechanism.

**The fix.** `_fleet_active_wall_advisory` (replacing the per-account loop) fires once when the active
account is walled with no relief, broadcasts to every mailbox repo, and latches epoch-free; the dead
`_fleet_slug_repos` per-account routing was removed. Watched-fail-first (the no-advisory-with-headroom
test was red against the old code, green after); 134 fleet tests pass; both vendored twins in sync.

# Lesson 126: the instruction set was the last thing anyone audited — and a wrong NAMESPACE degrades silently instead of failing

**Date:** 2026-08-16 · **Context:** the first audit of all 27 `/fabrik-*` command sources against the
live implementation, after a session spent auditing crons, hooks, gate checks, alerting and docs —
everything AROUND the corpus.

**The defect.** Four commands told agents to ground research with
`fanout(..., web_tools=["exa","brave","firecrawl","context7"])`. Those are **provider** names.
`WEB_TOOL_NAMES` accepts only **tool** names (`web_search`/`web_scrape`/`docs_lookup`/...), and
`loop.py` filters the advertised schemas by that set: an unknown name yields an EMPTY list, and
`merged.pop("tools")` then runs the agent with no tools at all. The "grounder" still returned
confident prose, `fanout` still returned a results table, the flywheel still recorded rows. Every
spec and plan grounded that way was ungrounded, for as long as the text stood.

**Why nothing caught it.** Both namespaces are real and adjacent — `exa`/`brave-search`/`firecrawl`/
`context7` ARE the valid MCP server names and the API-key names in the same subsystem. The call read
correctly to every human and every agent who saw it. There was no validation on the way in (an
unknown `web_tools` entry is silently filtered, never rejected) and no check on the corpus itself.

**The generalisable rule.** *A wrong name from an adjacent namespace does not fail — it degrades, and
degradation reports success.* This is the same shape as Lessons 122-124 (a check that cannot fail
buys the same green as one that works), one layer up: the INSTRUCTIONS can be vacuous exactly as the
CHECKS can. Wherever a subsystem accepts a list of names and filters the unrecognised ones out,
either it must reject them loudly or something must validate the callers — a silent filter plus a
confident-prose consumer is a lie generator.

**The secondary lesson — my own instrument lied about the fix.** The first pass at enumerating dead
chain references reported four (`/fabrik-autoheal`, `/fabrik-lib`, `/fabrik-lifecycle`,
`/fabrik-mail`). All four were false: the regex was matching PATHS (`/opt/fabrik-lib`,
`/run/fabrik-autoheal`, `docs/reference/fabrik-mail.md`). A check that cries wolf gets ignored just
as fast as one that greens on everything, so the boundary lookarounds are now the single most
tested part of `check_command_corpus.py`.

**Also found, same audit:** three commands described a BLOCKING gate as "advisory-WARN'd" and none
named its sanctioned escape valve; `/fabrik-spec` searched prior art with an uninstalled plugin;
`/design-review` allowlisted 7 MCP tools that no longer exist; 8 `CHANGELOG.md:<line>` citations in
the *destructive* decommission runbook had drifted ~2 700 lines onto unrelated text (a prepend-ordered
file cannot be cited by line — ever); 6 commit templates stamped a retired model into provenance
trailers; and one command fanned out subagents with its contract block configured but never included.

**Fix:** `scripts/enforcement/check_command_corpus.py` (Tier-2 BLOCKING) validates the corpus against
the LIVE registries — imported, never copied, because a copied allowlist drifts into exactly this
defect. Proven two ways: `--selftest` requires each predicate to fire on known-bad input and stay
silent on known-good, and separately it was proven RED on the real historical defect by reverting the
fix in a throwaway copy. Doc: `docs/reference/command-corpus-check.md`.

---

# Lesson 125: "advisory" is a DECISION with a blast radius, not a severity — and the audit that finds inert checks must not punish the honest fix

**Date:** 2026-08-16 · **Context:** resolving Lesson 124's eight inert registrations, plus the
inverse defect in `check_vps_docs` (`scripts/final_gate.py`, `scripts/sysadmin/liveness_audit.py`,
`scripts/enforcement/check_vps_docs.py`).

**1. The uniform answer was wrong in both directions, and only measurement said which.** "Promote
them all" and "make them all advisory" are equally cheap and equally unfounded. Running each rule
across this repo AND all 44 `/opt` repos changed four of the eight decisions and killed every
promotion candidate: `check_env_example` guards a REAL Doc Sync Matrix rule no other check covers
(`check_doc_sync` only fires `.env.example` → `CONFIGURATION.md`) and is still unpromotable at
2223 undeclared vars across 44/44 repos; `check_reusable_modules` looked like a live convention
because `[reusable]` INDEX.md tags are everywhere — but **not one of the 44 repos has a
`src/utils/` or `src/lib/` directory**, the only two it looks in, so it returned at its `if not
modules` guard in every repo on the box. **An empty UNIVERSE and a clean result are indistinguishable
from a "0 findings" run** — measure the denominator, not just the count.

**2. Volume is the weakest reason to retire a check; STRUCTURE is the strongest.**
`check_compose_services` had the smallest blast radius of the eight (30 findings, 19/44 repos) and
was still the clearest unwire, because reading it showed it only enters its services block when
`services:` is itself an ADDED diff line — so it sees a brand-new compose file and is **blind to a
service added to an existing one**, which is the entire use case. Its documentation test is a
case-insensitive substring over four docs, so `app`/`api`/`db` pass trivially. Two structural
faults beat any count. Conversely `check_test_coverage`'s 2063 findings were not the reason to
unwire it — its RULE (a test per public symbol) is the 100%-coverage dogma the Behavior Contract
explicitly rejects. **Read what the check asserts before you count what it finds.**

**3. The inverse defect is the same disease: a WARN that reds a blocking row.** Every finding
`check_vps_docs` can construct is `Severity.WARN`, yet its `__main__` exited 1 on ANY finding, so
"two ops docs have not been regenerated" failed a Tier-3 gate row. A gate that reds on nothing
teaches an operator to ignore it exactly as fast as one that greens on everything. Fix the exit to
follow the severity (`_check_runner.run_as_main`'s contract: ERROR fails, `--strict` promotes
WARN) — and do NOT fix it by generating the missing docs, which would hide the defect behind a
green and put a doc-refresh chore inside a code gate.

**4. `advisory=` and "cannot fail" were the SAME flag, and that is why nobody could see any of
this.** In this gate `advisory=True` only preserves stdout on exit 0 — five checks carrying it
(`check_docker`, `check_env_contract`, `check_doc_sprawl --strict`, `check_lint_ratchet`,
`check_subagent_flywheel`) genuinely fail on a real defect. Overloading it to also mean
"non-blocking" would have mislabelled all five. The separate `warn_only=True` declaration is what
lets the row print `[ADVISORY]`, and it must FAIL LOUDLY when false: a `warn_only` check that exits
non-zero still reds the gate and names the broken contract, or the flag becomes a way to disable
any check with one keyword.

**5. An audit whose verdict rule punishes the fix will not be obeyed.** `liveness_audit`'s vacuity
proof asked one question — "can it go RED?" — so correctly declaring a row advisory would have kept
reporting it INERT forever. The rule now forks on what the row CLAIMS: a **blocking** row must be
able to go red; a **declared-advisory or unwired** one must be able to SPEAK (its output diffed
against the clean control, so a check printing the same banner either way cannot pass as having
detected anything). DEAD then means exactly one thing — a gate row that claims to block and cannot
— and the count went 8 → 0 without a single check being weakened. Discovery of the `warn_only`
declarations is `ast`-parsed and fails CLOSED: an unparseable gate reads every row as blocking,
which can over-accuse but can never excuse.

**6. Two ratchets, because one direction is not enough.** "A warn-only check may never be
registered as an ordinary blocking row" stops the regression; "a `warn_only=True` declaration must
be backed by a written contract in `CANARIES`/`UNREACHABLE`" stops the label being used as a silent
downgrade. Writing the second immediately caught a real gap: `check_ticket_breadth` exits 1 only
under `--strict`, which the gate deliberately never passes — **the CHECK can fail while the ROW
cannot**, a distinction with no vocabulary until the ratchet demanded one (`row_warn_only`).

---

# Lesson 124: a gate row that can only WARN is a gate row that can never fail — and "no canary" hides it exactly as well as a bug would

**Date:** 2026-08-16 · **Context:** completing the canary corpus for all 43 checks
`final_gate.py` registers (`liveness_audit.CANARIES`, `tests/test_gate_check_canaries.py`),
after the liveness layer reported 40 of them UNPROVEN.

**1. Eight registered checks have no non-zero exit path at all.** Not a bug in the checks —
each is deliberately WARN-only in its own source (`return 0  # WARN-only — never blocks`,
`return True, errors  # Always pass - warnings only`, `(This is a WARNING - commit will
proceed)`). The defect is at the REGISTRATION: `check_compose_services`, `check_env_example`,
`check_env_updates`, `check_script_headers`, `check_test_coverage`, `check_reusable_modules`,
`check_retired_terms`, `check_doc_stubs` sit in `final_gate.py` next to checks that CAN red
the gate, and produce the identical green row. Four of them are not even registered
`advisory=True`, so their WARN text is discarded on exit 0 and the row is fully silent. **A
check's severity contract and its gate registration are two separate facts, and only the
second one is visible in the gate's output.** They are now declared `warn_only` in the canary
table with the source line that makes them toothless, and reported DEAD with that evidence —
the honest state, not UNKNOWN.

**2. "Unproven" and "vacuous" are indistinguishable from the outside, so the ratchet has to
be a test.** 40 checks were UNPROVEN not because anyone decided they were unimportant but
because that state accumulated one unexamined `run_optional_check(...)` at a time, invisibly.
`test_every_registered_gate_check_is_accounted_for` now fails on a new registration that has
neither a canary nor a written-down obstruction. Prose ("author a canary for new checks")
would have re-accumulated the same 40 — Lesson 116's rule, applied to a coverage denominator.

**3. A canary that survives a broken check proves nothing about a working one.** The obvious
trap in this work is authoring a fixture that reds for the wrong reason — a parse error, an
unrelated finding, a crash — and banking the green. `NEUTERS` deletes one check's core rule
in a throwaway copy of `scripts/enforcement/` and asserts the canary then goes GREEN. Two
fixtures failed exactly this way while being written: `check_no_host_ports` only flags a
`ports:` binding on a service that ALSO carries `traefik.enable=true` (a bare port binding is
not the rule), and neutering `check_user_guide`'s missing-directory branch still exited 1 via
its empty-directory branch — the first fixture was testing nothing, the second neuter was.

**4. Three checks are unreachable by any fixture, and that is a finding about the CHECK.**
`check_vps_docs` reads a hardcoded absolute `FABRIK_ROOT = Path("/opt/fabrik")` with no
`--root`, no env var and no cwd sensitivity — it cannot be pointed at a fixture, cannot be
unit-tested against one, and is currently **RED on the hub** (`docs/operations/vps-status.md`
and `vps-urls.md` do not exist) while its findings are all `Severity.WARN`. An untestable
check and a check nobody can prove are the same liability; `UNREACHABLE` records the
obstruction by name rather than letting the row read as covered.

---

# Lesson 123: absence of evidence is not evidence of absence — validate the instrument, and keep a third verdict

**Date:** 2026-08-16 · **Context:** building the liveness layer (`scripts/sysadmin/liveness_audit.py`)
after Lesson 122's audit found several "continuously monitored" surfaces that had never run.

**1. Three probes lied in one morning, and each lie reached the operator as fact.**

| Probe | Conclusion drawn | Truth |
|---|---|---|
| `grep -c "arg=sweep" ~/.claude/sound-debug.log` → nothing, rc=1 | "the reboot sweep has NEVER run" | one invalid UTF-8 byte at offset 1313 made GNU grep classify the file BINARY and suppress every match; `grep -a` shows **21** events, including that morning's boot |
| `ls specs/services/ | grep apprise` → nothing | "Apprise was never a fabrik-managed service" | the spec is `specs/infrastructure/apprise.yaml` — ONE directory searched, generalised to "does not exist" |
| `ssh vps docker ps` → EMPTY | "no apprise container, not even stopped" | the ssh user is not in the `docker` group; every call failed with permission denied and printed **nothing to stdout**. With `sudo`: 31 containers, apprise healthy, HTTP 200 |

Same shape every time: a **broken instrument** read as a **true negative**. Re-measured minutes
later, plain `grep` answered 21 — the binary suppression is **intermittent** as the file grows,
which is worse than a stable bug, because a one-off manual re-check would have "disproved" it.

**2. The fix is structural, not vigilance.** Verdicts are **LIVE / DEAD / UNKNOWN**, and one
constructor (`finding()`) is the only way to build one: any instrument fault collapses the verdict
to UNKNOWN and carries the fault forward by name — in **both** directions, since an instrument that
cannot prove absence cannot prove presence either. Every probe validates itself against a positive
control first: the log reader round-trips a file it just wrote containing an invalid UTF-8 byte
*before* the marker; `crontab -l` must return parseable entries before any cron is called missing;
`systemctl` is checked against `cron.service`; `ss` must list at least one socket. Neutering
`finding()` to a plain constructor turns 6 tests red, headline first.

Four corollaries, each of which cost a false finding today: read evidence as **bytes** with
`errors="replace"` and never shell out to `grep`; a missing file whose parent directory is absent or
unreadable is UNKNOWN, not DEAD; a `/tmp` evidence path is **volatile** (cleared on boot) so its
absence proves nothing; a systemd unit `not-found` at SYSTEM level must be re-queried with
`systemctl --user` before absence is claimed (`ai.traycer.host.service` is `not-found` system-wide
and `enabled` under `--user` — failure 3's exact shape, locally).

**3. The rule must survive the hand-off to a metric.** kaizen's `Missed crons` column — an em-dash
since it was pinned — is now computed from this audit as `DEAD/(LIVE+DEAD)`. UNKNOWNs are excluded
from **both** halves and named in the hand-off mail. Folding them into the denominator reports a
healthier box than was measured; folding them into the numerator invents defects. Either is the
honesty rule broken by a number that looks fine, and a wrong metric silently ends the investigation
it should start. Guarded by `test_an_unknown_surface_is_never_rendered_as_a_number`.

**4. A monitoring layer that blocks work gets disabled — and then it monitors nothing.** The audit
exits 0 by default and never raises; a proof that dies becomes an UNKNOWN finding rather than a
missing section. `--strict` is opt-in for CI, UNKNOWN never fails it (an instrument fault is not a
defect), but a **crashed proof** does: a silently skipped proof is exactly how a monitor learns to
say all-clear.

**5. False positives are how a truth-checker gets ignored, so tune the ORACLE, not the threshold.**
The doc-claim proof's first run produced 9 findings, 7 of them false: a naive "any negative word
means disabled" polarity called two correct docs stale (a unit documented as `enabled` that ends
`failed` on purpose is a TRUE doc), a prose word became a scheduled job named `"line"`, and four
files were called missing hooks because the oracle reads only ONE of four hook layers. Each was
fixed by making the oracle honest about its own scope — the hook check now returns UNKNOWN naming
the layer it cannot see — not by muting the check. Also: the audit's own doc prints a PROPOSED cron
line, which the parser dutifully reported as stale; a fenced block introduced as a proposal is not a
claim about the live box.

**First real audit:** `LIVE=52 DEAD=4 UNKNOWN=47`. Three crons with a schedule that have never
written evidence, one doc scheduling a pipeline whose crontab line is only a comment header, one
surface with no observable evidence channel at all, and 40 of 43 gate checks with no canary — the
honest size of the remaining gap, which is the number a green would have hidden.

---

# Lesson 122: a check that cannot fail buys the same green as one that works — and the tool you gather liveness evidence WITH can lie

**Date:** 2026-08-16 · **Context:** stop-the-bleeding pass after an audit found several
"continuously monitored" surfaces that were declared and never actually ran.

**1. Vacuity is the dominant failure mode of a large enforcement corpus.** Six Tier-3 checks
(`check_docker`, `check_ports`, `check_env_contract`, `check_deps_sync`, `check_watchdog`,
`check_health`) were registered in `final_gate.py` and reported PASS while asserting *nothing*:
each defined only `check_file()`, with no `__main__` and no top-level call, so running them exited
0 with **empty output**. Their logic was real and reachable — but only through
`validate_conventions.py`'s Cascade `post_write_code` path, and Cascade is dormant, so the only
live caller invoked them in the form where they did nothing. This is the second instance of the
class (`check_doc_sprawl` was inert since ≤2026-08-04). **The gate's green count is not evidence
that the gate checks anything.** A registered check owes a proof that it goes RED on a known-bad
fixture; without that pair, "N/N passed" measures registration, not coverage.

**2. Activating a check is a MEASUREMENT job, not a wiring job.** Running the six for the first
time against this repo *and* all 45 `/opt` repos changed almost every decision. Two earned
activation but only after their own false-positive classes were fixed: `check_env_contract` treated
a **missing** `.env.example` as "every compose var violates the contract" (13 errors in the hub, all
from `infra/vps1/*` deploy units whose secrets live on the VPS), and parsed **YAML comments** as
references — the scaffolder's template line `#   3. Environment variables (use ${VAR:?} …)` made 17
of 45 repos "require" a variable named `VAR`. Blast radius fell 17/45 → 2/45 on those two fixes
alone. Four checks were **unwired with the measurement recorded in the gate**, because their models
are structurally too coarse to gate on (a port-range rule keyed on the *file's* suffix rather than
the service's technology; a deps rule with no notion of optional groups or pinned transitives; a
per-project `watchdog.sh` convention nobody ever adopted; a health heuristic that cannot tell a
service with no dependencies from one skipping its probe). **An honest missing check beats a fake
green one** — and the reason belongs in the gate, next to the registration, not in a commit message.
Corollary: `advisory=True` output is not a substitute for a failure mode. If a check can never exit
non-zero, it is still vacuous; give it `--strict` or remove it.

**3. Activation pays for itself immediately, and the finding is the point.** `check_docker`'s first
real run found **5 of 45 repos shipping `platform: linux/arm64` in a production compose deployed to
an x86_64 VPS** — invisible for as long as the check was vacuous. Conversely, one of its ERROR rules
had to be *demoted*: `image: redis:7-alpine` / `postgres:16-alpine` are vendor images running in
production today, and the No-Alpine invariant targets **built** images (musl), so gating on them
would have red-lined the hub permanently for zero correctness gain. Read what a rule is *for* before
enforcing its letter.

**4. Split credentials fail with a status code that points at the wrong layer.** Alert delivery had
been dead: `/opt/fabrik/.env` stores only the **secret half** in `TELEGRAM_BOT_TOKEN` (the usable
token is `<bot_id>:<secret>`, in `TELEGRAM_FULL_BOT_TOKEN`), and posting to `/bot<secret>/sendMessage`
returns HTTP **404 Not Found** — which reads as "dead endpoint", not "malformed credential". A
sibling script (`claude-sound.sh`) already knew to compose the halves; the shared library did not.
Two lessons: **when two consumers read the same credential and only one works, diff their READ, not
their transport**; and a delivery layer must report *per method, with the cause* — the old single
line `Alert FAILED (all delivery methods)` named neither the methods nor the two independent
breakages behind it (the SSH→Apprise leg was separately down with remote curl exit 6). Every
alerting path now ships a `--selftest` an operator or cron can run to prove delivery end to end.

**5. ⚠️ The tool you gather liveness evidence with can lie to you.** The `@reboot` resume sweep was
reported as having **never run** — "zero `arg=sweep` events, ever" in `~/.claude/sound-debug.log`.
It was false. The sweep has run 21 times, including today's boot. One **invalid UTF-8 byte** at
offset 1313 makes GNU grep classify the whole file as binary and **silently suppress matching
lines** — `grep -c` prints nothing and exits 1, which reads exactly like "no matches". The
corroborating evidence was equally misleading: `~/.claude/sweep.log`'s stale mtime proves nothing,
because appending with `>>` never updates mtime when the process writes no stdout, and this script
logs to a different file by design. Rules: **`grep -a` when grepping a log** you did not write; a
file's mtime is not a run record; and before "fixing" a system reported dead, reproduce its
execution directly (running the sweep under a simulated cron env logged perfectly). The genuine
finding underneath was narrower and real — 4 sessions cut by today's reboot went un-notified
because their `cwd` was exactly `/opt`, which does not match `mesh-notify`'s `/opt/*` gate.

---

# Lesson 115: detection ≠ arming — the resume mesh has two independent legs, and a death-class detector keys on the structural invariant

**Date:** 2026-08-12 · **Context:** connection-failure auto-resume (plan-3), grounded from a live
`/opt/iterative_image_editor` ENOTIMP incident the operator hit ("havent you implemented this?").

Two lessons from one incident. (1) **Detection and arming are orthogonal.** The mid-stream plan
shipped death DETECTION; auto-continue also needs the session's `claude-selfwatch.sh` Monitor
ARMED to consume the `.errparked` record and wake the pane. Arming is ADVISORY — `session_orient.py`
emits an instruction ("ARM YOUR SELF-WATCH NOW … call Monitor(persistent…)") that the agent must
execute as its first tool action. Session 35204643 never did (0 arms in `sweep.log`), so even
perfect detection would have fired into the void. "Auto-resume is implemented" was only half-true:
the detection leg shipped, the arming leg depends on per-session agent compliance. Verify BOTH legs
before claiming a class is covered; a mechanical arm (a SessionStart hook spawning the Monitor
directly) is the real close, filed as a follow-up (harness-capability question). (2) **A death-class
detector keys on the STRUCTURAL invariant, not a string list.** The connection-failure family is an
open set of error codes (ENOTIMP/ECONNRESET/ECONNREFUSED/ETIMEDOUT/ENOTFOUND/…) — a code allowlist
rots the first time the CLI emits a new one. The actual death condition is `retryAttempt ==
maxRetries` (the CLI spent its own retry budget); keying on that covers the whole family and any
future sibling for free, while a still-retrying record (attempt < max) correctly stays non-death.
Same class as "ground enumerations from the registry, not memory" — but for runtime predicates:
find the invariant, not the instances.

# Lesson 114: race tests must run the real contended operation — and file-store locks converge on rename + traveling timestamps

**Date:** 2026-08-12 · **Context:** agent-roles wiring Phase C — the mail claim/ack resolve
window, 5 native review rounds.

Two halves. (1) **The fake-raise race test proves nothing:** my choreography test simulated a
lost race by RAISING FileNotFoundError on the actor's behalf instead of running the real
append — it stayed green with the O_CREAT guard reverted, asserting only that exceptions
propagate. The closer re-ran the same choreography with the REAL operation and the "closed"
case was wide open (a disposition landed on another agent's fresh claim). A race test's
sabotage hook may rearrange the WORLD (rename files, inject claims) but the operation under
test must be the production code path. (2) **Design convergence:** the resolve window went
through five shapes (direct append → shared window name → message-mtime age gate → per-pid
windows + post-rename utime → utime-BEFORE-rename). Each patch minted the next finding until
two invariants held: every transition uses the SAME lock primitive (atomic rename — the store's
native discipline), and any timestamp a guard reads must TRAVEL WITH the atomic op (stamp the
source, then rename), never be applied after it (a two-syscall gap is a theft window). When a
concurrency fix needs a second primitive, suspect the design instead.

# Lesson 113: a retirement decision must land in the machine-readable layer the same day — org markers beat mechanical probes

**Date:** 2026-08-12 · **Context:** capability-catalog audit before agent-ownership derivation
(operator: "I want to be 100% sure it is flawless because agent distinction will share from it").

The catalog advertised the dead Kilo-CLI review/dispatch family (12 files, incl. a `_bckp` copy)
and the retired `supabase` driver as `ok` — a month after both retirements — because `ok` means
"the mechanical probe passes" (`--help` exits 0 / the import succeeds), and the `# RETIRED`
line-start marker mechanism, built by the catalog plan for exactly this, was **never applied when
the retirement decisions landed** (2026-07-19 Kilo, 2026-07-03 Supabase). Third instance of the
registry-vs-decision drift class: the captcha "stays live" claim, the supabase driver, now the
kilo family. Two rules shipped: (1) a retirement decision's landing checklist includes marking
every affected file the catalog reads — the decision isn't landed until the machine layer says
so; (2) every enumerator must check org markers BEFORE its mechanical probe (`_enum_drivers`
imported first and never looked — fixed with a red-first fixture). Corollary from the same audit:
adjudicate per-file before marking — `kilo_model_sync.py` (daily cron) and `check_kilo_health.sh`
(invoked at `final_gate.py:1111`) carry the legacy name but are LIVE; a name-based sweep would
have killed real machinery.

# Lesson 112: a new `/opt/<dir>` auto-enrolls in governance-sync — exclude non-project dirs in the SAME change; sync-consciousness runs both directions

**Date:** 2026-08-12 · **Context:** fabrik-mail, operator-caught after the plan was EXECUTED + archived.

`sync_enforcement_to_projects.py::main()` discovers sync targets by iterating `/opt` and taking every
`is_dir()` that isn't hidden / `_`-prefixed / in `exclude_folders` — **it does NOT require `.git` or a
`project.yaml`.** So Phase C's `mkdir -p /opt/fabrik-mail` (the mail DATA store) was immediately adopted as
a "project" by the next governance-sync and filled with the entire governance corpus. The `<repo>` mailbox
dirs then sat *beside* CLAUDE.md/`.claude/`/`scripts/`/`libs/`/`docs/` — the exact "no governance-sync
races" the spec had frozen in prose but nothing enforced.

**Why every review missed it:** the phase reviews + the whole-plan cross-phase review all checked
**fleet-distribution coherence in the OUT direction** — `mail.py` ∈ `CORE_SCRIPTS`, `mail_notify.py` ∈
`AGENT_HOOK_FILES`, settings wiring, hub-guard — and confirmed the machinery reaches the ~46 projects. Not
one asked the IN direction: *does the new `/opt/fabrik-mail` directory itself get swept up as a sync
target?* A distribution review that only follows the artifacts outward is half a review.

**The rule:** any change that CREATES or LEAVES a new `/opt/<dir>` which is not a real project (a data
store, a socket/runtime dir, a cache) MUST add it to `exclude_folders` in `sync_enforcement_to_projects.py`
**in the same change** — and a distribution-surface review must assert it. Also: `fabrik-lib` and
`archived` were already excluded, so the pattern + its home existed; the plan simply never used it.

**Cleanup discipline (when it already happened):** the mail root is a live data store — a blanket `rm -rf`
would destroy real messages. The fix classified every top-level entry (KEEP anything containing an
`inbox/archive/malformed`; remove only the known governance surfaces) and guarded each removal with a
"contains a mailbox?" check — preserving a real `seo → fabrik-lib` upstream-feedback message that a project
had already sent ahead of item 6.

---

# Lesson 111: close a guard anti-pattern at every structural level in one wave — and split "new-code defect" from "pre-existing fragility" when costing the loop

**Date:** 2026-08-12 · **Context:** stalled-mid-stream auto-resume execution (plan-2), step-9 review
loop over `~/.claude/bin/claude-stop-decider.py`.

`X or default` guards against falsy, not truthy-wrong-type — `(m.get("content") or [])` still
iterates `content=99`, `float(v or 300000)` still crashes on `"not-a-number"` and silently returns
`1.0` for `True`. The review loop re-discovered this ONE anti-pattern at three structural levels in
three separate finder rounds: text leaves (round 8), containers (round 10, four CONFIRMED crash
repros on a production Stop-hook that fails open by RINGING), then bool scalars (rounds 12-15,
including a twin the closing finder flagged after the "class closed" claim). Each level cost a full
round: finder dispatch, red-first fixtures, suite, 114-fixture harness, DR push. The class was
grep-able from the first instance: sweep every `.get()`/coercion the same code path touches at ALL
levels (leaf, container, scalar) and close them in one wave — a "class closed" claim that only
covered one structural level is the exact false-convergence this file keeps re-learning.

The cost dual, surfaced by the operator's fair "4 hours for a simple hook?": most of those rounds
hardened PRE-EXISTING fragility the new walk exposed, not defects in the new feature. On a fail-open
production surface that's defensible (a decider crash = false ring). But the loop contract iterates
to no-op over both kinds without distinguishing them — the honest lever for small features is to
adjudicate pre-existing-adjacent findings as a batched hardening wave (or recorded-deferred), not to
round-trip each level through the full finder cycle.
...
**Context (2026-07-18):** Mid-session, the stop-hook DoD failed my turn on 2 new ruff errors (`119 → 121`). Every file I authored passed clean; the errors were in `/opt/ai-model-catalog/engine/microbench_review.py` — an **untracked** file a sibling agent had created **minutes earlier** and was still actively editing (it grew to 4 errors during diagnosis).
...
**Context (2026-07-14):** The Terminal-Bench runner (`/opt/ai-model-catalog/engine/microbench_terminal.py`) resumes an
...
**Rule.** An advisory check asserts the FACT, never the WORDING: read a committed constant, a ledger row, a file's presence, a parsed field — not English. When a prose-matcher must exist (a plan spine is prose), pin BOTH directions with tests the moment a seat raises a phrasing, record the shape as an ADJUDICATED standing row in the receipt, brief every later seat with it, and count only a plan-shaped phrasing a real spine would write; a further wording finding is CITED, not counted (D-048). Do not tune the regex a fourth time — leave it pinned; strictly better than another pass. **The discriminator at round 2, not round 8 (from the exchange with intel, 2026-09-08):** ask whether a fix ADDS AN ALLOWANCE or EXCLUDES A LEXICAL CLASS. Adding an allowance — a new verb, another adverb slot, one more synonym — fits the checker to a phrasing sample of size one; the next independent seat lands outside it again, and the commit subject gives it away (it names the phrasing it just accommodated). Excluding a structural class — hyphenation (`OFF\b(?!-)` shut every `OFF-*` compound at once, round 8), a word boundary, negation scope — is a fact about the language, not the sample, and converges over everything the fact covers. So prose-matchers DO converge, but only on structural constraints; every allowance-widening round is the signal the checker is fitting noise, and the second one is when to stop tuning and pin. Sibling of the mechanical-gate lesson: the gate that keyed on a credential file (no owner-visible state) was replaced by a committed constant for the same reason — a fact with a fixed point.
...
**Lesson (corrected the same day, after the class fired AGAIN in `fae20651`):** `git commit -- <paths>` commits the WORKTREE state of those paths — index surgery (`update-index --cacheinfo` with HEAD + your hunks) is a NO-OP against a pathspec commit, and `git diff --cached` proves nothing about what a pathspec commit will take. For CHANGELOG.md, INDEX.md, docs/DECISIONS.md, CLAIMS.yaml and every file more than one session appends to in a day: save the worktree file, WRITE the worktree file as HEAD + your own hunks, commit with the pathspec, then restore the saved worktree file (the sibling's WIP included) — and prove it with `git show HEAD -- <file>` showing only your lines. "No removed sibling lines" is not the test — ADDED sibling lines ride along just as silently. And commit per converged pass, not at the end: a 30-file unstaged set held for hours reds every sibling's diff-scoped gate and invites exactly this bundling in both directions.
...
rules and infra. Each time the answer was more surface: the duty written into both constitutions,
auto-appended to all 31 commands and 4 agent definitions, a `--feedback` flag, a verdict persisted to
the run record, a kaizen cell, and a grader. Then it was measured. **13 closes in 14 days, 12 with no
verdict, and zero `filed` verdicts ever recorded** — while distribution was flawless: 30/30 rendered
commands carried the text and the `~/.claude-fleet` copies were byte-identical to `~/.claude`.

**Lesson 1:** when a behaviour does not happen, measure the layers separately before adding another.
Distribution, recording, and measurement had all shipped and all worked; the missing layer was the
only one that binds — `done` returned 0 without the flag, and the grader is `warn_only`. On this box
exactly two things change agent behaviour: a command that REFUSES, and the Stop hook. Everything else
is a suggestion an agent reads once, hours before the moment it applies. "Is it installed?" and "does
anything happen if it is ignored?" are different questions, and only the second one predicts outcomes.

**Lesson 2:** the cheapest enforcement reuses an existing block. The refusal adds no new check and no
new hook — a refused close leaves the record `running`, which the Stop hook already blocks the turn
on. A new blocking check would have reddened ~46 repos on landing; this one cannot, because it is not
a check.

**Lesson 3 (a defect this change nearly shipped):** the requirement was keyed to a cutoff written as
"2026-08-28T00:00 UTC" while it was 21:18 UTC — a rounded-up local date. The mechanism would have
read as enforced and done nothing for hours. Every "effective from" constant is a live branch: write
it in UTC, set it BEHIND the current instant, and let an end-to-end test prove it fires now. The
end-to-end pair caught it; the unit tests over the helper all passed.

**Lesson 4:** making a duty binding costs the test suite. Twenty-three existing tests closed a run as
SETUP and broke instantly. That cost is the evidence the rule was previously optional — and the two
tests that failed for a *semantic* reason ("omitting feedback records `unstated`") were the old
contract, which now survives only on the grandfathered path. Rewrite those; do not weaken the rule to
keep them green.

## Fixing the authoring command leaves the REVIEW rubric still armed — and a rubric can contradict itself two axes apart (2026-08-27)

**Context:** transdoc filed (`01M12DN488`) that `/fabrik-flows` bans file paths in its hard exclusions
while its own re-freeze close-out MANDATES a Downstream impact table full of them. Correct, and a sibling
hub session fixed it in `commands/_sources/fabrik-flows.md` within the hour. The four false CRITICALs that
motivated the filing were still fully reproducible after that fix.

**Lesson 1:** an author-blind review does not read the authoring command — it reads the REVIEW rubric.
The unscoped law lived in BOTH (`fabrik-flows-review.md` axis F), and the reviewer applies the one that
was not fixed. When a contract is split across a producer/reviewer twin, a law stated in both is TWO
copies of the same law: fix the producer's copy alone and the defect keeps firing from the surface that
actually generates findings. Grep the twin before closing.

**Lesson 2:** the sharper defect was internal to the rubric. Axis **F** banned file paths and
implementation detail; axis **G**, twenty lines below, REQUIRED the Contract inputs section to list every
entity/field the journeys imply. One rubric, two clauses, opposite verdicts on the same section — and an
honest reviewer applying both in one pass must raise a finding against something the same rubric demands.
This is the third instance of the class in two days (`check_review_coverage` vs the `term-coverage` exit;
`/fabrik-features` vs `check_certification_coverage`). **A contract and its grader are one artifact; so
are two axes of one rubric.** When adding a clause that forbids a SHAPE, grep the same file for clauses
that require it.

**Lesson 3:** chasing the reporter's second, smaller finding is what surfaced the real one. The mail noted
in passing that the contract "calls the `[PRIMARY PATH]` markers the certification denominator" — and the
hub had told them that, in three places, and it had been false since the certification-denominator change
landed: the gauntlets resolve their denominator from a live registry and demote every doc to a
cross-check, with `check_certification_coverage.py:441` flagging a cert board sourced from a doc. A project
following the line verbatim would have built a board its own gate then failed. **The aside in a filing is
often the part the filer could not evaluate** — they cannot see the consumer's contract. Chase it.

## A convergence loop is only a loop if the round can ask the question again (2026-08-27)

**Context:** the first command of a 31-command corpus audit. `/fabrik-rivals` Phase 2 told the
agent to "re-run discovery each round, diff the new competitors, terminate on two consecutive DRY
rounds". The engine guards discovery with `if not discovery_done:` (`competitor_intel/orchestrator.py:566`)
and persists that flag in a per-`job_id` checkpoint; the driver derives `job_id` **deterministically
from the market**. So round 2 onward could not surface a new rival BY CONSTRUCTION. Condition 1
auto-satisfied at round 2 and the command reported CONVERGED.

**Lesson 1:** a loop's terminal condition must be checked against the mechanism that is supposed to
FEED it, not just stated. "No new X was found" and "the round could not have found a new X" produce
byte-identical evidence, and the second is the one that hides. When a convergence contract rests on
a re-run, open the code that the re-run calls and prove the re-run is not a replay — every caching,
checkpoint, memo or idempotency layer between the loop and the question is a candidate for turning
the loop into a formality. This is the same fail-silent-green class as a check that reports SUCCESS
when it could not ask its question; here the *loop itself* was the check.

**Lesson 2:** the obvious fix to a skipped step is to un-skip it — and that was WRONG here. Clearing
`discovery_done` alone would have been worse than the bug: the engine REPLACES the competitor list
(`orchestrator.py:572`) rather than merging it, so a round-2 discovery returning 9 of 12 would
silently drop 3 rivals, while their `reviews_done` entries survived so no later round re-mined them.
Before re-enabling a skipped stage, read what the stage DOES to the state it re-computes.
Accumulate-across-rounds needs an explicit union; it is never a side effect of re-running.

**Lesson 3 (the audit's own finding):** four of the five defects on this command were
*stale-companion* — the command source had been corrected days earlier and its reference doc, its
`INDEX.md` rows, its router entry and its grader did not move with it. A fix that lands only in the
artifact an agent is least likely to read is a fix with a countdown on it: the reference doc was
still routing project agents into the two-repo workflow that had already been deleted.
...
**Date:** 2026-05-14
**Context:** Running `scripts/final_gate.py --lean --json` for T1-01 returned `status: failure` on three ruff violations (N806 / B007) in files that were NOT in T1-01 scope: `scripts/generate_kilo_agents.py`, `/opt/ai-model-catalog/engine/post_filter.py`, `/opt/ai-model-catalog/engine/scrape_artificial_analysis.py`. Two had been modified in earlier sessions and left in the working tree; one was an untracked file from a prior session. The gate's diff-aware check rightly considers them "current state" and runs ruff over them. Result: T1-01's own work was clean, but the gate blocked progress on inherited lint debt.
...
## Refresh tokens are single-use and a chain runs ~30 days from its /login — a copy's refresh kills the real dir, and no ping extends the chain (2026-09-12)

- **What happened.** To learn whether a CLI refresh extends the OAuth refresh chain, a session ran `claude -p` on a TEMP COPY of mob@'s fleet dir with `expiresAt` forced past. The copy refreshed, received the ROTATED refresh token, and was deleted. The real dir kept the spent token; the tick's next refresh (15:50) was refused and the CLI wrote the logged-out shape (`accessToken: ""`, `expiresAt: 0`). mob@ was the only account with weekly headroom, so `--switch mob` was refused ("credentials … carry no access token") while the fleet was on HOLD. One `/login` in the real dir revived it. Same day, ob@ lapsed at its stored `refreshTokenExpiresAt` while it was the ACTIVE account serving a session every ten minutes: use does not extend the chain, and the weekly `--keepalive` had pinged nothing in three weeks because its idle gate read a credential mtime the tick renews daily.
- **The rules.** (1) A credential file is read-only for every probe: read the expiry fields, call the profile/usage endpoints with the ACCESS token (they rotate nothing), never force a refresh, never run `claude` in a copied dir — "on a copy" IS the consumption. (2) The chain is ~30 days from the last `/login` and nothing extends it; the recurring duty is a monthly `/login` per account in its own dir, and the tool's job is to say so in time (the 5-day warning carries the exact re-login line; the tick pushes once per chain inside 3 days). (3) A remedy printed by a tool is a claim: "run one claude turn" was printed for weeks and was false — test the remedy, not the warning.
- **Where it lives.** `scripts/sysadmin/claude_rotate.py` (`_relogin_block`, `_chain_expiry_push`, the retired `_keepalive_sweep`), `docs/workstation/claude-account-rotation.md` § Re-login, memory `feedback_never_refresh_on_a_credential_copy`.
