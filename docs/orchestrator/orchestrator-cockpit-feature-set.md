...
| F10 | **User-editable agent-selection-guide** consulted by every skill before spawning | **ADAPT** | Ours must route: Opus = orchestrator/authority, pool via `fabrik-lib/subagents` = breadth (D4) — i.e. the guide encodes `62-using-subagents.md`, not a free-text whim. |
