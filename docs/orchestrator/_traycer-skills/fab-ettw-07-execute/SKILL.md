---
name: fab-ettw-07-execute
description: "Epic-to-ticket step 07: dispatch and execute the tickets. Use after ticket breakdown, or when the user types /fab-ettw-07-execute."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/07-execute-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-07-execute (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/07-execute-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-ettw-07-execute --phases 7 \
...
- `python3 scripts/command_run.py done --command fab-ettw-07-execute --evidence "<what proves the terminal
  ...
  silence is not.
- `python3 scripts/command_run.py blocked --command fab-ettw-07-execute --reason "<what · searched · missing>"
