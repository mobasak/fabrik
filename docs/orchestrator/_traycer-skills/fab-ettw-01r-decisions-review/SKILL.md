---
name: fab-ettw-01r-decisions-review
description: "Epic-to-ticket step 01R: adversarially converge the DRAFT Decisions-Lock artifact to an md5-verified no-op, then stop for the operator's confirm, which flips it to LOCKED (Gate 1). Use right after 01, or when the user types /fab-ettw-01r-decisions-review."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/01R-decisions-review-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-01r-decisions-review (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/01R-decisions-review-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-ettw-01r-decisions-review --phases 2 \
...
- `python3 scripts/command_run.py done --command fab-ettw-01r-decisions-review --evidence "<what proves the terminal
  ...
  silence is not.
- `python3 scripts/command_run.py blocked --command fab-ettw-01r-decisions-review --reason "<what · searched · missing>"
