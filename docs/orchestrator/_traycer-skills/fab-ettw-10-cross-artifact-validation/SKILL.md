---
name: fab-ettw-10-cross-artifact-validation
description: "Epic-to-ticket step 10: cross-artifact consistency validation across the decisions lock, flows, plan, tickets, and code. Use before deploy, or when the user types /fab-ettw-10-cross-artifact-validation."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/10-cross-artifact-validation-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-10-cross-artifact-validation (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/10-cross-artifact-validation-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-ettw-10-cross-artifact-validation --phases 4 \
...
- `python3 scripts/command_run.py done --command fab-ettw-10-cross-artifact-validation --evidence "<what proves the terminal
  ...
  silence is not.
- `python3 scripts/command_run.py blocked --command fab-ettw-10-cross-artifact-validation --reason "<what · searched · missing>"
