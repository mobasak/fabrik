...
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/mega-epic-breakdown/00-trigger-mega-epic-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->
...
```bash
python3 scripts/command_run.py start --command fab-mega-00-trigger --phases 5 \
...
- `python3 scripts/command_run.py done --command fab-mega-00-trigger --evidence "<what proves the terminal
...
- `python3 scripts/command_run.py blocked --command fab-mega-00-trigger --reason "<what · searched · missing>"`
