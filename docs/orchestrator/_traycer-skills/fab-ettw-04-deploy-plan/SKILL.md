---
name: fab-ettw-04-deploy-plan
description: "Epic-to-ticket step 04: produce the deploy/registration plan (spec shape, fabrik apply path). Use after the tech plan, or when the user types /fab-ettw-04-deploy-plan."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/04-deploy-plan-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-04-deploy-plan (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/04-deploy-plan-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-ettw-04-deploy-plan --phases 9 \
...
- `python3 scripts/command_run.py done --command fab-ettw-04-deploy-plan --evidence "<what proves the terminal
  condition was met>"` — the evidence is the point; "finished" is not evidence.
- `python3 scripts/command_run.py blocked --command fab-ettw-04-deploy-plan --reason "<what · searched · missing>"`
