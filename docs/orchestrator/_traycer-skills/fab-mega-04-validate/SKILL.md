---
name: fab-mega-04-validate
description: "Mega-epic step 04: converging cross-epic validation + ticket-set integrity gate (absorbs retired 05) + emits the code-generated phased execution order. Use when the user types /fab-mega-04-validate."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/mega-epic-breakdown/04-cross-epic-validation-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-mega-04-validate (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/mega-epic-breakdown/04-cross-epic-validation-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-mega-04-validate --phases 4 \
...
- `python3 scripts/command_run.py done --command fab-mega-04-validate --evidence "<what proves the terminal
  condition was met>"` — the evidence is the point; "finished" is not evidence.
- `python3 scripts/command_run.py blocked --command fab-mega-04-validate --reason "<what · searched · missing>"`
