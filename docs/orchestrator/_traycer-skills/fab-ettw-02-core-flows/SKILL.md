---
name: fab-ettw-02-core-flows
description: "Epic-to-ticket step 02: define the epic's core user/system flows. Use after the decisions lock is set, or when the user types /fab-ettw-02-core-flows."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/02-core-flows-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-02-core-flows (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/02-core-flows-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-ettw-02-core-flows --phases 8 \
...
- `python3 scripts/command_run.py done --command fab-ettw-02-core-flows --evidence "<what proves the terminal
  ...
  silence is not.
- `python3 scripts/command_run.py blocked --command fab-ettw-02-core-flows --reason "<what · searched · missing>"
