---
name: fab-mega-03-expand
description: "Mega-epic step 03: expand the confirmed epic proposal into full epic files and PERSIST them to docs/development/epics/. Use when the user types /fab-mega-03-expand."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/mega-epic-breakdown/03-expand-epic-files-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-mega-03-expand (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/mega-epic-breakdown/03-expand-epic-files-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-mega-03-expand --phases 4 \
...
- `python3 scripts/command_run.py done --command fab-mega-03-expand --evidence "<what proves the terminal
...
- `python3 scripts/command_run.py blocked --command fab-mega-03-expand --reason "<what · searched · missing>"`
