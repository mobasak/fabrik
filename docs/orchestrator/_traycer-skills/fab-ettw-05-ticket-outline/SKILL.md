---
name: fab-ettw-05-ticket-outline
description: "Epic-to-ticket step 05: outline the ticket set and its dependency graph for parallel dispatch. Use after the deploy plan, or when the user types /fab-ettw-05-ticket-outline."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/05-ticket-outline-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-05-ticket-outline (thin wrapper → source-of-truth in /opt/fabrik)

Read the command specification at:

`/opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/05-ticket-outline-fabrik.md`
...
```bash
python3 scripts/command_run.py start --command fab-ettw-05-ticket-outline --phases 8 \
...
- `python3 scripts/command_run.py done --command fab-ettw-05-ticket-outline --evidence "<what proves the terminal
  condition was met>"` — the evidence is the point; "finished" is not evidence.
- `python3 scripts/command_run.py blocked --command fab-ettw-05-ticket-outline --reason "<what · searched · missing>"`
