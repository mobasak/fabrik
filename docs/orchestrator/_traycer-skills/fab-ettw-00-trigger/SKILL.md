---
name: fab-ettw-00-trigger
description: "Epic-to-ticket step 00: orient on the project, verify constraints, improve owner research, and route to the right workflow commands. Use when starting execution of a single epic, or when the user types /fab-ettw-00-trigger."
---

<!-- GENERATED by /opt/fabrik/commands/assemble_commands.py — DO NOT HAND-EDIT; edit _sources/ + the NEXT map and re-render -->
<!-- source-of-truth for THIS wrapper: /opt/fabrik/docs/orchestrator/epic-to-ticket-workflow/00-trigger-fabrik.md + the ORCH_SOURCES table in assemble_commands.py (NOT _sources/ — the banner above is the generic generator stamp; orchestrator wrappers render from the canonical doc named below) -->

# fab-ettw-00-trigger (thin wrapper → source-of-truth in /opt/fabrik)
...
- `python3 scripts/command_run.py done --command fab-ettw-00-trigger --evidence "<what proves the terminal
  ...
  silence is not.
- `python3 scripts/command_run.py blocked --command fab-ettw-00-trigger --reason "<what · searched · missing>"
