...
- Findings handled **autonomously**: surgical fixups dispatched (pool `pick_models("docs"/"spec"/"code")` or `claude -p`), re-reviewed, **looping to the lens-adjudicated exit (min-2 rounds, no ceiling)**; a rippling change routes to `09`; a >50%-rework batch routes to `06`; only the 3 BLOCKED cases pause (Telegram).
