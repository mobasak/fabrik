...
- **New / amended tickets** → dispatch them through **`07-execute-fabrik`** `[canonical: 07-execute-fabrik § Step 2 — coder dispatch by Complexity]`: each ticket to the coder tier its `Complexity` assigned `[canonical: 06-ticket-breakdown-fabrik § Step 9 — the coder tiers]` — `simple` → the cheapest OpenRouter pool `pick_models("code")` coder via `fanout` (flywheel-ranked, records the flywheel); `complex` → a mid pool coder OR **`claude -p sonnet`**; `critical` → **`claude -p opus`** in an isolated git worktree (auth/schema/migrations/concurrency/secrets) — and converge each ticket's diff to `/fabrik-review`'s **coverage-adjudicated exit** via `07`'s per-ticket review loop (pool `fanout("review")` recording the flywheel **AND** ≥1 native `fabrik-reviewer` on Opus), all through the **`libs/subagents` module**.
...
Then hand off: the next step is **`10-cross-artifact-validation-fabrik`** (`09`'s paired review — the spec-vs-spec audit across the revised artifacts), then the **deploy-out human gate** → `11-deploy-fabrik`. `09` never runs `fabrik apply`.
...
- **Deploy** — that is `11-deploy-fabrik` (the deploy-out gate). `09` stops after the revised tickets validate.
...
- **Re-execution autonomous** (Step 9): new/amended tickets driven through `07-execute-fabrik` (pool + `claude -p` coders via `libs/subagents`, per-ticket converge to `/fabrik-review`'s coverage-adjudicated exit), Done-but-affected amendments re-validated through `08-implementation-validation-fabrik`, **looping until every revised ticket is Done** — halting only on the 3 BLOCKED cases.
- Handoff to `10-cross-artifact-validation-fabrik` (`09`'s paired review), then the deploy-out gate → `11-deploy-fabrik`. Never runs `fabrik apply`.

---

**Next (CC1 pairing, north star § Command-chain build plan):** `09`'s paired review is **`10-cross-artifact-validation-fabrik`** `[canonical: north star § Command-chain build plan — CC5, "10 is the cross-cutting integration review + 09-revise's review"]` — the spec-vs-spec audit that runs the revised artifacts to its lens-adjudicated exit. Then the **deploy-out human gate** → `11-deploy-fabrik`. New/amended tickets re-enter via `07-execute-fabrik`; Done-but-affected amendments re-validate via `08-implementation-validation-fabrik`.
