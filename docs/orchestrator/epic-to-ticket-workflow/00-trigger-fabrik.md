...
2. **Agentic Implementation (WSL)** — structured tickets dispatched to agents (Claude Code — Max OAuth — plus the OpenRouter subagent pool; Windsurf Cascade and Kilo CLI are RETIRED). Agents write infra-aware code against the spec contract. `fabrik dev` for local iteration. `fabrik review` for pre-PR bundling.
