...
**Next (CC1 pairing, north star § Command-chain build plan):** `07`'s paired review is **`08-implementation-validation-fabrik`** `[canonical: north star § Command-chain build plan — CC5, "08 is 07-execute's review"]` — the epic-level code-vs-spec pass that forces the implemented epic to a no-op (the Step-3 per-ticket review is execution's own converge; `08` is the authoritative cross-ticket pass that catches regressions). Then `10-cross-artifact-validation-fabrik` (the integration review), and the **deploy-out human gate** → `11-deploy-command`. *(Downstream ettw twins are built incrementally; refs point to the live Traycer `-command` source and flip to `-fabrik` as each twin lands.)*
