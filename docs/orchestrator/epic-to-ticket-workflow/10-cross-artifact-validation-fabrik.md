<!-- ⚠️ FABRIK FACTORY WORKFLOW — CROSS-ARTIFACT VALIDATION (our own, tool-capable twin of
     ...
     build plan — CC5): the SPEC-vs-SPEC pass that validates the seams BETWEEN artifacts (Brief ↔ Flows ↔
     ...
     from the inline decision, or `(deeper, optional: …)` you may skip):
       ...
       · the INFRA-CHECK — Path A (`00-trigger-fabrik` output / Epic Brief Metadata) OR the dispatched epic
...
Read in order (the Reads budget): **Epic Brief** (Success Criteria, Out of Scope, Metadata) · **Core Flows** (when present — `[PRIMARY PATH]`, Flow Index, i18n decisions, Microcopy Hot-Spots) · **Tech Plan** (when present — Architecture, Data Model, Component Architecture, Shape Block, resilience table, Stack) · **Deploy Plan** (when present — may be SKIPPED entirely for a code-only Retrofit `[canonical: 04-deploy-plan-fabrik § Epic Flavor: Retrofit skip rule]`) · **Ticket Outline** (when present — the plan→execute run) · **Ticket Breakdown** (when present — + `[PRIMARY PATH]` Index) · **INFRA-CHECK** · `docs/operations/fabrik-lifecycle.md` · `docs/LESSONS_LEARNT.md`.

**INFRA-CHECK field set** `[canonical: 01-epic-brief-fabrik § Step 1 — Consume Trigger Context]`: **Path A = 13** (10 required — `Port`, `target_vps`, `Scaffold`, `User Guide`, `Shape`, `Concurrency`, `i18n`, `Responsive`, `Dark+Light`, `Rule Packs` — + 3 SaaS-conditional, `N/A` allowed — `Abuse Detection`, `Email`, `FINANCIALS`). **Path B = 16** (the 15-field Metadata block = the 13 Path-A fields + `Registrars` + `Universal categories`, **plus `Epic Flavor`**). ⚠️ `Internal APIs` is **informational, NOT a propagated field** — do not validate it as one. Consume the fields from Epic Brief Metadata **verbatim**; never re-derive them (a missing Path-B route → `00-trigger-fabrik`).
...
1. **Conceptual consistency** — terminology drift (same concept, different names); contradictory characterizations (Brief says admin-only, a flow shows a regular user); persona drift.
...
- **Needs full top-down propagation** (a change that ripples Brief → … → tickets) → **route to `09-revise-requirements-fabrik`**; `10` finds the contradiction, `09` propagates the fix. Do NOT run the propagation cascade here.
...
- **Validate rule-pack CONTENT** — `10` verifies the `Rule Packs` field *propagates* (Brief → Tech Plan → ticket Context Files); the pack semantics are the producer commands' job.
- **Re-derive INFRA-CHECK fields** — consume from Epic Brief Metadata verbatim; a missing Path-B route (Registrars / Universal categories / Epic Flavor) routes back to `00-trigger-fabrik`.
...
- **Flag a missing Deploy Plan / Epic Closure / a <5-SC Brief for a Retrofit** where `04`/`06` correctly skipped or the Retrofit default applies (per the Step-2 Retrofit adjustments); a Retrofit's 1:1 parallelism is not a budget failure.
...
- All present artifact surfaces internalized (Brief, Flows, Tech Plan, Deploy Plan, Outline, Tickets, INFRA-CHECK, lifecycle, `LESSONS_LEARNT.md`) — the Outline/Tickets only in the plan→execute run (an early spec-only run has none yet).
