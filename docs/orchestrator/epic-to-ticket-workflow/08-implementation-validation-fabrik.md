...
- Findings handled **autonomously**: scoped fixup tickets dispatched to coders (pool `pick_models("code")` or `claude -p`), re-reviewed, **looping to the lens-adjudicated exit (min-2 rounds, no ceiling)**; Product Misalignment routes to `09`; only the 3 BLOCKED cases pause (Telegram).
