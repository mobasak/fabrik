<!-- ⚠️ ORCHESTRATOR `-fabrik` COMMAND-CHAIN QA
     ...
     `docs/orchestrator/mega-epic-breakdown/` — the DOER chain an orchestrating
     agent runs directly:
     ...
     an orchestrating agent reading `epic_order.py --json`'s phased order and each
     epic's `owner:` field, and handing the epic to its named agent.
...
Every command in mega-epic-breakdown (00, 02, 03, 04 — `05` retired) must be evaluated against this list before it's considered complete. This checklist is for DECOMPOSITION quality — splitting a large vision into independent epics.
...
14. Is each epic SELF-CONTAINED enough to seed `/fabrik-spec`'s epic-file intake independently (scope, success criteria, constraints, the 15-field Metadata — no need to re-read the full vision)?
...
## Handoff to `/fabrik-spec`

30. Does each epic output file contain enough context for `/fabrik-spec`'s epic-file intake to START? (scope, success criteria, constraints, metadata)
...
34. Can `/fabrik-spec` run its epic-file intake with ONLY the epic output file + the infrastructure decisions file? (no need to re-read the full vision research)
...
48. Can one human + N named AI agents execute epics in the SAME PHASE CONCURRENTLY, one epic per agent (`owner:` in the frontmatter, filled deterministically by `epic_order.py --assign` — round-robin, no judgment)? (don't assume a single human orchestrates one epic at a time)
...
80. **Validation Report** (**04 output** — 05 retired): Feature Coverage, Epic Tickets (per-epic PASS/FAIL), Dependency Graph, Infrastructure Decisions (14-section check), Handoff Readiness (15-field Metadata check), **Ticket-set integrity** (the `scripts/epic_order.py --check --expected-count` gate absorbed from retired 05 — count-match, epic-number contiguity, duplicate/orphan detection, parallel-set disjointness + single-migration-owner proof), Overall result, and the **code-generated phased Execution Order** (`scripts/epic_order.py --json`, NOT hand-written). There is no downstream dispatch command: dispatch is an orchestrating agent reading the report's execution order (`epic_order.py --json`) and each epic's `owner:` field directly — no cockpit or driver was ever built to do this instead.

## Handoff Format (epic file → `/fabrik-spec`)

81. Each epic ticket (on-disk file under `docs/development/epics/`) MUST contain a `### Metadata` section with all **15 fields**: Scaffold, Port, target_vps, Shape, Concurrency, i18n, Responsive, Dark+Light, Rule Packs, HAS_USER_GUIDE, Registrars, Universal categories, Abuse Detection, Email, FINANCIALS (last 3 conditional — `N/A` allowed). Match per `03-expand-epic-files-fabrik` Metadata template + `04-cross-epic-validation-fabrik` Step 6 Handoff Readiness check + `/fabrik-spec`'s epic-file intake, which consumes every Metadata field as an intake row (the upstream `mega-epic-breakdown/03` enforces; the downstream `/fabrik-spec` consumes).
82. Each epic output file MUST contain a `## Success Criteria` section with numbered, testable criteria — matching what `/fabrik-spec`'s epic-file intake uses to validate coverage.
...
84. Each epic output file MUST be SELF-SUFFICIENT — `/fabrik-spec`'s epic-file intake reads ONLY this file + infrastructure-decisions.md. No need to load the full vision research.

## Persistence & Typed Frontmatter (added 2026-07-18 — the disk-of-record + code-driven-ordering rework)

This section encodes the north-star decisions D6 (persist on confirm), D7 (epic cards in the GUI immediately — never built, see 84d), D8 (disk = source of truth), D10 (ONE typed data model), and R8/D4 (control flow in code, not prose). Every command in the chain must satisfy its applicable items.

84a. **Typed frontmatter on every epic ticket** (03 output): each `docs/development/epics/YYYY-MM-DD-epic-<n>-<slug>.md` MUST open with the flat frontmatter block from `EPIC-ARTIFACT-SCHEMA.md` (scalars, inline lists preferred; a multi-line block list tolerated only for the three list fields, comments stripped — see EPIC-ARTIFACT-SCHEMA.md § Rules) — `kind` / `title` / `status` / `epic_n` / `depends_on` / `parallel_with` / `owned_paths` / `owner`. It is the ONE data model (D10) serving both consumers: `scripts/epic_order.py` reads `epic_n`/`depends_on`/`parallel_with`/`owned_paths`/`owner` (and WRITES `owner` via `--assign`); validation (04) reads all of it. `depends_on`/`parallel_with`/`owned_paths` are the **machine form** of the ticket's `### Dependencies` prose — they MUST be identical (a mismatch is a defect 04 flags). `owner` starts `""` (03 emits it empty) and is filled deterministically, post-integrity-check, by `epic_order.py --assign` — never authored by 03, never a judgment call.
...
84d. **No mirror — disk is the only copy (D8).** The cockpit/driver the chain would have handed an artifact projection to were never built; `docs/development/epics/*.md` is the sole persisted store, read directly by `scripts/epic_order.py` and `04-cross-epic-validation-fabrik`. There is no projection path and nothing named `$TRAYCER_EPIC_ID` — a command that reads or writes through a mirror is stale and must be corrected to read disk.
...
84f. **05 is retired, not merely unused.** No command may reference a live `05-dispatch` step, emit "dispatch instructions", or defer ticket-set integrity / execution-order to it. Its two jobs are now `epic_order.py` (absorbed into 04); its file lives in `_retired/` as a tombstone. Dispatch is an orchestrating agent reading `epic_order.py`'s phased order and each epic's `owner:` field — no cockpit or driver was ever built to do this instead.
...
93. Command's **document-style output** (free-form prose where the agent decides length, e.g., Vision Summary / Infrastructure Decisions / Compact Epic Proposal) doesn't have a clear token budget. → ADD budget. **Structure-bounded outputs** (an on-disk epic ticket file with a fixed section template, Validation Reports with PASS/FAIL row-per-check) do NOT need a numeric cap — the template structure bounds them and a numeric cap would force harmful truncation. The test: if length is bounded by "how thorough should I be?", needs a budget; if length is bounded by "fill this template once", doesn't.
...
97. **Feature-vs-scaffold drift in GUI mandates** — command applies i18n / Responsive / Dark+Light requirements based on scaffold TYPE (e.g. `saas-skeleton ⇒ mandatory`, `python-api ⇒ N/A`) instead of the GUI surface itself. A python-api with `shape.is_admin_dashboard: true` HAS a GUI and must carry the mandates. → REWRITE the trigger to be feature-based per `00-trigger-mega-epic-fabrik` § Rule-area applicability matrix (the c2ef2ee fix class). This defect was previously hit in 00 (`c2ef2ee`), 03 (`71dad46`), 04 (`5485644`), and ettw/05 (`ff2c427`).
...
These items port the enforcement/research disciplines that make the standalone design commands robust against agent shortcutting, ADAPTED to each command's role — a **producer/doer** (00, 02, 03) that self-audits ONCE before its checkpoint is NOT a **convergence twin** (04) that runs the heavy convergence loop to its lens-adjudicated exit. Do not demand a full md5 freeze-loop of a producer; do not let a producer skip its one fresh-eyes pass either.

103. **Decide-don't-drip Question bar** — does the command carry an explicit two-part bar (ask the user ONLY when the answer *materially changes the artifact* AND *cannot be resolved* from the spec/vision/convention/rule-packs — otherwise decide, apply the convention, note it in one line)? A checkpoint that mechanically asks a fixed list of questions every run (re-confirm a port, a slug, a scaffold pick) over-interrupts the owner; an artifact that hides a genuine two-way decision behind a silent guess is the mirror-image defect. → ADD a `## ⚠️ Question bar` section and reframe any "Questions for owner" list to surface only bar-clearing calls, each presented as a *made* decision + its alternative. (Reference implementations: `02` + `00` (both carry a checkpoint Question bar); `03` is **N/A** — a transform with no owner checkpoint. 2026-07-18.)
