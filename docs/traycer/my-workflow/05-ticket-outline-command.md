...
|---|---|---|---|
| **DB Schema & Migrations** | `shape.needs_database: true` | Schema design, migration files, seed data, indexes, constraints | `25-data-postgres` |
...
| **Internal API Consumption** | INFRA-CHECK `Internal APIs ≠ none` | M2M auth (`X-Internal-Token`), client wrappers with timeout/retry/circuit-breaker | `35-security-auth` + `58-resilience` |
| **External Service Integration** | Any external dep (Supabase, Backblaze, Paddle, Cloudflare, Gotenberg, Browserless, SMTP) | Client setup, credential config, resilience per dep, fallback, vendor balance checks | `58-resilience` |
...
| **GUI / Frontend** | saas-skeleton, static-site, chrome-extension, mobile-app, desktop-app | Components, pages, state management, routing, 5 UI states, Ocoron design system | `60-saas-ui` / `70-chrome-ext` / `80-mobile` |
...
| **Automation & Webhooks** | Services receiving/sending webhooks, n8n integration | Webhook endpoint, signature verification (see `85-payments-billing.md`), timeout/retry (see `58-resilience.md`), n8n workflows configured in n8n UI | `58-resilience` + `85-payments-billing` |
...
- LAST ticket is always "Epic Closure — Tier 3 systemic gate".
