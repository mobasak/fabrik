...
30. **Observability** — **`kind: service` / `worker` only:** the service exposes `/health` (Gatus, testing ALL real deps — not a static 200) **and** `/metrics` (Prometheus), per the mandate. ⚠️ **Exposing `/metrics` obliges the spec**: `shape.exposes_metrics: true` **AND** a `spec.domain` — with either missing, `fabrik apply` silently skips the Prometheus registrar and the service is never scraped. ⚠️ **`saas-skeleton` is *per ticket*, not automatic** (`55-observability.md:28` — that matrix is the declared source of truth). **`kind: static`** (`static-site`, `docusaurus`) and **`kind: wordpress`** have no app process serving these endpoints: **N/A**. `core/55-observability.md`.
...
**Informational:** x86_64, Duplicate, Internal APIs, Design System, Platform Debt, 12-Factor, Vector DB, **target_vps**, **Watchdog**, **LLM Gateway** — ⚠️ recorded on the **INFRA-CHECK line**, which IS the carrier that satisfies #27/#28's MUST-state (`03-tech-plan-command` Step 2 captures every INFRA-CHECK field). They do **not** enter `01`'s Epic-Brief Metadata — it has no slot for them — so do not expect them in the brief.
