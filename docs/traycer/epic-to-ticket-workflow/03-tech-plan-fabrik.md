<!-- ⚠️ FABRIK FACTORY WORKFLOW — TECH PLAN (our own, tool-capable twin of 03-tech-plan-command)
     ...
     TOOL-CAPABLE: it READS the Epic Brief + Core Flows + rule packs + AGENTS.md + fabrik-lib/README from
     ...
     from the inline decision, or `(deeper, optional: …)` you may skip):
       · the Epic Brief (`01-epic-brief-fabrik` output) · the `00-trigger-fabrik` INFRA-CHECK · Core Flows
...
1. **Epic Brief** — Summary, Context & Problem, Success Criteria, Out of Scope, Metadata. Every architectural decision traces to a Success Criterion or a `00-trigger-fabrik` constraint.
...
Missing required upstream → pause and ask. **Defensive case (no core-flows):** for `python-api`/`python-api-gpu`/`node-api`/`file-api`/`file-worker`/`docusaurus`, derive personas + primary paths from the Epic Brief Success Criteria; do not request core-flows retroactively. (`wordpress` is not in this workflow — not scaffoldable `[canonical: scaffold.py:5566]`, routes to `/opt/wpf`.)
...
- Re-read research files — rely on the Epic Brief's Context & Problem.
