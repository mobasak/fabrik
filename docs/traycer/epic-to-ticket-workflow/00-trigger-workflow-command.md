...
- **Observability** — every **`kind: service` / `worker`** exposes `/health` for Gatus and `/metrics` for Prometheus (and must then set `shape.exposes_metrics: true` + a `spec.domain`, or the Prometheus registrar is silently skipped). **`kind: static`** (`static-site`, `docusaurus`) has no server — N/A.
...
30. **Observability** — **`kind: service` / `worker` only:** the service exposes `/health` (Gatus, testing ALL real deps — not a static 200) **and** `/metrics` (Prometheus), per the mandate. ⚠️ **Exposing `/metrics` obliges the spec**: `shape.exposes_metrics: true` **AND** a `spec.domain` — with either missing, `fabrik apply` silently skips the Prometheus registrar and the service is never scraped. **`kind: static`** (`static-site`, `docusaurus`) has no server: **N/A** — do not mandate an endpoint it cannot serve. `core/55-observability.md`.
...
3. Constraint findings (all 31) — #29 (Resilience) and #30 (Observability) are **pass/fail checks** whose carrier is this findings block (they hold no value to propagate, like base constraints #1–#12); #31 carries a value and rides the INFRA-CHECK `target_vps` field.
