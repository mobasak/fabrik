...
- **i18n** — every GUI/user-facing service supports multi-language from day one (en + tr minimum). Translation validated via `scripts/validate_i18n.py` (3-level: structural, back-translation, native-speaker critique). Adding a language = adding a locale file, zero code changes. ⚠️ **The scaffolder only ships `scripts/validate_i18n.py` to `saas-skeleton`, `static-site`, `desktop-app`, `mobile-app`, `docusaurus`** (`I18N_ENABLED_TYPES`, `scaffold.py:186`). A **`python-api` / `node-api` / `file-api` / `chrome-extension`** epic that trips the i18n feature-trigger must therefore carry an explicit step to **vendor the kit** (`templates/i18n-kit/` → `scripts/`), or its Done-When cites a script the project will never have.
...
**Propagated downstream:** Port, Scaffold, User Guide, Shape, Concurrency, i18n, Responsive, Dark+Light, Rule Packs, **Epic Flavor** (drives the Success-Criteria branch in `01`/`05` — the only new field with a real downstream consumer).
**Informational:** x86_64, Duplicate, Internal APIs, Design System, Platform Debt, 12-Factor, Abuse Detection, Email, Vector DB, FINANCIALS, **Watchdog**, **LLM Gateway** (surfaced in the epic; `01`'s Metadata has no field for them — do not claim they are carried forward).
