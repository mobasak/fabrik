...
30. **Observability** — the service exposes `/health` (Gatus, testing ALL real deps — not a static 200) **and** `/metrics` (Prometheus), per the mandate. ⚠️ **Exposing `/metrics` obliges the spec**: `shape.exposes_metrics: true` **AND** a `spec.domain` — with either missing, `fabrik apply` silently skips the Prometheus registrar and the service is never scraped. `core/55-observability.md`.
