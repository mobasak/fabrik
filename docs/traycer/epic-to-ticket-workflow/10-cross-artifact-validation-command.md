...
7. **INFRA-CHECK** — Path A (per `01-epic-brief-command` § Step 5 → Metadata, the authority): Scaffold, Port, **target_vps**, User Guide, Shape, Concurrency, i18n, Responsive, Dark+Light, Rule Packs (**10 required**) + Abuse Detection, Email, FINANCIALS (**3 SaaS-conditional**) = **13 fields**. ⚠️ `Internal APIs` is **informational**, NOT propagated — do not validate it as a propagated field. Path B (multi-epic 15-field block per `ettw/00-trigger-workflow-command` § Entry Points → Multi-epic (consume mode) post-`5a48017` + `1eaf22a`): ALSO Registrars, Universal categories, Epic Flavor (Path B adds exactly 3 — per `01-epic-brief-command` § Step 5 → Metadata).
