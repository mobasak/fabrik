...
### Step 3: Receive + Review Each Result (the driver's in-loop converge)

For each returned ticket, validate through the gate AND a dispatched review — the driver's per-ticket converge step, so no broken ticket merges. (This is execution's own in-loop check; the **authoritative epic-level code-vs-spec review is 07's paired command `08-implementation-validation-command`** `[canonical: north star § Command-chain build plan — CC5, "08 is 07-execute's review"]`, run after the epic converges — it catches cross-ticket regressions a per-ticket pass can't; the cross-artifact pass is `10-cross-artifact-validation-command`.)

- **Gate (mechanical, blocking):** the agent's `final_gate.py --json` returned `status:"success"`? A self-report without gate output is rejected. If not success → fixup (Step 4).
- **Review dispatch (alignment):** run the review layer `[canonical: core/62-using-subagents.md § Dispatch policy]` — **BOTH**: the **pool breadth** via `fanout("review", …)` (`pick_models("review")`, records the flywheel) **AND ≥1 native `fabrik-reviewer` on Opus** (the authoritative pass — the pool never runs `anthropic/*`). Cover: **product lens** (Epic Brief + Core Flows — does the code meet the Success Criteria, does the `[PRIMARY PATH]` integration test exist and pass?) and **technical lens** (Tech Plan — shape block still matches code, resilience patterns applied, `Lessons Learnt:` stated — *silence = block*). You (Opus) refute/merge the findings.
...
- Each result reviewed: **gate** (`status:"success"`, no self-reports) **+ a dispatched review** (pool `pick_models("review")` breadth **AND** ≥1 native `fabrik-reviewer` on Opus) across the product + technical lenses; `Lessons Learnt` silence = block.
