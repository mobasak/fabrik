...
User-facing copy added or changed	Microcopy Hot-Spots from Core Flows referenced; literal copy follows .windsurf/rules/core/ocoron-design-system.md § Verbal Identity (lead with outcomes; specifics over adjectives; reject Forbidden Language list).
...
Logging code added or changed	Uses pre-scaffolded structured logger (src/{package}/logger.py for Python, src/logger.js for Node) per .windsurf/rules/core/55-observability.md; no print(), no console.log() in production code (enforced by scripts/enforcement/check_print_ban.py); correlation IDs included.
...
Governance Checklist (literal checklist; correct rule-pack paths):
 ...
 All logging uses structured logger (no print() / console.log()) per .windsurf/rules/core/55-observability.md.
