...
- **HAS_USER_GUIDE:** inherited from the existing project — a `03-expand-epic-files-command` Metadata field, NOT a Locked Decision (00 emits no such field). A retrofit does not change it: carry the project's current value.
...
**2f. fabrik-lib ladder — RUN IT unless an upstream Verdict table already did:**

⚠️ **Which case you are in depends on which `00` produced your Vision Summary — check first.**

- **Default on THIS (Traycer) path — RUN THE LADDER FOR EVERY CAPABILITY.** `00-trigger-workflow-command` is a GUI planner: it has **no tools, no ⛔BLOCKING N3k gate, and emits NO `## fabrik-lib Verdict` table.** So on the Traycer chain the ladder has **never been run** and inheriting nothing is the norm. **Skipping this step means BUILDING a capability `fabrik-lib` already vendors** — the single most expensive mistake this sub-step exists to prevent. For EACH capability, read `/opt/fabrik-lib/README.md` and stop at the first rung that fits: **vendor → enhance → build**.
- **Exception — a Vision Summary from the tool-capable `00-trigger-fabrik` twin** carries a **`## fabrik-lib Verdict`** table (its N3k-3 ladder produced it). If that table is present: **inherit those rows** — copy each verdict into the owning epic's scope as a *vendor* step, not a *build* step, and **do NOT re-litigate a row.** Run the ladder only for a capability the table does not cover — and say which.
...
| 14 | Retrofit | EXISTING mode only — one per `fix-now` Compliance Report row | Compliance Report from `00-trigger-workflow-command` (authored in Step E3.A–C; Step E5 only emits it) (consumed in 2b above) |
...
- Each per-epic compact entry carries **23 indented fields** under the `Epic [N]: [Name]` heading, in **five** groups: (1) **9 epic-shape fields** — Scope, Features, Scaffold, Depends on, Parallel with, Port, Delivers, Rule Packs, HAS_USER_GUIDE; (2) **6 inheritance-metadata fields** — Shape, Concurrency, i18n, Responsive, Dark+Light, Registrars; (3) **Universal categories** (1 field); (4) **3 conditional fields** — Abuse Detection, Email, FINANCIALS (each carries the project-wide Infrastructure Decisions value or `N/A` per the trigger); (5) **4 cross-epic-contract fields** — **Target host**, **Consumes**, **Produces**, **Owned paths** (the concurrency contract — see the parallel gate at 2c). 03's Metadata block consumes **15** of these (the 6 metadata + Scaffold + Port + **Target host** + Rule Packs + HAS_USER_GUIDE + Universal categories + the 3 conditionals); **Consumes**, **Produces** and **Owned paths** feed `03-expand-epic-files-command` § Dependencies (⚠️ **the arithmetic must close: 15 + 3 + 5 = 23.** Every field has exactly one destination; a field with none is a field that gets silently dropped at the boundary); the remaining 5 (Scope, Features, Depends on, Parallel with, Delivers) become other sections in 03's ticket (Summary, Scope > In, Dependencies, Dependencies, Success Criteria respectively). See `03-expand-epic-files-command` Metadata block and `epic-to-ticket-workflow/01-epic-brief-command` § Step 5 → Metadata.
