...
13. **Compose invariants** — every Fabrik-deployed service declares `container_name: <name>`, `deploy.resources.limits.memory`, `platform: linux/amd64`, no `ports:`, and joins the **`fabrik`** Docker network — ⚠️ but `_validate_compose()` runs **only** on the template + docker-image deploy paths (`deployer_ssh.py:373,481`); **`_deploy_git` never calls it** (`:404`) and per-service network *membership* is never checked, so a **git-sourced** project (the standard path) must self-enforce these (NOT `coolify` — renamed 2026-05-31; `fabrik apply` REJECTS the old name). Enforced fatally by `deployer_ssh._validate_compose()`. See `30-ops.md`.
