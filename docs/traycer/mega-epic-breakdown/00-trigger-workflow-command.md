...
These are **vision-level architectural commitments**. Every epic dispatched from this vision inherits them. **Violations block vision confirmation.** Per-epic verification happens later in `epic-to-ticket-workflow/00-trigger-workflow-command` Step 5 — but ⚠️ its overlay constraints #17–#24 cover only **8 of these mandates** (12-Factor, Concurrency, i18n, Shape, Responsive, Dark+Light, Abuse Detection, Email). Resilience, Observability and Fleet-topology (`target_vps`) are **now covered per-epic by overlay constraints #29–#31** (added because a standalone single-epic project enters at the epic-to-ticket file and never sees this one). The commitment is still made here at vision level; the epic re-checks it.
