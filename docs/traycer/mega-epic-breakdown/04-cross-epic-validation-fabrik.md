<!-- ⚠️ FABRIK ORCHESTRATOR COMMAND — OUR OWN TWIN OF `04-cross-epic-validation-command.md`
     Unlike the Traycer source, our orchestrator READS THIS FILE DIRECTLY — no GUI copy-paste.
     It is TOOL-CAPABLE: it reads epic tickets FROM DISK, runs commands, and fetches live sources.
     Keep it in lockstep with the Traycer twin; the ONLY intended differences are
     (a) the orchestrator framing, (b) the tool-capable inheritance from the -fabrik chain,
     and (c) the persistence model — we have NO Traycer store; epic tickets are FILES under
     docs/development/epics/ (written by `03-expand-epic-files-fabrik`), read with the Read/Grep tools.
     -->
...
You are a quality auditor who reads all epic-ticket FILES from disk (`docs/development/epics/`) and the specs from the conversation, and verifies the decomposition is complete, consistent, and ready for execution.

## Goal

Confirm that the mega-epic decomposition is ready for execution — every feature covered, no gaps, no overlaps, no broken dependencies, each epic ticket self-sufficient for `epic-to-ticket-workflow`. After this command, the owner can start dispatching epic tickets via `05-dispatch-epic-tickets-fabrik`.

## Core Philosophy

- **Read the ticket FILES from disk — do not rely on conversation memory.** ⚠️ We have **no** Traycer store and **no** `read_spec`/`read_ticket` tools — those are Traycer's. `03-expand-epic-files-fabrik` wrote each epic as a **file** under `docs/development/epics/YYYY-MM-DD-epic-<n>-<slug>.md`; read every one with the Read/Grep tools. The Infrastructure Decisions spec + Dependency Graph come from `02-epic-decomposition-fabrik` — from the conversation, or wherever the orchestrator persisted them on re-entry. Read them **fresh**: validating a ticket against your memory of it is how a stale ticket passes.
- **Validate, don't create.** This command finds problems. It does not fix them. If problems are found, route back to `02-epic-decomposition-fabrik` or `03-expand-epic-files-fabrik` to fix.
...
- Vision Summary spec (from `00-trigger-fabrik`) — conversation / persisted on re-entry
- Infrastructure Decisions spec (from `02-epic-decomposition-fabrik`) — conversation / persisted
- Dependency Graph (from `02-epic-decomposition-fabrik` — embedded in a spec or in conversation context)
- **Epic ticket FILES** (from `03-expand-epic-files-fabrik`) — one per epic, on disk at `docs/development/epics/YYYY-MM-DD-epic-<n>-<slug>.md`. Enumerate them with `ls docs/development/epics/` (or Glob) — the file count IS the epic count.

**Hard stop if:** any spec or ticket missing. State which and route back to the creating command.

**Additionally read:** `docs/operations/fabrik-lifecycle.md` — ⚠️ it covers **only lifecycle stages 3–4** (deploy/runtime behaviour + data safety); it carries **no** stage model. The 4-stage model (scaffold → implement → `fabrik apply` → `fabrik verify`) is asserted by the command chain itself. A **delta-feature** epic must be able to pass all four; a **Retrofit** epic on an already-deployed service creates **no new deploy unit**, so it has no Stage-1/Stage-3 of its own — its Stage-3 equivalent is the gate + the compliance-row flip (per `03-expand-epic-files-fabrik` § Success Criteria). Validate each epic against the stage set its **flavour** actually owns — not a blanket four.

## Processing User Request

### Step 1: Read All Artifacts

Take the Vision Summary + Infrastructure Decisions from the conversation (or wherever the orchestrator persisted them on re-entry).
List the epic-ticket files: `ls docs/development/epics/*.md` (or Glob `docs/development/epics/*.md`).
Read every ticket file, and the two specs, in full. There is no `list_specs`/`list_tickets`/`read_spec`/`read_ticket` here — those are Traycer's tools; our orchestrator reads files and the conversation directly.
...
| Check | PASS | FAIL |
|---|---|---|
| Vision Summary has Feature Inventory | Inventory section present AND at least 1 feature found | Inventory section missing OR empty — Vision Summary corrupted; route back to `00-trigger-fabrik` (do NOT continue 04) |
...
| Check | PASS | FAIL |
|---|---|---|
| Ticket Title format | Matches `Epic N — [Name]` exactly (em-dash with single spaces around it; optional `Retrofit:` prefix in [Name] for retrofit epics emitted by `02-epic-decomposition-fabrik` Step 2b) | Wrong format (en-dash, hyphen, missing space, wrong number) |
| Has `### Summary` | Present | Missing |
| Has `### Scope` with In + Out | Both present | Missing In or Out |
| Has `### Success Criteria` — count by epic flavour | Delta-feature epic: 5–8 items. Retrofit epic (Title `[Name]` starts `Retrofit:`): 3–5 items permitted per `03-expand-epic-files-fabrik` § Success Criteria | Below per-flavour minimum |
| Has deploy/gate-level criterion | Delta-feature: "`fabrik apply` succeeds" or "/health returns 200" found. Retrofit on existing service (no new deploy unit): `scripts/final_gate.py` success + Compliance Report gap moves Partial/Violates → Compliant per `03-expand-epic-files-fabrik` § Success Criteria | No deploy/gate criterion of either flavour |
...
| Has `### Out of Scope` | Present — names other epics, vision-level exclusions, OR explicit `- none — single-epic proposal` / `- none — no overlap with other epics` per `03-expand-epic-files-fabrik` § Scope → Out + § Out of Scope (Epic Level) | Missing, vague, OR fabricates `handled by Epic [N]` with non-existent Epic N |
...
**Graph form (checklist item 88):** `02-epic-decomposition-fabrik` emits the graph as a **mermaid diagram with `subgraph "Phase N"` blocks**. If it is prose instead, the owner cannot see the shape of their own decomposition — route back to `02`. **Terminology is `Phase`, never `Batch`** (consistent across 02 + 04 + 05; anti-pattern 101).
...
| **CRITICAL PATH stated** | `02-epic-decomposition-fabrik` sub-step **2d** emits `Critical path: Epic 1 → Epic 3 → Epic 5 (3 deep)` — the longest sequential chain. Confirm it is present **and matches the graph** you just validated | Missing, or contradicts the dependency graph. Route back to `02` sub-step 2d. ⚠️ Without it nobody knows what actually gates delivery — every other optimisation is guesswork |
...
| **Deferred Compliance appendix present** (EXISTING mode only) | If the Vision Summary's Compliance Report has any `fix-later` or `accept-as-legacy` row, a **"Deferred Compliance" appendix** exists listing every one of them (per `02-epic-decomposition-fabrik` Step 2b — those rows emit **no epic**, so the appendix is their *only* carrier). N/A in NEW mode, or when every row is `fix-now` | A `fix-later` / `accept-as-legacy` row exists in the Compliance Report but appears in **no** epic and **no** appendix → the owner's deliberate deferral has been silently dropped. Route back to `02-epic-decomposition-fabrik` Step 2b |
...
**ALL PASS:** "Validation complete. All checks passed. Proceed to `05-dispatch-epic-tickets-fabrik` to dispatch epic tickets in this order: [execution order]."

**ANY FAIL:** "Validation found [N] issues. Fix required before proceeding." List each failure with the specific fix needed. Route:

- **Scope/boundary issues** (orphans, duplicates, phantoms, wrong epic split) → "Run `02-epic-decomposition-fabrik` to fix boundaries, then `03-expand-epic-files-fabrik` to recreate tickets."
- **Missing ticket sections or thin metadata** (Step 3 / Step 6 failures) → "Run `03-expand-epic-files-fabrik` to recreate the affected ticket(s)."
- **Infrastructure Decisions missing section** (Step 5 failure) → "Run `02-epic-decomposition-fabrik` Step 3 to add the missing § section to the Infrastructure Decisions spec — do NOT recreate tickets unless they reference the missing decision."
- **Universal Coverage gap** (Step 5 missing a section that maps to a 2h category) → "Run `02-epic-decomposition-fabrik` sub-step 2h to re-audit the 14 universal categories AND Step 3 to add the absorbing § sub-section."
- **Vision Summary corruption** (Step 2 inventory missing/empty) → "Route back to `00-trigger-fabrik`; the upstream Vision Summary is broken — do NOT proceed with 02/03/04 until 00 produces a confirmed Vision Summary."
- **Title format violation** (Step 3 first row) → "Run `03-expand-epic-files-fabrik` to fix titles to the `Epic N — [Name]` format with em-dash."
...
- Does NOT fix problems — only finds them. Fixes happen in `02-epic-decomposition-fabrik` or `03-expand-epic-files-fabrik`.
- Does NOT create or modify specs or ticket files — read-only. It reads `docs/development/epics/*.md`; it never writes there (that is `03-expand-epic-files-fabrik`'s job). ⚠️ **Tool-capable advantage:** unlike the Traycer twin, this command CAN open `PORTS.md`, the rule packs, `spec_loader.py`, etc. — so every PASS/FAIL is grounded in the real repo, not in the ticket's own say-so. A ticket that *claims* `needs_database: true` is checked against whether it *lists* the postgres registrar; a cited pack is confirmed to exist. Verify, don't take the ticket's word.
- Does NOT re-derive the vision or epic boundaries — validates what exists.
- Does NOT dispatch tickets — the owner does that via `05-dispatch-epic-tickets-fabrik` after validation passes.

## Acceptance Criteria

- All epic tickets read as FILES from `docs/development/epics/` (Read/Glob); specs from the conversation — not from a Traycer store, and not from memory.
...
- ALL PASS → route to `05-dispatch-epic-tickets-fabrik` (dispatch) with execution order.
- ANY FAIL → route back to `02-epic-decomposition-fabrik` or `03-expand-epic-files-fabrik` with specific fixes.
