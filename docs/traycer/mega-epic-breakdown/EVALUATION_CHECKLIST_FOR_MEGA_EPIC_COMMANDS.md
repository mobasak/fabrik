...
77. **Compact Epic Proposal** (02 output): per `02-epic-decomposition-command` L406, 19 indented fields per epic in four groups — 9 epic-shape fields (Scope, Features, Scaffold, Depends on, Parallel with, Port, Delivers, Rule Packs, HAS_USER_GUIDE) + 6 inheritance-metadata fields (Shape, Concurrency, i18n, Responsive, Dark+Light, Registrars) + 1 Universal categories field + 3 conditional fields (Abuse Detection, Email, FINANCIALS).
...
81. Each epic ticket (Traycer-stored, not on-disk file) MUST contain a `### Metadata` section with all **14 fields**: Scaffold, Port, Shape, Concurrency, i18n, Responsive, Dark+Light, Rule Packs, HAS_USER_GUIDE, Registrars, Universal categories, Abuse Detection, Email, FINANCIALS (last 3 conditional — `N/A` allowed). Match per `03-expand-epic-files-command` Metadata template + `04-cross-epic-validation-command` Step 6 Handoff Readiness check + `epic-to-ticket-workflow/00-trigger-workflow-command` L77 consume-mode field list (the upstream `mega-epic-breakdown/03` enforces; the downstream `epic-to-ticket-workflow/01` Path B consumes).
