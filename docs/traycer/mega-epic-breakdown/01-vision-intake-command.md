...
- But research CAN BE WRONG. External AI sessions may suggest expensive, complex, or incompatible approaches. Challenge every technology choice in the research against the owner's decision criteria:
  ...
  4. **Easy to maintain** — simple over clever. Fewer moving parts = fewer things that break. But don't under-engineer — if the workload genuinely needs a dedicated tool (vector DB for millions of embeddings, message queue for high-throughput async), use the right tool. The rule is: start with what exists (pgvector, PostgreSQL job queue), escalate to dedicated tooling only when the existing solution hits a proven limit.
