...
2. **INFRA-CHECK** — Path A (single-epic): capture `Shape`, `Concurrency`, `Port`, **`target_vps`**, `Scaffold`, `Rule Packs`, `User Guide`, `i18n`, `Responsive`, `Dark+Light`, `Abuse Detection`, `Email`, `FINANCIALS` (the 10 required + 3 SaaS-conditional fields propagated by `01-epic-brief-command` Path A; `Internal APIs` is **informational** in 01, not propagated). ⚠️ **`target_vps` decides the deploy host AND the DB/cache host** — a spoke (`vps2`/`vps3`) reaches shared infra at `10.99.0.1`, never by Docker DNS. Path B (multi-epic 15-field block per `00-trigger-workflow-command` § Entry Points → Multi-epic (consume mode) post-`5a48017`+`1eaf22a`): ALSO capture `Registrars`, `Universal categories`, `Epic Flavor` (Delta-feature | Retrofit).
