...
Fabrik automatically reviews code after every edit and will run documentation updates for changes that the PostToolUse hook flags as needing docs coverage (API/CLI/config surface changes), whether edits come from Cascade, Kilo CLI, or any Factory-enabled tool.

Manual reviews and documentation updates can also be triggered using the `kilo_code_review.py` script.
