...
4. **Log row + mail** — the ISO-week row is upserted into both role logs (the write is
   ...
   path and lives at the fixed, unreaped `~/.claude/state/kaizen-log-locks/` (independent of
   `KAIZEN_STATE_DIR`; tempdir siting sat under systemd-tmpfiles' 30-day reaper), so a debug
   run and the cron meet on the same flock and no tracked dir grows a `.lock`; tmp+atomic-replace with the log's own mode; transient `tmp*.tmp`
...
| `scripts/command_feedback_report.py` | the close-out ledger reader — per-command backlog, `--queue`, `--observer-rank`, tokens/round |
| `scripts/enforcement/check_corpus_weight.py` | the corpus-weight byte ratchet over the six governance surfaces the hub OWNS (gate-advisory) |
| `commands/_sources/fabrik-command-improve.md` | the ACT half — one command, one edit, a commit trailer naming the rows it answers |
...
of the first 162 ledger rows carried a `change:` verdict that nothing acted on. Four pieces close
that gap — the operator's ask was *"make our commands better and better after each usage, not
weekly"*, and "after each usage" means **whenever a command's queue is non-empty**, never a cron.

| Piece | What it does | Where |
|---|---|---|
| 1 OBSERVE | every close's `change:` value is **axis-keyed** — `change: <axis>: <the one edit>` over `lean · fast · accurate · waste · infra · rules · manifesto`. The eighth axis, continuous improvement, has no per-run key: it is read ACROSS runs, which is why seven keys serve eight axes | `commands/_fragments/close-feedback.md`, which renders into every installed command |
| 2 ACT | `/fabrik-command-improve <command>` reads ONE command's queue and ships ONE edit, committed with `Agent-Context: command-improve <command> · rows <ts,…> · expects <series> <direction>` | `commands/_sources/fabrik-command-improve.md` |
| 3 KEEP LEAN | a byte ratchet over the six governance surfaces the hub owns, so an edit that adds a paragraph is expected to retire one | `scripts/enforcement/check_corpus_weight.py` |
| 4 PROVE | tokens/round per command, behind a mass rule that refuses to print a figure describing a minority of the rows | `scripts/command_feedback_report.py` |

The reader is `command_feedback_report.py`: `--queue <command>` prints that command's queue
(TAB-separated `<ts>` · bucket · verdict, newest first — `ts` is the only per-row handle, since the
ledger has no id and `sid` is per session), and `--observer-rank` names the commands whose closes
are expensive enough that a SEAT should write the verdict rather than the agent grading its own
run. The bucket is the axis read: one of the seven, or `unkeyed` · `bad-axis` · `placeholder`.

⚠️ **`check_corpus_weight.py` WRITES.** A bare run, `--seed`, `--reseed` and a bare `--strict` all
...
`change: make it faster` (no key) AND `change: banana: …` at rc 0. Of the 180 rows present on
2026-09-15, `_axis_of` buckets 176 `unkeyed` — but 5 of those are legitimate `change: none` verdicts
that carry no key BY CONTRACT, so the number that matters is **171 rows carrying a real verdict with
no axis**, against 4 keyed. ⚠️ A DATED snapshot of a growing file: re-derive it, never quote it
forward. The first cut of this sentence said "175 of 180 unkeyed", which is neither figure — it was
the non-`none` population — and the hand-carried number reached six files before a seat re-derived
it; `/fabrik-command-improve` had never run once (0 ledger rows, 0 commits carrying its
...
| nothing told anyone the queue existed | every close prints a line OPENING `QUEUE: /<command> has N unanswered verdict(s) of M filed — answer them with ` and closing with the two commands (the improve command, then the `--queue` read) — see `command_run.py` for the exact f-string rather than trusting a quote here. Advisory, one line, fail-soft: `_queue_depth` returns None on any unreadable input rather than raising on the close path |
...
```
30 6 * * 1 python3 /opt/fabrik/scripts/command_feedback_report.py --since 7 >> $HOME/.claude/command-feedback-report.log 2>&1
```
