...
| maestro | mobile/web UI test flows — **heaviest server on the box** | mobile-app | **724 MB / 4p** | ON mobile-app only. ⚠️ mcp_health false-DEAD: JVM answers slowly + exits on stdin close; Claude's own handshake connects it — confirm cold-start cases via `claude mcp list` |
