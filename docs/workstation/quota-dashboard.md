...
| `model weeklies: …` sub-line | per-account model-/feature-specific weekly windows the usage API returns beyond the general `seven_day` — shown ONLY when a window carries usage (>0%). Known keys are friendly-labeled (`seven_day_opus` → "Opus (wk)", `seven_day_sonnet` → "Sonnet (wk)"); an unknown-but-populated window (an Anthropic codename such as `nimbus_quill`, or a future **Fable-5** window) renders by its RAW key — so a model's window **self-identifies**: use that model, reload, and the key that lights up is its window. Absent today because only `five_hour`/`seven_day`/`nimbus_quill`(0%) are populated fleet-wide (2026-08-22) |
