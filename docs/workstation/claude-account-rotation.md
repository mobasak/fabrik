...
- **No headroom anywhere:** nothing flips; ONE advisory per wall episode goes to Telegram AND
  ...
  `walled` for the wall itself, `urgent-90` for the 90-line warning, which fires EIGHT points
  earlier on the default `ROTATE_THRESHOLD` of 98 — fewer when a `caps.json` cap binds first —
  because that runway is what a graceful stop needs. Line 1 stays the resume promise
