...
| Stop | `final_gate_stop.py` | Definition-of-done enforcer, SIX blocking causes: gate red on session-authored files (path-token attribution) · session's own work uncommitted · committed-but-UNPUSHED (branch ahead of upstream; the task-end push law — indeterminate/no-upstream never blocks) · checkpoint-stall (promises, plan-answered permission questions, passive obligations, assertive continuation claims, numbered `NEXT: round/pass N` footers naming undispatched own-loop work) · a command RUN RECORD still `running` (close it by name via `command_run.py done/blocked`) · UNREVIEWED SPONTANEOUS WORK (added 2026-08-29, made PER-CHANGE 2026-09-06: code files authored AFTER the last CLOSED command's record — a closed command covers the edits before its close and nothing after; a running record is the fifth cause's business. The per-session "ever opened a record" exemption let one 01:45 `/fabrik-review-scoped` license ten unreviewed plain-chat commits across a day — the hook's own author, measured. Remedy `/fabrik-review-scoped`. **Review 2026-09-06 (pass 1 A-F2..F8, pass 2 P1-1..P1-7, closing C-1..C-7/E8):** the coverage is EVERY window a command of the session closed — the record's `covered` ledger (`command_run.py` appends `[started_epoch, close]` at each agent close, `start` carries it across its overwrite and seeds a pre-ledger closed record's own window, a nested run's windows join its caller's, deduped) plus the live record's `[started_epoch, ∞)`; edits before the SessionStart baseline are a resumed transcript's and never count, and the floor is the LATEST of THREE bounds (`_sixth_cause_floor`): that baseline, an EDIT-AGE window (`_SIXTH_CAUSE_MAX_EDIT_AGE_S`, 24 h, added 2026-09-15 by T5.4), and the ledger's own birth (`_LEDGER_EPOCH` = ff887758, 2026-09-06 19:56 +0300). ⚠️ On this box the AGE bound is the one that usually binds — 87 of 97 SessionStart baselines are older than it — because the reported shape (01M25Y93RB) is a RESUMED transcript, which HAS a baseline and is merely old: the first cut bounded only the MISSING-baseline case and so left the reported case untouched, blocking on a file that was committed and reviewed to `done`. The cost is stated where the constant lives: work older than the window drops out of this cause entirely, which is the fail-open direction it takes by charter — edits older than the ledger were adjudicated by the per-session rule of their day and no ledger holds their closes, so re-judging them only re-blocked (16 reviewed edits, three attempts per turn, 2026-09-07; transitional: no session started after that instant carries an older edit); closed states = `command_run.py::AGENT_CLOSED_STATES` = {done, blocked, handoff} (the coroner's `died`/`expired` cover NOTHING — a reaped run had no review); a `started_epoch` ≤ 0 covers nothing; a `running` record covers only while the fifth cause still acts on it, and the operator's `COMMAND_RUN_STALE_H<=0` hatch disarms this cause too; NaN/Infinity/bool timestamps read as no record; warn-through re-arms; `step/round` refuse a closed record). **Phase C of the mail-triage plan, 2026-09-14 (T5.1–T5.3, all three EXECUTED before they were written):** the windows now include every frame PARKED on the live record's `stack` — a nested `start` parks the running parent there and gives the child an EMPTY `covered` ledger on purpose, so for the whole life of a nested `/fabrik-review` both the parent's live window and every window the session had already closed were invisible and work authored minutes earlier under the parent's own contract read as unreviewed (measured 1 vs 0 with the same parent live; `stack` is flat, so no recursion is owed, and a frame's RUNNING window rides the live record's freshness while both ledgers of closed windows survive staleness) · a RUNNING review-family record EXEMPTS BY NAME the files its `--surface` spells (`_surface_reviewed`, `_REVIEW_FAMILY` bound to `command_run.py::REVIEW_FAMILY` by a parity grader like `_CLOSED_STATES`): the sixth cause fired on exactly the files the running review was at that moment reviewing, and the reach-back cannot move earlier because a `blocked` or `handoff` close must never certify the gap. Equality on a whole token only — `scripts/` exempts nothing under it; a colon is not a path character in the splitter, so `tests/b.py:44` and `a.py::test_x` end their token at it for free (an explicit strip for those suffixes was UNREACHABLE and was deleted in round 1 of this change's own review), while a leading `./` IS stripped because both its characters survive the split. A frame's running window counts only while the LIVE record says `running`: `kaizen_coroner.py` writes `died`/`expired` on the top-level record alone, so a reaped session's parked frames would otherwise cover `[started, ∞)` — closing review C-2's 37 h abandoned plan in a nested hat, measured 0 unreviewed against 1 non-nested · a review-family `done` close whose ledger held NO earlier run covers back to the SESSION FLOOR — `command_run.py` records how far its contract reached in a DURABLE `first_review_reach` field (carried across every later `start` exactly as `covered` is, and joined at a nested pop), and `_first_review_base_case` floors that reach with the SessionStart baseline it alone knows. Round 1 of this change's own review found the first cut DERIVED the same thing per stop from the live record, so the coverage evaporated at the session's very next `start` — the permanent block deferred rather than closed; the three "must not launder" guarantees (a `blocked`/`handoff` close certifies nothing, a non-review command records nothing, a ledger with an earlier run stands the field down) therefore live in the WRITER now and are graded there. A corrupt ledger pair also stands it down: an unreadable pair is still evidence of an earlier run. Why it is owed at all: the writer's reach-back has nothing to reach back to with an empty ledger and falls back to the review's own `started_epoch`, so a session interrupted before any command closed was PERMANENTLY unclearable (01M21JAET — 28ca7443 closed only the pre-ledger half and is transitional; this half is not). Narrow by construction: a window ADDED not a pair rewritten, `done` only, review-family only, and it stands down the moment any ledger window starts before the record's own start. It reads the LIVE record only — a nested review pops into its caller before the next stop, so its identity is gone by then; RECORDED, destination `docs/STRATEGIC_BACKLOG.md`. The three composed in one gradeable reader, `_unreviewed_spontaneous`, because the call site sits hundreds of lines inside `main` — the count is deliberately not written here, because round 2 found the absolute line cite that replaced "~40" stale by 34 in the very commit that called it re-derived; the function NAME is the durable address where nothing could grade the wiring. Quote-span/negation/deadline exemptions; `BLOCKED:` exempts globally; human-gate wording is line-scoped AND (since 2026-08-29) a bare deferral phrase — "operator decision", "your call", "say the word" — no longer disarms the stall guard by itself: it must share its line with a HARD-STOP class (cross-repo · gate 1/2 · deploy/publish · spend/quota · irreversible/destructive · policy · rule-conflict citing `path:line`); 3-attempt warn-through per cause, independent counter slots (6). **Yields to the quota hold (2026-09-06, D-158):** while `quota_stop.py`'s `fleet-exhausted` stamp stands the stop is ALLOWED outright — the hold denies the very tools that clear these causes (`final_gate.py` is not in its allow-list), so blocking only forced more assistant turns, and turns burn quota with every tool denied; that deadlock, not a late warning, is how infra ran into its session limit. Emits `stop_allowed_quota_hold`; fail-open. **Review 2026-09-06 (A-F1):** the yield requires the stamp AND a tick log fresher than `QUOTA_STOP_TICK_STALE_S` (900 s) — the same seam `quota_stop.py` fails OPEN on; a stale stamp with a dead cron used to disable all six causes while nothing was held |
...
installer copy and is NOT read by the CLI** — editing it changes nothing for a running session while appearing to work. ⚠️ **But it IS the `--sync-shared` copy SOURCE** (`claude_rotate.py` reads `CLAUDE_DIR/settings.json` as "the canonical" and overwrites every account dir's copy with it; `--from` selects only the ROSTER source). So a user-level hook is registered THERE and then synced — an entry added to an account dir's copy is reverted by the next sync (2026-09-06: three hooks vanished from all five dirs that way, caught by the verify step). The four
...
| UserPromptSubmit (user-level, every window) | `/opt/fabrik/scripts/sysadmin/selfwatch_check.py` | **The mechanical half of the self-watch** (2026-09-06, D-163): is this session's `selfwatch.lock` HELD? The watcher holds a `flock` on it for its whole life (`claude-selfwatch.sh:30-37`), so held IS armed — no registry, no pgrep; since 2026-09-07 the rotation tick carries a VERBATIM vendored copy of this decider (`claude_rotate.py::_sid_is_armed`, lockstep-graded against `_armed` in `tests/test_claude_rotate_v2.py`) to enumerate the armed watches it wakes at a hold lift — the aro-wake twin ships as one file and cannot import this module. Unarmed → prints the arm order with the LITERAL sid on every prompt until armed; armed → silent. **Review 2026-09-06 (B4–B7, B11, B12):** the probe is READ-ONLY — `/proc/locks` by inode first, an `O_RDONLY` flock probe as fallback (taking `LOCK_EX` to ask could make the watcher's own `flock -n` see "already armed"); a read-only lock file no longer crashes into silence and any exception prints one stderr line; an unwritable lock DIR prints the fix-the-dir diagnosis instead of an arm order that would die at once; `CLAUDE_MESH_AUTONOMOUS=1` workers (`claude -p`, no Monitor tool) are silent; the sid transform is byte-wise like the watcher's `tr`. Same gates as the SessionStart ORDER except `source=compact`, which is the point: a compact-resumed session never saw the order. Measured before shipping: 4 of 12 live /opt sessions unarmed. Fail-open. Tests: `tests/test_selfwatch_check.py` |
| UserPromptSubmit + PreToolUse (user-level, every window) | `/opt/fabrik/scripts/sysadmin/user_hook_gate.py <hub hook>` | Runs the hub's `mcp_watch.py` (UserPromptSubmit) and `quota_stop.py` (PreToolUse `.*`) for any window whose PROJECT does not already wire that hook — closes the sync-excluded gap (fabrik-lib had no MCP banner and no quota hold) without editing a synced hook or the exclusion ruling. Detection (review B2/B3): the project ROOT is `CLAUDE_PROJECT_DIR` or the nearest ancestor of cwd holding `.claude/settings.json`; that file (+ `settings.local.json`) is PARSED and the gate defers only on a hook entry under the SAME `hook_event_name` naming this basename — a substring anywhere used to defer, silently stripping a window of its hold; `disableAllHooks` never defers. The wrapped `quota_stop.py` denies via stdout JSON with exit 0 (B1) — forwarded byte for byte; a hang or a missing hook is fail-open with one stderr line (B8/B9), inner timeout 8 s under a 10 s registration; in the 42 synced repos the project copy fires and the gate exits silently (no double banner, no double deny). Exit code and stdout pass through untouched. Fail-open toward COVERAGE (unreadable settings → the hook runs). D-163. Tests: `tests/test_user_hook_gate.py` |
| SessionStart (user-level, every window) | `/opt/fabrik/scripts/scratch_sweep.py --hook` | **The scratch advisory** (2026-09-08, D-184/D-187): one line naming how many stale entries sit in THIS session's scratchpad, the dry-run command and the apply command — and what the sweeper never touches. Fires at SessionStart only: a `UserPromptSubmit` trigger was measured BEFORE it shipped and rejected — 9 of 9 live sessions fired on every prompt, because a working session never becomes clean, and that is wallpaper. A count-recording stamp inside the session's own dir (`<root>/<slug>/<sid>/.scratch-nagged`, never `~/.claude*/state`, which the sweeper's refusal set forbids it to write) holds the count seen at every COMPLETE evaluation — a silent zero included, a budget-truncated walk never, so an agent that complied and swept can be told again — and the line reprints only when that count has GROWN. A budget-truncated count is a lower bound and never reprints (measured: it wobbled and re-fired on the three largest scratchpads). Gates mirror `selfwatch_check.py`: real sid · not headless · not autonomous · an `/opt` cwd. Its freshness walk has a 4 s deadline (raised from 1 s: the largest live scratchpad's holder probe alone costs 2.2 s, so at 1 s the budget was exhausted before the first stat on 3 of the 6 largest pads and the advisory was structurally dead there); `SCRATCH_SWEEP_HOOK_BUDGET` is its stat-cap test seam. Fail-open, exit 0 on every path, one stderr line on an exception. Fire rate is bounded by the stamp, not by a census: the line speaks only when the count EXCEEDS the last one recorded for that sid, the stamp records on every evaluation (silent zero included), and a budget-truncated walk neither speaks nor records. Each of those three is graded — `test_the_hook_line_reprints_only_when_the_candidate_count_grows`, `test_hook_line_fires_only_with_candidates_and_never_blocks`, `test_a_truncated_walk_never_makes_the_hook_line_reprint` — because a live-session census cannot be re-derived later without writing stamps into other sessions' directories, which this row will not ask a reader to do. Tests: `tests/test_scratch_sweep.py` |
| (installer, not a hook) | `/opt/fabrik/scripts/sysadmin/install_user_hooks.py [--check [--warn]]` | Creates / verifies the four user-level registrations above in the canonical `~/.claude/settings.json` and every account dir (review 2026-09-06, B10): they were hand-written and pushed by `--sync-shared`, with no creator, verifier or restorer, and `check_hooks_index.py` derives its required set FROM the file so it cannot see a MISSING entry. `--check --warn` runs as the warn-only `User-Level Hooks Registered` row of `final_gate.py` (a bare `--check` exits 1 on drift, and a warn-only row that exits non-zero still FAILS the gate — closing review R1 / non-author F10) (hub-only — the script is not synced). Tests: `tests/test_install_user_hooks.py` |
