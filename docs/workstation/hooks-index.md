...
| SessionStart (user-level, every window) | `/opt/fabrik/scripts/scratch_sweep.py --hook` | **The scratch advisory** (2026-09-08, D-184/D-187): one line naming how many stale entries sit in THIS session's scratchpad, the dry-run command and the apply command — and what the sweeper never touches. Fires at SessionStart only: a `UserPromptSubmit` trigger was measured BEFORE it shipped and rejected — 9 of 9 live sessions fired on every prompt, because a working session never becomes clean, and that is wallpaper. A count-recording stamp inside the session's own dir (`<root>/<slug>/<sid>/.scratch-nagged`, never `~/.claude*/state`, which the sweeper's refusal set forbids it to write) holds the count seen at EVERY evaluation — a silent zero included, so an agent that complied and swept can be told again — and the line reprints only when that count has GROWN. A budget-truncated count is a lower bound and never reprints (measured: it wobbled and re-fired on the three largest scratchpads). Gates mirror `selfwatch_check.py`: real sid · not headless · not autonomous · an `/opt` cwd. Its freshness walk has a 4 s deadline (raised from 1 s: the largest live scratchpad's holder probe alone costs 2.2 s, so at 1 s the budget was exhausted before the first stat on 3 of the 6 largest pads and the advisory was structurally dead there); `SCRATCH_SWEEP_HOOK_BUDGET` is its stat-cap test seam. Fail-open, exit 0 on every path, one stderr line on an exception. Confirmed fire rate: 9 of 11 live sessions on first sight, 0 of 11 on the next, 1 of 11 when one genuinely grew. Tests: `tests/test_scratch_sweep.py` |
