...
| UserPromptSubmit | `mcp_watch.py` | D-041 per-message MCP forcing: STALENESS banner when .mcp.json/roster postdate the session's transcript (catches resumed-conversation tool universes) + cached `mcp_health` dead-server banner (TTL 15m, detached refresh); fail-open, <100ms |
