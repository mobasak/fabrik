...
| (installer, not a hook) | `/opt/fabrik/scripts/sysadmin/install_user_hooks.py [--check]` | Creates / verifies the three user-level registrations above in the canonical `~/.claude/settings.json` and every account dir (review 2026-09-06, B10): they were hand-written and pushed by `--sync-shared`, with no creator, verifier or restorer, and `check_hooks_index.py` derives its required set FROM the file so it cannot see a MISSING entry. `--check` runs as the warn-only `User-Level Hooks Registered` row of `final_gate.py` (hub-only — the script is not synced). Tests: `tests/test_install_user_hooks.py` |
