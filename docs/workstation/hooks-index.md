...
| PreToolUse | `quota_stop.py` (project, synced) | **Fleet-wide graceful stop** on the rotation tick's `~/.claude/state/fleet-exhausted` stamp — set by `claude_rotate.py::_fleet_active_wall_advisory` ONLY when the ACTIVE account is walled and no successor exists (or rotation is paused), cleared by the tick on relief. Matcher `.*`; while it stands the rule is DEFAULT-DENY — Edit/Write/MultiEdit/NotebookEdit, MCP editors (serena replace/insert/rename, browser clicks), Agent/Workflow dispatch and any Bash that is not ONE simple checkpoint/read command (no `&&`/`;`/`|`/`$( )`/file redirection; `git reset` only in its `HEAD --` realign form; `sed -n` never with `-i`) are DENIED with one instruction (commit + push with explicit pathspecs, close the run record, end the turn); reads, git, `command_run.py`, `mail.py`, `thread_anchor.py` stay allowed so the Stop hook's commit-and-push law can be met. FAIL-OPEN: no stamp / unreadable → allow; tick log older than `QUOTA_STOP_TICK_STALE_S` (900s) → allow + one warning (a dead cron never freezes the fleet). Hooks load at session START: a session already running when `settings.json` synced gets the hook on its next start; the stamp can outlive relief by one tick (≤5 min). Operator rule 2026-09-02; the mail broadcast still goes out, this is what reaches a session MID-TURN. |
