...
**Status:** REVISION 8 (2026-09-22). The program of revisions 4–7 (the old § 5: remove the apparatus) was
...
| `/opt/fabrik-lib/CLAUDE.md` | 115,366 B | **115,366 B** / 758 lines | 0 | GENERATED by their `scripts/refresh-governance.sh`; the hub edits it only by mail — the lean text for its QUOTA bullet is mailed (`01M327TF7ERCBYHRA9448DSD01`) and parked until fabrik-lib applies it |
...
| 1b(i) · the commit recipe → a tested script | 2026-09-21 | `fa463ffcc` (D-328) — committed BY the script | `scripts/private_index_commit.py` + nine red-first graders in `tests/test_private_index_commit.py`; the graders caught the prose's own step 5b missing (working file not brought up to the new base after a CAS retry) — now a three-way merge, proven by mutant; the 11,136-byte recipe is a 1,302-byte pointer; twelve T6 pins retired. File **61,400 B** — step 1 complete: 134,466 → 61,400 (46%), every remaining pinned span kept verbatim |
