...
| 7b · the rollout of chunk 7's two shared fragments to the rest of the corpus (§ 5.3: the four-call close is the edit all 38 commands share) | 2026-09-23 | D-366 | `close-chain` in 37 of 38 sources — every command but `/fabrik-task`, which keeps its own close (§ 5.3) — the assembler filling the command's name and, where the command sets none, a pointer to the check its own close names; `delegated-reads` in the 23 commands that read large sources themselves (the producing, converge, certification and utility commands), never in the review family, whose seats read | measured on the next run of each |
