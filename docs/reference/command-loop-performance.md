...
**Status:** REVISION 25 (2026-09-26) — every chunk in § 5.2's table has landed (1–3, 5, 5b, 6, 6b, 7, 7b; the numbering skips 4, which was never defined); the § 1 goal is not met in any measured chain (§ 6: ~9.3 h, ~18.8 h, ~16.2 h against ≤ 2 h), and the stage-yield reading is the next measurement. The program of revisions 4–7 (the old § 5: remove the apparatus) was
...
**Second measured chain (2026-09-24/25, the work-tracking spec and plan `archived/2026-09-24-plan-2-work-tracking`, EXECUTED 02744deea):** `/fabrik-spec` 54 min · `/fabrik-spec-review` 32 min over 4 passes · `/fabrik-plan-after-chat` 79 min · `/fabrik-plan-review` 64 min over 7 passes · `/fabrik-execute-plan` 988 min wall for eleven tickets, inside which eleven ticket reviews took 2–5 rounds each (14–162 min; T04's record stayed open 437 min while T02, T06 and T03 merged ahead of it in Merge Order), three fix reviews found while adopting, `/fabrik-docs-review` 20 min (6 → 0 → 0) and the D7 whole-plan review 64 min (7 → 1 → 1 → 0). Spec start → EXECUTED: **~18.8 h** wall (a WSL restart inside it) against the ≤ 2 h goal. No loop exceeded 7 passes; as in the first chain the cost sat in execution, and this chain fired **16 review stages inside execution for one change** (11 ticket reviews, 3 fix reviews, the docs review, D7; 18 with the spec and plan reviews) — the unmeasured lever named in § 4.9 finding 35 (b). The next measurement is that lever's yield: confirmed defects per stage across both chains, read from the receipts (`docs/development/reviews/2026-09-2[34]-plan-*`).

**Third measured chain (2026-09-25/26, the single-tracker spec and plan `archived/2026-09-25-plan-1-work-store-single-tracker`, EXECUTED e41fb3b70, D-423)** — the first chain with every chunk of § 5.2 in place, read from the feedback ledger (run records of session `dd3c06d1`): `/fabrik-spec` 81 min, with `/fabrik-spec-review` run inside it (57 min over 5 passes, 27 → 7 → 5 → 2 → 0) · `/fabrik-plan-after-chat` 75 min · `/fabrik-plan-review` 59 min over 3 passes (24 → 4 → 0) · `/fabrik-execute-plan` 811 min wall for ten tickets, inside which eleven review-family runs (ten ticket reviews and the T08 whole-plan review) took 2–5 passes each and 701 min together (19–238 min; every one closed at 0 confirmed). Spec start → EXECUTED: **~16.2 h** wall against the ≤ 2 h goal. Every review closed within 5 passes, so § 5's loop shape held; as in both earlier chains the hours sat in execution — thirteen review stages for one change. What remains unmeasured is still § 4.9 finding 35 (b): which of those stages found defects the others did not.

**Review-stage yield across the three chains (2026-09-26, W-992909ca — § 4.9 finding 35 (b)).** Read from each stage's receipt Pass Ledger (`docs/development/reviews/2026-09-23-plan-1-*`, `2026-09-24-plan-2-*`, `2026-09-25-plan-1-*`) and, for the spec, plan and docs reviews, the run ledger's confirmed series; minutes are the run records' wall time. *First-pass* is a stage's round-1 confirmed count (defects in what it was handed); *residue* is every later round's confirmed count (defects in the review's own fixes). The population is each chain's own stages — its spec, plan, ticket, docs and whole-plan reviews and the fix reviews inside its execution: 42 stage receipts, 138 passes, 2,534 min. Three review runs in the same hours are not a chain's stage and are left out (a kaizen test fix, 9 min; the NEXT-footer escape fix below, 41 min; the `BLOCKED:` format ruling D-391, 27 min), and two stop-and-compaction receipts (T01b, T02a) have no run record of their own (T01b's review ran inside T01a's), so a count of run records reads 43 runs and 2,612 min. Where a receipt and its run record differ, the receipt's adjudicated count is used (work-tracking's whole-plan review: 7 in the receipt, 8 in the record, one finding reclassified as by design after it was counted):

| Stage | Runs | Minutes | First-pass | Residue | First-pass per 10 min |
|---|---:|---:|---:|---:|---:|
| Spec review | 3 | 159 | 95 | 46 | 6.0 |
| Plan review | 3 | 168 | 68 | 29 | 4.0 |
| Ticket reviews | 28 | 1,920 | 322 | 96 | 1.7 |
| Docs review (stop-and-compaction, work-tracking) | 2 | 103 | 16 | 0 | 1.6 |
| Whole-plan review (D7) | 3 | 123 | 15 (7 · 7 · 1) | 3 | 1.2 |
| Fix reviews inside execution (work-tracking) | 3 | 61 | 4 | 0 | 0.7 |
| **All** | **42** | **2,534** | **520** | **174 (25%)** | 2.1 |

Four readings. (1) **Ticket reviews are 76% of all review minutes and 62% of first-pass finds** — the stage that finds most, not the stage to drop; its cost is its COUNT: 28 separate runs, and a run that confirms almost nothing still costs 9–29 min (work-tracking T07 14 min for 1, the fix reviews 9–29 min for 1–2). One work-tracking row (T04, 437 min) is a record left open while three tickets merged ahead of it, not review work. (2) **The whole-plan review's yield falls as the ticket reviews tighten** — 7, 7, then 1 — and the docs review found 16 as a separate stage while the single-tracker chain's doc ticket review found 16 in 31 min with no separate docs stage. (3) **A quarter of every confirmed defect is residue from the review's own fixes** (spec 33%, plan 30%, tickets 23%). (4) **Escapes past every stage:** two fixes on a chain's own code within 14 days of its close — `21a831986` (the Stop hook's NEXT footer in bold, bulleted and quoted forms, 47 minutes after stop-and-compaction closed at `03a94e9a3`) and `3e75fd9ac` (D-425: a NEXT rewrote the named item's `next`, destroying migrated text — designed by the work-tracking spec (its NEXT rule 3, "updates that item's `next` field"), built in its T01b and kept by the single-tracker spec's own D3, so it passed both chains' spec, plan, ticket and whole-plan reviews; the first reader to hit it was another agent using the store).

**Proposed, approved by the operator and applied 2026-09-26 (D-429, `6457ce01e`) — (d) as applied covers a fix the whole-plan validation finds (reviewed in the next or the still-open last run), while a fixup inside a wave's own review round stays in that round as before; (e) as applied also asks who wrote the content a write overwrites:** (a) **review per merge wave, not per ticket** — the tickets a wave merges together go through ONE partitioned review run with one slice per ticket, so the per-run floor is paid per wave, not per ticket (by the Depends columns, the single-tracker plan's 10 build tickets sit on 7 dependency levels and work-tracking's 10 on 5 — T08 and T10 are each plan's integration ticket); (b) **fold the whole-plan review into the last wave's run** as its cross-ticket slice, since it is then the same seats over the same merged tree; (c) **drop the separate docs review when the plan has a doc ticket** — that ticket's review is the docs review (the single-tracker chain already did this); (d) **fold a fix found during execution into the next wave's run** instead of its own review; (e) **add one question to the spec review's hunt list — "every write to an existing record: what does it overwrite?"** — the one class that escaped two chains was a design decision every later stage was built to follow.
...
**Revision 25 (2026-09-26):** the Status line catches up — every chunk in § 5.2's table has landed (the numbering skips 4, never defined) — and § 6 gains the third measured chain (the single-tracker plan, ~16.2 h against the ≤ 2 h goal, thirteen review stages for one change, every review closed within 5 passes). The goal is not met in any of the three chains; the next measurement is each stage's yield (§ 4.9 finding 35 (b), work item W-992909ca).
