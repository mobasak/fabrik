...
| `done --command <name> --evidence "<proof>" [--commit <sha>]` | terminal — the contract IS met. `--commit` is the `fabrik-task` lane's flag on the three close verbs — REQUIRED on `done` there, optional on `blocked`/`handoff` (which may close before any commit exists), REFUSED on every other command; it names THIS run's commit, read from the capture file written in the same shell as the commit, and the close re-measures that commit's diff to write `oversized_mini` |
