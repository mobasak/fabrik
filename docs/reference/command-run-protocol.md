...
| Command | Effect |
|---|---|
| `start --command <name> --phases <N> [--terminal "<cond>"] [--budget <minutes>] [--surface "<s>"] [--file <path>…] [--declare k=v,…]` | begin a run at phase 1 (a running record is pushed onto `stack`). `--file` (repeatable, ONE repo path per occurrence — a path that does not exist yet is accepted, so the grader the change is about to add is declarable) and `--declare` (all five of `decision`/`heavy`/`mechanism`/`oneway`/`tradeoffs`, each `yes` or `no`) belong to `--command fabrik-task` and are REFUSED under any other command; there they are BOTH REQUIRED and the pair IS the lane's SIZE gate — a missing flag, a path outside the repo, a directory, a path already dirty against `HEAD`, more than 3 files, a governance-sync path or a `yes`/`no` that names another lane REFUSES the start with the correction or the lane (rc 1). It persists `declared: {files, sha, sync_test, decision, heavy, mechanism, oneway, tradeoffs}` (`sync_test` is `unavailable` when the hub's governance-sync filter could not be read — the test fails OPEN. ⚠️ The close never reads this key back: it RE-DERIVES readability at close time with a fresh `_sync_filter_source()` call (`command_run.py:3284`) and keys `unmeasurable=sync_test-unavailable` on THAT reading alone — a filter readable at start and unreadable at close once scored a clean `0`, `command_run.py:3347-3353`) and prints `RECORD: <started_at>` for the phase-2 scratch path |
| `step --phase <N> [--title "<t>"] [--design <path>]` | advance. `--design` (`fabrik-task` only) stores that FILE'S TEXT once as the run's `design` — the six DESIGN fields, so they outlive the scratch directory and a `phase_title` the next `step` overwrites; stored WHOLE at any length (no cap — D-314; the design never reaches the ledger row's capped fields); an unreadable path is REFUSED |
...
from both `command_run.py` and `dispatch_headroom.py` (`--delta` is an unknown flag): a later pass is sized by the
slices with open claims, on the seats that owned them in round 1.
...
- **The report:** `python3 scripts/command_feedback_report.py [--since DAYS] [--command NAME]
  [--agent NAME] [--observer-rank] [--queue COMMAND] [--json]` — per command: runs, done/blocked/handoff, median and max wall-clock, median
...
- **The `change:` queue is classified into FOUR buckets, in a fixed precedence** (`_axis_of`):
  `placeholder` (the grammar's own noun phrase, bracketed or not, KEYED or not) → `bad-axis` (a
  leading `word:` outside the seven axes — an attempt that missed) → the named axis → `unkeyed` (no
  `word:` head at all). The order is what makes the four counts a PARTITION: a value can satisfy two
