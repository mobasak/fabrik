...
- **`docs/development/PLANS.md`** — the `AUTO-GENERATED:PLANS` block, `| Epic/Plan | Owner | Status | Phase |`,
  ...
  so. `—` is an untagged row, filled by adoption (below), together with untagged
  `STRATEGIC_BACKLOG.md` rows.
- **Adoption** — `python scripts/docs_updater.py --adopt <name>[,<name>…]`, run ONCE by agent-1 in
  the main checkout (refuses below 2 live sessions unless `--single-window` overrides it; D-154,
  D-155): seeds the PLANS markers, stamps `**Owner:**` on every open unowned plan round-robin, tags
  every untagged `STRATEGIC_BACKLOG.md` row in its own shape (a hub row's `Owner`/`Tag` cell, a
  project row's `Item` cell, or a bullet's text right after its checkbox), appends the ledger's
  `MERGE OWNER: <first name>` row (the LAST row whose `what` cell opens with `MERGE OWNER:` wins —
  read it with `python3 scripts/decisions.py --merge-owner .`; a changed owner is a NEW superseding
  row, never an edit), and delegates the epic half to `epic_order.py --assign` wherever that script
  is present (hub-only, never synced to projects). Once declared, the PLANS block's second header
