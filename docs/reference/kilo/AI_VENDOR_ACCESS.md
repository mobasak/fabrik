...
| ModelScope | qwen, deepseek, z-ai, moonshotai, minimax, mistralai, stepfun, nex-agi, meituan-longcat, shanghai-ai-lab, paddlepaddle, xiaomimimo, tencent-hunyuan, alibaba-iic, llm-research, medaibase, musepublic, opencompass, opengvlab, xgenerationlab | env `MODELSCOPE_API_KEY` | ✅ | credits pool | 55-model Alibaba model-hub gateway on `https://api-inference.modelscope.cn/v1` (OpenAI-compatible). Notable: ZhipuAI direct (GLM 5.2/5.1/5/4.7-Flash), Shanghai AI Lab Intern-S series (InternLM3 successors — Intern-S1 / S1-mini / S2-Preview), PaddlePaddle ERNIE-4.5 (0.3B / 21B-A3B / 300B-A47B / VL-28B), Xiaomi MiMo-V2-Flash, Tencent Hunyuan Hy3, XiYanSQL SQL coder, plus Qwen (20 models) / DeepSeek V3.2-V4 / MiniMax M2.5-M3 / Kimi K2.5 overlap. Format: `ms-*`. Full list: `python scripts/kilo-benchmarks/scrape_modelscope_catalog.py` (also flips `via_modelscope=1` on affected agents rows). |
