Last refresh: 2026-09-25
⚠️ Evidence age: newest ranked run 2026-09-08 (17d old) · 16,855 runs in the 90-day window · NO NEW EVIDENCE since then — this ranking is FROZEN, and the window empties on 2026-12-07, after which every task type falls back to the unrestricted vendored table. `Last refresh` above is the day the ranker ran, not the age of its data.
...
_⚠️ These tables are the measurement record, NOT what `pick_models` returns. Routing reads the `### <task_type>` sections further down, after the operator deny (`OPERATOR_DENY`) and allowlist (`OPERATOR_ALLOW`, D-159) are applied — a model can be gate-eligible here and not routable there. `claude-code/*` rows are spawn-native and are never pool-routed._
...
| rank | model | shrunk_q | success | avg_cost | avg_quality | quality_tier | grounding | n |
|---:|---|---:|---:|---:|---:|:-:|:-:|---:|
| 1 | `deepseek/deepseek-v4-flash` | 2.75 | 0.89 | $0.0041 | 2.74 | 2 | ✓ | 1521 |
| 2 | `deepseek/deepseek-v3.2-exp` | 2.92 | 0.89 | $0.0046 | 2.91 | 3 | ✓ | 1567 |
...
| `claude-code/haiku` | A | 4.21 | 0.73 | 1.00 | $35.549 | $5.00 | $1.0665 | $0.019788 | 16.5 | 60 | 22 | 8 | ✅ |
...
| `claude-code/fable` | A | 4.05 | 0.68 | 1.00 | $448.486 | $50.00 | $13.4546 | $0.024327 | 10.3 | 16 | 22 | 8 | ✅ |
| `claude-code/opus` | A | 4.05 | 0.68 | 1.00 | $215.978 | $25.00 | $6.4794 | $0.023655 | 8.0 | 17 | 22 | 8 | ✅ |
| `claude-code/sonnet` | A | 4.05 | 0.68 | 1.00 | $106.899 | $10.00 | $3.2070 | $0.037469 | 12.4 | 35 | 22 | 8 | ✅ |
...
|---|:-:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| `claude-code/haiku` | A+ | 5.00 | 1.00 | 1.00 | $41.500 | $5.00 | $0.8300 | $0.014873 | 25.2 | 60 | 10 | 10 |
| `claude-code/sonnet` | A+ | 5.00 | 1.00 | 1.00 | $110.976 | $10.00 | $2.2195 | $0.021933 | 16.3 | 28 | 10 | 10 |
| `claude-code/fable` | A+ | 4.74 | 0.90 | 1.00 | $494.925 | $50.00 | $9.8985 | $0.017403 | 12.1 | 15 | 10 | 10 |
| `claude-code/opus` | A | 4.44 | 0.80 | 1.00 | $237.073 | $25.00 | $4.7415 | $0.016932 | 13.8 | 10 | 10 | 10 |
...
_`claude-code/*` rows: `$/1k` = ① API-equivalent (a list-price valuation of the subscription run's tokens, comparable to the pool — a RATE). (no per-row ② column here — this harness doesn't persist raw tokens per run.) Context — ② amortized ≈$0.015/M (over 2026-08-27→2026-09-25, 52.0B tokens over 4 accounts) · ③ last run's weekly-quota draw ≈0.0% (from `claude_p_cost.json`; ③ is a capacity estimate, not a precise meter). A `claude-code/*` `✅` reflects the QUALITY floors only — the carve-out bypasses the printed cost/latency gate, and these tiers are **spawn-native (display-only, NOT pool-dispatched)**, so `pick_models` never returns them._
