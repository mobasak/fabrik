...
What type of web product?
    ...
    │       │           • Fabrik override: e-commerce → Medusa (MIT, headless) via /opt/web-ecommerce-factory; billing = iyzico (TR) / Paddle (intl) / RevenueCat (mobile IAP). Not Shopify; Stripe is unavailable to the TR entity
    │       │
    │       └─► Custom / Full control?
...
- **Last updated**: 2026-08-30 (e-commerce override Vendure→Medusa: the reference impl `/opt/web-ecommerce-factory` switched to Medusa (MIT) — neither engine ships a ready iyzico provider so the TR payment handler is custom on either path, which removed the case for carrying GPLv3; billing guidance unchanged. Prior 2026-07-19: Fabrik-overrides reconciled to binding constraints — OpenRouter-only LLM gateway, pool model selection, Pattern-A auth, Astro factory for websites, iyzico/Paddle e-commerce billing, rent-don't-own GPU, shape-driven Redis; owned-services catalog pointer added)
