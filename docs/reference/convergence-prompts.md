...
`found: F, new: N, confirmed: 0, fixed: 0, unexecuted: 0` (or carries no `unexecuted` cell)
AND both gate runs show "status":"success". Quiet is zero CONFIRMED code or doc defects —
never zero raised.
...
`|` (escape it, or move it out of the table).
