...
| Verb | Caller | Does |
|---|---|---|
| `harvest` | Stop hook (`final_gate_stop.py`, best-effort, 5s timeout, skips if unsynced). Runs BEFORE the hook's `final_gate.py` eligibility return, resolving the script from the payload cwd with a `__file__` fallback — a post-compact Stop arrived with a drifted payload and silently blacked out the register (2026-08-29); memory must not depend on enforcement eligibility. Each attempt emits an `anchor_harvest` kaizen event (`tp`, `chars`), so "hook never ran" and "ran, found nothing" are distinguishable. | Extracts the last `NEXT:` of the final message. **Anchor shapes** — `N of M`, a `docs/development/{plans,epics,certifications}/` path, `phase X` — persist; keyed with digits masked, so "15 of 31" *updates* the "14 of 31" anchor rather than stacking. Plain successors just roll the latest-NEXT slot. |
