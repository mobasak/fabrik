...
(`cmd_where`, `scripts/thread_anchor.py:645`), built ONLY from records — never from an
...
3. An open DECISION block (below), rendered verbatim, prefixed `- OPEN DECISION — awaiting the
   operator's answer; never treat it as settled:` (`scripts/thread_anchor.py:680`).
...
`UserPromptSubmit` (`cmd_clear_decision`, `:475`) UNLESS that prompt's WHOLE first token,
...
**Tests:** `tests/test_thread_anchor.py` — 43 behaviors (`grep -c "^def test_"`), watched-fail-first;
