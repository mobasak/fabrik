...
**Denominator (measured 2026-09-02):** 158 distinct systems = the union of this doc's previous entries ∪ `scripts/service_catalog.json` (99 vendors) ∪ every vendor-shaped key in `/opt/*/.env.example` (40 files) + `specs/services/*.yaml` (72 specs) ∪ the live `docker ps` of vps1/vps2/vps3 ∪ `docs/reference/apis/*.md`. **115 are fleet-used** (≥1 env key, spec or running container); the rest are catalogued-only or retired and say so.
...
- **Research notes** _(2026-09-02)_: Core Google auth service; quotas can be increased via Cloud Console.

### Microsoft 365 / Graph

**Type:** infra-platform · **Reach:** REST API (env key) · **Used by:** 3 project(s) — email-reader, spec:email-reader, tojlo-mail · **Env keys:** `M365_*`, `MICROSOFT_*` · **Docs:** https://learn.microsoft.com/graph

| # | Field | Value (source) |
|---|---|---|
| 1 | Limits & quota | Global 130,000 req/10 s per app across tenants; service-specific tables (e.g. Excel 1,500/10 s per app per tenant; call records 1,500/20 s). _(src: https://learn.microsoft.com/en-us/graph/throttling-limits, 2026-09-02)_ |
| 2 | Behaviour AT the cap | 429 TooManyRequests + Retry-After; batch sub-requests 429 individually while batch returns 200. _(src: https://learn.microsoft.com/en-us/graph/throttling, 2026-09-02)_ |
| 3 | Concurrency & parallelism | Scoped per app/tenant, per app all tenants, per tenant; Bookings 4 concurrent per app+mailbox. _(src: https://learn.microsoft.com/en-us/graph/throttling-limits, 2026-09-02)_ |
| 4 | Identity posture | Per app registration + tenant; ToS n/a (M365 product terms) — UNKNOWN not fetched. _(src: https://learn.microsoft.com/en-us/graph/throttling-limits, 2026-09-02)_ |
| 5 | Failure & resume | Honour Retry-After; delta query (@odata.deltaLink, $deltatoken=latest) is the resumable cursor; @odata.nextLink paging. _(src: https://learn.microsoft.com/en-us/graph/delta-query-overview, 2026-09-02)_ |
| 6 | Cost model | No per-call charge for standard Graph in fetched docs; metered APIs UNKNOWN; Data Connect for bulk. _(src: https://learn.microsoft.com/en-us/graph/throttling, 2026-09-02)_ |
| 7 | Usage observability | No usage endpoint — only 429 + Retry-After. _(src: https://learn.microsoft.com/en-us/graph/throttling, 2026-09-02)_ |
| 8 | Health signal | Service communications API /v1.0/admin/serviceAnnouncement (healthOverviews, issues, messages; ServiceHealth.Read.All) — machine-readable but authenticated. _(src: https://learn.microsoft.com/en-us/graph/api/resources/service-communications-api-overview, 2026-09-02)_ |
| 9 | Credential lifecycle | Access tokens 60–90 min (CAE 24–28 h); refresh via OAuth; expired -> 401; subscription validation tokens ~1 h. _(src: https://learn.microsoft.com/en-us/entra/identity-platform/configurable-token-lifetimes, 2026-09-02)_ |
| 10 | Interface lifecycle | v1.0 + beta; GA deprecation >=24 months notice; changelog; Deprecation/Sunset/Link headers observed on beta (StackOverflow 2022), not in official doc. _(src: https://learn.microsoft.com/en-us/graph/versioning-and-support, 2026-09-02)_ |
| 11 | Data contract | Backward-compatible without version bump: new nullable props, new enum members, paging introduced, error-code changes — do NOT strict-reject unknown enums. _(src: https://learn.microsoft.com/en-us/graph/versioning-and-support, 2026-09-02)_ |
| 12 | Push delivery (webhooks/streams) | Webhooks: 2xx within 3 s; retries up to 4 h backoff; slow (>10% timeouts) delayed, drop (>15%) dropped unrecoverably; no sent-list — reconcile via delta. _(src: https://learn.microsoft.com/en-us/graph/change-notifications-delivery-webhooks, 2026-09-02)_ |
| — | **Resilience posture (58)** | per-dependency card owed in the consuming project's `docs/RESILIENCE.md` §2b (timeout · retry layer · breaker · pause key · failover · backup) — this index records the VENDOR side only |
...
**Type:** email · **Reach:** REST API (env key) · **Used by:** 0 project(s) (no env key/spec reference found) · **Docs:** https://aws.amazon.com/ses

| # | Field | Value (source) |
|---|---|---|
| 1 | Limits & quota | - Sandbox: 200 emails/day - Production: Request limit increase _(carried from the 2026-06-02 entry — re-verify)_ |
| 2 | Behaviour AT the cap | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 3 | Concurrency & parallelism | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 4 | Identity posture | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 5 | Failure & resume | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
...
| 7 | Usage observability | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 8 | Health signal | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 9 | Credential lifecycle | - Type: SMTP Credentials - Env Vars: `SES_SMTP_HOST`, `SES_SMTP_PORT`, `SES_SMTP_USER`, `SES_SMTP_PASS` - Generate at: AWS Console → SES → SMTP Settings → Create SMTP Credentials — expiry/rotation UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 10 | Interface lifecycle | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 11 | Data contract | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| 12 | Push delivery (webhooks/streams) | UNKNOWN — tried: catalog, the 2026-06-02 entry, rules corpus, vendor docs; live re-verify pending |
| — | **Resilience posture (58)** | per-dependency card owed in the consuming project's `docs/RESILIENCE.md` §2b (timeout · retry layer · breaker · pause key · failover · backup) — this index records the VENDOR side only |
...
| 4 | Identity posture | Keys and limits bound to organization, not user. Cloud use governed by Groq Services Agreement / Acceptable Use Policy (index fetched; agreement text not opened) → multi-account clause UNKNOWN. _(src: https://console.groq.com/docs/api-keys, 2026-09-02)_ |
...
| 9 | Credential lifecycle | Keys org-bound; expiry not documented on api-keys page → UNKNOWN; invalid key → 401. _(src: https://console.groq.com/docs/api-keys, 2026-09-02)_ |
