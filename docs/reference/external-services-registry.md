...
Every step is non-fatal to the caller but never silent: a non-zero exit (124 = the 900 s timeout; 137 = the SIGKILL that follows an ignored SIGTERM 30 s later) alerts via `alerting.send_alert` (the retired orchestrator's alert, kept) — a bounded-prune refusal, an exhausted pool or a dead registry DB reaches the operator, not just the log — and the dashboard is NOT rewritten, so the liveness heartbeat ages and reads DEAD. Newly classified providers raise an info alert. A manual run racing the cron is safe: `gather_envs` and `classify` write atomically (`os.replace`), `registry_sync` is one transaction, and `classify` holds a non-blocking lock (`.tmp/external-services/classify.lock`) so two runs never process the same slice or clobber the cursor — the loser prints a line and exits 0.
