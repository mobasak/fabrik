...
**Catalog conventions** (`scripts/service_catalog.json`): a `_`-prefixed top-level key is METADATA (`_README`), never a provider — `gather_envs.load_catalog` drops it, `classify_services` keeps it aside and writes it back first, and `derive_provider` never mints one; every provider key is a single whitespace-free token (`load_catalog` refuses anything else, loudly — a hand-edited `pro v` would round-trip as a different provider); a model answer's `category` is normalised and must be an enum value, else the provider is NOT identified (it is tombstoned on the daily run, never merged into a curated entry, never reported or scored as classified); `cost`/`status` are normalised too but only DEFAULT (`?` / `active`) — they never gate identification (AX1).
