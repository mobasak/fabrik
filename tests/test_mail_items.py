...
_VERB_CASES = [
    pytest.param("claim", "open_linked", [], id="claim"),
    pytest.param("ack", "close_linked", ["--disposition", "done"], id="ack"),
    pytest.param("requeue", "close_linked", [], id="requeue"),
]


def _expected_path(mail_root: Path, verb: str, mid: str) -> Path:
    sub = "inbox" if verb == "requeue" else "archive"
    return mail_root / "alpha" / sub / f"{mid}.md"


@pytest.mark.parametrize(("verb", "work_attr", "extra_args"), _VERB_CASES)
def test_note_mail_survives_a_raising_store(
    alpha, monkeypatch, capsys, verb, work_attr, extra_args
):
    """A plain exception from the store call (``open_linked`` for claim, ``close_linked``
    for ack/requeue) must never touch the verb's rc or its printed path, and must surface
    as exactly one stderr line — for all three CLI verbs, not just claim (M2-O3)."""
    repo, mail_root = alpha
    mid = mail._ulid()
    _plant(mail_root, "alpha", mid, "boom test\n")

    if verb in ("ack", "requeue"):
        # a real, UNMUTATED claim first: moves the message into archive (so ack/requeue has
        # something to act on) and opens a real item via the real open_linked, so only the
        # verb-under-test's own store call is mutated below.
        assert mail.main(["claim", mid, "--repo", "alpha"]) == 0
        capsys.readouterr()  # discard the claim's own stdout/stderr
    ...
    monkeypatch.setattr(w, work_attr, _raiser)

    rc = mail.main([verb, mid, "--repo", "alpha", *extra_args])

    assert rc == 0
    out = capsys.readouterr()
    assert out.out == f"{_expected_path(mail_root, verb, mid)}\n"
    assert out.err.count("\n") == 1, out.err
    assert "mail item not" in out.err
    if verb == "claim":
        assert _mail_items(repo) == []
    else:
        items = _mail_items(repo)
        assert len(items) == 1 and items[0]["status"] == "open", items


@pytest.mark.parametrize(("verb", "work_attr", "extra_args"), _VERB_CASES)
def test_note_mail_survives_a_printing_systemexit_store(
    alpha, monkeypatch, capsys, verb, work_attr, extra_args
):
    ...
    call routes it to stderr instead) and must never change the verb's rc — a store-side
    ``sys.exit`` must not exit this process — for all three CLI verbs (M2-O3)."""
    repo, mail_root = alpha
    mid = mail._ulid()
    ...
    monkeypatch.setattr(w, work_attr, _printer_then_exit)

    rc = mail.main([verb, mid, "--repo", "alpha", *extra_args])

    assert rc == 0
    out = capsys.readouterr()
    expected_path = _expected_path(mail_root, verb, mid)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "---\nid: X\nkind: request\n---\n**Subject: x**\n",
            "x",
            id="whole-line-bold-subject-trailing-stars",
        ),
        pytest.param(
            "---\nid: X\nkind: request\n---\n---\n---\nreal title\n",
            "real title",
            id="two-leading-rule-lines",
        ),
        pytest.param(
            "---\nid: X\nkind: request\n---\n**Subject:** **bold value**\n",
            "**bold value**",
            id="bold-prefix-bold-value",
        ),
        pytest.param(
            "---\nid: X\nkind: request\n---\nSubject: fix **urgent**\n",
            "fix **urgent**",
            id="plain-subject-bold-tail",
        ),
        pytest.param(
            "---\nid: X\nkind: request\n---\n-----\nreal title\n",
            "real title",
            id="four-plus-dash-rule",
        ),
        pytest.param(
            "---\nid: X\nkind: request\n---\n" + ("x" * 400) + "\n",
            "x" * 300,
