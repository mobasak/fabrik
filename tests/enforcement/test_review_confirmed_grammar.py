...
def test_corpus_every_committed_receipt_parses_and_grades_exactly_as_it_did_at_the_base_sha(
    ...
    for p in files:
        ...
        # one_committed_row`. In THIS FILE it reaches no gate output — the receipt carries no
        # Coverage Checklist, so `check_file` returns early and `_committed_nonquiet` skips it, and
        # the `errs`/advisory comparisons below stay byte-identical. That is a fact about this
        # receipt, NOT about the rule: the same idiom on a receipt with a checklist section is
        # refused by `check_file` (with the citation repair). The pin is at
...
def test_a_citing_cell_right_of_a_gap_is_refused_fail_closed(tmp_path):
    ...
    This file's adjudicated policy on that exact tie is fail-CLOSED: a cited counter is one edit to reword (fencing does NOT
    repair this shape — see the repair matrix over `_joined_row`); a missed join grades a receipt quiet off another round's numbers. Both halves of
