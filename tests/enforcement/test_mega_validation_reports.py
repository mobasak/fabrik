...
def test_code_spanned_hashes_satisfy_both_halves_not_just_the_surface_line(repo: Path) -> None:
    """The mirror the first code-span fix MISSED — and missing it made things worse, not neutral.

    `_MEGA_SURFACE` was made quote-tolerant while `_MEGA_HASH_PAIR` was not. An author who
    code-spans hashes CONSISTENTLY then flipped from "both halves fail, agreeing" to "presence
    passes, the pair fails" — the operator pointed at the wrong half with "carries no full
    md5(start) -> md5(end) pair" while the pair sat right there in code spans. Both halves are
    asserted here so neither can be widened alone again.
    """
    h = _shell_hash(repo)
    body = _report(h)
    # code-span BOTH the Surface value and the ledger's hash pair, the way markdown invites.
    body = body.replace(f"Surface: {h}", f"**Surface:** `{h}`")
    body = body.replace(f"{h} -> {h}", f"`{h}` -> `{h}`").replace(
        f"{h} \u2192 {h}", f"`{h}` \u2192 `{h}`"
    )
    rc, out = _gate(repo, "2026-08-19-mega-vision-validation-review.md", body)
    assert rc == 0, f"a uniformly code-spanned report was refused: {out}"
...
def test_a_surface_hash_may_not_be_borrowed_from_a_later_line(repo: Path) -> None:
    ...
    spec = importlib.util.spec_from_file_location("crc_nl", CHECK)
...
def test_checkfile_ledger_cannot_be_silenced_by_quoted_contract_prose(repo: Path) -> None:
    ...
    body = (
        "# Review — some diff (/fabrik-review)\n"
        "Surface: abc123\n\n"
        "# REVIEW RUBRIC — inject into EVERY finder prompt (generated by review_rubric.py) here\n\n"
        "## Coverage Checklist\n"
        "| Class | Verdict | Evidence |\n|---|---|---|\n"
...
def test_narrative_counters_in_a_cell_cannot_swap_the_real_values(repo: Path) -> None:
    ...
    # ⚠️ The `method: re-derivation` cell lives on PASS 1, deliberately, and both probed rows carry
    # ONLY narrative counters. Two reasons, both learned the hard way on 2026-09-01:
    #   (1) the masked half was VACUOUS for a day — the probed row had no re-derivation text, so it
    #       tripped "no re-derivation ledger row" and returned rc=1 no matter what the counter logic
    #       did. Deleting the round-11 token defence (`_FOUND_TOK`'s `(?!\s*\w)`) still left 895/895
    #       green. Hence the `"raised 4"` message assertion below: rc alone cannot prove the reason.
    #   (2) it must not go on a TRAILING row either — a quiet `Pass 3` would become the FINAL ledger
    #       round and invert the masked assertion. Pass 1 satisfies the obligation while leaving the
    #       final row free to carry the counters under probe.
    base = (
        "# Review — some diff (/fabrik-review)\nSurface: abc123\n\n"
        "# REVIEW RUBRIC — inject into EVERY finder prompt (generated by review_rubric.py)\n\n"
        "## Coverage Checklist\n| Class | Verdict | Evidence |\n|---|---|---|\n"
        "| fail-open cost boundary untested behavior | CLEAN | scripts/x.py hunted |\n\n"
        "## Pass Ledger\n"
        "| Pass 1 | initial method: re-derivation | found: 3 · fixed: 1 | notes |\n{row}\n"
    )
    masked = base.format(
    ...
    assert "raised 4" in out, f"refused, but for the WRONG reason — the decoy may be masking: {out}"
...
def test_a_bulleted_surface_declaration_with_a_code_spanned_hash_is_still_refused(
    repo: Path,
) -> None:
    """The `_SURFACE_QUOTE` MIRROR — the wrap matcher composes it too, and shipped with NO guard.

    `_WRAPPED_HEADINGISH` was given quote-tolerance in the same change as `_MEGA_SURFACE`, but only
    the latter got a test: reverting the wrap hunk left the whole 895-test suite green (measured
    2026-09-01). It is live code — a report that BULLET-wraps its Surface declaration with a
    code-spanned hash would otherwise slip past the container-wrap refusal, which is the vanishing
    class this matcher exists to close.
    """
    ...
    rc, out = _gate(repo, "2026-08-30-wrapped-surface-review.md", body)
    assert rc == 1, f"a list-wrapped Surface declaration escaped the container refusal: {out}"
    assert "LIST-WRAPPED heading/declaration" in out, f"refused for the wrong reason: {out}"
...
def test_bare_decoy_blocked_headings_do_not_inflate_the_cardinality(repo: Path) -> None:
    ...
    body = (
        ...
        "## Pass Ledger\n| Pass 1 | x | found: 3 · fixed: 3 | y |\n| Pass 2 | x method: re-derivation | found: 0 · fixed: 0 | y |\n\n"
