...
def test_a_sentence_final_surface_still_yields_a_usable_plan_stem():
    ...
    assert (
        cr._plan_stem({"surface": "docs/development/plans/2026-09-11-plan-1-x/"})
        == "2026-09-11-plan-1-x"
    )


def test_the_own_fix_note_covers_every_command_the_stop_can_fire_on(run_dir, tmp_path):
    ...
    ⚠️ THIS TEST INVOKES THE NOTE PATH. Its first cut asserted only `_tokish` and set membership,
    and stayed GREEN through two mutations of the line it named — the gate reverted to
    `in REVIEW_FAMILY`, and the gate deleted outright. A guard that never runs the branch it
    protects is the class this whole review was hunting, committed by the review itself.
    """
    ...
    def run(*a, session):
        env = {
            **os.environ,
            "COMMAND_RUN_DIR": str(run_dir),
            "COMMAND_RUN_TRANSCRIPT": str(tmp_path / f"{session}.jsonl"),
            "KAIZEN_EVENTS_DIR": str(tmp_path / "kz"),
            "HOME": str(tmp_path / "home"),
            "FABRIK_SCRATCH_SWEEP": "0",
        }
        return subprocess.run(
            [sys.executable, str(_SCRIPT), *a, "--session", session],
            capture_output=True,
            text=True,
            env=env,
            timeout=120,
    ...
    for cmd in ("fabrik-spec-review", "fabrik-plan-review", "fabrik-docs-review", "fabrik-review"):
        ...
        assert "--own-fix" in r.stderr, f"{cmd} got no NOTE: {r.stderr!r} / {r.stdout!r}"

    # and a per-unit command, where the stop stands down, must NOT be nagged
    sid = "probe-note-exec"
    run(
        "start", "--command", "fabrik-execute-plan", "--phases", "2", "--terminal", "x", session=sid
    )
    r = run("round", "--findings", "3", "--confirmed", "2", session=sid)
    assert "--own-fix" not in r.stderr, r.stderr
