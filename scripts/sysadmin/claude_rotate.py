#!/usr/bin/env python3
# AFTER-EDIT: scripts/sysadmin/bot.py, scripts/aro-wake/claude_rotate.py
...
def _cmd_switch(name: str) -> int:
    ...
    if _activate_snapshot(target):
        sys.stdout.write(f"switched active Claude account → {target.name} ({_account_email(target)})\n")
        _reload_hint()
        return 0
