...
def scan(p: Path):
    ...
    cwd = title = summary = None; first = last = None; user_txt = None
    with open(p, errors="replace") as fh:
        for i, line in enumerate(fh):
            ...
            if not user_txt and o.get("type") == "user":
                ...
                t = (t or "").strip()
                if t and not t.startswith(("<", "[", "Caveat:")): user_txt = t
    title = (summary or user_txt or p.stem)[:120]
    return cwd, title, first, last

def main() -> int:
    if not DB.exists():
        print("claudeck DB not found — run claudeck once first"); return 1
    bak = DB.with_suffix(f".bak-{date.today():%Y%m%d}")
    if not bak.exists(): shutil.copy2(DB, bak)
    db = sqlite3.connect(DB)
    ins = upd = 0
    for f in STORE.glob("*/*.jsonl"):
        sid = f.stem
        if len(sid) != 36: continue  # session files are uuid-named
        cwd, title, first, last = scan(f)
        if not cwd or not cwd.startswith("/opt/"): continue
        if Path(cwd).name in EXCLUDE and "--include-all" not in sys.argv: continue
