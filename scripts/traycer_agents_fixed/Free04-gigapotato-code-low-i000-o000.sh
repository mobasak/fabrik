#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Free Tier #04 (FIXED)
# ════════════════════════════════════════════════════════════════════════════
#
...
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Free04-gigapotato-code-low-i000-o000.sh
#
...
# Specialty: Evaluation period free model
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Free04-giga-potato-code-low" >&2
    echo "[DEBUG] Model: kilo/giga-potato" >&2
...
SESSION_ID="ses_free04_${TIMESTAMP}_$$"
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Free04"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Free04-code-low\",\"model\":\"kilo/giga-potato\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
