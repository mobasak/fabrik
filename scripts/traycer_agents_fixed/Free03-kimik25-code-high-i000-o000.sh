#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Free Tier #03 (FIXED)
# ════════════════════════════════════════════════════════════════════════════
#
...
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Free03-kimik25-code-high-i000-o000.sh
#
...
# AGENT DETAILS
# ════════════════════════════════════════════════════════════════════════════
# Model: kilo/moonshotai/kimi-k2.5:free
# Role: code | Variant: high
# Specialty: Agentic capabilities, tool use, reasoning (FREE)
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Free03-kimi-k2.5-code-high" >&2
    echo "[DEBUG] Model: kilo/moonshotai/kimi-k2.5:free" >&2
...
SESSION_ID="ses_free03_${TIMESTAMP}_$$"
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Free03"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Free03-code-high\",\"model\":\"kilo/moonshotai/kimi-k2.5:free\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
