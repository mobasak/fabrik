#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Free Tier #06 (FIXED)
# ════════════════════════════════════════════════════════════════════════════
#
...
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Free06-qwen3coder-code-high-i000-o000.sh
#
...
# Specialty: Agentic coding, function calling, long-context (FREE)
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Free06-qwen3-coder-code-high" >&2
    echo "[DEBUG] Model: kilo/qwen/qwen3-coder" >&2
...
SESSION_ID="ses_free06_${TIMESTAMP}_$$"
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Free06"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Free06-code-high\",\"model\":\"kilo/qwen/qwen3-coder\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
