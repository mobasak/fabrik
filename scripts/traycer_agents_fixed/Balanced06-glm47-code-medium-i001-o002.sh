#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Balanced Tier #06
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Balanced06-glm47-code-medium-i001-o002.sh
#
...
# Specialty: Chinese/multilingual
# Pricing: $0.01/1M input, $0.02/1M output
# ════════════════════════════════════════════════════════════════════════════

# Debug mode (KILO_DEBUG=1)
if [ "$KILO_DEBUG" = "1" ]; then
    set -x  # Print all commands
    echo "[DEBUG] Agent: Balanced06-glm-4.7-code-medium" >&2
    echo "[DEBUG] Model: kilo/z-ai/glm-4.7" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Balanced06"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Balanced06-code-medium\",\"model\":\"kilo/z-ai/glm-4.7\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
