#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Review Agent - Economy Tier #04
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Economy04-seed20mini-review-max-i003-o031.sh
#
...
# Role: review | Variant: max
# Specialty: Efficient reasoning
# Pricing: $0.03/1M input, $0.31/1M output
# ════════════════════════════════════════════════════════════════════════════

# Debug mode (KILO_DEBUG=1)
if [ "$KILO_DEBUG" = "1" ]; then
    set -x  # Print all commands
    echo "[DEBUG] Agent: Economy04-seed-2.0-mini-review-max" >&2
    echo "[DEBUG] Model: kilo/bytedance-seed/seed-2.0-mini" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Economy04"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Economy04-review-max\",\"model\":\"kilo/bytedance-seed/seed-2.0-mini\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
...
KILO_MODEL="kilo/bytedance-seed/seed-2.0-mini"
