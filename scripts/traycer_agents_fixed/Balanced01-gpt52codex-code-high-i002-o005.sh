#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Balanced Tier #01
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Balanced01-gpt52codex-code-high-i002-o005.sh
#
...
# Pricing: $0.02/1M input, $0.05/1M output
# ════════════════════════════════════════════════════════════════════════════

# Debug mode (KILO_DEBUG=1)
if [ "$KILO_DEBUG" = "1" ]; then
    set -x  # Print all commands
    echo "[DEBUG] Agent: Balanced01-gpt-5.2-codex-code-high" >&2
    echo "[DEBUG] Model: kilo/openai/gpt-5.2-codex" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Balanced01"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Balanced01-code-high\",\"model\":\"kilo/openai/gpt-5.2-codex\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
...
KILO_MODEL="kilo/openai/gpt-5.2-codex"
