#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Free Tier #09 (FIXED)
# ════════════════════════════════════════════════════════════════════════════
#
...
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Free09-kimik2-code-high-i000-o000.sh
#
...
# Specialty: Advanced tool use, reasoning, code synthesis (FREE)
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Free09-kimi-k2-code-high" >&2
    echo "[DEBUG] Model: kilo/moonshotai/kimi-k2" >&2
...
SESSION_ID="ses_free09_${TIMESTAMP}_$$"
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Free09"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Free09-code-high\",\"model\":\"kilo/moonshotai/kimi-k2\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
