#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Review Agent - Balanced Tier #03
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Balanced03-glm5-review-high-i100-o320.sh
#
...
# Specialty: Advanced reasoning, multilingual
# Pricing: $1.00/1M input, $3.20/1M output
# ════════════════════════════════════════════════════════════════════════════

# Debug mode (KILO_DEBUG=1)
if [ "$KILO_DEBUG" = "1" ]; then
    set -x  # Print all commands
    echo "[DEBUG] Agent: Balanced03-glm-5-review-high" >&2
    echo "[DEBUG] Model: kilo/z-ai/glm-5" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Balanced03"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Balanced03-review-high\",\"model\":\"kilo/z-ai/glm-5\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
