#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Review Agent - Balanced Tier #05
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Balanced05-gpt52chat-review-max-i175-o1400.sh
#
...
# Specialty: Code review, conversation
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Balanced05-gpt-5.2-chat-review-max" >&2
    echo "[DEBUG] Model: kilo/openai/gpt-5.2-chat" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Balanced05"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Balanced05-review-max\",\"model\":\"kilo/openai/gpt-5.2-chat\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
...
KILO_MODEL="kilo/openai/gpt-5.2-chat"
