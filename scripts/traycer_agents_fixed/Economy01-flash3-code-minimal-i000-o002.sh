#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Economy Tier #01
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Economy01-flash3-code-minimal-i000-o002.sh
#
...
# Role: code | Variant: minimal
# Specialty: Fast prototyping, cheap
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Economy01-gemini-3-flash-preview-code-minimal" >&2
    echo "[DEBUG] Model: kilo/google/gemini-3-flash-preview" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Economy01"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Economy01-code-minimal\",\"model\":\"kilo/google/gemini-3-flash-preview\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
...
KILO_MODEL="kilo/google/gemini-3-flash-preview"
