#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Balanced Tier #04
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Balanced04-grok41fast-code-high-i020-o050.sh
#
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Balanced04-grok-4.1-fast-code-high" >&2
    echo "[DEBUG] Model: kilo/x-ai/grok-4.1-fast" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Balanced04"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Balanced04-code-high\",\"model\":\"kilo/x-ai/grok-4.1-fast\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
...
KILO_MODEL="kilo/x-ai/grok-4.1-fast"
