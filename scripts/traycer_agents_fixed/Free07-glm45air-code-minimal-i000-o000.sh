#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Free Tier #07 (FIXED)
# ════════════════════════════════════════════════════════════════════════════
#
...
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Free07-glm45air-code-minimal-i000-o000.sh
#
...
# Specialty: Lightweight agent-centric (FREE)
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Free07-glm-4.5-air-code-minimal" >&2
    echo "[DEBUG] Model: kilo/z-ai/glm-4.5-air" >&2
...
SESSION_ID="ses_free07_${TIMESTAMP}_$$"
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Free07"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Free07-code-minimal\",\"model\":\"kilo/z-ai/glm-4.5-air\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
