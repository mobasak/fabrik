#!/bin/sh
# ════════════════════════════════════════════════════════════════════════════
# Kilo Code Agent - Economy Tier #03
# ════════════════════════════════════════════════════════════════════════════
#
# 📛 SCRIPT NAME: Economy03-glm47flash-code-minimal-i007-o040.sh
#
...
# Specialty: Fast, cheap, multilingual
...
if [ "$KILO_DEBUG" = "1" ]; then
    ...
    echo "[DEBUG] Agent: Economy03-glm-4.7-flash-code-minimal" >&2
    echo "[DEBUG] Model: kilo/z-ai/glm-4.7-flash" >&2
...
if [ $EXIT_CODE -eq 124 ]; then
    echo '{"error": "timeout", "duration": '$TIMEOUT', "agent": "Economy03"}' >&2
...
if [ -n "$KILO_TRACK_COST" ]; then
    ...
    echo "{\"timestamp\":\"$(date -Iseconds)\",\"agent\":\"Economy03-code-minimal\",\"model\":\"kilo/z-ai/glm-4.7-flash\",\"task_id\":\"$TRAYCER_TASK_ID\",\"exit_code\":$EXIT_CODE,\"duration\":$DURATION}" >> "$USAGE_LOG"
...
KILO_MODEL="kilo/z-ai/glm-4.7-flash"
