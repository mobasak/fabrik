...
_step() {  # $1 label, rest = command; records timing, alerts + flags on failure
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    _alert "external-services chain: step $label FAILED (exit $rc)" "gather_envs -> classify_services -> gather_envs -> registry_sync -> gen_dashboard lost step $label in today's run (exit $rc; 124 = timeout ${STEP_TIMEOUT}s, 137 = SIGKILL 30 s after an ignored SIGTERM, 2 = registry_sync could not read the catalog: provenance UNKNOWN, every credential stored unattributed; 1 = for gather_envs: it refused its INPUTS — a catalog it cannot trust (unreadable, undecodable, not an object, a whitespace key, a prefix two providers claim, a catalog emptied while the last consolidation knew vendors), or ripgrep could not run, or no /opt/*/.env was found; for any other step, 1 is that step's own failure — read its first stderr line). A bounded-prune refusal needs REGISTRY_PRUNE_FORCE=1 after a look; an exhausted pool or a dead registry DB needs a hand. The dashboard is NOT rewritten on a failed chain, so liveness will read DEAD until this is fixed. Log: $LOG_FILE" warning
  fi
  return $rc
}
