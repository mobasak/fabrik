...
Subcommands:
  ...
  round [--findings <N>] [--confirmed <N>] [--seats <N>] [--classes-swept a,b] [--classes-new c,d]
...
def scope_growth_warning(rows: list[Any], command: str = "") -> str:
    ...
    arrow = " → ".join(f"{'?' if c is None else c}/{'?' if o is None else o}" for c, o in pairs)
    ...
    def _qualifies(c: int | None, o: int | None) -> bool:
        ...
        return c is not None and o is not None and c > 0 and 0 <= o <= c and o * 3 >= c * 2
...
def _trend_series(rounds: list[Any]) -> list[int]:
    ...
    questions ("what did you raise" and "what did you reproduce") into one arrow. Shared by the
...
def _round_report(rec: dict[str, Any]) -> str:
    ...
    last_confirmed = _confirmed(last)
    ...
    if quiet and len(rounds) >= 2 and failing:
        lines.append(
            "⛔ NOT TERMINAL — "
            + "; ".join(
                f"slice {s['name']} has {s['claims'] - s['verified']} open claim"
                + ("s" if s["claims"] - s["verified"] != 1 else "")
...
# ── per-run TOKEN usage from the session transcript (operator, 2026-09-07: "input, output,
# cached tokens must be recorded per run") ─────────────────────────────────────────────────
...
def _giant_line_epoch(q: Path, size: int) -> float | None:
    """The envelope stamp of a last line longer than the walk (a tool result past 4 MB). Two
    live shapes (round 17, measured on 8,778 seat files: 1,011 lines over 64 KB): the stamp
    trails the message payload — 860 of them, read from the line's last 64 KB — or it PRECEDES
    a giant `toolUseResult` (151, all `type: user`; the envelope key order is …, message,
    uuid, timestamp, toolUseResult, …), read from the line's first 64 KB after a scan back for
    the previous newline that is capped at one extra window (F347 stays closed). Exactly one
    stamp in the window is the envelope's; two mean a nested one and `_line_epoch` refuses the
    truncated chunk → None → "skipped". The caller has already ruled out a torn tail."""
    with q.open("rb") as fh:
        fh.seek(max(size - (64 << 10), 0))
        e = _line_epoch(fh.read())
        if e is not None:
            return e
        pos = max(size - _SEAT_TAIL_BYTES, 0)
        floor = max(size - 2 * _SEAT_TAIL_BYTES, 0)
        while pos > floor:
            step = min(1 << 20, pos - floor)
            pos -= step
            fh.seek(pos)
            ...
            if pos > 0:
                return None  # the line's head is further back than one extra window
        fh.seek(pos)
...
def _seat_last_epoch(q: Path) -> float | str:
    ...
    its own bytes (`_giant_line_epoch`, tail then bounded head), never by its mtime (rounds
    15–17); any file with a TORN tail skips the giant path — "skipped" (rounds 16–17)."""
    try:
        size = q.stat().st_size
        torn = False
        if size:
        ...
        for raw in _iter_lines_backwards(q, _SEAT_TAIL_BYTES):
            if raw is None:
                if was_torn:
                    # a torn tail poisons the FILE-tail read the giant path makes (round 17:
                    # a stamp fragment after a complete giant line read as a second stamp)
                    break
...
def _transcript_path(sid: str, repo_root: str) -> Path | None:
    override = os.environ.get("COMMAND_RUN_TRANSCRIPT")
    if override:
        return Path(override)
    if not sid or not repo_root:
        return None
    ...
    try:
        ...
        return None
    for base in (home / ".claude" / "projects", home / ".claude-fleet" / "active" / "projects"):
        cand = base / slug / f"{sid}.jsonl"
        if cand.is_file():
            return cand
    return None
...
def _minutes_used(rec: dict[str, Any]) -> int:
    """Whole minutes since the record's start (0 when the start epoch is unreadable)."""
    started = rec.get("started_epoch") or 0.0
    try:
        return max(0, int((time.time() - float(started)) // 60)) if started else 0
...
def _feedback_line(rec: dict[str, Any], fields: dict[str, str], wall_s: float) -> str:
    ...
    tokens = _tokens_clause((rec.get("usage") or {}).get("tokens") or {})
    if tokens:
        head += f" · {tokens}"
    return f"{head} · {tail}"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "round",
        help="record one convergence round (--confirmed <n> is the exit counter)",
        parents=[common],
    )
    p.add_argument("--findings", type=int, default=0)
    # D-206/D-203: quiet is zero CONFIRMED code or doc defects, never zero RAISED — a refuted or
    # recorded candidate never reopens the loop, so the raw candidate count cannot be the exit
    # test. `None` (not stated) is a THIRD answer that keeps every untouched fleet caller on the
    # old `--findings 0` rule; a default of 0 would close their loops for them.
    p.add_argument(
        "--confirmed",
        type=int,
        default=None,
    ...
    p.add_argument(
        "--slices",
        default=None,
        help="the per-slice claim ledger this round verified — `A:12/12,B:5/6` (verified/claims "
        "per round-1 slice); the terminal is every slice verified (D-335)",
    )
...
def _mutate(sid: str, args: argparse.Namespace, outbox: dict[str, Any]) -> int:
    ...
    if args.cmd == "round":
        ...
        if args.confirmed is None and (rec.get("command") or "") in CONFIRMED_REQUIRED_COMMANDS:
            print(
                f"[command_run] REFUSED — /{rec.get('command')} is a review-family run: every "
                "`round` states `--confirmed <n>` (the exit counter; 0 on a quiet pass) — the "
                "terminal and the oscillation advisory read the confirmed series only (D-335)",
                file=sys.stderr,
            )
            return 2
        ...
        rounds.append(
            {
                ...
                # ABSENT when not stated (unlike `seats`, whose 0 IS "nobody dispatched"): the
                ...
                **({} if slices is None else {"slices": slices}),
                "phase": _phase_now,
            }
        )
        ...
        fields = _queue(
            ...
            "round",
            {
                ...
                # absent when not stated, exactly as in the record (docs/workstation/
                # kaizen-event-stream.md) — the emitter takes **fields with no allowlist
                **({} if args.confirmed is None else {"confirmed": args.confirmed}),
                "classes_swept": swept,
                "classes_new": new_c,
