...
CHECKLISTS = {
    "mega": Path("docs/orchestrator/mega-epic-breakdown/EVALUATION_CHECKLIST_FOR_MEGA_EPIC_COMMANDS.md"),
    "ettw": Path("docs/orchestrator/epic-to-ticket-workflow/EVALUATION_CHECKLIST_FOR_EPIC_COMMANDS.md"),
}
...
def build_rubric(changed: list[str], workflow: str | None, root: Path) -> str:
    ...
    out: list[str] = ["# REVIEW RUBRIC — inject into EVERY finder prompt (generated by review_rubric.py)"]
    out.append(
    ...
    for rel in FLOOR_PACKS:
        ...
        if rel in by_rel:
            ...
            out.append(f"- (pack missing at {root}/.windsurf/rules/{rel} — arm from the matched set)")
        emitted.add(rel)
...
def main() -> int:
    ...
    ap.add_argument("--changed", nargs="+", required=True, help="changed file paths (repo-relative)")
    ap.add_argument("--workflow", choices=sorted(CHECKLISTS), help="ALSO inject this command-chain checklist")
