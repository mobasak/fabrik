...
GOVERNANCE_TEMPLATES = [
    ...
    # Multi-agent-per-repo adoption artifact #1 (design spec § Lifecycle "Adoption"):
    # copies every gitignored governance/enforcement/vendored file into a linked worktree
    # at creation. Naming this pair here is what actually distributes the tracked
    # templates/governance/.worktreeinclude template into every project — listing the
    # path only in gitignore_dest_paths() would never reach a project (see that
    # function's own docstring warning).
...
def gitignore_dest_paths() -> dict[str, list[str]]:
    ...
    return {
        ...
        # SEED_IF_MISSING dests are EXCLUDED: they are project-owned DATA the moment they
        # exist, and their git history IS the who/when corroboration layer — an ignored
        # ledger can never be committed (caught live 2026-08-30: the first rollout
        # gitignored docs/DECISIONS.md in all 48 repos).
        "Governance files": list(GOVERNANCE_FILES)
        + [dest for _src, dest in GOVERNANCE_TEMPLATES if dest not in SEED_IF_MISSING],
...
def worktreeinclude_text() -> str:
    """The full ``.worktreeinclude`` file content — the multi-agent-per-repo adoption
    artifact that copies every gitignored governance/enforcement/vendored file into a
    linked worktree at creation (design spec § "Environment inside a worktree": Claude
    Code copies a gitignored file into a new worktree when it also matches a
    ``.worktreeinclude`` pattern, so a bare ``git worktree add`` — which only checks out
    ...
    for paths in gitignore_dest_paths().values():
        for p in paths:
            if p not in seen:
                seen.add(p)
                patterns.append(p)
    for extra in (".env", ".mcp.json"):
        if extra not in seen:
            seen.add(extra)
            patterns.append(extra)
    patterns = [p for p in patterns if p != ".claude/settings.local.json"]
    lines = [
        "# Fabrik-managed — DO NOT EDIT (centrally generated by fabrik_synced_manifest.py)",
...
if __name__ == "__main__":  # pragma: no cover - thin CLI
    ...
    g.add_argument(
        ...
        action="store_true",
        help=(
            "render the .worktreeinclude content — regenerate the tracked template with: "
            "python3 scripts/fabrik_synced_manifest.py --worktreeinclude "
            "> templates/governance/.worktreeinclude"
        ),
    )
    args = ap.parse_args()
    if args.worktreeinclude:
        print(worktreeinclude_text(), end="")
    else:
