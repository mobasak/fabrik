#!/usr/bin/env python3
"""
Apply Self-Review Workflow to Economy Tier Agents

Applies the same fixes as free tier agents:
1. Real self-review (separate Kilo call, not placeholder)
2. Mandatory workflow (always runs)
3. Self-review validation

Usage:
    python scripts/fix_economy_tier_agents.py
"""
...
def main():
    ...
    # Find economy tier agents
    economy_agents = sorted(source_dir.glob("Economy*.sh"))

    if not economy_agents:
        print("❌ Error: No economy tier agents found")
        return 1

    print(f"Found {len(economy_agents)} economy tier agents")
    print()
    ...
    # Process each agent
    for agent_file in economy_agents:
    ...
    print()
    print("Economy tier agents ready for testing")
