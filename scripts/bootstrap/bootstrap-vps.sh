...
#   04. Install Wireguard (DOCKER-USER persistence via iptables-docker-user.service, step 10)
...
step_04_install_wireguard() {
    log "step 04: install Wireguard on the spoke"
    ...
    # (Remove: ufw:amd64), which then breaks step_15's aro-wake UFW rule
    # (`ufw: command not found`). DOCKER-USER persistence is instead provided by
    # iptables-docker-user.service (installed in step_10), matching the vps1 hub
    # pattern. UFW remains the canonical spoke firewall (installed in step_02). (G5)
    remote 'sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq wireguard'
    ok "step 04 done"
}
...
step_10_apply_firewall_rules() {
    log "step 10: DOCKER-USER chain + persist via iptables-docker-user.service (hub-matching)"
    if $DRY_RUN; then
    ...
    # Build add/rm scripts + the systemd unit locally, then scp + install — the
    # scp-to-/tmp-then-sudo-install pattern (see step_15) avoids the remote-bash
    # quote-nesting hazard (90-bootstrap-scripts.md Rule 2). DOCKER-USER rules
    # are (re)applied at boot by a oneshot unit identical in shape to vps1's
    # /etc/systemd/system/iptables-docker-user.service — NOT iptables-restore of a
    # full save (which can clobber Docker's live chains on a Docker host). (G5)
    local tmpdir
    tmpdir=$(mktemp -d -t fabrik-iptables-XXXX)

    cat > "${tmpdir}/add-docker-user-rules.sh" <<EOF
#!/bin/bash
# Spoke DOCKER-USER rules — generated by bootstrap-vps.sh step_10.
