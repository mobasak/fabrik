...
def _future_or_far(reset: object, now: float, far: float) -> float:
    """A sort key for perishable-first: the validated reset when it lies ahead of *now*, else *far*."""
    ts = _usable_ts(reset)
    return ts if ts is not None and ts > now else far
...
def _rotate_state_dir() -> Path:
    """The tick's state dir. Creates it when absent (the leaf 0700 less the umask, any missing
...
def _usable_ts(ts: object) -> float | None:
    """A JSON number as a float, or None when it cannot be one — the ONE validator for every
...
def _pick_flip_target(
    ...
    *exclude* emails, walled ones (either window ≥100%), CAP-walled ones (weekly ≥ its
    ...
    for row in accounts:
        ...
        # seat, RECORDED → this fix). The scoped review and the heavy review it routed up to then
        # closed every sibling bare site of the class — grep `_usable_ts(` for the current set;
        # a list here was incomplete the day it was written (round 1 seat 3, F6)
...
def _fleet_row_warnings(accounts: list[dict]) -> list[str]:
    ...
    for row in accounts:
        ...
        if isinstance(exp, (int, float)) and math.isfinite(exp):
            ...
            if left <= 0:
                warns.append(
                    f"⚠ {row['email']}: refresh chain EXPIRED {abs(left) / 86400:.1f}d ago — a claude "
...
def _fleet_readings(accounts: list[dict], picture: dict) -> dict:
    ...
    WHY the band is what it is. An empty pool yields no reading; whether that is scarcity or a
    blackout is `_fleet_measured`'s answer, and `_fleet_band` reads it.
    """
    states = {
...
def _fleet_band(
    ...
    *,
    ...
    # seat 1: capped active + session-exhausted sibling, executed). That is RED; the WALL stamp
...
def _quota_posture(
    ...
    return {
        ...
        "active": {
            ...
            "band": _fleet_band(
                fleet_w,
                _band_of(hot, hold, drain, urgent),
                hold,
                drain,
                urgent,
                fable=False,
        ...
        "fleet": {
            ...
            "windows": fleet_w,
            # how many accounts a required window was READ for: the renderers print
            # `nobody serves it` for an absent window only when this is > 0 (F2)
            "measured": measured,
...
def _posture_status_line(posture: dict | None, now: float, stale_s: float = 900.0) -> str:
    ...
    # an EMPTY map is "nobody can serve" when someone was measured and a blackout otherwise —
    # gated on the map's truthiness, the worst case (both required windows unserved) rendered no
    # fleet section at all (Delta 9 seat A, F2)
    scarce = (
    ...
    if fw or scarce:
        ...
        for key, label in (("five_hour", "5h"), ("seven_day", "weekly"), ("fable", "Fable")):
            ...
            # a key PRESENT but unusable is a malformed reading: omitted, as the hook's `_util`
            # omits it — the first cut rendered `5h — (x)` under a comment claiming omission
            # (Delta 9 seat A, F5)
    ...
    try:
        ...
        if (
            ...
            for key in burn:
                ...
                # the PREVIOUS reading is json.loads of the memory file — same validator: a giant
...
def _row_has_reading(row: dict | None) -> bool:
    """The ONE no-reading predicate: `_active_account_walled` answers "not walled" on it and the
    relief wake refuses to fire on it (a probe blackout is not relief). A row with ANY non-None
    window utilization is a reading — the same rule on both sides, by construction."""
    return row is not None and not all(v is None for v in _row_utils(row).values())
...
def _active_account_walled(accounts: list[dict], threshold: float) -> tuple[bool, dict | None]:
    ...
    if row is None:
        return False, None
    utils = _row_utils(row)
    if not _row_has_reading(row):
        return False, row  # no reading → no claim
    return _flip_churn_excluded(utils, row.get("weekly_cap"), threshold), row


def _clear_stamp(stamp: Path) -> bool:
    """Unlink the fleet-exhausted stamp; True when it is gone. A stamp that cannot be removed
    (permissions, a directory in its place) is reported and left — the hold then STANDS, so no
    wake may follow: the tick must never tell the fleet the hold lifted while the hook still reads
    the stamp (scoped review of Phase A, 2026-09-07)."""
    try:
...
def _sid_is_armed(sid: str) -> bool:
    """Vendored VERBATIM from `scripts/sysadmin/selfwatch_check.py:65-86` — the ONE armed decider:
    a HELD `<safe>.selfwatch.lock` is an armed watch. `/proc/locks` first, then a non-blocking
    SHARED flock probe on an O_RDONLY fd (a held exclusive lock refuses it at once; a shared probe
    can never hand the watcher's own `flock -n` a false "already armed")."""
    path = _selfwatch_lock_dir() / f"{_selfwatch_safe(sid)}.selfwatch.lock"
    if not path.exists():
        return False
    st = os.stat(path)
    if _selfwatch_held_per_proc_locks(st):
        return True
...
def _wake_held_sessions(now: float, reason: str, reading_ok: bool) -> dict:
    """The RELIEF WAKE (D-177/D-178): the fleet-exhausted stamp just went away, so every session
    ...
    try:
        ...
        if reading_ok:
            for sid in armed:
                target = _selfwatch_lock_dir() / f"{_selfwatch_safe(sid)}.holdlifted"
                try:
        ...
        counts["errors"] += 1
    _ledger_append({"event": "hold-lifted", "ts": now, "reason": reason, "site": site, **counts})
    print(
        f"tick: hold lifted ({reason}) — woke {counts['woken']} of {counts['armed']} armed "
        f"self-watch(es); dead {counts['dead']}, pending {counts['pending']}, errors {counts['errors']}"
    )
    return counts
...
def _next_session_relief(
    ...
    for row in accounts:
        ...
        # guarded it — the wall stamp and the ledger then carried a resume epoch `_promised_resume`
        # could never reach, silencing the re-arm for a week (heavy review round 1 seat 1, F3)
        fr = _dateable_ts(fh.get("resets_at_epoch"))
...
def _open_wall_episode(email: str, now: float | None = None) -> tuple[dict | None, bool]:
    ...
    End rows are FLEET-wide, exactly like the stamp they stand in for: a RELIEF-site
    `hold-lifted`, a `flip`, or a `wall-episode-closed` row ends EVERY account's open episode.
    An episode for an account the pointer has since left is therefore closed by the next relief
    of whoever is active, instead of stranding until the week re-arm (seat B, F6). The cost,
    accepted over stranding: an account still walled inside its own promise re-arms on a
    sibling's relief and speaks once more per pointer round-trip (Delta 9 seat B, F5).
    A DWELL-site `hold-lifted` row is NOT an end: the dwell branch clears the stamp and wakes
    the held sessions on the EXPECTATION of a flip while the account is still walled, and
...
def _advisory_ledger_latch(email: str, now: float) -> bool:
    ...
    - within `_ADVISORY_MIN_GAP_S` of that row the fleet is latched unconditionally — the floor
      that bounds a storm at 2/h even if the STAMP is wrong (an unreadable ledger fails open
      past it — F4);
    ...
    """
    last, _readable = _open_wall_episode(email, now)
    if last is None:
        return (
...
def _fleet_active_wall_advisory(accounts: list[dict], now: float, threshold: float) -> None:
    ...
    if not _switch_paused() and _validated_pick(accounts, {row["email"]}) is not None:
        ...
        # which is the episode's honest end. Nor is the `hold-lifted` row the dwell-site wake
        # above writes an end: `_open_wall_episode` skips `site: dwell` — counting it re-armed
        # the ledger latch every tick-pair, 7/h with a WRITABLE stamp (Delta 9 seat B, F1).
        return
...
def _chain_push_stamp(email: str) -> Path:
    ...
    fallback dir (D-249 to D-251: a repeat of this push is bounded by the notifier's own 30-minute
...
def _write_stamp(path: Path, key: str) -> None:
    ...
    try:
        ...
        os.fchmod(fd, 0o600)
        buf = key.encode()
        while buf:  # a regular file never short-writes except on error; a 0-byte write is one
            n = os.write(fd, buf)
            if n <= 0:
...
def _chain_expiry_push(accounts: list[dict], now: float) -> int:
    ...
    The stamp holds the chain's expiry epoch: the same chain never pushes twice, a re-minted
    ...
    for row in accounts:
        ...
        left = exp - now
        state = (
            f"EXPIRED {abs(left) / 86400:.1f}d ago"
            if left <= 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    Fleet dirs (the login-once architecture — one config dir, one OAuth chain, one login):
        ...
        ``--touch``         RETIRED 2026-09-13 — a no-op that says why (rc 0): the temp-dir COPY
                            refresh consumed the single-use refresh token; only a /login re-mints.
