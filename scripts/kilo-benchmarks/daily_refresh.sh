...
mkdir -p "$(dirname "$LOG_FILE")"
{
  ...
  if ! mkdir -p "$KB/cache"; then
    ...
    bash "$KB/pipeline_alert.sh" 'daily_refresh.sh: heartbeat cache dir creation FAILED' "Disk full or permission denied on $KB/cache/. Next-day staleness alert will fire as backup but operator should investigate immediately." >> "$LOG_FILE" 2>&1 || true
  fi
  if ! date -u +'%Y-%m-%dT%H:%M:%S+00:00' > "$KB/cache/daily_refresh_last_success.txt" 2>/dev/null; then
    ...
    bash "$KB/pipeline_alert.sh" 'daily_refresh.sh: heartbeat timestamp write FAILED' "Could not write to $KB/cache/daily_refresh_last_success.txt. Tomorrow heartbeat will alert as STALE; please investigate disk / permissions now." >> "$LOG_FILE" 2>&1 || true
  fi
