...
_USAGE = {"input_tokens": 10, "output_tokens": 20, "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0}
...
def test_review_branch_builds_directresult(monkeypatch):
    ...
    monkeypatch.setattr("claude_p.claude_p_call", lambda model, prompt, *, system, timeout: ("[]", _USAGE))
...
def test_review_branch_pricing_error_becomes_error_result(monkeypatch):
    ...
    monkeypatch.setattr("claude_p.claude_p_call", lambda model, prompt, *, system, timeout: ("ok", _USAGE))
...
def test_run_direct_routes_openrouter_to_direct_call_not_shim(monkeypatch):
    ...
    monkeypatch.setattr(mr, "_direct_call", lambda model, *a, **k: mr._DirectResult(model, text="ok", out_tokens=1))
    monkeypatch.setattr(
    ...
    assert results[0].model == "openrouter/x" and hits["claudep"] == 0  # shim NOT called for an OR model
...
def test_code_branch_builds_gen(monkeypatch):
    ...
    monkeypatch.setattr(
        "claude_p.claude_p_call", lambda model, prompt, *, system, timeout: ("```python\nx=1\n```", _USAGE)
    )
...
def test_code_cost_cap_ignores_claude_spend(monkeypatch):
    ...
    big = {"input_tokens": 1_000_000, "output_tokens": 1_000_000, "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0}

    def slow(model, prompt, *, system, timeout):
        _t.sleep(0.03)  # keep the 2 workers busy so a cap-breach WOULD cancel not-yet-started futures (observable)
        return ("```python\nx=1\n```", big)

    monkeypatch.setattr("claude_p.claude_p_call", slow)
    probs = [mc.Problem(question_id=f"q{i}", prompt="s", starter_code="", sample={}) for i in range(6)]
...
def test_write_cost_sidecar_three_keys(tmp_path):
    ...
    data = derive_cost.write_cost_sidecar(10.0, 42.0, p, when=datetime.datetime(2026, 7, 20, 12, 0, 0))
