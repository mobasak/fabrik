...
def _hist(tmp_path, day: str, tokens: dict):
    ...
    hist.write_text(json.dumps({"version": 1, "days": {day: {"byModel": {"claude-opus-4-8": tokens}}}}))
    return hist
...
def test_amortized_rate_from_fixture(tmp_path):
    ...
    hist = _hist(tmp_path, today, {"input": 100, "output": 50, "cacheRead": 1000, "cacheCreation": 200})
    assert dc.amortized_rate(hist, _accounts(tmp_path, 3)) == pytest.approx((200.0 * 3) / 1350)  # $/token


def test_amortized_rate_ignores_days_outside_30d_window(tmp_path):
    old = (datetime.date.today() - datetime.timedelta(days=45)).isoformat()  # gapped/stale → excluded
    hist = _hist(tmp_path, old, {"input": 100, "output": 50, "cacheRead": 1000, "cacheCreation": 200})
...
def test_amortized_rate_malformed_history_fails_soft(tmp_path):
    ...
    for bad in ("[]", "5", '"x"', json.dumps({"days": {"2026-07-21": None}}),
