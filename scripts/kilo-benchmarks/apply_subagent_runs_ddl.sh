...
#
# Auth: uses `sudo -u postgres psql` (peer auth on unix socket) — WSL pg_hba.conf
...
DB_EXISTS=$(sudo -u postgres psql -tAc "SELECT 1 FROM pg_database WHERE datname='fabrik_analytics'" 2>/dev/null || echo "")
if [ "$DB_EXISTS" != "1" ]; then
  sudo -u postgres psql -c "CREATE DATABASE fabrik_analytics"
...
echo "$DDL" | sudo -u postgres psql -d fabrik_analytics

# Verify columns landed.
sudo -u postgres psql -d fabrik_analytics -tAc \
