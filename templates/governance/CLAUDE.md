...
**(a) ARM the self-watch** — `Monitor(persistent: true, command: "bash ~/.claude/bin/claude-selfwatch.sh <sid>", description: "resume-mesh self-watch")`; RE-ARM after every delivered wake (each fires once). `description` is tool-REQUIRED, and `<sid>` is a **literal** id — there is no `$CLAUDE_SESSION_ID` and an empty arg exits 1, ending the watch as you arm it; take it from the SessionStart arming line, or post-compact from the transcript path in the compaction notice. This line exists because that hook **skips on `source=compact` and headless** — a session that never armed pre-compact is never told again. Never a `nohup … &` arm: it eats the death marker and wakes nothing (authority: `/opt/fabrik/docs/workstation/hooks-index.md`).
