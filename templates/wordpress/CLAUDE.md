# Claude Code — WordPress Factory Bootstrap

> Tactical bootstrap for Claude Code when working on the **Fabrik WordPress Factory engine** (Phases 2–5 of the build). For per-generated-site work, the site's own `CLAUDE.md` applies.
...
Claude Code is reading this file when it is editing the Fabrik monorepo's WordPress factory engine: the 13-stage deployer at `src/fabrik/wordpress/`, the golden-base image work (Phase 2), the `fabrik wp create / preview / promote` wrappers (Phase 2), per-site spec files at `specs/sites/*.yaml`, and the manifests under `templates/wordpress/`. It governs the **build of the factory**, not the WordPress sites the factory produces. Site-level conventions (themes, plugins inside `wp-content/`, content schemas) are out of scope here — they belong to each site's own `CLAUDE.md` once the site is generated. If you are unsure which side of the line you are on, ask before editing.
...
- [`../../CLAUDE.md`](../../CLAUDE.md) — repo-root Claude Code bootstrap (contract, completion gate, hard stops). This file is a topical extension of that one; the repo-root rules still apply.
...
When in doubt about any rule, read the rule pack rather than guess; cross-reference, never duplicate it back into this file.
...
|---|---|---|
| **1 — Foundation (current)** | `FABRIK_EXEC_MODE` env-gate; finalised `ocoron.com` spec (DRAFT → READY, WPML → Polylang); first end-to-end VPS deploy; agent guardrails (this file + `KILO_CLI_RULES.md`) | Golden-base image, layered profile images, `fabrik wp create / preview / promote`, `fabrik-api`, GUI, Watchdog, multi-tenant fields |
