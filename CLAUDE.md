...
**(a) ARM the self-watch** — `Monitor(persistent: true, command: "bash ~/.claude/bin/claude-selfwatch.sh <sid>")`, and RE-ARM first thing after every delivered wake (each watch fires once). ⚠️ `<sid>` is a **literal** session id — there is NO `$CLAUDE_SESSION_ID` in the environment, and an empty arg arms a watch that protects nothing; take it from the SessionStart arming line, or on a compact-resume from the transcript path in the compaction notice. That hook **skips on `source=compact` and headless**, so a session that never armed pre-compact is never told again — this line is the backstop. Never a `nohup … &` Bash arm: it consumes the death marker and wakes nothing (`docs/workstation/hooks-index.md` is the authority).
