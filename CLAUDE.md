...
5. **EXIT** — Gate green → **COMMIT your own work NOW** (explicit pathspecs only — `git commit -- <your files>` — with Agent Provenance Trailers; never bundle files you didn't author). **An uncommitted task is an UNFINISHED task**: parked WIP is the only work that can be silently destroyed (pre-commit stash, resets) and it reds every sibling's diff-scoped gates. Stop-hook-enforced. **Push stays operator-authorized** (unless said this turn). **Ad-hoc branch/worktree work** (NON-plan — any `/fabrik-execute-plan` run keeps its own §Finish): unless the operator already named the disposition this turn (then do that and say which), end by PRESENTING four options and STOPPING — never execute a choice you weren't given: merge to base locally · push (operator-gated, as ever) · keep the branch as-is (work already committed on it) · discard (only a branch/worktree THIS run created — never a sibling's tree). On merge: resolve base as the MAIN checkout's branch — `MAIN=$(git worktree list --porcelain | sed -n '1s/^worktree //p')` — and pin every mutation (`git -C "$MAIN"`), then merge → verify (tests on the MERGED result) → only then clean up the worktree → delete the branch.
