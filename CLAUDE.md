...
FEEDBACK: /<command> · <wall-clock> · rounds <n> (<findings trend>) · tokens <input> input / <output> output (<n>% cached) · confusion: <…|none> · waste: <…|none> · change: <the one edit to the command/rule | none> · filed: <mail id(s) to a beat | none — surfaces exercised: …> [· cost: <pool $>]
```
