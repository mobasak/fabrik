...
- **Subagent fan-out** (detail: `.windsurf/rules/core/62-using-subagents.md`): **native Claude Task subagents by default** (line-precise grounding, review recall, GUI, the decide/refute/merge you own). The **OpenRouter pool** (`run_agents`) is the gated cost-lever for parallel implementation / cheap review breadth — phased in (`/fabrik-execute-plan` implementers → `/fabrik-review` finders) only after the plumbing is PROVEN (`from libs.subagents import record_agent_run` resolves + a smoke row lands in `subagent_runs`). **Flywheel rule:** a **pool** dispatch owes both a `results_table` and a `record_agent_run(spec, result)` per unit (one 0–5 verdict in both). A **native** Task subagent produces no `AgentResult` — it does **not** record. `record_run(result, …)` silently no-ops — always `record_agent_run(spec, result, …)`.
