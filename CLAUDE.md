...
5. **EXIT** — Gate green → **COMMIT your own work NOW** (explicit pathspecs only — `git commit -- <your files>` — with Agent Provenance Trailers; never bundle files you didn't author). **An uncommitted task is an UNFINISHED task**: parked WIP is the only work that can be silently destroyed (pre-commit stash, resets) and it reds every sibling's diff-scoped gates. Stop-hook-enforced. **Push stays operator-authorized** (unless said this turn). **Ad-hoc branch/worktree work** (NON-plan — plan sets keep their own §Finish): end by presenting exactly four options — merge to base locally · push + PR · keep as-is · discard — and on merge the order is merge → verify (tests on the MERGED result) → only then clean up the worktree → delete the branch.
