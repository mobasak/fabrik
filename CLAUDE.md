...
Cite the URL and the engine that found it in the artifact you produce (code, spec, ledger row, `CLAIMS.yaml`). **A research FAN-OUT's results are filed whole before anything is synthesized from them:** every fact a seat or engine returned goes verbatim into `docs/reference/research/<date>-<topic>-ledger.md`, each dispositioned `USED → <where>` · `DUPLICATE of <id>` · `REJECTED — read whole: <reason>` · `UNREACHABLE — …; tried …`; a source is rejected only after it is read in full, never on its quote — `scripts/check_research_ledger.py` refuses the commit otherwise. 3. After 3 QUERIES with no usable answer, tier 2 included:
