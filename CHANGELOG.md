...
- `scripts/kilo-benchmarks/embedding_export_markdown.py` — generates two human-readable reports daily under `docs/reference/kilo/`: `KILO_EMBEDDING_SELECTION_GUIDE.md` (today's winners per role with floor explanation) and `KILO_EMBEDDING_CAPABILITIES.md` (full catalog table sorted by cost). Both are fully auto-generated each run (rewritten, not appended). YAML block-scalar role notes are flattened to a single sentence for the role section header.
...
- `tests/kilo_benchmarks/test_embedding_export_markdown.py` — 8 integration tests for the markdown generator. Asserts both files exist with required headings, every role from the YAML appears as a section, every `embedding_roles.model_id` is rendered in the guide, every `embedding_models.id` is rendered in the capabilities catalog, the total-count line matches the actual row count, two consecutive runs produce byte-identical output, and the `qwen/qwen3-embedding-8b` P1 row appears in the `multilingual_primary` section specifically.
